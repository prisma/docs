...
The following query creates ([`create()`](/6.0/orm/reference/prisma-client-reference#create)) a single user with two fields:
...
The following [`createMany()`](/6.0/orm/reference/prisma-client-reference#createmany) query creates multiple users and skips any duplicates (`email` must be unique):
...
The following queries return a single record ([`findUnique()`](/6.0/orm/reference/prisma-client-reference#findunique)) by unique identifier or ID:
...
The following [`findMany()`](/6.0/orm/reference/prisma-client-reference#findmany) query returns _all_ `User` records:
...
The following [`findFirst()`](/6.0/orm/reference/prisma-client-reference#findfirst) query returns the _most recently created user_ with at least one post that has more than 100 likes:
...
The following query uses a combination of [operators](/6.0/orm/reference/prisma-client-reference#filter-conditions-and-operators) to return users whose name start with `E` _or_ administrators with at least 1 profile view:
...
See [Working with relations](/orm/prisma-client/queries/relation-queries#filter-a-list-of-relations) to find out how to combine [`include`](/6.0/orm/reference/prisma-client-reference#include) and `where` for a filtered list of relations - for example, only include a user's published posts.

## Update

### Update a single record

The following query uses [`update()`](/6.0/orm/reference/prisma-client-reference#update) to find and update a single `User` record by `email`:
...
The following query uses [`updateMany()`](/6.0/orm/reference/prisma-client-reference#updatemany) to update all `User` records that contain `prisma.io`:
...
The following query uses [`upsert()`](/6.0/orm/reference/prisma-client-reference#upsert) to update a `User` record with a specific email address, or create that `User` record if it does not exist:
...
From version 4.6.0, Prisma Client carries out upserts with database native SQL commands where possible. [Learn more](/6.0/orm/reference/prisma-client-reference#database-upserts).
...
Use [atomic number operations](/6.0/orm/reference/prisma-client-reference#atomic-number-operations) to update a number field **based on its current value** - for example, increment or multiply. The following query increments the `views` and `likes` fields by `1`:
...
Refer to [Working with relations](/orm/prisma-client/queries/relation-queries) for information about disconnecting ([`disconnect`](/6.0/orm/reference/prisma-client-reference#disconnect)) and connecting ([`connect`](/6.0/orm/reference/prisma-client-reference#connect)) related records.

## Delete

### Delete a single record

The following query uses [`delete()`](/6.0/orm/reference/prisma-client-reference#delete) to delete a single `User` record:
...
The following query uses [`deleteMany()`](/6.0/orm/reference/prisma-client-reference#deletemany) to delete all `User` records where `email` contains `prisma.io`:
...
The following query uses [`deleteMany()`](/6.0/orm/reference/prisma-client-reference#deletemany) to delete all `User` records:
...
When you know the order in which your tables should be deleted, you can use the [`deleteMany`](/6.0/orm/reference/prisma-client-reference#deletemany) function. This is executed synchronously in a [`$transaction`](/orm/prisma-client/queries/transactions) and can be used with all types of databases.
