...
This example shows how an `undefined` parameter impacts the results returned by a query that uses the [`OR`](/6.0/orm/reference/prisma-client-reference#or) operator.
...
This is in contrast to the [`AND`](/6.0/orm/reference/prisma-client-reference#and) and [`NOT`](/6.0/orm/reference/prisma-client-reference#not-1) operators, which will both return all the users
