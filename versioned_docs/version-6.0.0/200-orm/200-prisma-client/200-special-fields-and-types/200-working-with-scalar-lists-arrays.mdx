...
The following example demonstrates how to [`set`](/6.0/orm/reference/prisma-client-reference#set-1) the value of a scalar list (`coinflips`) when you create a model:
...
The following example demonstrates how to [`unset`](/6.0/orm/reference/prisma-client-reference#unset) the value of a scalar list (`coinflips`):
...
Use the [`push`](/6.0/orm/reference/prisma-client-reference#push) method to add a single value to a scalar list:
...
Use [scalar list filters](/6.0/orm/reference/prisma-client-reference#scalar-list-filters) to filter for records with scalar lists that match a specific condition. The following example returns all posts where the tags list includes `databases` _and_ `typescript`:
