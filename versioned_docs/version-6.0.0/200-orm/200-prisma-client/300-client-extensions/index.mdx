...
- Use the client-level [`$extends`](/6.0/orm/reference/prisma-client-reference#client-methods) method
...
`$on` and `$use` are not available in extended clients. If you would like to continue using these [client-level methods](/6.0/orm/reference/prisma-client-reference#client-methods) with an extended client, you will need to hook them up before extending the client.
...
To learn more, see our documentation on [`$on`](/6.0/orm/reference/prisma-client-reference#on) and [`$use`](/6.0/orm/reference/prisma-client-reference#use)

### Usage of client-level methods in extended clients

[Client-level methods](/6.0/orm/reference/prisma-client-reference#client-methods) do not necessarily exist on extended clients. For these clients you will need to first check for existence before using.
