...
Use the `PrismaClient` [`log`](/6.0/orm/reference/prisma-client-reference#log) parameter to configure [log levels](/6.0/orm/reference/prisma-client-reference#log-levels) , including warnings, errors, and information about the queries sent to the database.

Prisma Client supports two types of logging:

- Logging to [stdout](https://en.wikipedia.org/wiki/Standard_streams) (default)
- Event-based logging (use [`$on()`](/6.0/orm/reference/prisma-client-reference#on) method to [subscribe to events](#event-based-logging))
...
The exact [event (`e`) type and the properties available](/6.0/orm/reference/prisma-client-reference#event-types) depends on the log level.
