...
Case-insensitive filtering is a preview feature and [can be enabled for PostgreSQL](case-sensitivity#options-for-case-insensitive-filtering). The feature is not required for MySQL and MariaDB because these databases are case-insensitive by default. Add the highlighted feature flag to enable case-insensitive filtering capabilities:
