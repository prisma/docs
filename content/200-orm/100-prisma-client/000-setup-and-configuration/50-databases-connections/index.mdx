...
If the query engine cannot [process a query in the queue before the time limit](/concepts/components/prisma-client/working-with-prismaclient/connection-pool#how-the-connection-pool-works) , you will see connection pool timeout exceptions in your log. A connection pool timeout can occur if:
...
The [Data Proxy](/data-platform/classic-projects/data-proxy) is a managed external connection pooler built by Prisma that is integrated in the [Prisma Data Platform](/data-platform) and handles connection pooling for you.
...
Due to the way AWS RDS Proxy pins connections, [it does not provide any connection pooling benefits](/guides/deployment/caveats-when-deploying-to-aws-platforms#aws-rds-proxy) when used together with Prisma Client.
