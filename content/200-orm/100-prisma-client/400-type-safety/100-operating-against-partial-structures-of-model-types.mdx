...
The `User` type only contains the model's [scalar](/orm/prisma-schema/data-model#scalar-fields) fields, but doesn't account for any relations. That's because [relations are not included by default](/orm/prisma-client/queries/select-fields#return-the-default-selection-set) in Prisma Client queries.
