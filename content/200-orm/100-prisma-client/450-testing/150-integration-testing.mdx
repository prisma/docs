...
Before the test suite is run the database is seeded with data. After the test suite has finished a [`deleteMany`](/reference/api-reference/prisma-client-reference#deletemany)  is used to clear the database of its data.
