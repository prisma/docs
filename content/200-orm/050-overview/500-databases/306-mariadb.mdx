---
title: 'MariaDB'
metaTitle: 'MariaDB database connector'
metaDescription: 'This page explains how Prisma can connect to a MariaDB database using the MariaDB/MySQL database connector.'
tocDepth: 3
---

<TopBlock>

The MariaDB/MySQL data source connector connects Prisma ORM to a [MariaDB](https://mariadb.org/) database server.

Prism fully supports MariaDB: The database connection is made using [MariaDB NodeJS Connector](https://www.npmjs.com/package/mariadb) and the CI of Prism includes testing MariaDB 10.11 compatibility on every commit in Prism.

By default, the MariaDB/MySQL connector contains a database driver responsible for connecting to your database. You can use a [driver adapter](/orm/overview/databases/database-drivers#driver-adapters) (Preview) to connect to your database using a JavaScript database driver from Prisma Client.

</TopBlock>

## Example

To connect to a MariaDB database server, you need to configure a [`datasource`](/orm/prisma-schema/overview/data-sources) block in your [Prisma schema file](/orm/prisma-schema):
...
- `url`: Specifies the [connection URL](#connection-url) for the MariaDB database server. In this case, an [environment variable is used](/orm/prisma-schema/overview#accessing-environment-variables-from-the-schema) to provide the connection URL.

## Connection details

### Connection URL

Here's an overview of the components needed for a MariaDB connection URL:

![Structure of the MariaDB connection URL](mysql-connection-string.png)

The method `mysql:` is used for both MariaDB and MySQL.
...
| Database | `DATABASE`  | Name of the [database](https://mariadb.com/kb/en/create-database/) you want to use, e.g. `mydb`                     |
...
To connect to your MariaDB database via sockets, you must add a `socket` field as a _query parameter_ to the connection URL (instead of setting it as the `host` part of the URI).
The value of this parameter then must point to the directory that contains the socket, e.g. on a default installation of MariaDB on Ubuntu or Debian use: `mysql://USER:POST@localhost/database?socket=/run/mysqld/mysqld.sock`
...
## Type mapping between MariaDB to Prisma schema

The MariaDB connector maps the [scalar types](/orm/prisma-schema/data-model/models#scalar-fields) from the Prisma ORM [data model](/orm/prisma-schema/data-model/models) as follows to native column types:
...
When introspecting a MariaDB database, the database types are mapped to Prisma ORM according to the following table:

| MariaDB                   | Prisma ORM    | Supported | Native database type attribute                | Notes                                                              |
| ------------------------- | ------------- | --------- | --------------------------------------------- | ------------------------------------------------------------------ |
...
| `year`                    | `Int`         | ✔️        | `@db.Year`                                     |
| `Longtext`                | `Json`        | ✔️        | `@db.Longtext`                                 | See https://mariadb.com/kb/en/json-data-type/                      |
...
MariaDB uses the [InnoDB storage engine by default](https://mariadb.com/kb/en/server-system-variables/#default_storage_engine). If you use a different engine and introspect the database, relations in the Prisma Schema are not created (or may be lost, if the relation already existed).

## Permissions

A fresh new installation of MariaDB has by default only a `root` database user. Do not use `root` user in your Prisma configuration, but instead create a database and database user for each application. On most Linux hosts (e.g. Ubuntu) you can simply run this as the Linux `root` user (which automatically has database `root` access as well):

```
mariadb -e "CREATE DATABASE IF NOT EXISTS $DB_PRISMA;"
mariadb -e "GRANT ALL PRIVILEGES ON $DB_PRISMA.* TO $DB_USER@'%' IDENTIFIED BY '$DB_PASSWORD';"
```
...
```
mariadb -e "GRANT CREATE, DROP, REFERENCES, ALTER ON *.* TO $DB_USER@'%';"
```
