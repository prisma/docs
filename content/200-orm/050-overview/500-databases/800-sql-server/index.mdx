...
#### Raw queries with `String @db.VarChar(n)` field s/ `VARCHAR(N) columns

`String` query parameters in [raw queries](/orm/prisma-client/queries/raw-database-access/raw-queries) are always encoded to SQL Server as `NVARCHAR(4000)` (if your `String` length is <= 4000) or `NVARCHAR(MAX)`. If you compare a `String` query parameter to a column of type `String @db.VarChar(N)`/`VARCHAR(N)`, this can lead to implicit conversion on SQL Server which affects your index performance and can lead to high CPU usage. 
