...
PlanetScale does not use foreign key constraints in its database schema by default. On the other hand, Prisma ORM relies on foreign key constraints in the underlying database to enforce referential integrity between models in your Prisma schema.
