...
const adapter = new PrismaBetterSQLite3({
  url: "file:./prisma/dev.db" // note: no `file:` prefix here!
});
const prisma = new PrismaClient({ adapter });
```

Notice that this code requires the `DATABASE_URL` environment variable to be set to your SQLite connection string. You can learn more about the connection string below.
