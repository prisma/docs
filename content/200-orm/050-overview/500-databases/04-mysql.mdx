...
- `url`: Specifies the [connection URL](#connection-url) for the MySQL database server. In this case, an [environment variable is used](/orm/prisma-schema#accessing-environment-variables-from-the-schema) to provide the connection URL.
...
| `sslcert`          | No       |                        | Path to the server certificate. Certificate paths are [resolved relative to the `./prisma folder`](/orm/prisma-schema/data-sources#securing-database-connections) |
...
The MySQL connector maps the [scalar types](/orm/prisma-schema/data-model#scalar-fields) from the Prisma [data model](/orm/prisma-schema/data-model/models) as follows to native column types:
