...
- connect to your database, using the [`mongodb` database connector](/orm/)
...
Further reference information is available in the [MongoDB connector documentation](/orm/).
...
See also: [Defining ID fields in MongoDB](/orm/prisma-schema/data-model#defining-ids-in-mongodb)
...
The MongoDB connector maps the [scalar types](/orm/prisma-schema/data-model#scalar-fields) from the Prisma [data model](/orm/prisma-schema/data-model/models) to MongoDB's native column types as follows:
...
When introspecting a MongoDB database, Prisma uses the relevant [scalar types](/orm/prisma-schema/data-model#scalar-fields). Some special types also get additional native type annotations:
