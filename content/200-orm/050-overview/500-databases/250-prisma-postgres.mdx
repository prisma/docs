...
Prisma Postgres currently has a fixed connection limit of 10, which cannot be increased. This limitation is temporary and applies only while Prisma Postgres is in Early Access. Once Prisma Postgres reaches General Availability (GA), it will support configurable connection limits, allowing you to scale connections as needed.
