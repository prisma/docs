...
### Driver adapters and Nuxt

Using a driver adapter with [Nuxt](https://nuxt.com/) to deploy to a Edge Function environment unfortunately does not work out of the box, but adding the `nitro.experimental.wasm` configuration option fixes that:
...
See [this example project](TODO) for a full example that can be deployed to Cloudflare Pages.
