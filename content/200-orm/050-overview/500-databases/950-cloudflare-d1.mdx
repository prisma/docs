...
The workflow for creating an initial migration looks as follows. Assume you have an fedgeresh D1 instance without any tables.
...
#### 5. Locate the SQLite database file in `.wrangler`
...
Store this file path since you'll need it for any future migrations. The instructions will refer to the file path as:
...
For any further migrations, you can use the same workflow but instead of using `--from-empty`, you'll need to use `--from-url file:./.wrangler/state/v3/d1/miniflare-D1DatabaseObject/__YOUR_DB__.sqlite` because your source schema for the `prisma migrate diff` command is your local D1 instance, while the target remains your (then updated) Prisma schema.
...
Like before, you now need to create the migration file:
...
As explained above, you now need to use `--from-url` instead of `--from-empty` to specify a source schema:
...
- `--from-url file:./.wrangler/state/v3/d1/miniflare-D1DatabaseObject/__YOUR_DB__.sqlite`: The source for the SQL statement is a SQLite database file at `./.wrangler/state/v3/d1/miniflare-D1DatabaseObject/__YOUR_DB__.sqlite`.
- `--to-schema-datamodel ./prisma/schema.prisma`: The target for the SQL statement is the data model in `./prisma/schema.prisma`.
- `--script`: Output the result as SQL. If you omit this option, the "migration steps" will be generated in plain English.
