...
- **Remote and embedded SQLite databases**. D1 uses HTTP to connect to the remote SQLite database. When using Prisma ORM with D1, you need to use the `sqlite` database provider and the `@prisma/adapter-d1` [driver adapter](/orm/overview/databases/database-drivers#driver-adapters).
