...
2.  Next, link a field to a TypeScript type using an [AST comment](/orm/prisma-schema/overview#comments).
    ...
    ```prisma highlight=4;normal file=prisma/schema.prisma showLineNumbers
    model Log {
      id   Int @id
...
### Inline and Advanced Types

The generator also supports defining types directly in your schema for simple cases or creating arrays of typed objects.
...
This results in `post.status` being typed as `'draft' | 'published'` and `post.meta` as `LogMetaType[]`.
