...
We strongly recommend you define an output path. In Prisma ORM version `6.6.0`, not defining an output path will result in a warning. In Prisma ORM 7, the field will be required.

:::

### Using a custom `output` path

You have to specify a custom `output` path on the `generator` configuration, for example (assuming your `schema.prisma` file is located at the default `prisma` subfolder):
...
For improved compatibility with ECMAScript modules (ESM) and to ensure consistent behaviour of Prisma ORM across different Node.js runtimes, we recommend using the [TypeScript-only Prisma Client](/orm/prisma-client/setup-and-configuration/ts-only-client). This client is specifically engineered to handle common challenges with module resolution and runtime variations, providing a smoother integration experience.
