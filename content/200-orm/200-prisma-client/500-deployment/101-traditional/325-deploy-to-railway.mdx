...
This guide explains how to deploy an app that uses Prisma ORM and Prisma Postgres to [Railway](https://railway.app). The app exposes a REST API and uses Prisma Client to query a Prisma Postgres database. Both the app and database will be hosted on Railway.
...
To get a pre-wired Next.js Project with Prisma ORM, Prisma Postgres, and Railway, use the [Official Prisma Railway Template](https://railway.app/deploy/prisma-postgres).
...
If you used the example proejct, you should see three api endpoints already set up:
