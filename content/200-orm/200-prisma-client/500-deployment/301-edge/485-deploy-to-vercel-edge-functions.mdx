...
When deploying a Vercel Edge Function that uses Prisma ORM, you need to use an [edge-compatible driver](tbd) and its respective [driver adapter](tbd) for Prisma ORM.
...
- use the `@prisma/adapter-neon` database adapter (via the `driverAdapters` Preview feature) because Vercel Postgres uses [Neon](tbd) under the hood
