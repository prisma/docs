...
- [`@libsql/client`](https://github.com/tursodatabase/libsql-client-ts) is used to accessed Turso databases
...
Cloudflare has a [size limit of 1 MB on free accounts](https://developers.cloudflare.com/workers/platform/limits/). If your application bundle with Prisma ORM exceeds that size, we recommend upgrading to a paid account or using Prisma Accelerate to deploy yout application.

If you're running into this problem with `pg` and the `@prisma/adapter-pg` package, you can replace the `pg` with the custom `@prisma/pg-worker` package.
...
> **Note**: For Cloudflare Pages, using `node_compat` is not officially supported, but you can use a pre-release version of the CLI that adds support for `--node-compat` to `pages deploy`. Learn more in this [feature request](https://github.com/cloudflare/workers-sdk/issues/1074).
