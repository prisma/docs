...
- manually execute schema changes using [`wrangler d1 execute`](https://developers.cloudflare.com/workers/wrangler/commands/#execute) instead of using Prisma Migrate
...
If your Prisma schema only contains the `User` model but your D1 database is still empty, you need to make sure that there is a table in D1 that mirrors the structure of the `User` model. You therefore need to execute a SQL statement against D1 that will create this table.

You can generate this SQL statement using the [`prisma migrate diff`]() command as follows:

```terminal
npx prisma migrate diff --from-empty --to-schema-datamodel ./prisma/schema.prisma --script > prisma/schema.sql
```

Note that the resulting SQL statement is stored in a file in the `prisma` directory called `schema.sql` which looks as follows:

```sql file=prisma/schema.sql no-copy
-- CreateTable
CREATE TABLE "User" (
