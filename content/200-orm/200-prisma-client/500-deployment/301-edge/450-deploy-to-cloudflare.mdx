...
This page covers everything you need to know in order to deploy a Cloudflare Worker or Cloudflare Pages that uses Prisma Client for talking to a database.
...
> **Note**: There's [also work being done](https://github.com/sidorares/node-mysql2/pull/2289) on the `node-mysql2` driver which will enable to access traditional MySQL databases from Cloudflare Workers and Pages as well.
