...
This will create a new context before each test is run via the `createMockContext` function. This (`mockCtx`) context will be used to make a mock call to Prisma ORM and run a query to test. The `ctx` context will be used to run a scenario query that is tested against.
