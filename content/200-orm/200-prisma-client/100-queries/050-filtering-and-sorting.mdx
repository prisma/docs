...
In [3.5.0+](https://github.com/prisma/prisma/releases/3.5.0) for Postgres and [3.8.0+](https://github.com/prisma/prisma/releases/3.8.0) for MySQL, you can sort records by relevance to the query using the `_relevance` keyword. This uses the relevance ranking functions from full text search features.
...
Enabling the `fullTextSearch` preview feature breaks the imports of the `<Model>OrderByWithRelationInput` TypeScript types. It is is renamed to `<Model>OrderByWithRelationAndSearchRelevance`.
