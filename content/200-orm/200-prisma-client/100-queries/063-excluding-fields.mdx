...
- If you are omitting a field due to security reasons, it's best to omit it globally. For example: Globally omitting the password field from a User model so that sensitive information doesn't accidentally get exposed.
- If you are omitting a field because it's not needed in a query, it's best to omit it locally.

Locally omitted fields are omitted at the query level. This means that what fields are omitted can differ per-query, but there is no "default" omitted fields. On the other hand, globally omitted fields are defined at when a Prisma Client is instantiated and are then automatically omitted from every subsequent query on that Client. 

To omit different fields or select previous omitted fields, you will need to instantiate a new Prisma Client or [override a global omit with a local select](#how-to-select-a-previously-omitted-field).

## Excluding the password field without using `omit`

:::note

The `omitApi` Preview feature, released in Prisma ORM 5.13.0, is the preferred way of omitting fields from a query result.
