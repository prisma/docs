...
Use `where` to filter the fields returned by the `_count` output type. You can do this on [scalar fields](/orm/prisma-schema/data-model/models#scalar-fields), [relation fields](/orm/prisma-schema/data-model#relation-fields) and fields of a [composite type](/orm/prisma-schema/data-model#defining-composite-types).
