...
If you're using the [new `prisma-client` generator](/orm/prisma-schema/overview/generators#prisma-client-early-access), the `PromiseReturnType` type is no longer available. Instead, you can use a combination of the built-in TypeScript type `Awaited` and `ReturnType` to achieve the same result. 
