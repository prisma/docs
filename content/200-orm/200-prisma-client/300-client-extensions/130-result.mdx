...
When using the [`omit` (Preview) option](https://www.prisma.io/docs/orm/reference/prisma-client-reference#omit) with result extensions, there are certain considerations to keep in mind. 

If you `omit` a field that is needed by a computed field, it will still be read from the database even though it will not be included in the query result. 
...
```
In this case, although `password` is omitted from the result, it will still be queried from the database because it is a dependency of the `sanitizedPassword` computed field. 

To ensure omitted fields are not queried from the database at all, you must also omit dependent fields. For example, omitting both `password` and `sanitizedPassword` will exclude both from the result as well as prevent the `password` field from being read from the database. 
