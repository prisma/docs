...
This page builds upon the [legacy raw query methods](/orm/prisma-client/using-raw-sql/raw-queries) available in Prisma Client. Many cases for using raw SQL in Prisma Client are covered by [TypedSQL](/orm/prisma-client/using-raw-sql/typedsql), but this is still the recommended approach for working with `Unsupported` fields in Prisma Client.
