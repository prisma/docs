...
1. In your SQL file, use placeholders for the parameters you want to pass. The syntax for placeholders depends on your database engine:
   ...
   In SQLite, there are a number of different placeholders you can use. For this example, we'll use named placeholders `:minAge` and `:maxAge`:
   ...
   See below for information on how to type placeholders in your SQL files.
...
### Typing arguments in your SQL files
...
As an example, if you needed to type a single string argument with the alias `name`, you would add the following comment to your SQL file:
```sql
-- @param {String} $1:name
```
