...
With Prisma ORM 5.18.0, we have released [TypedSQL](/orm/prisma-client/using-raw-sql). TypedSQL is a new way to write type-safe SQL queries that are type-safe and even easier to add to your workflow. 

We strongly recommend using TypedSQL queries over the legacy raw queries described in this document whenever possible.
