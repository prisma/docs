...
With Prisma ORM 5.18.0, we have released [TypedSQL](/orm/prisma-client/raw-database-access). TypedSQL is a new way to write type-safe SQL queries that are type-safe and even easier to add to your workflow. Whenever possible, we recommend using TypedSQL queries over legacy raw queries.
...
If you cannot use tagged templates, you can instead use [`$queryRawUnsafe`](/orm/prisma-client/raw-database-access/raw-queries#queryrawunsafe) or [`$executeRawUnsafe`](/orm/prisma-client/raw-database-access/raw-queries#executerawunsafe) but **be aware that your these functions make it much more likely that your code will be vulnerable to SQL injection**.
