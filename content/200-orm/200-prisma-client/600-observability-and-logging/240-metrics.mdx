...
Prisma Client metrics give you a detailed insight into how Prisma Client interacts with your database. You can use this insight to understand better what your application is doing, or help diagnose performance issues with your application.
...
Prisma Client provides the following metrics when using a [native build-in driver](/orm/overview/databases/database-drivers#default-built-in-drivers):
...
- Counters (always increase):
  ...
  - `prisma_pool_connections_opened_total`: The total number of pool connections opened.
...
When using a [driver adapter](/orm/overview/databases/database-drivers#driver-adapters), the following metrics are not available as the connection management and the connection pool is handled by the external database driver itself:
...
When you retrieve metrics in JSON format, you can use them in the format they are returned, or for example [send them to StatSD](#use-prisma-client-metrics-with-statsd) to visualize how they change over time.
