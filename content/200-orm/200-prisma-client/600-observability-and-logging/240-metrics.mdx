...
Prisma ORM metrics give you a detailed insight into how Prisma Client interacts with your database. You can use this insight to help diagnose performance issues with your application.
...
To use Prisma ORM metrics, you must do the following:
...
When you retrieve metrics in JSON format, you can use them in the format they are returned, or [send them to StatSD](#use-prisma-orm-metrics-with-statsd) to visualize how they change over time.
...
### Use Prisma ORM metrics with StatsD

You can send JSON-formatted metrics to [StatsD](https://github.com/statsd/statsd) to visualize your metrics data over time.

<Admonition type="info">

Note: You must provide counter metrics to StatsD as a series of values that are incremented or decremented from a previous retrieval of the metrics. However, Prisma ORM counter
...
When you retrieve Prisma ORM metrics in Prometheus format, you can use them in the format they are returned, or [send them to the Prometheus metrics system](#use-prisma-orm-metrics-with-the-prometheus-metrics-system) to visualize how they change over time.
...
1. Multiple cumulative counters for observation buckets. These counters are suffixed with `_bucket{le="<upper inclusive bound>"}`. For example, `prisma_datasource_queries_duration_histogram_ms` has a counter exposed as `prisma_datasource_queries_duration_histogram_ms_bucket{le="1"}`
   ...
   When an observed value is less than or equal to the upper inclusive bound of a bucket, then Prisma ORM metrics increments that bucket by 1. Suppose that you have buckets with the upper inclusive bounds 0, 1, 5, 10, and 50 respectively. If the observed value is 5 then Prisma ORM metrics increments the third bucket onwards, because the value is greater than 0 and greater than 1, but less than or equal to 5, 10, and 50.
...
### Use Prisma ORM metrics with the Prometheus metrics system
...
The following example shows how to combine Prisma ORM metrics with other Prometheus client libraries that are also served with a REST API endpoint in conjunction with `Express.js`:
