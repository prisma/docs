...
TODO: merge when tracing goes GA:
...
1. [Install OpenTelemetry packages](#2-install-opentelemetry-packages).
...
Install the latest version of the `prisma`, `@prisma/client`, and `@prisma/instrumentation` npm packages. The minimum version to use tracing is 4.2.0.
...
<details>
<summary>Expand if you're using a Prisam ORM version older than v6</summary>

If you're using a Prisam ORM version older than v6, you need to enable the `tracing` feature flag in the `generator` block of your `schema.prisma` file:
