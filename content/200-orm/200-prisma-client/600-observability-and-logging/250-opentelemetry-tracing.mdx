...
For each trace, Prisma Client outputs a series of spans. The number and type of these spans depends on the Prisma Client operation, and the [database driver](/orm/overview/databases/database-drivers) being used. 

### Native built-in drivers

A typical Prisma trace for the [native build-in drivers](/orm/overview/databases/database-drivers#default-built-in-drivers) has the following spans:
...
When using a [driver adapter](/orm/overview/databases/database-drivers#driver-adapters), the trace contains less spans as the connetion management of the database driver it out of the control of Prisma. Its structure usually looks lik this:
