...
While Prisma Client operates on a higher-level of abstraction, it strives to expose the full power of the underlying database and lets you drop down to [raw SQL](/orm/prisma-client/raw-database-access/raw-queries) at any time if your use case requires it.
