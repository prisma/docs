...
Note that Prisma Client's nested write here save an initial query where first the `User` record needs to be retrieved by its `email`. That's because, with Prisma ORM you can connect records in relations using any unique property.
