...
You can learn how Prisma compares to Sequelize on the [Prisma vs Sequelize](/orm/more/comparisons/prisma-and-sequelize) page.
...
The first step to adopt Prisma is to [install the Prisma CLI](/orm/reference/prisma-cli-reference#installation) in your project:
...
Once that's done, you can configure your [database connection URL](/orm/reference/connection-urls) in the `.env` file. Here's how the database connection from Sequelize maps to the connection URL format used by Prisma:
...
To learn more how Prisma does this, you can read more [`@default(now())`](/orm/reference/prisma-schema-reference#now) and [`@updatedAt`](/orm/reference/prisma-schema-reference#updatedat) here.
...
The models that were generated via introspection currently _exactly_ map to your database tables. In this section, you'll learn how you can adjust the naming of the Prisma models to adhere to [Prisma's naming conventions](/orm/reference/prisma-schema-reference#naming-conventions).
...
Also note that you can rename [relation fields](/orm/prisma-schema/relations#relation-fields) to optimize the Prisma Client API that you'll use later to send queries to your database. For example, the `post` field on the `user` model is a _list_, so a better name for this field would be `posts` to indicate that it's plural.
...
Prisma on the other hand maps the model name to the table name in your database [modelling your data](/orm/prisma-schema/data-model/models).
...
Prisma offers you the flexibility to define these fields in your model. You add the `createdAt` and [`updatedAt`](/orm/reference/prisma-schema-reference#updatedat) fields by defining them explicitly in your model.
...
Similar to the `belongsToMany()` association method in Sequelize, Prisma allows you to [model many-to-many relations _implicitly_](/orm/prisma-schema/relations/many-to-many-relations#implicit-many-to-many-relations). That is, a many-to-many relation where you do not have to manage the [relation table](/orm/prisma-schema/relations/many-to-many-relations#relation-tables) (also sometimes called JOIN table) _explicitly_ in your schema. Here is an example with Sequelize:
