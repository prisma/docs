...
Note that Prisma Client's nested write here saves the initial query where the `User` record is first retrieved by its `email`. That's because, with Prisma ORM you can connect records in relations using any unique property.
