...
Prisma ORM 5.0.0 introduces a number of changes, including the usage of our new JSON Protocol, [which make Prisma ORM faster by default](https://www.prisma.io/blog/prisma-5-f66prwkjx72s). A full list of these changes can be found [in our release notes](https://github.com/prisma/prisma/releases/tag/5.0.0).
...
## Upgrade the <inlinecode>prisma</inlinecode> and <inlinecode>@prisma/client</inlinecode> packages to Prisma ORM 5
...
When we released Prisma ORM 2, the `prisma2` executable was used in order to differentiate from Prisma ORM 1. In a later release, the `prisma2` cli took over the `prisma` executable name.
