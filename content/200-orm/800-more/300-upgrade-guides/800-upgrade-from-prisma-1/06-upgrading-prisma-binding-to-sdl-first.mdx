...
metaTitle: 'Upgrading from Prisma ORM 1 with prisma-binding to SDL-first'
metaDescription: 'Learn how to upgrade existing Prisma ORM 1 projects with prisma-binding to Prisma ORM 2 (SDL-first).'
---

## Overview

This upgrade guide describes how to migrate a Node.js project that's based on [Prisma ORM 1](https://github.com/prisma/prisma1) and uses `prisma-binding` to implement a GraphQL server.
...
With `prisma-binding`, your approach for defining your GraphQL schema (sometimes called [application schema](https://v1.prisma.io/docs/1.20/data-model-and-migrations/data-model-knul/#a-note-on-the-application-schema)) is based on _importing_ GraphQL types from the generated `prisma.graphql` file (in Prisma ORM 1, this is typically called [Prisma GraphQL schema](https://v1.prisma.io/docs/1.20/data-model-and-migrations/data-model-knul/#the-prisma-graphql-schema)). These types mirror the types from your Prisma ORM 1 datamodel and serve as foundation for your GraphQL API.
...
<CodeBlock languages={["Before (with graphql-import)", "After (used with Prisma ORM 2)"]}>
...
Similar to the `prisma-binding` instance from Prisma ORM 1, you also want to attach your `PrismaClient` from Prisma ORM 2 to GraphQL's `context` so that in can be accessed inside your resolvers:
...
You can start by removing npm dependencies that were related to the Prisma ORM 1 setup:
...
Next, delete the files of your Prisma ORM 1 setup:
