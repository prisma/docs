...
metaTitle: 'Upgrade Prisma ORM 1 with nexus-prisma to @nexus/schema'
metaDescription: 'Learn how to upgrade existing Prisma ORM 1 projects with nexus-prisma to Prisma ORM 2 and Nexus.'
---
...
This upgrade guide describes how to upgrade a project that's based on [Prisma ORM 1](https://github.com/prisma/prisma1) and uses [`nexus`](https://www.npmjs.com/package/nexus) (< v0.12.0) or [`@nexus/schema`](https://github.com/graphql-nexus/schema) together with [`nexus-prisma`](https://www.npmjs.com/package/nexus-prisma) (< v4.0.0) to implement a GraphQL server.
...
To get started, you can remove the old Nexus and Prisma ORM 1 dependencies:
...
> **Note**: If you see the output of the Prisma ORM 1 CLI when running `npx prisma -v`, be sure to delete your `node_modules` folder and re-run `npm install`.
...
The only thing that needs to be updated for this query is the call to Prisma ORM since the new Prisma Client API looks a bit different from the one used in Prisma ORM 1.
...
You now need to adjust the call to your `prisma` instance since the new Prisma Client API looks a bit different from the one used in Prisma ORM 1.
...
If you haven't already, you can now uninstall dependencies that were related to the Prisma ORM 1 setup:
