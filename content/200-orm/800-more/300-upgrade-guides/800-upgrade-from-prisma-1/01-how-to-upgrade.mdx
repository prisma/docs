...
metaTitle: 'How to upgrade from Prisma ORM 1 to Prisma ORM version 2.x and later'
metaDescription: 'Learn how to upgrade your Prisma ORM 1 project to Prisma ORM version 2.x and later'
---

## Overview

This page helps you make an informed decision on when and how to upgrade from Prisma ORM 1 to Prisma ORM version 2._x_ and later.
...
- [Upgrading the Prisma layer](/orm/more/upgrade-guides/upgrade-from-prisma-1/upgrading-the-prisma-layer-postgresql): No matter what your Prisma ORM 1 setup looks like, you should **always start your upgrade process by following this guide**.
...
- [prisma-binding to Nexus](/orm/more/upgrade-guides/upgrade-from-prisma-1/upgrading-prisma-binding-to-nexus): Choose this guide if you're currently running Prisma ORM 1 with `prisma-binding` and want to upgrade to [Nexus](https://www.nexusjs.org/#/).
...
## Main differences between Prisma ORM 1 and Prisma ORM version 2._x_ and later

On a high-level, the biggest differences between Prisma ORM 1 and Prisma ORM versions 2._x_ and later are summarized below.
...
- make the features of Prisma ORM 1 more modular and splits them into dedicated tools:
  - Prisma Client: An improved version of Prisma Client 1.0
  - Prisma Migrate: Data modeling and migrations (formerly `prisma deploy`).
- use the [Prisma schema](/orm/prisma-schema), a merge of Prisma ORM 1 datamodel and `prisma.yml`.
...
Prisma ORM 2._x_ and later versions do not yet have full feature parity with Prisma ORM 1. The biggest feature that is still missing from Prisma ORM versions 2._x_ and later is real-time subscriptions.
...
The database schema that is created when running `prisma deploy` in Prisma ORM 1 is only partially compatible with the one that Prisma ORM versions 2._x_ and later creates. This section gives a quick overview of the general incompatibilities and the potential workarounds. -
...
- **Problem**: A short description of the problem when upgrading from Prisma ORM 1 to Prisma ORM versions 2._x_ and later
...
- **Breaking Prisma ORM 1**: Do the SQL statements break the Prisma ORM 1 setup? This is only relevant when you're choosing the gradual side-by-side [upgrade strategy](#upgrade-strategies).

| Problem                                                                  | SQL     | Prisma schema | Breaking Prisma ORM 1              |
| ------------------------------------------------------------------------ | ------- | ------------- | ---------------------------------- |
...
| `@db` attributes from Prisma ORM 1 are not transferred to the Prisma schema  | No      | Yes           | No                                 |
...
## Prisma ORM 1 Upgrade CLI

The [Prisma ORM 1 Upgrade CLI](https://github.com/prisma/prisma1-upgrade) helps you apply the workarounds that are explained on the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql) page. It generates the SQL statements to fix the database schema and make it compatible with Prisma ORM versions 2._x_ and later. Note that you are in full control over the operations that are executed against your database, the Upgrade CLI only generates and prints the statements for you. The Upgrade CLI also takes care of the workarounds in the Prisma schema.
...
- **Upgrade all at once**: Entirely remove Prisma ORM 1 from your project and move everything over to Prisma ORM version 2._x_ or later at once.
- **Gradual upgrade side-by-side**: Add Prisma ORM version 2._x_ and later to the existing Prisma ORM 1 project and gradually replace existing Prisma ORM 1 features with the newer Prisma features while running them side-by-side.

Note that if you are planning to run Prisma ORM 1 and Prisma ORM 2._x_ or later version side-by-side, you must not yet resolve the [schema compatibilities](#schema-incompatibilities) that are breaking the Prisma ORM 1 setup.
...
In case your project already sees a lot of traffic and has a lot of user data stored in the database, you might want to consider the **gradual** upgrade strategy where you're running Prisma ORM 1 and Prisma ORM 2 or later side-by-side for a certain amount of time until you've replace all former Prisma ORM 1 functionality with Prisma ORM 2 or later version.

Note that you won't be able to fix the [schema incompatibilities](#schema-incompatibilities) that require a "Breaking Prisma ORM 1" change if you choose the gradual upgrade strategy and intend to run Prisma ORM 1 and Prisma ORM version 2._x_ or later side-by-side. That's because these data migrations are breaking the schema that Prisma ORM 1 expects. This means that your Prisma Client API might not feel as idiomatic as it could, but you still get the full feature set of Prisma Client.
...
1. Use the Prisma ORM version 2._x_ or later CLI to introspect your Prisma ORM 1 database and generate your Prisma schema
1. Run the [Prisma ORM 1 Upgrade CLI](https://github.com/prisma/prisma1-upgrade) to "fix" the Prisma schema
