...
> If you're using TypeScript with PostgreSQL or SQLite, see also: [@snaplet/seed](#seeding-your-database-with-snapletseed).
...
enabling you to generate production-accurate data and efficiently seed your database.
...
2. Execute `npx @snaplet/seed init` to introspect your database. This command generates the seed client and creates a `seed.mts` file.
3. Utilize the generated `@snaplet/seed` client within the `seed.mts` file to compose your seeds.

Here's how to create new users and posts by modifying the initially generated `seed.mts` file:

```ts file=seed.mts highlight=14-20;normal
/**
...
4. Add `tsx` as a development dependency:

   ```
   npm install -D tsx
   ```

5. Insert the `prisma.seed` field into your `package.json` file and configure commands to seed your database and ensure the client remains up-to-date with your schema:

   ```json file=package.json highlight=5,8-9;normal
   {
     "name": "my-project",
     "version": "1.0.0",
     "prisma": {
...
7. To ensure the client remains synchronized with your schema, use the `migrate` command to run migrations, which automatically triggers the `postmigrate` script to sync the client:
   ```
   npm run migrate
   ```
