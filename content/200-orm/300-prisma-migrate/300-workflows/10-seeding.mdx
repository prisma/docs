...
3. Add `typescript`, `tsx`, `@types/node`, `@prisma/adapter-pg`, `pg`, and `dotenv` development dependencies:
   ```
   npm install -D typescript tsx @types/node @prisma/adapter-pg pg dotenv
   ```
