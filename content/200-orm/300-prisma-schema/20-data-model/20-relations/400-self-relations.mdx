...
- One relation field must be a [fully annotated](/orm/prisma-schema/relations#relation-fields). In this example, the `successor` field defines both the `field` and `references` arguments.
...
Note that you can also require each user to have a teacher by making the `teacher` field [required](/orm/prisma-schema/data-model#optional-and-mandatory-fields).
...
Note that for relational databases, this many-to-many-relation is [implicit](many-to-many-relations#implicit-many-to-many-relations). This means Prisma maintains a [relation table](/orm/prisma-schema/relations/many-to-many-relations#relation-tables) for it in the underlying database.
