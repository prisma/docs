...
Models represent the entities of your application domain. Models are represented by [`model`](/orm/reference/prisma-schema-reference#model) blocks and define a number of [fields](/orm/reference/prisma-schema-reference#model-fields). In the [example data model](data-model), `User`, `Profile`, `Post` and `Category` are models.
...
- [Scalar types](data-model#scalar-fields) (includes [enums](data-model#defining-enums)) that map to columns (relational databases) or document fields (MongoDB) in the database - for example, [`String`](/orm/reference/prisma-schema-reference#string) or [`Int`](/orm/reference/prisma-schema-reference#int)
...
You can optionally define a [custom primary key constraint name](/orm/prisma-schema/names-in-underlying-database#constraint-and-index-names) in the underlying database.
...
**Constraint names in relational databases** <br />
You can optionally define a [custom unique constraint name](/orm/prisma-schema/names-in-underlying-database#constraint-and-index-names) in the underlying database.

</Admonition>

By default, the name of this field in Prisma Client queries will be `authorId_title`.

You can also provide your own name for the composite unique constraint using the [`@@unique`](/orm/prisma-schema/names-in-underlying-database#constraint-and-index-names) attribute's `name` field:
...
You can optionally define a [custom index name](/orm/prisma-schema/names-in-underlying-database#constraint-and-index-names) in the underlying database.
