...
The `view` block may be generated with a `@@ignore` attribute if Prisma cannot fully support the view.
...
- Introspected views may include a `@@ignore` attribute if Prisma cannot safely query them. This prevents Prisma from generating invalid or partially supported queries on views.
