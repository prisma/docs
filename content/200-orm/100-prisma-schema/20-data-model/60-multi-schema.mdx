...
Many database providers allow you to organize database tables into named groups:
- In PostgreSQL, CockroachDB, and SQL Server, these groups are known as _schemas_. 
- In MySQL, these are called _databases_. 

In both cases, these groups act as a _namespace_ for logically grouping tables (e.g. to avoid name collisions or to have clearer domain separation) and let you define foreign key constraints across them. To avoid ambiguity, this page will refer to these namespaces as _database schemas_.
...
Multi-schema support is currently in [Preview](/orm/more/releases#preview) and has been available:
- since [v4.3.0](https://github.com/prisma/prisma/releases/tag/4.3.0) for PostgreSQL, CockroachDB, and SQL Server
- since [v6.12.0](https://github.com/prisma/prisma/releases/tag/6.12.0) for MySQL
