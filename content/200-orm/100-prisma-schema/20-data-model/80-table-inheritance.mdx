...
metaDescription: Table inheritance
tocDepth: 3
---

## Overview

Table inheritance is a software design pattern that allows to model hierarchical relationships between entities. Using table inheritance on the database-level can also enable the use of union types in your JavaScript/TypeScript application or share a set of common properties accross multiple models.

On this page, we introduce two approaches to table inheritance and explain how to use them with Prisma ORM.

A common use case for table inheritance may be a situation where an application needs to display a _feed_ of some kind of _content activities_. A content activity in this case could be a _video_ or an _article_. As an example, let's assume that:
...
Union types are a convenient feature in TypeScript that allow developers to work more flexibly with the types in their data model.
...
If you have a use case, where multiple models should share a certain set of properties, you can model this using table inheritance as well.

For example, if both the `Video` and `Article` models from above should have a common `title` property, you can achieve this with table inheritance as well.
...
- **Single-table inheritance (STI)**: Uses a _single_ table to store data of _all_ the different entitites in one location. In our example, there'd be a single `Activity` table that has the `id`, `url` as well as te `duration` and `body` column. It also uses a `type` column that indicates whether an _activity_ is a _video_ or an _article_.
- **Multi-table inheritance (MTI)**: Uses _multiple_ tables to store the data of the different entitites separately and links them via foreign keys. In our example, there'd be an `Activity` table that has the `id`, `url` column, a `Video` table with the `duration` and a foreign key to `Activity` as well as an `Article` table with the `body` and a foreign key. There also is a `type` column that acts as a discriminator and indicates whether an _activity_ is a _video_ or an _article_. Note that multi-table inheritance is also sometimes called _delegated types_.
...
- The model-specific properties `duration` and `body` need to be marked as optional (i.e. with `?`). That's because a record in the `Activity` table that represents a _video_ must not have a value for `body`. Conversely, an `Activity` record that represents an _article_ can never have a `duration` set.
- The `type` discriminator column indicates for each record whether it represents a _video_ or an _article_ item.

### Prisma Client API

Due to how Prisma ORM generates types and an API for the data model, there is only going to be an `Activity` type and the CRUD queries that belong to it (`create`, `update`, `delete`, ...) available to you.

#### Querying for videos and articles

In general, you can now query for only _videos_ or _articles_ by filtering on the `type` column. For example:
...
If you want to have type safety for these objects as well, you need to define dedicated types for them. You can do this for example by using the generated `Activity` type and TypeScript `Omit` utility type to remove properties from it:
...
If you want to access the shared properties, you need to use `include` in order to fetch the relation to `Activity`.
...
Here's how you can define `Video` and `Article` types by combining Prisma ORM's generated `Video` and `Article` types with the `Activity` type. This creates a new type with the desired properties. Note that we're also omitting the `type` discriminator column because that's not needed any more on the specific types:
...
Once these types are defined, you can define mapping functions to convert the types that you receive from the queries above into the desired `Video` and `Article` types. Here's the example for the `Video` type:
...
- **Performance**: MTI may come with a performance cost because you need to join the parent and child tables to get access to _all_ properties relevant for your model.
...
While Prisma ORM doesn't natively support union types nor polymorphism at the moment, you can check out Zenstack who are building additional features for the Prisma schema. Read their [blog post about polymorphism in Prisma ORM](https://zenstack.dev/blog/polymorphism) to learn more.

<!-- With Zenstack, you can model the example scenario from this page as follows:
...
model Video extends Activity {
  duration Int
}

model Article extends Activity {
  body String
}
```

Zenstack also adds the new `Video` and `Article` models to the Prisma Client API and enables CRUD queries for them:

```ts
await db.video.create({
  data: { ownerId: userId, duration: 100 }
});
