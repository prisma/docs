...
<versioninfo>
  Version [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and
  later support **native database type attributes** (type attributes) that
  describe the underlying database type:
</versioninfo>
...
- [Lists](/orm/reference/prisma-schema-reference#-modifier) of static values, such as `[5, 6, 8]` (`Int[]`) or `["Hello", "Goodbye"]` (`String`[]). <versioninfo>These are available in versions `4.0.0` and later, when using databases where Prisma supports them (PostgreSQL, CockroachDB and MongoDB)</versioninfo>
