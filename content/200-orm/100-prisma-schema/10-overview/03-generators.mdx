...
Currently, `prisma-client-js` is generally available, while [`prisma-client-ts` is in preview](/orm/prisma-client/setup-and-configuration/ts-only-client). Be sure to define a custom output folder for the generated assets using the output property.
...
## Prisma Client: `prisma-client-ts` (`Preview`)

The generator for Prisma's Typescript Client accepts multiple additional properties:

- `previewFeatures`: [Preview features](/orm/reference/preview-features) to include
- `binaryTargets`: Engine binary targets for `prisma-client-ts` (for example, `debian-openssl-1.1.x` if you are deploying to Ubuntu 18+, or `native` if you are working locally)
- `output`: The directory where Prisma ORM will generate all the type definitions. *This field is required*.
