...
- No need for the `@prisma/client` npm package
...
| `runtime`                | `nodejs`                     | Target runtime environment. <br />Supported values: <br />`nodejs` (alias `node`), `deno`, `bun`, `workerd` (alias `cloudflare`), `edge-light` (alias `vercel`), `react-native`. |
