...
Multi-file Prisma schemas are Generally Available since [v6.7.0](https://github.com/prisma/prisma/releases/tag/6.7.0). Before that, they could be used via the `prismaSchemaFolder` Preview feature flag.
...
- Organize your files by domain: group related models into the same file. For example, keep all user-related models in `user.prisma` while post-related models go in `post.prisma`. Try to avoid having "kitche" sink” schema files.
