...
Prisma now resolves the schema file or folder in this order:
...
Any paths referenced within a schema (e.g. SQLite file paths) are resolved **relative to the schema file** itself.

#### Migrations, views, and typed SQL

The `migrations`, `views`, and `sql` directories are now placed **next to the schema file that contains the `datasource` block**.
...
Making this placement more configurable is planned for **6.7.0** via `prisma.config.ts`.
