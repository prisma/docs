...
The Prisma schema (or _schema_ for short) is the main method of configuration for your Prisma ORM setup. It is typically a single file called `schema.prisma` (or multiple files with `.prisma` file extension) and consists of the following parts:
...
Looking to split your schema into multiple files? Multi-file Prisma schema is supported via the [`prismaSchemaFolder` preview feature](/orm/prisma-schema/data-model/location#multi-file-prisma-schema) in Prisma ORM 5.15.0 and later.
