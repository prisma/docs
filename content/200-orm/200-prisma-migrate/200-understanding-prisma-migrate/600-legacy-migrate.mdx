...
Legacy Prisma Migrate is a tool that lets you _change your database schema_, e.g. by creating new tables or adding columns to existing tables. These changes are called _schema migrations_. legacy Prisma Migrate is available as part of the [Prisma CLI](/orm/reference/prisma-cli-reference#installation) via the `legacy Prisma Migrate` command.
...
Add two models with [relation fields](/orm/prisma-schema/relations#relation-fields) to your Prisma schema:
...
Notice that in addition to the [annotated relation fields](/orm/prisma-schema/relations#annotated-relation-fields) and its relation scalar field (which represent the foreign keys), you must also specify the Prisma-level [relation fields](/orm/prisma-schema/relations#relation-fields) on the other side of the relation.
...
Add a [field](/orm/prisma-schema/data-model#defining-fields) to the `Post` model:
