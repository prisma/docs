...
Bootstraps a fresh Prisma ORM project within the current directory. The project uses a [local Prisma Postgres](/postgres/database/local-development) by default but you can 
...
The `dev` command starts a [local Prisma Postgres](/postgres/database/local-development) database that you can run Prisma ORM commands against. It is useful for development and testing purposes and also allows you to switch to [Prisma Postgres](/postgres) in production easily. Y
