...
Introspection with the `db pull` command on the [MongoDB connector](/orm/) samples the data instead of reading a schema.
...
This command is currently not supported on [MongoDB](/orm/).
...
Instead of `migrate dev` and related commands, [`db push`](/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/orm/).
...
This command is not supported on [MongoDB](/orm/). Use [`db push`](/orm/prisma-migrate/workflows/prototyping-your-schema) instead.
...
This command is only partially supported for [MongoDB](/orm/). See the command options below for details.
