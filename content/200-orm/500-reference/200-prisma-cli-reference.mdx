...
The `dev` command starts a local [PGlite server](https://pglite.dev/) that you can run Prisma ORM commands against. It is useful for development and testing purposes and also allows you to switch to [Prisma Postgres](/postgres) easily. You can learn more about the PGlite server [in this section](/postgres/database/local-development). 
