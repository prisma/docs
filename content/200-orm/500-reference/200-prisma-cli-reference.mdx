...
Here’s the new **section for `npx create-db`, `npx create-pg`, and `npx create-postgres`** that you can add at the end of your **Prisma CLI reference** page:

## `npx create-db` commands

The `create-db` command provisions a temporary [Prisma Postgres](/postgres) database with a single command. This is a standalone utility that can be invoked using `npx`. It's ideal for quickly testing, prototyping, or integrating with Prisma Postgres.
...
For full usage details, options (such as `--region` and `--interactive`), and examples, see the [npx create-db documentation](/postgres/introduction/npx-create-db).
