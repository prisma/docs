...
The `platform` command provides access to the Prisma Data Platform through the Prisma CLI. To get started, ensure you have the [Prisma Client](/concepts/components/prisma-client) updated to version `5.7.0` or later. This is necessary to access the Platform through the Prisma CLI.

The integration, currently in [Early Access](/platform/maturity-levels#early-access), extends the functionality of the Prisma CLI, enabling you to manage your Platform projects with Prisma Accelerate directly from the CLI. For the complete list of available commands, navigate to this section [here](/platform/platfrom-cli/commands).
