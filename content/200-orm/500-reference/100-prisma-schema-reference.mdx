...
```prisma no-lines
@unique(map: String?, length: number?, sort: String?, where: raw(String)?)
```
...
> ```prisma no-lines
> @@unique(_ fields: FieldReference[], name: String?, map: String?, where: raw(String)?)
> ```
...
```prisma no-lines
@@index(_ fields: FieldReference[], map: String?, where: raw(String)?)
```
