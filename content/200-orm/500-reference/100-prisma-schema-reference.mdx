...
| `moduleFormat`   | No       | Enum (`esm` or `cjs`)                       | Generates ESM or CommonJS modules. Determined on the basis of whether `import.meta.url` or `__dirname` is used. **Default**: Inferred from environment                                                 |
