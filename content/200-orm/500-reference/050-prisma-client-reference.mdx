...
| `relationLoadStrategy`          | `'join'` or `'query'`    | No       | **Default: `join`**. Specifies the [load strategy](tbd) for a relation query. Only available in combination with `include` (or `select` on a relation field) and since 5.9.0.                                                                                     |
...
- `query`: Sends multiple queries (one per table) to the database and joins them on the application level.

Learn more about join strategies [here](tbd).
...
##### Load the `posts` relation via a database-level JOIN
