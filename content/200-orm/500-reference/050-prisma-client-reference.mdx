...
See: [Raw database access (`findRaw()`)](/orm/prisma-client/queries/raw-database-access/raw-queries#findraw).

### `aggreagateRaw()`

See: [Raw database access (`aggreagateRaw()`)](/orm/prisma-client/queries/raw-database-access/raw-queries#aggregateraw).
...
`not` will return all items that do not match a given value. However, if the column is nullable, `NULL` values will not be returned. If you require null values to be returned, use an [`OR`](#or) to include `NULL` values.
...
See: [Raw database access (`$queryRawUnsafe()`)](/orm/prisma-client/queries/raw-database-access/raw-queries#queryrawunsafe).
...
See: [Raw database access (`$executeRawUnsafe()`)](/orm/prisma-client/queries/raw-database-access/raw-queries#executerawunsafe).
