...
- Prisma Client's dataloader [automatically batches `findUnique` queries](/orm/prisma-client/queries/query-optimization-performance#solving-n1-in-graphql-with-findunique-and-prisma-orms-dataloader) with the same `select` and `where` parameters.
