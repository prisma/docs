...
Defines an instance of a [driver adapter](/concepts/components/database-drivers#driver-adapters). See also [Database drivers](/orm/overview/introduction/databases/database-drivers) .
...
- It is for use on optional [scalar](/orm/prisma-schema/data-model#scalar-fields) fields only. If you try to sort by nulls on a required or [relation](/orm/prisma-schema/data-model#relation-fields) field, Prisma Client throws a [P2009 error](/orm/reference/error-reference#p2009).
...
Supported by [PostgreSQL](/orm/overview/introduction/databases/postgresql) and [MySQL](/orm/overview/introduction/databases/mysql) with different syntaxes for the `path` option. PostgreSQL does not support filtering on object key values in arrays.
...
- If you want to compare a field with a [unique constraint](/orm/prisma-schema/data-model#defining-a-unique-field)
