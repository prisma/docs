...
- If the related record does not exist, Prisma Client throws an exception:
  ...
  ```
- When using `set` and `connect` together, their order significantly impacts the result. If `set` is used before `connect`, the connected records will reflect the final state after the connect operation, as set clears all previous connections and connect establishes new ones. However, if `connect` is used before `set`, the set operation will override the connect action by clearing all connected records.
