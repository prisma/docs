...
Determines the level and formatting of errors returned by Prisma ORM.
...
The example below uses the [Neon driver adapter](/orm/overview/databases/neon#how-to-use-neons-serverless-driver-with-prisma-ormpreview)
...
- Prisma ORM's dataloader [automatically batches `findUnique` queries](/orm/prisma-client/queries/query-optimization-performance#solving-n1-in-graphql-with-findunique-and-prisma-orms-dataloader) with the same `select` and `where` parameters.
...
When Prisma ORM does an upsert, it first checks whether that record already exists in the database. To make this check, Prisma ORM performs a read operation with the `where` clause from the upsert operation. This has two possible outcomes, as follows:

- If the record does not exist, then Prisma ORM creates that record.
- If the record exists, then Prisma ORM updates it.
...
- They are faster than upserts handled by Prisma ORM
...
### <inlinecode>relationLoadStrategy</inlinecode> (Preview)
...
Prisma ORM metrics give you a detailed insight into how Prisma Client interacts with your database. You can use this insight to help diagnose performance issues with your application. Learn more: [Metrics](/orm/prisma-client/observability-and-logging/metrics).

Prisma ORM metrics has the following methods:

- `$metrics.json()`: [Retrieves Prisma ORM metrics in JSON format](/orm/prisma-client/observability-and-logging/metrics#retrieve-metrics-in-json-format).
- `$metrics.prometheus()`: [Retrieves Prisma ORM metrics in Prometheus format](/orm/prisma-client/observability-and-logging/metrics#retrieve-metrics-in-prometheus-format).
...
Utility types are helper functions and types that live on the Prisma ORM namespace. They are useful for keeping your application type safe.
