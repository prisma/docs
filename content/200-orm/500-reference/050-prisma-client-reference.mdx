...
In most cases, you can carry out batch inserts with the [`createMany()`](#createmany) or [`createManyAndReturn()`]() queries. However, [there are scenarios where `create` is the best option to insert multiple records](#remarks-10).
