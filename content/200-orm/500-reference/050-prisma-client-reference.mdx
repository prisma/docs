...
`findUniqueOrThrow()` retrieves a single data record in the same way as [`findUnique()`](#findunique). However, if the query does not find the requested record, it throws a `PrismaClientKnownRequestError`.

Note that [before Prisma v6](/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-6#replaced-notfounderror), it would throw a `NotFoundError: No User found error`. 
...
See: [Using Raw SQL (`aggreagateRaw()`)](/orm/prisma-client/using-raw-sql/raw-queries#aggregateraw).
