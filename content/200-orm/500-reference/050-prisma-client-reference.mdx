...
| `exclude`                       | `XOR<UserExclude, null>` | No       | Specifies which properties to omit on the returned object.                                                                                                                                                                    |
...
| `exclude`              | `XOR<PostExclude, null>`                                      | No       | Specifies which properties to omit on the returned object.                                                                                                                                                                    |
| `relationLoadStrategy` | `'join'` or `'query'`                                         | No       | **Default: `join`**. Specifies the [load strategy](/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) for a relation query. Only available in combination with `include` (or `select` on a relation field). In [Preview](/orm/more/releases#preview) since 5.9.0. |
| `where`                | `UserWhereInput`                                              | No       | Wraps _all_ model fields in a type so that the list can be filtered by any property.                                                                                                                                                                                                         |
...
| [`exclude`](#exclude)  | `XOR<UserExclude, null>`                                 | No       | Specifies which properties to omit on the returned object.                                                                                                                                                                    |
| `relationLoadStrategy` | `'join'` or `'query'`                                    | No       | **Default: `join`**. Specifies the [load strategy](/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) for a relation query. Only available in combination with `include` (or `select` on a relation field). In [Preview](/orm/more/releases#preview) since 5.9.0. |
|                        |
...
`exclude` defines which fields are omitted in the object that Prisma Client returns.

#### Remarks

- You cannot combine `exclude` and `select` since they serve opposite purposes

#### Examples

##### Exclude the `password` field from all User records
...
##### Exclude `title` of all User posts with `include`
...
#### Generated types for `exclude`

The following example demonstrates how to use the [`validator`](/orm/prisma-client/type-safety/prisma-validator) with `exclude`:

```ts
const excludePassword = Prisma.validator<Prisma.UserExclude>()({
  password: true,
})
```
