...
Configures how Prisma ORM locates and loads your schema file(s). Can be a file or folder path. Relativ paths are resolved relative to the `prisma.config.ts` file location. See [link to schema location docu] for more info about schema location options.
