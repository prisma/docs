---
title: "Build a real-time application with Prisma Postgres and Cloudflare Workers"
metaTitle: "Build a real-time application with Prisma Postgres and Cloudflare Workers"
description: "Learn step-by-step how to develop and deploy a Hono.js app that uses Prisma Postgre's realtime features to Cloudflare"
sidebar_label: "Build a real-time application with Prisma Postgres and Cloudflare Workers"
...
1. Create a new Hono.js project:  

   ```terminal
   npm create hono@latest realtime-app
   ```

2. Navigate into the newly created app directory:  
...
3. Follow the CLI prompts to configure your project:  
   a. Select the `cloudflare-workers` template.  
   b. Agree to install the project dependencies.  
   c. Choose `npm` as your package manager (or use your preferred manager but update commands accordingly throughout this guide).  
...
### 1. Clear the existing `src/index.ts` file  

Remove all content from the `src/index.ts` file to start with a clean slate.

### 2. Set up dependencies and environment bindings  
...
### 4. Create a route to establish a Websocket connection  
...
app.get(
  ...
  upgradeWebSocket(async (c) => {
    ...
    let listeningToPulseStream = false;

    return {
      onMessage(event, ws) {
        if (!listeningToPulseStream) {
...
### 5. Create a `POST` route which enables you to save `Points` in the database 
...
### 6. Create a `GET` route that serves an HTML page  

This route serves an HTML page with a form and scatter plot. Also establishes a connection to the websocket route and receives and reflects events from Pulse in real-time.
...
app.get("/", async (c) => {
  ...
  return c.html(`
    ...
    <html>
    <head>
      <title>Real-Time Scatter Plot</title>
      <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    </head>
    <body>
      <form id="addPointForm">
        <input name="x" type="number" placeholder="X coordinate" required />
        <input name="y" type="number" placeholder="Y coordinate" required />
      ...
      <script>
        ...
        document.getElementById('addPointForm').onsubmit = async (e) => {
          e.preventDefault();
          const form = new FormData(e.target);
          await fetch("/", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ x: +form.get("x"), y: +form.get("y") }),
          });
          e.target.reset();
        };
...
### 7. Start the server and test your application  
...
If you don’t already have a Cloudflare account, [sign up](https://dash.cloudflare.com/sign-up). After creating your account, ensure you have access to the Cloudflare dashboard.

### 2. Deploy the application with Wrangler
...
### 3. Configure secrets for the application
...
- Add [caching](https://www.prisma.io/docs/accelerate/caching) for performance using Prisma Accelerate.
- Explore the [Prisma Postgres documentation](https://www.prisma.io/docs/orm/overview/databases/prisma-postgres).