...
Next, bring the Prisma Client into your `index.astro` file by importing it from the `prisma.ts` file you just created:
...
Now, let's fetch the `Users` data from your database, making sure to include each user's `Posts` by adding them to the `include` field:
...
With the variables from the frontmatter now available throughout your file, you can display the list of users just below the `<h1>` tag:
...
Finally, display the `Posts` below the respective `User`:
