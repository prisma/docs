...
This guide uses Prisma Postgres as the primary example, but you can adapt it for SQLite or MySQL. [Choose your database setup below](#21-database-setup-options).
...
If you are using a different database provider (SQL Server), install the corresponding driver adapter package. For more information, see [Database drivers](/orm/overview/databases/database-drivers).
...
- A new database (for PostgreSQL) or local database file (for SQLite)
