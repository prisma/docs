...
You will be prompted to initalize a new git repository and install dependencies with npm. Select **Yes** for both.
...
Then, run `prisma init --db` to create a Prisma Postgres database and initialize Prisma ORM for your project:
...
This will then create a new `prisma` directory in your project, with a `schema.prisma` file inside of it. The `schema.prisma` file is where you will define your database models.
...
Next, change the generator to use the [`prisma-client`](/orm/prisma-schema/overview/generators#prisma-client-early-access) provider and add two models to your `schema.prisma` file, a `User` model and a `Post` model:
...
Now you can view the form at the new URL. It looks good, but it doesn't do anything yet. Let's update the `action` to save the post to the database:
