...
Add an `action` function to the route as well as a `Toast` to display a message when a note is created alongside importing the `createNote` function you created earlier in `models/note.server.js` and the `useActionData` hook and `useAppBridge` hook assigned to a new variable called `app`:
...
Next, add the form that will call the `action` function to the `ProductNotes` component:
...
Run `npm run dev` and navigate to the `Product Notes` route. Select a product, add a note, and verify that notes are displayed and saved correctly.
