...
sidebar_label: 'Set up SolidStart with Prisma ORM'
...
In this guide, you'll learn how to integrate Prisma ORM with a Postgres database in a SolidStart project from scratch.
...
- **Node.js 18+**
- **A PostgreSQL database** (or use [Prisma Postgres](https://www.prisma.io/postgres) for a managed option)
...
Now, let's fetch data from the database using a API route.
...
export default function App() {
  //add-next-line
  const [data, { mutate, refetch }] = createResource<UserWithPosts[]>(fetchUsers);
...
export default function App() {
  const [users, { mutate, refetch }] =
    createResource<UserWithPosts[]>(fetchData);
  ...
  return (
    <main>
      ...
      <Show when={!data.loading} fallback={<p>Loading...</p>}>
        <Show when={!data.error} fallback={<p>Error loading data</p>}>
          //add-end
          <For each={users()}>
            {(user) => (
              <div>
