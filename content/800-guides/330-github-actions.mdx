---
title: 'How to manage Prisma Postgres with GitHub Actions'
metaTitle: 'How to manage Prisma Postgres with GitHub Actions'
...
* Runs the workflow for PR open, reopen, and close events
* Supports manual runs using the GitHub UI
* Dynamically generates a sanitized database name per PR or manual run
...
### 5.1 Retrieve the project ID where you want to provision Prisma Postgres databases

As you ran the `npx prisma init --db` command earlier, you should have a Prisma Postgres project created in Prisma Data Platform. If you want to use a different project for the CI workflow, you can create a new one or use an existing project.

You can create a new project in the Prisma Data Platform by following these steps:

1. Go to the [Prisma Console](https://console.prisma.io).
2. Select your workspace.
3. Click on **New project**.
4. Then follow the prompts in the UI to create a new project.

Once the project is created, you will be using the [Platform CLI](/platform/platform-cli/commands#project-show) to retrieve your project ID. Head into your CLI and login to Prisma Data Platform using the CLI so that the CLI can access your project and workspace:

```terminal
npx prisma platform auth login --early-access
```

Then run the following command to retrieve your workspace ID:

```terminal
npx prisma platform workspace show --early-access
```

Then use the workspace ID to retreive your desired project ID where you want to automatically provision Prisma Postgres databases from the CI workflow:

```terminal
npx prisma platform project show --workspace $INSERT_WORKSPACE_ID --early-access 
```

Copy the Project `id` from the CLI output and store it somewhere securely. You will use this project ID in the GitHub Actions workflow as `PRISMA_PROJECT_ID`.

### 5.2. Retrieve your Prisma Postgres service token
...
5. Copy the generated token and store it securely. You will use this token in the GitHub Actions workflow as `PRISMA_POSTGRES_SERVICE_TOKEN`.
