...
This guide shows you how to automatically create and delete [Prisma Postgres](https://www.prisma.io/postgres) databases using GitHub Actions and [the Prisma Postgres management API](https://api.prisma.io/v1/swagger-editor). The setup provisions a new database for every pull request, seeds it with sample data, and deletes it once the PR is closed.

This is useful when testing database changes in isolation or when each feature branch needs its own temporary environment.
...
```terminal
npm install prisma tsx @prisma/client dotenv --save-dev
      ...
      - name: Get Connection String for Existing Database
        ...
        run: |
          ...
          CONNECTION_RESPONSE=$(curl -s -X POST \
            ...
            "https://api.prisma.io/v1/projects/${{ env.PRISMA_PROJECT_ID }}/databases/${{ steps.check-db.outputs.db-id }}/connections")
        ...
        run: |
          ...
          if [ ! -z "$DB_EXISTS" ] && [ "$DB_EXISTS" != "null" ]; then
            ...
            DELETE_RESPONSE=$(curl -s -X DELETE \
              -H "Authorization: Bearer ${{ env.PRISMA_POSTGRES_SERVICE_TOKEN }}" \
              -H "Content-Type: application/json" \
              "https://api.prisma.io/v1/projects/${{ env.PRISMA_PROJECT_ID }}/databases/$DB_EXISTS")
...
4. Under the **Netlify** section, click the **Generate integration token** button.
5. Copy the generated token and save it in your `.env` file as `PRISMA_POSTGRES_SERVICE_TOKEN`. This token is required for the next step's script and must also be added to your GitHub Actions secrets.

### 5.2 Retrieve the project ID where you want to provision Prisma Postgres databases

To avoid conflicts with your development databases, you'll now create a dedicated project specifically for CI workflows. Use the following script to provision a new Prisma Postgres project via the Prisma Management API. Save it as `create-prisma-project.sh` at the root of your repository:

:::note 

Make sure your `.env` file has the `PRISMA_POSTGRES_SERVICE_TOKEN` set to the token you generated in the previous step.

:::


```bash file="create-prisma-project.sh"
#!/usr/bin/env bash
...
if [[ -z "${PRISMA_POSTGRES_SERVICE_TOKEN:-}" ]]; then
  echo "❌ Please set the PRISMA_POSTGRES_SERVICE_TOKEN environment variable."
  echo "   Example: export PRISMA_POSTGRES_SERVICE_TOKEN=your-token"
  exit 1
fi
...
if [[ -z "$REGION" || -z "$NAME" ]]; then
  echo "Usage: ./create-prisma-project.sh <region> <project-name>"
  echo "Example: ./create-prisma-project.sh us-east-1 my-gha-preview"
  exit 1
fi

# -------- Make request --------
echo "🚀 Creating Prisma project '$NAME' in region '$REGION'..."

API_URL="https://api.prisma.io/v1/projects"
RESPONSE_FILE=$(mktemp)
HTTP_STATUS=$(curl --silent --output "$RESPONSE_FILE" --write-out "%{http_code}" \
  -X POST "$API_URL" \
  -H "Authorization: Bearer $PRISMA_POSTGRES_SERVICE_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"region\": \"$REGION\", \"name\": \"$NAME\"}")
...
if [[ "$HTTP_STATUS" -ne 201 ]]; then
  echo "❌ Failed to create project (HTTP $HTTP_STATUS)"
  echo "Response body:"
  echo "$RESPONSE_BODY"
  exit 1
fi
...
PROJECT_ID=$(echo "$RESPONSE_BODY" | grep -oE '"id":"[^"]+"' | head -n1 | cut -d':' -f2 | tr -d '"')

if [[ -z "$PROJECT_ID" ]]; then
  echo "❌ Project creation returned 201 but no project ID found."
  echo "Raw response:"
  echo "$RESPONSE_BODY"
  exit 1
fi
...
Then execute the script in your terminal to create a new Prisma Postgres project:

```bash
bash create-prisma-project.sh $REGION $PROJECT_NAME
```


Replace `$REGION` with your desired region (e.g., `us-east-1`) and `$PROJECT_NAME` with a name for your project (e.g., `my-gha-preview`). The script will create a new Prisma Postgres project in the specified region.

The CLI output will then look like this:

```bash
📦 Loading environment variables from .env
🚀 Creating Prisma project $PROJECT_NAME in region $REGION...
✅ Project created successfully!
📦 Project ID: $PRISMA_PROJECT_ID  
```
