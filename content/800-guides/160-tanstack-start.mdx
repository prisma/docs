...
sidebar_label: 'Set up Tanstack Start with Prisma ORM'
...
We'll also need a `.gitignore` file, so let's set that up now:
...
You'll also need React, the Vite React plugin, and TypeScript:
...
You should be seeing an error about `routeTree.gen.ts` not being existing. This is expected. It will be generated when you run TanStack Start for the first time.
...
Now, run the following command to set up Prisma:
...
Instead of using Prisma Studio, you can seed your database with data using the following command. See [Prisma Seed](https://www.prisma.io/docs/orm/prisma-migrate/workflows/seeding) for more information.

</Admonition>

We can't see the post on our app yet, so let's fix that.
