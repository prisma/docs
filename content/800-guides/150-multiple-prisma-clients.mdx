...
description: 'Learn how to use multiple Prisma Clients in a single app to connect to different databases, handle migrations, and deploy your application to Vercel.'
sidebar_label: 'Using multiple Prisma Clients in an app'
image: '/img/guides/multiple-prisma-clients.png'
...
- Two instances of [Prisma Postgres](https://www.prisma.io/postgres) databases (or any other PostgreSQL databases)
...
We are using [Prisma Postgres](/postgres/getting-started) in this guide. If you are not using a Prisma Postgres database, you will not need to add the `--db` flag. In that case, you will have to add your own database url to the `.env` file.
...
This will open up two browser windows, one in [`http://localhost:5555`](http://localhost:5555) and one in [`http://localhost:5556`](`http://localhost:5556`). Navigate to those windows and add sample data to both databases.
...
Navigate to [`http://localhost:3000`](http://localhost:3000) to see your Next.js app display data from both databases.


Congratulations, you have a Next.js app running with two Prisma Client instances querying different databases.

## 5. Deploy your Next.js app with multiple Prisma Clients to Vercel
...
Congratulations! Your have deployed an application with multiple Prisma Clients connecting to two different databases and is now live and fully operational on Vercel.
