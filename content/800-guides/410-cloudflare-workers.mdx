...
```terminal
npm create cloudflare@latest prisma-cf-worker -- --type=hello-world --ts=true --git=true --deploy=false
```
...
When prompted, paste your database connection string from the `.dev.vars` file.
