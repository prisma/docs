...
- **Management API**: A set of endpoints that allow approved partners to provision and manage Prisma Postgres databases on behalf of their users.
- **Projects vs Databases**: In Prisma Postgres, a "project" is a container that can hold multiple databases. When you provision a project, you are creating a new project instance that includes a default database for your user.
...
- **Tokens**: There are two main types of tokens:
  ...
  - **User Access Token**: Obtained via OAuth2 when a user authenticates, scoped to the user's workspace and can be used to transfer database ownership to the user's workspace.
