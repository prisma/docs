...
When you write about a specific version, make it clear _what product_ you are referring to. For example, in the following sentence, version 5.11.1 might refer to Prisma or to MongoDB:

_This filter is available for MongoDB only in versions 5.11.1 and later._

When the product is not clear from the context, explicitly mention the product name in front of the version number.

_This filter is available for MongoDB only in Prisma versions 5.11.1 and later._
...
From v5.0.0, the `command name` command is deprecated.

<!-- Bad -->

From v5.0.0, the `command name` command is deprecated.
We plan to remove `command name` in v.6.0.0.
```
...
We made composite types generally available in version 5.12.0.
They were previously available in preview from version 5.10.0.
