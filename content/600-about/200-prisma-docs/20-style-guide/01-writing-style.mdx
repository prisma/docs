...
## Provide context at the start of a page

Do not assume that readers will come into a page from a specific page in the docs (such as the preceding page in the side navigation). A reader might jump around in the side navigation, reach a page from a link in another page, or drop into any page from an external search engine. Therefore, always provide adequate context at the start of a page. This should consist of [a self-explanatory page title](/about/prisma-docs/style-guide/spelling-punctuation-formatting#titles-and-headings), and sufficient orienting material - or links to it - in the page introduction.

Exception: If your page is part of a tutorial that we've broken into multiple pages, then it's reasonable to expect that readers will consume it as part of a specific sequence. However, we should still provide some contextual material at the start of the page.
