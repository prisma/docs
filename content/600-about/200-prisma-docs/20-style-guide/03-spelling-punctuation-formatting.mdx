...
- VS Code (this is an official Microsoft short form, so is acceptable. Be sure to include the space. Do not use the long form Visual Studio Code)
...
- Do not use punctuation at the end of a title or heading. Exception: question marks are OK.
- Use `<inlinecode>code</inlinecode>` for code in headings - this is required by our navigation elements.
- Do not place a heading immediately at the top of a page, below the page title and before the first body text. It's implicit that the first paragraph on a page is an overview or introduction. There's no need to have a heading stating this.
- Ensure that page titles are self-sufficient. Readers might come in from a link or a search and the page title should make it clear what the subject of the page is. Do not force the reader to use the side navigation or breadcrumbs to figure out the subject matter. For example, the page title "Overview" is not self-explanatory, so avoid. "Metrics overview" would be better.
  - Headings in a page do not need to be self-sufficient. Even if readers reach the heading from a link, the context or text of a good link makes the meaning of the heading clear.
...
- are spelled in all-lowercase letters
- are prefixed and suffixed with an angle bracket
- use hyphens to join multiple words
- use descriptive terms

As an example, consider the following code block where `<database-connection-url>` is a placeholder for the PostgreSQL connection URL:
