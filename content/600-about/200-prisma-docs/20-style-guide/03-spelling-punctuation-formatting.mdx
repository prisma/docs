...
As an example, consider the following code block where `__database-connection-url__` is a placeholder for the PostgreSQL connection URL:

```prisma
datasource db {
  provider = "postgresql"
  url      = "__database-connection-url__"
}
```
