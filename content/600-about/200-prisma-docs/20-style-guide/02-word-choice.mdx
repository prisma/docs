...
`prisma` CLI commands only work with `prisma` included. Therefore, in most cases we should include the root command to help docs users who drop into the docs at that point. Use the following guidelines when you write about this command:
...
- In headings, for example in reference docs, it is OK to omit `prisma` for the sake of readability. [Example](/reference/api-reference/command-reference#init)
