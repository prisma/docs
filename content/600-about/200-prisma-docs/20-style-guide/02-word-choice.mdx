...
`prisma` CLI commands always work with `prisma` included, but they do not always work without it. Therefore, in most cases we should include the root command to help docs users who drop into the docs at that point. Use the following guidelines when you write about this command:
...
- In headings, for example in reference docs, you can omit `prisma`. [Example](/reference/api-reference/command-reference#init)
