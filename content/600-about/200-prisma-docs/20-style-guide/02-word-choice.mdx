...
When you refer to errors, use the following guidelines.

### Handling errors

When you want to say that Prisma (or another system) handles an error, use the verb "handle". Avoid other terms, such as "catch".
...
#### Errors returned by a CLI, API, or functions

When you want to refer to errors returned by Prisma (or another system), use the verb "return".

```md
<!-- Good -->

From version 4.0.0, the query returns an error:
```

#### Errors displayed by a GUI or by VS Code's IntelliSense

When you want to refer to errors returned by a GUI or by VS Code's IntelliSense (syntax validation), use the verb "display".

```md
<!-- Good -->

The Prisma Status page displays the following information:
```
