...
`prisma` CLI commands only work with `prisma` included. In most cases, we should include the root command to help docs users who drop into the docs at that point. Use the following guidelines when you write about this command:
