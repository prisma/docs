...
Avoid gerunds (the "ing" form of verbs). For example, use "Get started" instead if "Getting started". This guideline applies to headings and body text.
...
When you are writing about a specific version, be sure to specify _to what product_ you are referring. For example, in the following sentence it might be unclear to users whether version 3.11.1 refers to Prisma or to MongoDB:
...
_This filter is available for MongoDB only in Prisma version 3.11.1 and later._
...
Avoid the constructions "click here" or "here" for link text, because they are considered bad practice on the web.
...
When it is obvious what the link destination is (for example if you've explained the concept in the proceeding sentence), then a very concise (and maintainable) way to provide a link to further information is as follows:
