...
`prisma` CLI commands only work with `prisma` included. In most cases, we should include the root command to help docs users who drop into the docs at that point. Docs users can copy and paste a working command into their terminal windows.
...
- When you include a `prisma` command in a procedure, include `prisma`.
