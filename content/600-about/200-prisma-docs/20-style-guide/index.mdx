...
### Write in a calm, assured style

Do not use exclamation points (exclamation marks). Exception: they are acceptable in congratulatory messages, for example: "Congratulations - you've completed the tutorial!".
...
- Test the certificate using a browser
- Exclude fields
...
Although it can be tempting to use abbreviations like "Postgres" (instead of the official proper noun "PostgreSQL") or not worry about casing when writing "Javascript" (instead of "JavaScript"), we use the official forms of proper nouns. A few common examples are:
...
We want our docs to be as accessible as possible to a global audience. Use the following guidelines to write for a global audience.

- Stick to short, simple sentences. Where possible, avoid sentences with multiple clauses. Break these up into multiple sentences.
- English has many synonyms - use the simplest word you can. Examples: "use" instead of "utilize", "in" instead of "within".
- Avoid words borrowed from other languages (example: use "with" instead of "via": "Now you can start to send queries with the generated Prisma Client API").
- Avoid gerunds (the "ing" form of verbs). So use "Get started" instead if "Getting started". Cast your sentences to avoid gerunds. Example: Instead of "You can do this by rebuilding Prisma Client", use "To do this, rebuild Prisma Client".
...
Use lower case for these. Use capitals for the abbreviation: GA.
...
- Compile-time (when you use it as an adjective: "This compile-time...". When it's a noun, don't use a hyphen: "... at compile time")
