...
We write in short, clear sentences for a [global audience](#write-for-a-global-audience).
...
The Prisma docs are very technical, and we have to use a lot of jargon. However, strive to use as little jargon as possible.

When you do include jargon, use the following guidelines:

- If you can explain the jargon in a few words, then you might prefer to explain it there and then in your doc. Use your judgement to decide what is best for your doc.
...
- When you link to an external definition, choose carefully:

  - Use a credible source. Wikipedia is acceptable, and official third-party documentation is better.

  - Ensure that the external definition is relevant to the scope of the doc. For example, a database provider's docs might define a term, but this definition might only be valid for that database type. If your doc covers multiple database types, then this definition might be misleading. In this instance, you must find a definition that is relevant to the meaning of the jargon term in the doc you're working on.
