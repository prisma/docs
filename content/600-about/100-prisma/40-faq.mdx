...
Each of these migration directories contain a `migration.sql` file about the respective migration. The status of the migrations is tracked in the `_prisma_migrations` table in your database/schema. See [migrate status](/reference/api-reference/prisma-cli/command-reference#migrate-status).
