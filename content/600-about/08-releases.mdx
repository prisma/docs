...
Prisma's approach for versioning adheres to Semantic Versioning ([SemVer](https://semver.org/)) starting with `3.0.0`.
...
Beginning with version `3.0.0`, Prisma adheres strictly to the [SemVer](https://semver.org/) versioning scheme.

Here is a brief overview of how Prisma's follows SemVer:

- Breaking changes in stable surface (i.e. [General Availability](https://www.prisma.io/docs/about/releases#generally-available-ga)) will only be introduced in new `MAJOR`.
...
> **Note:** Up until version version `2.27.0`, Prisma did not follow SemVer versioning strictly. This means that releases in the `2.MINOR.PATCH` range, `MINOR` versions may have included breaking changes. To learn more about Prisma's adoption of SemVer, check out the [blog post](https://www.prisma.io/blog/prisma-adopts-semver-strictly).