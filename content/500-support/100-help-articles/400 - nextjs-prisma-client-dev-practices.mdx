...
There's a related [discussion](https://github.com/prisma/prisma/discussions/4399) and [issue](https://github.com/prisma/prisma/issues/5103) for the same. The reason is that in development due to hot reloading, a new instance of `PrismaClient` is created due to `next dev` clearing NodeJS cache which makes a lot of connections to the database in which is not ideal. This leads to database connections getting exhausted because each `PrismaClient` instance holds its own connection pool.
