...
| `tags` | `["user"]` | `String[]` | No | An optional array of strings used to [invalidate](/accelerate/api-reference#accelerateinvalidate) the cache, where each tag may contain only alphanumeric characters and underscores, with a maximum length of 64 characters. |
