...
A static IP address is an IPv4 or an IPv6 address that doesn’t change. Unlike dynamic IP addresses, which can change unpredictably, traffic from static IP addresses are predictable. The predictability of traffic originating from static IP addresses allows you to precisely identify traffic sources. This is important when your database is using IP allowlisting or a firewall to minimize exposure of your database by limiting access to only trusted IP addresses.

![What is a static IP](./images/static-ip.png)

After you enable static IP within your Accelerate configuration in your project environment, add the IP address you obtained in your database firewall's IP allowlist, ensuring secure access for Accelerate while blocking unauthorized requests to your database.
