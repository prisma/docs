...
## When do I have to enable static IP for Prisma Accelerate?

A static IP address is an IPv4 or an IPv6 address that doesn’t change. Unlike dynamic IP addresses, which can change unpredictably, traffic from static IP addresses are predictable. The predictability of traffic originating from static IP addresses allows you to precisely identify traffic sources. IP addresses need to be predictable when implementing security measures such as IP allowlisting or firewall configurations to restrict database access to trusted sources only.

![What is a static IP](./images/static-ip.png)

If your database is using IP allowlisting, enable static IP within your Accelerate configuration in your project environment and then add the IP address you obtained in your database firewall's IP allowlist, ensuring secure access for Accelerate while blocking unauthorized requests to your database.
