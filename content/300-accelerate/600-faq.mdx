...
You can invalidate the cache programmatically via the [`$accelerate.invalidate` API](/accelerate/api-reference#accelerateinvalidate) if you're on a [paid plan](https://www.prisma.io/pricing#accelerate), or you can invalidate your entire cache, on a project level, a minimum of five times a day. This limit increases depending on [your plan](https://www.prisma.io/pricing#accelerate). You can manage this via the Accelerate configuration page.
...
- Your application data _always_ needs to be up-to-date on retrieval, making it difficult to establish a reasonable cache strategy or possibly exceeding [the known cache invalidation limits](/accelerate/known-limitations#limits-on-programmatic-cache-invalidation).
