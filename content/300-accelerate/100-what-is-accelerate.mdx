...
While Accelerate is beneficial for all types of applications, edge function deployment options like [Vercel Edge Functions](https://vercel.com/docs/concepts/functions/edge-functions), [Cloudflare Workers](https://workers.cloudflare.com/), and [Deno Deploy](https://deno.com/deploy) get even more benefits because Accelerate is also served from edge locations. Cache hits can be served from data centers near the user regardless of the region of the database.

## See Accelerate in action

We built the [Accelerate Speed Test](https://accelerate-speed-test.prisma.io/) to compare the performance of cached and uncached queries side by side. The app is [open source](https://github.com/prisma/accelerate-speed-test) and you can clone it to try it yourself!
