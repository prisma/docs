...
The connection pool timeout is the maximum number of seconds to wait for a new connection to be opened. If the connection is not opened within this time frame, an exception is thrown. A value of 0 means there is no timeout.
