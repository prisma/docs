...
By default, Accelerate has a default connection limit of `10`. In most cases, this connection limit is sufficient. However, in high traffic or compute environments, you may need to [increase this value](#configuring-the-connection-pool-size). For the [Pro plan]() there is a maximum connection limit of `100`. For [Business plan]() users the maximum is `1000`.
...
With the Pro and Business plans of the Prisma Data Platform, you can increase your connection limit above the default of `10`. This allows Accelerate to allocate resources to scale up or down based on the traffic to your application. Specifically, Accelerate will begin allocating new resources when you have consistent usage greater than 90% of your currently alloted connection limit. If traffic continues to increase, Accelerate will scale up further. If traffic decreases, Accelerate will scale down.
