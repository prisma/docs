...
#### Incorrect/Missing <inlinecode>[where](/orm/reference/prisma-client-reference#where)</inlinecode> or <inlinecode>[select](/orm/reference/prisma-client-reference#select)</inlinecode> clause

In certain cases, a large number of records or fields are unintentionally fetched, which results in exceeding the 5MB limit. This could happen when the `where` clause in the query is incorrect or entirely missing. If you only need to use specific fields from the table, then you can use the select clause in your queries to prevent unnecessary data from being fetched.

**Suggested solution:** Double-check if the `where` clause is filtering the data as you expect. Consider using [pagination](/orm/prisma-client/queries/pagination) to prevent a large number of records from being fetched. Using the [select](/orm/reference/prisma-client-reference#select) clause to return only the fields that are being consumed can also be helpful in reducing the response size.
...
For runtimes that support Prisma ORM natively, you could consider creating two `PrismaClient` Instances. One with the Accelerate connection string (prefixed with `prisma://`) and the other one with the direct database connection string (prefixed with `postgres://`, `mysql://` etc)
