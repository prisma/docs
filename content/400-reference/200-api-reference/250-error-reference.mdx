...
As Accelerate connections operate differently from local database connections, error codes are different from regular ones. In general, all Accelerate-related errors start with `P6xxx`.

#### <inlinecode>P6004</inlinecode>

**QueryTimeout**—The global timeout of Accelerate has been exceeded. You can find the limit [here](/data-platform/accelerate/limitations#query-timeout-limit).

#### <inlinecode>P6005</inlinecode>

**InvalidParameters**—The user supplied invalid parameters. Currently only relevant for transaction methods. For example, setting a timeout that is too high. You can find the limit [here](/data-platform/accelerate/limitations#interactive-transactions-query-timeout-limit).

#### <inlinecode>P6009</inlinecode>

**ResponseSizeLimitExceeded**—The global response size limit of Accelerate has been exceeded. You can find the limit [here](/data-platform/accelerate/limitations#response-size-limit).
