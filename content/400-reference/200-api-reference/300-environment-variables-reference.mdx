...
`PRISMA_BINARIES_MIRROR` can be used to specify a custom CDN (or server) endpoint to download the engines binaries for the CLI/Client. The default value is `https://binaries.prisma.sh`.

See [`Navigating a corporate firewall`](../../concepts/overview/under-the-hood#navigating-a-corperate-firewall-with-prisma_binaries_mirror) for a conceptual overview of how to use this environment variable.
