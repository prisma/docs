...
When your application tries to perform two or more concurrent upsert operations, then a race condition might happen where one or more operations do not find the record and therefore try to create that record. In this situation, one of the operations succesfully creates the new record but the other operations fail and return a unique key constraint error.
