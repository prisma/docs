...
- It is for use on optional [scalar](https://www.prisma.io/docs/concepts/components/prisma-schema/data-model#scalar-fields) fields only. If you try to sort by nulls on a required or [relation](https://www.prisma.io/docs/concepts/components/prisma-schema/data-model#relation-fields) field, Prisma Client throws a [P2009 error](/reference/api-reference/error-reference#p2009).
...
The following examples show you how to to compare columns in separate tables.

- From version 4.3.0, you can also use these operators to [compare columns in the same table, with the `<model>.fields` field](#compare-columns-in-the-same-table).
...
From version 4.3.0, you can compare columns in the same table directly.

<Admonition type="info">

Before version 4.3.0, you can [use raw queries to compare columns in the same table](/guides/database/troubleshooting-orm/help-articles/comparing-columns-through-raw-queries).

</Admonition>

**TBA: the list of exceptions where you can't do this - users will need to use the rawQuery workaround**
...
To compare columns the same table, use the `<model>.fields` field. In the following example, the query returns all records where the value in the `prisma.product.quantity` field is less than or equal to the value in the `prisma.product.warnQuantity` field.
...
**TBD: Are there any operators you can't use?**
