...
`upsert` updates an existing database record _or_ creates a new database record.
...
- From version 4.6.0, Prisma carries out native upsers for Postgres, SQLite, and CockroachDB databases, if specific crieria are met. [Learn more](#native-upsert-support)
...
In a native upsert, Prisma Client hands over an `upsert` operation to the database. This has the following advantages:

- The database handles the `upsert` faster than if Prisma Client handles it
- It avoids [unique key constraint errors](#unique-key-constraint-errors-on-upserts).

From version 4.6.0, Prisma Client automatically carries out native upserts for all `upsert` operations for which all of the following criteria apply:

- You use SQLite, PostgreSQL, or CockroachDB as your database
- You do not have any nested queries in your `upsert`'s `create` and `update` [options](#options-7)
- Your upsert operation updates only one model
- There is only one unique field in your `upsert`'s `where` option
- The unique field defined in your `upsert`'s `where` option has the same value as defined in your `upsert`'s `create` option

Advantages: There won't be unique key constraint errors and it will be a lot faster.
So we should link to this from the unqiue key constraint errors section.

**How does it do it?**
This adds support for using the built in INSERT ... ON CONFLICT SET .. WHERE
This is via the ON CONFLICT .. UPDATE` sql command prisma.io/dataguide/postgresql/inserting-and-modifying-data/insert-on-conflict
...
If the upsert query is like this:

```ts
prisma.testModel.upsert({
  where: {
...
Prisma will use the native upsert. But if we have a query with multiple unique values in the `where` clause:
...
This it will use Prisma's upsert instead of the database's upsert functionality.

The unique field defined in where clause has the same value as defined in the create arguments

For the same schema as above, if the unique field defined in the where clause has a different value to the same unique defined in the create clause, then it will not use the database's upsert functionality.

For example this query Will work because the field in the where clause is "a-unique-value" and the field in the create is also "a-unique-value":
...
But this query would use Prisma's upsert instead of the database, because the field in the where clause is "a-unique-value" and the field in the create is "a-different-value":
