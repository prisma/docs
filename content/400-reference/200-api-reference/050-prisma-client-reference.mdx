...
`findUniqueOrThrow` retrieves a single data record in the same way as [`findUnique`](#inlinecodefinduniqueinlinecode). However, if the query does not find a record, it returns `NotFoundError: No User found error`.
...
`findFirstOrThrow` retrieves the first record in a list in the same way as [`findFirst`](#inlinecodefindfirstinlinecode). However, if the query does not find a record, it returns `NotFoundError: No User found error`.
