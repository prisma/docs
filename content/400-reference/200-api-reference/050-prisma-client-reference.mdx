...
`rejectOnNotFound` is deprecated in v4.0.0. From v4.0.0, use TBD . +++!!!new parameters TBA!!!+++
...
<Admonition = "info">
We introduced `findUniqueOrThrow` in v4.0.0. It replaces the [`rejectOnNotFound`](#inlinecoderejectonnotfoundinlinecode) option.
</Admonition>

`findUniqueOrThrow` retrieves a single database record. If no record is found, then the query returns an error. **!!!what error TBW!!\***

`findUniqueOrThrow` is otherwise identical to `findUnique`, with the following exceptions:
