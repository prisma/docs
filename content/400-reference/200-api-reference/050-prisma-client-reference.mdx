...
- Scalar list / array filters [ignore `NULL` values](../../../concepts/components/prisma-client/working-with-fields#null-values-in-arrays) <span class="concept"></span>. Using `isEmpty` or `NOT` does not return records with `NULL` value lists / arrays, and `{ equals: null }` results in an error.
