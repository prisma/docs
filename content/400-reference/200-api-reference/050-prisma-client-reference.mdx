...
**Note**: In PostgreSQL, the value of `array_starts_with` must be an array and not a string, even if the array only contains a single value.
...
**Note**: In PostgreSQL, the value of `array_ends_with` must be an array and not a string, even if the array only contains a single value.
