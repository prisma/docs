...
| ------------------------------- | ------------------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `where`                         | `UserWhereUniqueInput`   | **Yes**  | Wraps all _unique_ fields of a model so that individual records can be selected. <br></br>From version 4.5.0, this option wraps all fields of a model. [Learn more](#use-non-unique-fields-with-where)                                                            |
...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  From version 4.5.0, this option exposes all fields on the model. [Learn more](#use-non-unique-fields-with-where)
...
## Use non-unique fields with <inlinecode>where</inlinecode>

From version 4.5.0, the generated type `UserWhereUniqueInput` on [`where`](#where) exposes all fields on the model, not just unique fields. This means that you can specify unique and non-unique fields in your `where` statements. You must specify at least one unique field, but you can specify any number of additional unique and non-unique fields.

Example use cases:

- Optimistic concurrency control on updates **_Do we want to over this as a use case? If so, I will need help with this example. And do we want to update the dedicated OCC doc?_**
  Example:

  ```ts
  // At least one field must be unique
...
- Permission checks (for example, to check that a user is the author of a post before we let them update the content of that post)
- Soft deletes (for example `prisma.Post.findUnique({where: {id: postId, isDeleted: false}})`)

### Enable non-unique fields with where

To use this feature, enable the `extendedWhereUnique` preview flag in your `schema.prisma` file's `generator` block, as follows:
...
Do we want to say something about boolean operations not being possible on unique fields? But you can use boolean operations on non-unique fields.

The following is not possible, for example:

```ts
await prisma.user.update({
  where: { id: 1, OR: [{ email: "bob@prisma.io" }, { email: "alice@prisma.io" }] },
  //                    ^^^^^^^^                    ^^^^^^^^
  //                    | `email` is not part of that type
  data: { ... }
})
```

Do we want to say anything about one-to-one relations?
...
Let’s consider a nested delete on a 1-1 relation with the current API:

```ts
let connectedUser = await prisma.user.findUnique({ where: { id: 1 } })
...
The where input suggests you need to return a specific record. But you're only trying to delete the plan if the user doing it is authorized to do so - you're not trying to select any specific plan.
**_Will need help with this, too_**
