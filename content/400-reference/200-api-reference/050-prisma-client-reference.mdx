...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  From version 4.5.0, this type exposes all fields on the model. You can filter for a single record, and check additional non-unique and unique fields at the same time. [Learn more](#filter-on-non-unique-fields-with-userwhereuniqueinput).
...
From version 4.5.0, the generated type `UserWhereUniqueInput` on [`where`](#where) exposes all fields on the model, not just unique fields. [How to enable this feature](#enable-the-ability-to-filter-on-non-unique-fields-with-userwhereuniqueinput)

You must specify at least one unique field in your `where` statement, but you can specify any number of additional unique and non-unique fields.

You can use this to add filters to an operation that returns a single record. For example, you can use this feature for the following:
...
To perform optimistic concurrency control, we recommend that you use a `version` field to check whether the data in the record has changed while your code executes. Before version 4.5.0, you could not evaluate the `version` field in an `update` operation, because this field is non-unique. This is possible from version 4.5.0.

**TBA:**

**1. explain the example - what happens in this sequence?**

**2. consider updating the OCC doc**

In the following example, **need to add details on what happens with the new feature - which of these updates gets applied?**:
...
Note: Although you can perform optimistic currency control on nested updates, you must fetch the nested model first to get its version. **TBA: this needs to be clarified, plus we could give a short example of this**
...
In the following example, a user wants to update a post title. The `where` statement checks the field `authorId` to confirm that the user is the author of the post. The application only updates the post title if the user is the post author.
...
In the following example, we do not want to return a post if it is soft-deleted. The operation only returns the post if the value in the `isDeleted` field is `false`.
...
With `UserWhereUniqueInput`, you cannot use boolean operations to test unique fields in conjunction with other unique fields or with non-unique fields.

In the following example, we try to test `id`, a unique field, in conjunction with `email`. This is not possible. **TBA: state what happens - an error? nothing returned?**

```ts
await prisma.user.update({
  where: { id: 1, OR: [{ email: "bob@prisma.io" }, { email: "alice@prisma.io" }] },
  // ^^^ You cannot test `id` (a unique field) in conjunction with another field
  data: { ... }
})
```
...
**TBW: need to add any caveats the user needs to know about 1-2-1 relations, depending on implementation**
