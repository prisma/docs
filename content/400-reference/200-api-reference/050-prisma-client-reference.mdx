...
<Admonition type="warning">
  `$on` is not available in [extended
  clients](https://www.prisma.io/docs/concepts/components/prisma-client/client-extensions).
  Please either migrate to client extensions or use the `$on` method prior to
