...
From version 5.0.0, the generated type `UserWhereUniqueInput` on [`where`](#where) exposes all fields on the model, not just unique fields. This feature is also available in versions 4.5.0 and later using the `extendedWhereUnique` preview flag.
