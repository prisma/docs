...
### <inlinecode>createMany</inlinecode> (Preview)

`createMany` creates multiple records in a transaction. To enable this feature, add `createMany` to the `previewFeatures` list in your schema:
...
- You can nest a [`createMany`](#createmany-preview-1) inside an `update` or `create` query - for example, add a `User` and two `Post` records at the same time.
...
export type UserOrderByInput = {
  ...
  role?: SortOrder
  coinflips?: SortOrder
  profile?: ExtendedProfileOrderByInput  
}
...
You can also use a nested [`createMany`](#createmany-preview-1) to achieve the same result.
...
The `$disconnect()` method closes the database connections that were established when `$connect` was called and stops the process that was running Prisma's query engine. See [Connection management](../../..//concepts/components/prisma-client/connection-management) for an overview of `$connect()` and `$disconnect()`.
