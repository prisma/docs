...
**Deprecated:** `rejectOnNotFound` is deprecated in v4.0.0. From v4.0.0, use the queries [`findUniqueOrThrow`](#inlinecodefinduniqueorthrowinlinecode) or [`findFirstOrThrow`](#inlinecodefindfirstorthrowinlinecode).
...
- If you want the query to throw an error if the record is not found, then consider using [`findUniqueOrThrow`](#inlinecodefinduniqueorthrowinlinecode) instead.
...
| `rejectOnNotFound` (deprecated) | `RejectOnNotFound`       | No       | If true, throw a `NotFoundError: No User found error`. You can also [configure `rejectOnNotFound` globally](#rejectonnotfound). <br></br>**Note:** `rejectOnNotFound`is deprecated in v4.0.0. From v4.0.0, use [`findUniqueOrThrow`](#inlinecodefinduniqueorthrowinlinecode) instead. |
...
We introduced `findUniqueOrThrow` in v4.0.0. It replaces the [`rejectOnNotFound`](#inlinecoderejectonnotfoundinlinecode) option. `rejectOnNotFound` is deprecated in v4.0.0.

</Admonition>

`findUniqueOrThrow` retrieves a single database record. If no record is found, then the query returns `NotFoundError: No User found error`. `findUniqueOrThrow` is otherwise identical to [`findUnique`](#inlinecodefinduniqueinlinecode), with the following exceptions:
...
- If you want the query to throw an error if the record is not found, then consider using [`findFirstOrThrow`](#inlinecodefindfirstorthrowinlinecode) instead.
...
| `rejectOnNotFound` (deprecated) | `RejectOnNotFound`                                           | No       | If true, throw a `NotFoundError: No User found error`. You can also [configure `rejectOnNotFound` globally](#rejectonnotfound). <br></br>**Note:** `rejectOnNotFound`is deprecated in v4.0.0. From v4.0.0, use [`findFirstOrThrow`](#inlinecodefindfirstorthrowinlinecode) instead. |
...
We introduced `findFirstOrThrow` in v4.0.0. It replaces the [`rejectOnNotFound`](#inlinecoderejectonnotfoundinlinecode) option. `rejectOnNotFound` is deprecated in v4.0.0.

</Admonition>

`findFirstOrThrow` returns the first record in a list that matches your criteria. If no record is found, then the query returns `NotFoundError: No User found error`. `findFirstOrThrow` is otherwise identical to [`findFirst`](#inlinecodefindfirstinlinecode), with the following exceptions:
