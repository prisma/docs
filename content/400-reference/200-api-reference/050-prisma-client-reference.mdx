...
Where possible, Prisma Client hands over an `upsert` query to the database, instead of handling the upsert itself. This is called a _database upsert_.
...
Prisma Client uses a database upsert automatically when [specific criteria](#criteria-for-database-upserts) are met.

To use a database upsert, Prisma Client sends the SQL construction [`INSERT ... ON CONFLICT SET .. WHERE`](www.prisma.io/dataguide/postgresql/inserting-and-modifying-data/insert-on-conflict) to the database.
...
The following query has multiple unique values in the `where` clause. Prisma Client does _not_ use a database upsert:
