...
- Prisma's dataloader [automatically batches `findUnique` queries](/prisma-orm/performance-and-optimization/query-optimization#solving-n1-in-graphql-with-findunique-and-prismas-dataloader) with the same `select` and `where` parameters.
