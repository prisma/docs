...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  From version 4.5.0, this type exposes all fields on the model. You can filter for a single record, and check additional, non-unique fields at the same time. [Learn more](#filter-on-non-unique-fields-with-userwhereuniqueinput).
