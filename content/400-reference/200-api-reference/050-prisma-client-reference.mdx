...
**Note:** The following examples show how to compare columns in the same table.

- From version 4.3.0, you can also use these operators to [compare _fields_ in the same model](#compare-columns-in-the-same-table), with the `<model>.fields` property.
...
- `$on` and `$use` client methods do not exist on extended client instances which are extended using [`$extends`](#extends)<span class="api"></span>
...
See [middleware examples](/concepts/components/prisma-client/middleware#samples) <span class="concept"></span>.
