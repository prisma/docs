...
Full-Text Search is currently in **Preview** and only available for **PostgreSQL**. To use `search`, you'll need to enable the `fullTextSearch`
preview feature.
...
Available for MongoDB only in Prisma 3.12.0 and later.
...
##### Set the <inlinecode>shippingAddress</inlinecode> composite type within a new order
...
Use `unset` to unset the value of a composite type. Unlike `set: null`, this removes the field entirely.

#### Examples

##### Remove the <inlinecode>billingAddress</inlinecode> from an order
...
Use `update` to update fields within a composite type.
...
Use `equals` to filter results by matching a composite type or a list of composite types. Requires all fields of the composite type to match.
...
##### Find products with photos that match the given <inlinecode>url</inlinecode>s
...
Use `some` to filter for lists of composite types where some items in the list match the condition.
