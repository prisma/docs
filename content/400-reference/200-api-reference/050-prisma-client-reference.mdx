...
- The query does _not_ include a selection with nested fields **<-- wording candidate A **
- The query does _not_ include nested selection criteria **<-- wording candidate B **
...
In the following query, there are nested `select` statements, so Prisma Client does _not_ use a database upsert.
