...
The `$connect()` method establishes a physical connection to the database via Prisma's query engine. See [Connection management for an overview of `$connect()` and `$disconnect()`](../../..//concepts/components/prisma-client/connection-management).
