...
- Corresponding database type: [`ObjectId`](https://docs.mongodb.com/manual/reference/method/ObjectId/)
- Can be annotated with a [`@default()`](#default) value that uses [the `dbgenerated()` function](#dbgenerated) to auto-generate an `ObjectId`
- Must be defined on a `String` field
...
- ID field must be decorated with `@db.ObjectId`, where `db` is the `datasource` name
- `autoincrement()` is **not supported** and not required - `dbgenerated()` prompts MongoDB to generate an `ObjectId`
- [`cuid()`](#cuid) and [`uuid()`](#uuid) are not guaranteed to generate a valid `ObjectId` and **should not be used** in the context of an `@id` - however, both functions can be used to generated default values for other fields
...
For relational databases, use `autoincrement()` and `Int`:
...
For MongoDB, use `dbgenerated()` and `String`:
...
- Default values can be a static value (`4`, `"hello"`) or one of the following [functions](#attribute-functions):
  ...
  - [`dbgenerated()`](#dbgenerated) // TODO: Does this work for any field?
  - [`cuid()`](#cuid)
  - [`uuid()`](#uuid)
  - [`now()`](#now)
...
**SQLite and SQL Server not supported** <br />
