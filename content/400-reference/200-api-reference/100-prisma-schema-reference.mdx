...
In this example, the target database is located in a file named `dev.db`:
...
The [MongoDB connector](../../database-connectors/mongodb) does not currently support the `Unsupported` type.
...
```prisma
model User {
  id    Int     @id @map("_id") @db.ObjectId @default(dbgenerated())
  name  String
}
```
...
**Not supported by MongoDB** <br />
The [MongoDB connector](../../database-connectors/mongodb) does not support composite IDs.
...
```prisma
model User {
  id      Int      @id @default(dbgenerated()) @db.ObjectId @map("_id")
  email   String   @unique
  name    String?
|  role    Role     @default(USER)
  posts   Post[]
...
model Post {
  author    User    @relation(fields: [authorId], references: [id])
  authorId  Int     @unique @db.ObjectId
  title     String
  published Boolean @default(false)
}
...
- A `@@unique` block is required if it represents the only unique constraint on a model without an `@id` / `@@id` (relational databases) or `@id` (MongoDB)
...
| :----------- | :----------------- | :---------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------- |
| `name`       | `String`           | Sometimes (for example, to disambiguate a relation)                                                                                               | Defines the name of the relationship. In an m-n-relation, it also determines the name of the underlying relation table.                   | `"CategoryOnPost"`, `"MyRelation"`                    |
...
**Not supported by MongoDB** <br />
The [MongoDB connector](../../database-connectors/mongodb) does not support the `autoincrement()` function.
...
The [SQLite connector](../../database-connectors/sqlite) and the The [SQL Server connector](../../database-connectors/microsoft-sql-server) do not support the `enum` type.
...
- Enums are implemented at Prisma level in MongoDB, and stored as // TODO
