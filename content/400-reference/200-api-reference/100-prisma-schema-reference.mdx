...
| `clustered` | **No**   | `Boolean`          | **SQL Server only.** Defines whether the index is clustered or non-clustered. Defaults to `true`.                                                     |
...
> **Note:** In versions `4.0.0` and later, and in versions `3.5.0` and later with the `extendedIndexes` Preview feature enabled, new `length` and `sort` arguments are available as a Preview feature. In versions `3.14.0` and later a new `ops` argument is available. See [Index configuration](../../../../concepts/components/prisma-schema/indexes) for more details.
...
| `ops`    | **No**¹  | `identifier` or a `function` | **PostgreSQL only.** Allows defining the index operators for certain index types.                                                                  |
...
| `map`       | **No**   | `String`           |                                                                                                           |
| `clustered` | **No**   | `Boolean`          | **SQL Server only.** Defines whether the index is clustered or non-clustered. Defaults to `false`.        |
...
| `type`      | **No**   | `identifier`       | **PostrgreSQL and CockroachDB only.** The index access method. Defaults to `BTree`.                                                                                                                                                         |
| `clustered` | **No**   | `Boolean`          | **SQL Server only.** Defines is the index clustered, or non-clustered. Defaults to `false`.                                                                                                                                                 |
...
Composite types are available **for MongoDB only** in versions 3.12.0 and later, and in versions 3.10.0 and later if you enable the `mongodb` Preview feature flag.
