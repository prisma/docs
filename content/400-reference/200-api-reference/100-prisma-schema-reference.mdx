...
As of 2.28.0, and the introduction of `referentialActions`, the signature changes.
...
As of 2.29.0, if the `namedConstraints` preview flag is enabled, and used in conjunction with `referentialActions`, the signature changes again (unless you are using SQLite).
