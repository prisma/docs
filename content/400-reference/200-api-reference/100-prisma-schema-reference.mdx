...
#### Specify a SQL Server data source
...
```prisma
model User {
  email String  @unique @map("_id") // `email` can not be optional because it's the only unique field on the model
  name  String?
}
```
...
- Corresponding database type: [`PRIMARY KEY` constraint]()
...
- [`cuid()`](#cuid) and [`uuid()`](#uuid) are supported but do not generate a valid `ObjectId` - use `dbgenerated()` instead
...
| Name     | Required | Type               | Description                                                                 |
| -------- | -------- | ------------------ | --------------------------------------------------------------------------- |
| `map` ([2.27.0](https://github.com/prisma/prisma/releases/tag/2.27.0) and later)   | No       | `String`           | The constraint name in the underlying database. Default naming convention: `{table_name}_pk` (for example, `users_pk`)    |
...
Do not attempt to use `cuid()` to generate an `ObjectId` - this requires specific syntax.
...
Do not attempt to use `uuid()` to generate an `ObjectId` - this requires specific syntax.
...
| `name`   | No       | `String`           | The name of the field in the generated client - defaults to `field1_field2` |
| `map` ([2.27.0](https://github.com/prisma/prisma/releases/tag/2.27.0) and later)   | No       | `String`           | The constraint name in the underlying database. Default naming convention: `{table_name}_pk` (for example, `users_pk`)    |
...
In this example, `email` acts as the unique identifier and therefore requires `@map("_id")`.
...
- Corresponding database type: [`UNIQUE` constraint]()
...
- A field annotated with `@unique` _must_ include `@map("_id")` if it represents the only unique constraint on a model without an `@id` - MongoDB requires an `_id`

#### Arguments

| Name  | Required | Type     | Description                                                              |
| ----- | -------- | -------- | ------------------------------------------------------------------------ |
| `map` ([2.27.0](https://github.com/prisma/prisma/releases/tag/2.27.0) and later)| No       | `String` | The constraint name in the underlying database. Default naming convention: `{table_name}_{column_names}_uniq` (for example, `users_col1_uniq`) |
...
```prisma
model User {
  id    String  @unique @default(cuid()) @map("_id")
  name  String
}
```
...
| `name`   | **No**   | `String`           | The name of the unique combination of fields that is [reflected in the generated Prisma Client](#specify-a-custom-name-for-a-multi-field-unique-attribute) - defaults to `fieldName1_fieldName2_fieldName3`             |
| `map` ([2.27.0](https://github.com/prisma/prisma/releases/tag/2.27.0) and later)  | No   | `String`             | The constraint name in the underlying database. Default naming convention: `{table_name}_{column_names}_uniq` (for example, `users_col1_col2_uniq`) |
...
@@unique([title, author])
@@unique(fields: [title, author], name: "titleAuthor", map: "myConstraintName")
```
...
Introspection and Prisma Migrate do not currently support MongoDB, which means:

- Prisma Migrate will never use the `@@index` attribute to create an index in the database
- Introspection will never add an `@@index` attribute to the Prisma schema

Manually defining an `@@index` in the Prisma schema serves no purpose.
...
| -------- | -------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fields` | **Yes**  | `FieldReference[]` | A list of field names - for example, `["firstname", "lastname"]`                                                                                                                                                                            |
| `map` ([2.27.0](https://github.com/prisma/prisma/releases/tag/2.27.0) and later)  | No   | `String`             | The index name in the underlying database. Default naming convention: `{table_name}_{column_names}_idx` (for example, `users_col1_col2_idx`) for globally unique indexes and `{column_names}_idx` for indexes that are only unique at table level (for example: `col1_idx1`) |

The _name_ of the `fields` argument on the `@@index` attribute can be omitted:

```prisma no-lines
@@index(fields: [title, author])
@@index([title, author], map: "MyConstraintName")
```
...
- Corresponding database types: [`FOREIGN KEY` / `REFERENCES` constraint]()

##### MongoDB

- If your model's primary is of type `ObjectId` in the underlying database, both the primary key _and_ the foreign key must be have the `@db.ObjectId` attribute
...
| `map` ([2.27.0](https://github.com/prisma/prisma/releases/tag/2.27.0) and later)  | No   | `String`             | The index name in the underlying database. Default naming convention: `{table_name}_{column_names}_fkey` (for example, `post_userid_fkey`)|
...
@relation("UserOnPost", references: [id], map: "")
...
```ts no-lines
@relation(_ name: String?, fields: FieldReference[]?, references: FieldReference[]?, map: String?)
```
...
Maps a field name or enum value from the Prisma schema to a column or field with a different name in the database. If you do not use `@map`, the field name matches the column name exactly.
...
- At least one field on your model must define `@map("_id")`. For example:
  ...
  ```prisma
  model User {
  email  String  @db.ObjectId @unique @map("_id")
  }
  ```
...
| Name   | Type     | Required | Description                        | Example                         |
| :----- | :------- | :------- | :--------------------------------- | :------------------------------ |
| `name` | `String` | **Yes**  | The database column or field name. | `"comments"`, `"someFieldName"` |
...
Maps the Prisma schema model name to a table or collection with a different name, or an enum name to a different underlying enum in the database. If you do not use `@@map`, the model name matches the table or collection name exactly.
...
| Name   | Type     | Required | Description                            | Example                                     |
| :----- | :------- | :------- | :------------------------------------- | :------------------------------------------ |
| `name` | `String` | **Yes**  | The database table or collection name. | `"comments"`, `"someTableOrCollectionName"` |
