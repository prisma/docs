...
- [`cuid()`](#cuid), [`uuid()`](#uuid), and [`nanoid()`](#nanoid) are supported but do not generate a valid `ObjectId` - use `auto()` instead for `@id`
...
You cannot use `nanoid()` to generate a default value if your `id` field is of type `ObjectId`. Use the following syntax to generate a valid `ObjectId`:
...
### <inlinecode>nanoid()</inlinecode>

Generate a globally unique identifier based on the [Nano ID](https://github.com/ai/nanoid) spec.

#### Remarks

- Compatible with `String`
- Implemented by Prisma and therefore not "visible" in the underlying database schema. You can still use `nanoid()` when using [introspection](/concepts/components/introspection) by [manually changing your Prisma schema](/concepts/components/prisma-client/working-with-prismaclient/use-custom-model-and-field-names) and [generating Prisma Client](/concepts/components/prisma-client/working-with-prismaclient/generating-prisma-client), in that case the values will be generated by Prisma's [query engine](/concepts/components/prisma-engines/query-engine).

<Admonition type='info'>

**Note (Relational databases)**: If you do not want to use Prisma's `nanoid()` function, you can use [a native database function with `dbgenerated`](#override-default-value-behavior-for-supported-types).

</Admonition>

##### MongoDB

- `nanoid()` does not generate a valid `ObjectId` - [use the `@db.ObjectId` syntax](#generate-objectid-as-ids-mongodb-only) if you want to use `ObjectId` in the underlying database. However, you can still use `nanoid()` if your `_id` field is not of type `ObjectId`.
...
##### Generate `nanoid()` values of a specific length as IDs
...
```prisma
model User {
  id   String @id @default(nanoid(7)) @map("_id")
  name String
}
```
