...
| `start`     | `@default(sequence(start: 2))`<br />The value the sequence starts at, if it's restarted or if the sequence hits the `maxValue` and `noCycle` is not set.                                                           |
| `noCycle`   | `@default(sequence(noCycle))`<br />By default the sequence will wrap around when the sequence value hits the maximum or minimum value. If `noCycle` is set, the sequence will not wrap.                            |
