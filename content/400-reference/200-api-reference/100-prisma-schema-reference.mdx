...
Note that the above `generator` definition is **equivalent** to the following because it uses the default values for `output`, `engineType` and `binaryTargets` (and implicitly `previewFeature`):
...
This example shows how to define a custom `output` location of the generated asset to override the default.
...
With SQLite, the signiture changes to:
...
The [SQLite connector](../../concepts/database-connectors/sqlite) and the The [Microsoft SQL Server connector](../../concepts/database-connectors/microsoft-sql-server) do not support the `enum` type.
