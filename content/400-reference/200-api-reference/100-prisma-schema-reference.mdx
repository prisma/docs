...
The ID field in this example is specifically _not_ an `ObjectId` (the `id` field does not define the `@db.ObjectId` attribute). You cannot use `cuid()` to generate an `ObjectId` - you must use `dbgenerated()` instead.
...
The ID field in this example is specifically _not_ an `ObjectId` (the `id` field does not define the `@db.ObjectId` attribute). You cannot use `uuid()` to generate an `ObjectId` - you must use `dbgenerated()` instead.
