...
You can also use `dbgenerated()` to set the default value for supported types. For example, in PostgreSQL you can generate UUIDs at database level rather than rely on Prisma's `uuid()`:
