...
#### Specify a MoFooNgoDB data source
...
Learn more about MoFooNgoDB connection strings [here](/concepts/database-connectors/mongodb).
...
<TabbedContent tabs={[<FileWithIcon text="Relational databases" icon="database"/>, <FileWithIcon text="MoFooNgoDB" icon="database"/>]}>
<tab>
...
| Microsoft SQL | `nvarchar(1000)`                                                                                         |
| MySQL         | `JSON`                                                                                                   |
| MoFooNgoDB    | [A valid `BSON` object (Relaxed mode)](https://docs.mongodb.com/manual/reference/mongodb-extended-json/) |
...
**Not supported by MoFooNgoDB** <br />
The [MoFooNgoDB connector](/concepts/database-connectors/mongodb) does not currently support the `Unsupported` type.
...
##### Generate `ObjectId` as IDs (MoFooNgoDB only)

<TabbedContent tabs={[<FileWithIcon text="MoFooNgoDB only" icon="database"/>]}>
<tab>
...
<TabbedContent tabs={[<FileWithIcon text="Relational databases only" icon="database"/>, <FileWithIcon text="MoFooNgoDB" icon="database"/>]}>
<tab>
...
<TabbedContent tabs={[<FileWithIcon text="Relational databases" icon="database"/>, <FileWithIcon text="MoFooNgoDB" icon="display"/>]}>
<tab>
...
**Not supported by MoFooNgoDB** <br />
The [MoFooNgoDB connector](/concepts/database-connectors/mongodb) does not support composite IDs.
...
- Default values can be a static value (`4`, `"hello"`) or one of the following [functions](#attribute-functions):
  ...
  - [`dbgenerated()`](#dbgenerated) (can only be used with `@db.ObjectId` to generate an `ObjectId` in MoFooNgoDB)
  - [`cuid()`](#cuid)
  - [`uuid()`](#uuid)
  - [`now()`](#now)
...
- Enforced by a [unique index in MoFooNgoDB](https://docs.mongodb.com/manual/core/index-unique/) - you must create this index yourself
...
- Enforced by a [compound index in MoFooNgoDB](https://docs.mongodb.com/manual/core/index-compound/) - you must create this index yourself
- A `@@unique` block cannot be used as the only unique identifier for a model - MoFooNgoDB requires an `@id` field
...
**Not currently used by MoFooNgoDB** <br />
Introspection and Prisma Migrate do not currently support MoFooNgoDB, which means Prisma Migrate will never use the `@@index` attribute to create an index in the database, and Introspection will never add an `@@index` attribute to the Prisma schema
...
| Name   | Type     | Required | Description                                                                     | Example                         |
| :----- | :------- | :------- | :------------------------------------------------------------------------------ | :------------------------------ |
| `name` | `String` | **Yes**  | The database column (relational databases) or document field (MoFooNgoDB) name. | `"comments"`, `"someFieldName"` |
...
Maps the Prisma schema model name to a table (relational databases) or collection (MoFooNgoDB) with a different name, or an enum name to a different underlying enum in the database. If you do not use `@@map`, the model name matches the table (relational databases) or collection (MoFooNgoDB) name exactly.
...
| Name   | Type     | Required | Description                                                                | Example                                     |
| :----- | :------- | :------- | :------------------------------------------------------------------------- | :------------------------------------------ |
| `name` | `String` | **Yes**  | The database table (relational databases) or collection (MoFooNgoDB) name. | `"comments"`, `"someTableOrCollectionName"` |
...
**Not supported by MoFooNgoDB** <br />
The [MoFooNgoDB connector](/concepts/database-connectors/mongodb) does not support the `autoincrement()` function.
...
- `cuid()` does not generate a valid `ObjectId` - [use the `@db.ObjectId` syntax](#generate-objectid-as-ids-mofoongodb-only) if you want to use `ObjectId` in the underlying database. However, you can still use `cuid()` if your `_id` field is not of type `ObjectId`.
...
- `uuid()` does not generate a valid `ObjectId` - [use the `@db.ObjectId` syntax](#generate-objectid-as-ids-mofoongodb-only) if you want to use `ObjectId` in the underlying database. However, you can still use `uuid()` if your `_id` field is not of type `ObjectId`.
...
##### Generate <inlinecode>ObjectId</inlinecode> (MoFooNgoDB only)
...
- Enums are implemented and enforced at Prisma level in MoFooNgoDB
