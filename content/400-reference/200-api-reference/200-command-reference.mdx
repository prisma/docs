...
#### <inlinecode>schema</inlinecode> path
...
#### <inlinecode>seed</inlinecode> option

The path to the file or script to populate a database with sample database is specified in the `prisma.seed` entry in the `package.json` file. The file specified is used when `prisma db seed` is invoked. Both absolute and relative paths are supported.
