...
##### Run `npx prisma init --url mysql://user:password@localhost:3306/mydb`
...
| -------------- | -------- | --------------------------------------------------------------------------------------------------------------------- | ------- |
| `--data-proxy` | No       | The `generate` command will generate Prisma Client for use with the [Prisma Data Proxy](/data-platform/data-proxy).   |
...
- Take the SQL script from standard input and execute it on the database specified by the data source URL given in the `DATABASE_URL` environment variable:
  ...
  ```terminal
  $ echo 'TRUNCATE TABLE dev;' | npx prisma db execute --stdin --url="$DATABASE_URL"
  ```
