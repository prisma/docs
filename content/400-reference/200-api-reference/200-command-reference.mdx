...
While `--data-proxy`, `--accelerate`, and `--no-engine` are all available and work similarly, we recommend `--no-engine` as it prevents an engine from being downloaded which will greatly impact the size of apps deployed to serverless and edge functions.
