...
> **Note**: There is currently no built-in functionality to re-populate existing data or seed the database.
...
| `--rolledback` | No*       | Record a specific migration as rolled back - for example `--rolledback "20201231000000_add_users_table"`. | `./schema.prisma`<br /> `./prisma/schema.prisma` |
| `--schema`    | No       | Specifies the path to the desired schema.prisma file to be processed instead of the default path. Both absolute and relative paths are supported. | `./schema.prisma`<br /> `./prisma/schema.prisma` |

You must specify either `--rolledback` _or_ `--applied`.
