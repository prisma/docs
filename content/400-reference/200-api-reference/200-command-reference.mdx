...
If a [schema drift](/concepts/components/prisma-migrate/shadow-database#detecting-schema-drift) is detected while running `prisma migrate dev` using `--create-only`, you will be prompted to reset your database. If you choose to reset your database, the `--create-only` flag will be ignored and the migration will be applied. If a migration name is not provided, `prisma migrate dev` will prompt you for a name.
