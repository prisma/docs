...
- In 2.18.0 and later you can pass `--compiler-options` into the `ts-node` command that runs seed scripts (for example, if you are working with Next.js, whic has specific requirements for `ts-node`.) Prisma's script is specifically named `ts-node`:
