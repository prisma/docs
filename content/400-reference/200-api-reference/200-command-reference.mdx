...
```
Environment variables loaded from ./prisma/.env prisma : 2.13.0-beta.3 Current platform
...
```
Environment variables loaded from ./prisma/.env prisma : 2.0.0-beta.3 Current platform
: windows Query Engine : query-engine 2fb8f444d9cdf7c0beee7b041194b42d7a9ce1e6 (at
C:\Users\veroh\AppData\Roaming\npm\node_modules\@prisma\cli\query-engine-windows.exe) Migration
Engine : migration-engine-cli 2fb8f444d9cdf7c0beee7b041194b42d7a9ce1e6 (at
C:\Users\veroh\AppData\Roaming\npm\node_modules\@prisma\cli\migration-engine-windows.exe)
...
error: The preview feature "orderByRelationnp" is not known. Expected one of: microsoftSqlServer, groupBy, createMany, orderByRelation, napi
  -->  schema.prisma:3
   | 
 2 |   provider        = "prisma-client-js"
 3 |   previewFeatures = ["orderByRelationnp"]
   | 
...
> **Note**: `db push` and `db seed` are currently in [Preview](../../../about/releases#preview). Features in Preview may still have some issues, and are not recommended for use in production environments. To access commands in Preview, you must opt-in via the `--preview-feature` flag. 
...
### <inlinecode>db push</inlinecode>  (Preview)

The `db push` command pushes the state of your Prisma schema file to the database without using migrations. This command is a good choice when you do not need to version schema changes, such as during prototyping and local development.

See also:

* [Conceptual overview of `db push` and when to use it over Prisma Migrate]() <span class="concept"></span>
* [Schema prototyping with `db push`]() <span class="guide"></span>
...
- In [2.18.0](https://github.com/prisma/prisma/releases/tag/2.18.0) and later you can pass `--compiler-options` into the `ts-node` command that runs seed scripts (for example, if you are working with Next.js, whic has specific requirements for `ts-node`.) Prisma's script is specifically named `ts-node`:
...
The `migrate dev` command updates your database using migrations during development. See also:

* [Conceptual overview of Prisma Migrate]() <span class="concept"></span>
* [Developing with Prisma Migrate]() <span class="guide"></span>
