...
- [Conceptual overview of `db push` and when to use it over Prisma Migrate](/concepts/components/prisma-migrate/schema-prototyping) <span class="concept"></span>
...
Instead of `migrate dev` and related commands, [`db push`](/concepts/components/prisma-migrate/schema-prototyping) is used for [MongoDB](/concepts/database-connectors/mongodb).
...
This command is not supported on [MongoDB](/concepts/database-connectors/mongodb). Use [`db push`](/concepts/components/prisma-migrate/schema-prototyping) instead.
