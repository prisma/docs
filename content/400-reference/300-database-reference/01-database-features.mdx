...
- † [Caveats apply when using the `UNIQUE` constraint with Microsoft SQL Server](../../concepts/components/preview-features/sql-server#cannot-use-column-with-unique-constraint-and-filtered-index-as-foreign-key)
...
- \* Only available if natively supported by database.
- † Only supports JSON operations, but doesn't have a JSON column type. Therefore client JSON operations are not supported.
