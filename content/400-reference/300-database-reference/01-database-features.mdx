...
`autoincrement()` is commonly used to generated autoincrementing [ID](../../concepts/components/prisma-schema/data-model#defining-an-id-field) values. `now()` is commonly used to initialize "createdAt"-fields that store the point in time when a record was created.
