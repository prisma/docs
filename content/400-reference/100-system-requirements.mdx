...
The following two tables show the supported Linux distro' family, OpenSSL versions and C standard libraries for each CPU architecture.
Note that, in case of Prisma not being able to resolve the OpenSSL version on your system (e.g. because it is not installed), it will default to OpenSSL 1.1.x

On `AMD64` (`x86_64`) architectures:
...
On `ARM64` (`aarch64`) architectures:
