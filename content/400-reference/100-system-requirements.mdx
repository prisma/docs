...
- OpenSSL 1.0.x (except Alpine Linux), 1.1.x or 3.x
- On most distributions except Alpine Linux, glibc 2.17 or newer on x86_64 systems and glibc 2.24 or newer on ARM64 systems
- zlib (`libz.so.1`)
- libgcc (`libgcc_s.so.1`)
- C standard library (glibc on most Linux distributions or musl libc on Alpine Linux)

The following table shows the required C standard library version for each supported combination of a Linux distribution, CPU architecture and OpenSSL version:

| Distribution           | CPU architecture | OpenSSL version   | libc version        |
| ---------------------- | ---------------- | ----------------- | ------------------- |
| RHEL-like              | x86_64           | 1.0.x, 1.1.x, 3.x | glibc 2.17 or newer |
| Debian-like            | x86_64           | 1.0.x             | glibc 2.19 or newer |
| Debian-like            | x86_64           | 1.1.x, 3.x        | glibc 2.24 or newer |
| RHEL-like, Debian-like | ARM64            | 1.0.x, 1.1.x, 3.x | glibc 2.24 or newer |
| Alpine Linux           | x86_64, ARM64    | 1.1.x, 3.x        | musl libc 1.2.x     |

Note that systems that can run the supported versions of Node.js will most likely have zlib and libgcc present on the system. One notable exception is Google's Distroless images, where `libz.so.1` needs to be copied from a compatible Debian system.
...
Prisma supports macOS 10.15 or newer on Macs with Intel CPUs and macOS 11 or newer on Macs with Apple Silicon chips. There are no additional platform-specific requirements on macOS other than what is listed for all platforms in the [Software requirements](#software-requirements) section.
