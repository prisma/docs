...
Prisma Postgres bills based on operations and storage consumed. Visit the [pricing page](https://www.prisma.io/pricing) for rates and the [operations-based billing](https://www.prisma.io/blog/operations-based-billing) blog post for a detailed explanation on operations-based pricing.
...
### How do I monitor my Prisma Postgres operation usage?

You can monitor your operation usage by integrating an application performance monitoring tool like Prometheus with Prisma ORM's [`metrics` preview feature](/orm/prisma-client/observability-and-logging/metrics). Once you enable the `metrics` preview feature, you can look at the `prisma_client_queries_total` metric. 
...
Prisma Postgres bills by operation. The more you can perform using a single operation, the lower your bill. Some suggestions to reduce the number of operations:
...
You can use the three example scenarios and estimate bills for small, medium, and large apps. Each combines a realistic number of monthly active users (MAUs), a typical level of daily activity per user, and a rounded storage footprint:

- **Small**: A hobby or early-stage side-project with ≈`500` MAUs. Each user clicks around `10` times a day, and the entire database uses about `0.5` GB of storage.
- **Medium**: A growing SaaS product serving ≈ `5000` MAUs. Power users average `40` actions per day, and the app stores roughly `6` GB of data.
- **Large**: A production-grade, consumer-facing application handling ≈ `50000` MAUs. Heavy usage with `60` actions per user per day drives significant traffic, and the dataset reaches ≈ `40` GB.

All three examples assume the standard allowances (`100000` free operations and `1` GB of free storage each month) plus the published rates:
...
- **Business plan** - \$6 per million operations
  - Storage - 10 GB free then \$1 per additional GB


The workloads above translate into monthly charges with the same set of equations; just drop in your own MAUs, activity, and storage.

```
total_ops            = MAUs x actions_per_user_per_day x 30      
billable_ops         = MAX(total_ops - 100_000, 0)
ops_cost             = (billable_ops ÷ 1_000_000) x plan_rate

billable_storage_GB  = MAX(storage_used_GB - free_storage_for_plan, 0)
storage_cost         = billable_storage_GB x storage_rate_for_plan

total_monthly_cost   = ops_cost + storage_cost
```

Applying those formulas to the three sample workloads:

Small app on the Starter plan:
- `total_ops` = `500` x `10` x `30` = `150000`
- `billable_ops` = `50000`
- `ops_cost` = (`50000` ÷ `1000000`) x $`18` ≈ $ `0.90`
- `storage_cost` = 0 GB (0.5 GB is below the 1 GB free tier)

`total_monthly_cost` ≈ $ 0.90 per month

Medium app on the Pro plan:
- `total_ops` = `5000` x `40` x `30` = `6000000`
- `billable_ops` = `5900000`
- `ops_cost` = (`5900000` ÷ `100000`) = `5.9` x $`8` ≈ $ `47.20`
- `storage_cost` = (`6` GB - `5` GB) x $`1.50` = $ `1.50`

`total_monthly_cost` ≈ $ 48.70 per month

Large app on the Business plan:
- total_ops = `50000` x `60` x `30` = `90000000`
- billable_ops = `89900000`
- ops_cost = (`89900000` ÷ `100000`) = `89.9` x $`6` ≈ $ `539.40`
- storage_cost = (`40` GB - `10` GB) x $1 = $ `30.00`

`total_monthly_cost` ≈ $ 569.40 per month

Swap in your own MAU count, activity level, and storage footprint to project costs on any plan.

### Why aren't cached queries billed separately?
