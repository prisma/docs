...
### How do I estimate my Prisma Postgres operation usage?

You can estimate your operation usage by integrating an application performance monitoring tool like Prometheus with Prisma ORM's [`metrics` preview feature](/orm/prisma-client/observability-and-logging/metrics). Once you enable the `metrics` preview feature, you can look at the `prisma_client_queries_total` metric. 
...
We will demonstrate three example workloads and estimate bills for small, medium, and large workloads. Each combine a realistic number of monthly active users (MAUs), a typical level of daily activity per user, and a rounded storage footprint. Here are the assumptions about the workloads:
...
Here are the rates for each pricing plan:
...
- **Business Annual plan** - \$4 per million operations
  - Storage - 10 GB free then \$1 per additional GB

You can also learn more about the rates for each plan on the [pricing page](https://www.prisma.io/pricing?utm_source=docs).
...
You can place in your own MAU count, activity level, and storage footprint to project costs on any plan using the equations above.

:::

We will associate each workload with a plan and its corresponding rates, for example, the **Starter plan** for the small workload, the **Pro plan** for the medium workload, and the **Business Annual plan** for the large workload. Then we will apply the equations to the example workloads to generate a rough estimate of a monthly bill. For example:
...
- `ops_cost` = (`89900000` ÷ `100000`) = `89.9` x $`4` ≈ $ `359.6`
