...
A hobby or early-stage side-project with ~`500` MAUs. Each user clicks around `10` times a day, and the entire database uses about `0.5` GB of storage. Based on the assumptions, you would calculate the monthly bill using the following equations:
...
A growing SaaS product serving ~`5000` MAUs. Power users average `40` actions per day, and the app stores roughly `6` GB of data. Based on the assumptions, you would calculate the monthly bill using the following equations:
