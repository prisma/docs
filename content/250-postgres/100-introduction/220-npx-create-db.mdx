...
`create-db` is an open-source CLI tool that provisions [**temporary Prisma Postgres databases**](/postgres) with a single command.
...
You can also use the following aliases, which behave exactly the same way as `npx create-db@latest`:
