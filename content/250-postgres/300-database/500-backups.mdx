...
On [Pro and Business plans](https://www.prisma.io/pricing), Prisma Postgres automatically creates snapshots of your database to support recovery and backup workflows.

Snapshots are created _daily_, but only on days when the database has activity. For example, if your database is active on 5 out of the last 12 days, you will have 5 snapshots available.

Depending on your plan, you will see a different number of available snapshots:

| Plan     | Snapshot retention |
| -------- | ------------------ |
| Pro      | 7 most recent      |
| Business | 30 most recent     |
...
## Creating a backup file via `@prisma/ppg-tunnel`
