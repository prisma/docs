...
### Custom engine binaries

Reasons to set custom engine binaries include:

* Automated download not possible
* Creating your own engine binary - e.g. to support a particular OS

#### Project `.env` file

#### Global environment variable

To set the `PRISMA_QUERY_ENGINE_BINARY` variable globally:

<TabbedContent tabs={[<FileWithIcon text="schema.ts" icon="Unix, MacOS"/>  , <FileWithIcon text="Windows" icon="display"/>]}>	

<tab>	

```bash-symbol
export PRISMA_QUERY_ENGINE_BINARY=/Users/customuser/Dev/repro/repro-binary-log-version/query-engine-custom npx prisma -v
```
...
```bash-symbol
set PRISMA_QUERY_ENGINE_BINARY=/Users/j42/Dev/repro/repro-binary-log-version/query-engine-custom
```
