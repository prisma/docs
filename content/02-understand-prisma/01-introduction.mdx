...
Every project that uses a tool from the Prisma toolkit starts with a [Prisma schema file](../cconcepts/tools-and-interfaces/prisma-schema/prisma-schema-file). The Prisma schema allows developers to define their _application models_ in an intuitive data modeling language. It also contains the connection to a database and defines a _generator_:
...
> **Note**: The Prisma schema has powerful data modeling features. For example, it allows you to define "Prisma-level" [relation fields](../cconcepts/tools-and-interfaces/prisma-schema/relations) which will make it easier to work with [relations in the Prisma Client API](../cconcepts/tools-and-interfaces/prisma-client/relation-queries). In the case above, the `posts` field on `User` is defined only on "Prisma-level", meaning it does not manifest as a foreign key in the underlying database.
...
On this page, the focus is on the data model. You can learn more about [Data sources](../cconcepts/tools-and-interfaces/prisma-schema/data-sources) and [Generators](../cconcepts/tools-and-interfaces/prisma-schema/generators) on the respective docs pages.

#### Functions of Prisma models

The data model is a collection of [models](../cconcepts/tools-and-interfaces/prisma-schema/models). A model has two major functions:
...
- Generating the data model from [introspecting](../cconcepts/tools-and-interfaces/introspection) a database
- Manually writing the data model and mapping it to the database with [Prisma Migrate](../cconcepts/tools-and-interfaces/prisma-migrate)

Once the data model is defined, you can [generate Prisma Client](../cconcepts/tools-and-interfaces/prisma-client/generating-prisma-client) which will expose CRUD and more queries for the defined models. If you're using TypeScript, you'll get full type-safety for all queries (even when only retrieving the subsets of a model's fields). 
...
Learn more about the available operations in the [Prisma Client API reference](../cconcepts/tools-and-interfaces/prisma-client/api).
...
1. Optionally (re-)configure your Prisma Client API ([learn more](../cconcepts/tools-and-interfaces/prisma-client/configuring-the-prisma-client-api))
...
1. Manually adjust your [Prisma data model](../cconcepts/tools-and-interfaces/prisma-schema/data-model)
