...
You can configure Prisma Pulse for an existing Railway database by enabling logical replication.

## Configure Logical replication for your Railway database
...
## Railway templates for Prisma Pulse (Deprecated)
...
These templates are available for quick experimentation with Prisma Pulse, but they are **no longer actively maintained**. For a more supported and up-to-date setup, we recommend using [Prisma Postgres](/postgres), which is quick to set up and makes enabling Prisma Pulse straightforward.
