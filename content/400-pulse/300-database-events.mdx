...
You can configure **Event persistence** for Pulse in your Console [project](/platform/about#project). Only with **Event persistence** enabled, you will be able to take advantage of "at least once" and "right order" delivery guarantees by Prisma Pulse via the `stream()` API.
...
- **Events reads**: The number of database events _read_ and _delivered_ by Pulse
- **Event storage**: The amount of disk space the stored events consume (in GiB)

See the [subscription plans](https://www.prisma.io/pricing) for more details.

<Admonition type="warning">

This pricing applies regardless of whether you use `subscribe()` or `stream()`.
