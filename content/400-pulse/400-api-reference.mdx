...
const stream: PulseSubscription<
  PulseCreateEvent<{ /* model fields */ }> | 
  PulseUpdateEvent<{ /* model fields */ }> | 
  PulseDeleteEvent<{ /* model fields */ }>
...
Passing an empty object to the `subscribe` function will also behave the same as passing no argument to the function:
...
The events received by the async iterable returned by `.stream()` have a predefined structure. There are a few fields that all events have in common and that will always carry a value:

| Name        | Type     | Example value | Description                                                                                 |
| :---------- | :------- | :------------ | :------------------------------------------------------------------------------------------ |
| `id`        | `string` | `0/1981650`   | A unique identifier / idempotency key.                                                      |
| `modelName` | `string` | `User`        | The name of the model affected by this event. This is a model name from your Prisma schema. |
| `action`    | `string` | `create`      | The kind of write-operation performed in the database: `create`, `update` or `delete`.      |

There are two more fields that will carry a value depending on the `action`:
- `before`: Carries a value only for `update` and `delete` operations.
- `after`: Carries a value only for `update` operations.

Generally, `before` and `after` have the type of the model that's affected by the event.
