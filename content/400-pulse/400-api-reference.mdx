...
| `before`    | `User`   | `null`                            | An object with the _old_ values of the record that was just updated. This only works with when the `REPLICA IDENTITY` in your database is set to `FULL`. Otherwise the value will always be `null`. |
