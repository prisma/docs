...
- Usage of `stream()` requires [event persistence](/pulse/events#event-persistence) to be enabled in your Pulse project.
...
- Events delivered with `subscribe()` are _epheremal_, meaning they won't be delivered if your subscription isn't active during the time when the event happens in the database (e.g. because your server is down).
...
| `before`    | `User`   | null                              | An object with the _old_ values of the record that was just created (i.e. _before_ the `update` operation). |
| `after`     | `User`   | See `after` in the example below. | An object with the _new_ values of the record that was just created (i.e. _after_ the `update` operation).  |
...
| `deleted`   | `User`   | `{ id: 3 }`   | An object with the `id` value of the record was just deleted.                               |
