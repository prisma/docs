...
The size limit of [databasase events](/pulse/database-events) depends on your [subscription plan](https://www.prisma.io/pricing).

Note that database events are base64 encoded for transmission. The size limited applies to the _encoded_ not the _original_ size of the event.

If your events exceed the size limit, they will be dropped by Prisma Pulse and won't arrive on the receiver side. You can see how many events have been dropped by Prisma Pulse in the Pulse Dashboard in the [Console](https://console.prisma.io/).
