...
If you're using [`"moduleResolution": "bundler"`](https://www.typescriptlang.org/docs/handbook/modules/reference.html#bundler) in your `tsconfig.json` file, and your Pulse client extension version is `1.2.0` or greater, you need to import the Pulse Client extension in a way that matches your target runtime environment:
...
This is needed because Prisma Pulse extension offers separate implementations tailored for various runtimes, such as Node.js and Cloudflare Workers. This will instruct TypeScript to match the correct type definitions of the Prisma Pulse extension, as well as any other packages that expose multiple entrypoints.

This runtime-specific import approach also resolves the following error:

```
Cannot find module '@prisma/extension-pulse' or its corresponding type declarations.
```

Alternatively, you can ensure the types are working for the target-runtime by configuring your `tsconfig.json` file and explicitly setting [`customConditions`](https://www.typescriptlang.org/tsconfig/#customConditions) to either `node` or `workerd` depending on your target runtime:
...
We recommend this approach if you're building a package that's using the Prisma Pulse Client extension as a dependency.

:::

This configuration will also address the same module resolution error and ensure the correct runtime-specific types of the Prisma Pulse extension is used.