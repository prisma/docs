...
Note that because Neon doesn't provide [superuser](https://www.postgresql.org/docs/current/role-attributes.html) access to the database, it's not available on the Starter plan.

## Does Pulse store my database events?

Pulse persists database events if you enable the **Event persistence** feature in your [project](/platform/about#project) in the Console. This allows usage of the `stream()` API and take advantage of delivery guarantees.

## How does persisting events impact pricing?

Pulse stores the events that happen in your database in the exact same shape that they're delivered. 

Usage of Pulse is billed according to these factors:
- **Events in**: The number of database events captured by Pulse
- **Events out**: The number of database events delivered by Pulse and acknowledged by the subscriber
- **Storage**: The amount of disk space the stored events consume (in GiB)
