...
1. decide whether you want to use the **Automatic setup** for [**Database replication**](/pulse/concepts#logical-replication) (only available on paid plans)
1. enabled **Event persistence** to use delivery guarantees with `.stream()` (or disable it if you want to use `.subscribe()` for fully ephemeral events without delivery guarantees)
