...
When using `"moduleResolution":"bundler"` in your `tsconfig.json` file and your Pulse client extension version is `1.2.0` or greater, you must import the Pulse Client extension in a way that matches your target runtime environment:
...
If you don't use the [runtime-specific import aproach](/pulse/faq#how-do-i-import-the-pulse-client-extension-when-using-moduleresolutionbundler-in-my-tsconfigjson), you may run into the following error when using the `@prisma/extension-pulse` in your application code:
