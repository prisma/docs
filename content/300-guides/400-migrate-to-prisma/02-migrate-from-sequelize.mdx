...
For this guide, we'll use a REST API built with Express as a [sample project](https://github.com/prisma/migrate-from-sequelize-to-prisma) to migrate to Prisma. It has four models/entitites:
...
In this guide, the route handlers are located in the `src/controllers` directory. The models/entities are located in the `src/models` directory. From there, they are pulled into a central `src/routes.js` file which is used to set up the required routes in `src/index.js`:
...
```
DATABASE_URL="sqlserver://localhost:1433;database=blog-sequelize;user=alice;password=myPassword42;trustServerCertificate=true;encrypt=DANGER_PLAINTEXT"
```
...
We'll adjust the `createdAt` and `updatedAt` fields in our models. Sequelize doesn't add the `DEFAULT` constraint to `createdAt` when creating the tables in the database. 
Therefore, we'll add `@default(now())` and `@updatedAt` attributes to the `createdAt` and `updatedAt` columns respectively.
...
First, to set up the `PrismaClient` instance that you'll use to send database queries from the various route handlers. Create a new file called `prisma.ts` in the `src` directory:
...
```js file=src/controllers/GetPostByIdAction.ts
const getPostById = async (req, res) => {
  const { postId } = req.params
...
To set the `createdAt` field in your model, add the `default(now())` to the column. In order to set the `updatedAt` column, update your model by adding the `@updatedAt` attribute to the column.
...
Similar to the `belongsToMany()` association method in Sequelize, Prisma allows you to [model many-to-many relations _implicitly_](../../concepts/components/prisma-schema/relations#implicit-vs-explicit-many-to-many-relations). That is, a many-to-many relation where you do not have to manage the [relation table](../../concepts/components/prisma-schema/relations#relation-tables) (also sometimes called JOIN table) _explicitly_ in your schema. Here is an example with TypeORM:
