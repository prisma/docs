...
- A new directory called `prisma` with a `schema.prisma`: Your Prisma schema file that specifies your database connectionmodels
- `.env`: A [`dotenv`](https://github.com/motdotla/dotenv) at the root of your project, if it didn't exist, to configure your database connection URL as an environment variable
...
Update the 1-n relationship between `Post` <-> `User`

<!-- TODO: tab with diff and another with final version -->
...
<!-- highlight the mapping of author to authorId -->

Update the m-n between `Post` <-> `Category` references
...
model posts {
  ...
  categories   categories[] @relation(fields: [category_ids], references: [id])
...
If your schema includes a [`versionKey`](https://mongoosejs.com/docs/guide.html#versionKey), change it to an optional field and add the `@ignore` attribute to the `v` field to exclude it from the generated Prisma Client.
...
<!-- TODO: add @default(false) to published field -->
...
You can model relations between documents by either using:

- Embedded documents
- References

<!-- TODO: expound on this and how to model the different kinds of relations -->
