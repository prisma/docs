...
For a more optimal development experience when working with Prisma, refer to [editor setup](/guides/development-environment/editor-setup) – syntax highlighting, formatting, auto-completion and a lot more cool features.
...
If your schema includes a [`versionKey`](https://mongoosejs.com/docs/guide.html#versionKey), update it by adding the `@ignore` attribute and adding a `@default(0)` to the `v` field. This means the field will be excluded from the generated Prisma Client. Prisma does not handle document versioning.
