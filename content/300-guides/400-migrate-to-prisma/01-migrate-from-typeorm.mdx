...
For this guide, we'll use a REST API built with Express as a [sample project](https://github.com/prisma/migrate-from-typeorm-to-prisma) to migrate to Prisma. It has four models/entitites:
...
Note that if you are running on macOS **[and a version of Prisma before 2.15.0](https://github.com/prisma/tiberius/issues/65)**, you must use `encrypt=DANGER_PLAINTEXT` to work around the current [TLS limitation](https://github.com/prisma/prisma/issues/4075):
...
Note that the way how Prisma Client includes the `author` relation is absolutely type-safe. The TypeScript compiler would throw an error if you were trying to include a relation that does not exist on the `User` model.
...
export async function filterPostsAction(req, res) {
  ...
  const filteredPosts = await postRepository.find({
    where: [{ title: Like(`%${searchString}%`) }, { content: Like(`%${searchString}%`) }],
  })
...
Similar to the `@manyToMany` decorator in TypeORM, Prisma allows you to [model many-to-many relations _implicitly_](../../concepts/components/prisma-schema/relations#implicit-vs-explicit-many-to-many-relations). That is, a many-to-many relation where you do not have to manage the [relation table](../../concepts/components/prisma-schema/relations#relation-tables) (also sometimes called JOIN table) _explicitly_ in your schema. Here is an example with TypeORM:
