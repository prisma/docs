...
1. Add the following package to your project, [`jest-mock-extended`](https://github.com/marchaos/jest-mock-extended). This will be used to assign the mocked client to a variable, which will then be used to mock the Prisma client.

```terminal
npm install jest-mock-extended --save-dev
```

2. Create a `context.ts` file and add the following.
...
3. Next, inside your `__tests__` folder create a new file `unit-tests.ts` and import the new mock contexts.
...
Open up the `unit-tests.ts` and modify it by adding the following test.
...
// This function mocks updating a user in the db. The user is found by their id.
function updateUsername(user: UpdateUser, ctx: Context) {
...
test('should update a users name ', async () => {
  // The user has added their surname to their account
