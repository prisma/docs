...
When using a single `.env` file to hold different connection URLs to each of your environments, you run the risk of accidentally using your production database for testing instead of a dedicated testing database. 

Given that the data in the database is wiped and seeded before and after each test, this could lead to you losing all of your production data.

One approach you could take to solve this potential issue, is to have multiple `.env` files which each represent different environments. In practice this means you create a file for each of your environments:
...
It is not recommended to keep your production credentials stored locally, even if they are git ignored. 
