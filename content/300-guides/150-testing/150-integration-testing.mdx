...
If you are using MySQL as your database of choice you will have to change the `afterAll` function to perform the truncate successfully. 

This is due to the foreign key constraints, they will need to be removed prior to truncating each table, then set back again once the truncate is finished. These operations are performed within a transaction.

```ts file=__tests__/create-order.ts highlight=2-10;delete|1-33;add
...
> **Note**: MySQL will throw an error if your table names clash with a reserved word. One way to escape this issue is to use quotes around the table name in the raw query. 
Another altogether easier way to handle this is to make sure your table names (models) don't clash with the [MySQL reserved words](https://dev.mysql.com/doc/refman/8.0/en/keywords.html).
