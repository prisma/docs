...
With Docker and Docker compose both installed on your machine you can add use them in your project.
...
3. To create the container in a detached state so that you can continue to use the terminal tab, run the following command from a terminal:

```terminal
docker compose up -d
```

4. Next you can check that the database has been created by executing a `psql` command inside the container.
...
1322e42d833f   postgres:latest   "docker-entrypoint.s…"   2 seconds ago   Up 1 second   0.0.0.0:5433->5432/tcp   integration-tests-prisma
```
...
💾 Run `psql` in the container, login with the created user and check the database is created:
