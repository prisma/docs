...
it("should show 'Out of stock' message if productId doesn't exit", async (done) => {
  // The existing customers email
  const customer: Customer = {
    email: "harry@hogwarts.io",
  };
...
```json file=package.json
  "scripts": {
    ...
    "test": "yarn docker:up && yarn migrate && jest -i"
  },
```
...
1. Runs a migration on the database using your projects schema, this creates the tables in the containers database.
