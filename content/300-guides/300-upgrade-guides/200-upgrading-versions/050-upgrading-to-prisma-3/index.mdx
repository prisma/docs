...
See [Referential action upgrade paths](./referential-actions) to understand how to proceed.
...
We changed the convention followed by Prisma to name constraints and indexes. We also introduced a `map` attribute in the PSL to allow defining these names explicitly from the Prisma schema.
