...
Prisma 4 introduces a number of **breaking changes** when you upgrade from Prisma 3 version (`3.x.x`). This guide explains how this upgrade might affect your application and gives instructions on how to handle any changes.
...
We recommend that you re-test your usage of `$queryRaw` and double-check that the types you pass into your raw queries match the types that PostgreSQL expects.
...
This is because PostgreSQL’s `length(string)` function expects a string as the parameter. To fix this, explicitly coerce the incoming number into a string as follows:
...
If you now get a type error when you pass `Prisma.DbNull` as the value of a JSON field, then this probably indicates a bug in your code that our types did not catch before v4.0.0. The field where you tried to store `DbNull` is probably not nullable in your schema. As a result, a literal "DbNull" string was stored in the database instead of NULL.
