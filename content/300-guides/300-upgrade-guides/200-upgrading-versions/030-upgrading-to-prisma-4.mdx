...
This is because PostgreSQL’s `length` function expects `text` as input. Prisma used to silently coerce `42` to `text`, but does not do this in v4.0.0. To fix this, explicitly cast `42`to `text` as follows:
