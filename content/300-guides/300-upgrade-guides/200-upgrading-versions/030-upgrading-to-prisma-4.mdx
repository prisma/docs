...
2. Review the Prisma schema validation errors (via `prisma validate`, or via the Prisma VS Code extension).
...
4. Run `prisma db pull` to upgrade the Prisma schema to all new capabilities (e.g. `extendedIndexes`).
