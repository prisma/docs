...
Prisma 4 introduces a number of **breaking changes** if you are upgrading from an Prisma 3 version (`3.x.x`). This guide explains how this upgrade might affect your application and gives instructions on how to handle any changes.
...
- upgrade to the new Prisma 4 packages following [these instructions](#upgrading-the-prisma-and-prismaclient-packages-to-prisma-4)
...
After upgrading to Prisma 4, any one-to-one relations without a `@unique` attribute on the relation scalar will trigger a validation error. To upgrade, you will need to:
...
After upgrading to Prisma 4, any implicit many-to-many relations with a `references` argument will trigger a validation error. To upgrade, you will need to:
