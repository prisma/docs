...
2. Backup your existing `prisma.schema` file
3. Use `npx prisma db pull --force` in order to overwrite your existing schema (including native types) to those that are on your CockroachDB instance.
4. Review changes between your schema backup and the new schema generated by `db pull`. You can either use the new schema as is, or update it to include your preferred spacing, comments, etc.
