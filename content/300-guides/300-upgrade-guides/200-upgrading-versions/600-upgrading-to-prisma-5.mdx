...
```terminal
yarn upgrade prisma@latest @prisma/client@latest
```
...
```terminal
pnpm up prisma@latest @prisma/client@latest
```
...
Prisma 5 includes some minimum version changes for Node.js, TypeScript, PostgreSQL, and SQLite. To use Prisma version 5.0.0 and up, you will need to have at least the minimum versions below:
...
You could also create a reusable error adapter which can be used in a `catch` in your code.
...
With Prisma version 5.0.0, we are requiring the CockroachDB instances be connected to with a `cockroachdb` provider. Previously, we had accepted `postgresql` as well, but we are removing that option.

If you were using [native database types](/reference/api-reference/prisma-schema-reference#postgresql) and also the `postgresql` provider, you will most likely need to `diff` your database to see if anything has changed:

1. Update your provider from `postgresql` to `cockroachdb`
2. use `npx prisma db pull --force` in order to overwrite your existing schema (including native types) to those that are on your CockroachDB instance.
3. delete your existing migrations. We will be [performing a baseline](/getting-started/setup-prisma/add-to-existing-project/relational-databases/baseline-your-database-typescript-cockroachdb) in order to make your local setup agree with your existing CockroachDB instance.
4. Perform the [baselining steps](/getting-started/setup-prisma/add-to-existing-project/relational-databases/baseline-your-database-typescript-cockroachdb). After these steps, you'll have migrated successfully from the `postgresql` provider to the `cockroachdb` provider!
