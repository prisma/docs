...
- Prisma 2 makes the features of Prisma 1 more modular and splits them into dedicated tools:
  ...
  - Prisma Migrate: Data modeling and migrations (formerly `prisma1 deploy`).
...
The database schema that is created when running `prisma1 deploy` in Prisma 1 is only partially compatible with the one that Prisma 2 creates. This section gives a quick overview of the general incompatibilities and the potential workarounds.
