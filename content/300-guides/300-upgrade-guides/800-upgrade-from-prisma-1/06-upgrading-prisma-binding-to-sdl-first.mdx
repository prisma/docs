...
![Downloading the GraphQL schema with GraphQL Playground](https://res.cloudinary.com/prismaio/image/upload/v1628761154/docs/BbgLyp2.png)
...
```js line-number highlight=10-13;delete|14;add copy
...
```js line-number highlight=8-23;add copy
const resolvers = {
  Query: {
...
```js line-number highlight=11-26;add copy
const resolvers = {
  Query: {
    // ... your query resolvers
  },
...
```js line-number highlight=14-22;add copy
const resolvers = {
  Query: {
    // ... your query resolvers
  },
...
```js line-number highlight=17-25;add copy
const resolvers = {
  Query: {
    // ... your query resolvers
  },
...
```js line-number highlight=3-11;normal copy
const resolvers = {
  Query: {
...
```js line-number highlight=3-7;normal copy
const resolvers = {
  Query: {
    user: (_, args, context) => {
...
```js line-number highlight=3-15;normal copy
const resolvers = {
  Mutation: {
    createDraft: (_, args, context, info) => {
...
```js line-number highlight=3-12;normal copy
const resolvers = {
  Mutation: {
    updateBio: (_, args, context, info) => {
...
```js line-number highlight=3-13;normal copy
const resolvers = {
  Mutation: {
    addPostToCategories: (_, args, context, info) => {
      const ids = args.categoryIds.map((id) => ({ id }))
