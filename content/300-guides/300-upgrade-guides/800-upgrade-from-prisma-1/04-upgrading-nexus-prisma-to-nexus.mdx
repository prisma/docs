...
```ts line-number highlight=1-3;delete copy
...
```ts line-number highlight=1-3;add copy
...
```ts file=./src/index.ts line-number highlight=2,12-14;add|1,8-11;delete copy
...
```ts file=./src/types.ts highlight=2,6;add|1,5;delete copy
import { Prisma } from './generated/prisma-client'
import { PrismaClient } from '@prisma/client'
...
```ts line-number highlight=6,9,12,13;normal copy
queryType({
  definition(t) {
    t.list.field('posts', {
      type: 'Post',
...
```ts line-number highlight=6,12-17;normal copy
...
```ts line-number highlight=11-19;normal copy
...
```ts highlight=13-23;normal copy
const Mutation = mutationType({
  definition(t) {
    t.field('updateBio', {
      type: 'User',
...
```ts line-number highlight=14-21;normal copy
const Mutation = mutationType({
  definition(t) {
    t.field('addPostToCategories', {
      type: 'Post',
