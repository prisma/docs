...
- introspected your database and resolved potential [schema incompatibilities](/guides/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgres)
...
![Exposing Prisma model fields with t.model](https://res.cloudinary.com/prismaio/image/upload/v1628761153/docs/p6Ke4KS.png)
...
```ts line-number highlight=10-13;add copy
const User = objectType({
  name: 'User',
  definition(t) {
    t.model.id()
...
```ts line-number highlight=2;normal copy
export const schema = makeSchema({
...
![Using t.crud to generate resolvers](https://res.cloudinary.com/prismaio/image/upload/v1628761153/docs/4nuiXcJ.png)
...
```ts line-number highlight=3-6;add copy
const Query = queryType({
  definition(t) {
    t.crud.users({
      filtering: true,
...
```ts line-number highlight=5-9;add copy
const Query = queryType({
  definition(t) {
    // ... previous queries
...
```ts line-number highlight=9-21;add copy
const Query = queryType({
  definition(t) {
    // ... previous queries
...
```ts line-number highlight=1,3-9,15-23;add copy
...
```graphql line-number highlight=3,7-10;normal copy
type Query {
  posts(searchString: String): [Post!]
  user(userUniqueInput: UserUniqueInput!): User
  users(after: UserWhereUniqueInput, before: UserWhereUniqueInput, first: Int, last: Int, orderBy: Enumerable<UserOrderByInput>, skip: Int, where: UserWhereInput): [User!]!
...
```ts line-number highlight=22-29;add copy
const UserUniqueInput = inputObjectType({
  name: 'UserUniqueInput',
  definition(t) {
    t.string('id')
...
![Generating resolvers with t.crud](https://res.cloudinary.com/prismaio/image/upload/v1628761153/docs/kgweyv1.png)
...
```ts line-number highlight=3-5;add copy
const Mutation = mutationType({
  definition(t) {
...
```ts line-number highlight=5-12;add copy
const Mutation = mutationType({
  definition(t) {
    // ... previous mutations
...
```graphql line-number highlight=3;normal copy
type Mutation {
  createUser(data: UserCreateInput!): User!
  createDraft(title: String!, content: String, authorId: String!): Post!
}
...
```ts line-number highlight=12-22;add copy
const Mutation = mutationType({
  definition(t) {
    // ... previous mutations
...
```ts line-number highlight=5-14;add copy
const Mutation = mutationType({
  definition(t) {
    // ... previous mutations
...
```graphql line-number highlight=4;normal copy
type Mutation {
  createUser(data: UserCreateInput!): User!
  createDraft(title: String!, content: String, authorId: String!): Post!
  updateBio(bio: String!, userUniqueInput: UserUniqueInput!): User
...
```ts line-number highlight=14-26;add copy
const Mutation = mutationType({
  definition(t) {
    // ... previous mutations
...
```graphql line-number highlight=5;normal copy
type Mutation {
  createUser(data: UserCreateInput!): User!
  createDraft(title: String!, content: String, authorId: String!): Post!
  updateBio(bio: String, userUniqueInput: UserUniqueInput!): User
...
```ts line-number highlight=13-23;add copy
const Mutation = mutationType({
  definition(t) {
    // ... mutations from before
    t.field('addPostToCategories', {
