...
1. You set up Prisma by installing the Prisma versions 2._x_ and later CLI and running `prisma init`.
1. You connect to your database and introspect it with `prisma db pull`.
...
1. You invoke the Upgrade CLI with `prisma-upgrade`.
...
1. You run the `prisma-upgrade` command again.
