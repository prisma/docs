---
title: 'Upgrade Prisma Migrate'
metaTitle: 'Upgrade from legacy Prisma Migrate to the most recent Prisma Migrate.'
---

<TopBlock>

The following guide describes how to upgrade from legacy Prisma Migrate (an Experimental feature available until version 2.12.0) to Prisma Migrate.

</TopBlock>

### Introduction

For users who are transitioning from the experimental Prisma Migrate, the conceptual flow is pretty much the same.

For those coming from a third-party migration tool, it's worth calling out that the flow with this new version of Prisma Migrate no longer implies modifying your database schema and then relying on introspection to bring in the changes from your database schema into your Prisma schema.

You can read more on the different flows in the corresponding section.

1. **Make sure your Prisma schema is in sync with your DB schema**. If you are using a previous version of Prisma Migrate, this should be the case. You can always use `prisma introspect` to pull the DB schema into your Prisma schema file.
2. Delete or rename the current migrations folder so that it does not collide with this new version
3. **Initialize a new migration history** by calling `prisma migrate dev`. Prisma Migrate will do the following
...
2. Call `prisma migrate dev` to reset the development database
