...
This means that you currently have to be careful when searching for `null` and undefined fields. Searching for records with `name: null` will return both records:
...
Instead, to exclude undefined fields, use the [`isSet` filter](/reference/api-reference/prisma-client-reference#isset) to explicitly search for fields which are either `null` or not set:
...
```ts
const findNullNotUndefined = await prisma.user.findMany({
  where: {
    OR: [
      {
        name: null,
...
console.log(findNullNotUndefined)
```
