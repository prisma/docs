...
The deletion behavior `CASCADE` in this case means that when you delete a `AnotherUser` record that's referenced by one or more `AnotherPost` records, these `AnotherPost` records will be deleted as well.
