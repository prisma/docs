...
This page explains how to configure cascading deletes on [foreign key constraints](https://www.sqlite.org/foreignkeys.html#fk_actions) (relations) in your SQLite database.
...
Next, make sure that your SQLite database server is running. Then execute the following command in your terminal to create a new database called `CascadingDeletesDemo`:
...
Create a new file named `cascading-deletes-no-action.sql` and add the following code to it:
...
The database connection URL is set via an environment variable. The Prisma CLI automatically supports the [`dotenv`](https://www.npmjs.com/package/dotenv) format which automatically picks up environment variables defined in a file named `.env`.
