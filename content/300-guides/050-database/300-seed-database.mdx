...
## Configuring your seed script

Prisma's integrated seeding functionality expects a command in the `"seed"` key in the `"prisma"` key of your `package.json` file. This points to a file which holds your seed script. The location of the file is optional, although we recommend placing it in the `/prisma` folder by convention.
...
1. Create a new file named `seed.ts`.
...
Prisma Migrate integrates seamlessly with your seeds, assuming you follow the steps in the previous section. When Prisma Migrate resets the development database, seeding is triggered automatically. When Prisma Migrate resets the development database, seeding is triggered automatically. 
