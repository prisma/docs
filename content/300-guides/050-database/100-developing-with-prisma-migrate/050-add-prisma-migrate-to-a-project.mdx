...
1. Run the following command to create a `migrations` directory inside with your preferered name. This example will use `0_init` for the migration name

Run the following commands to create the first migration **without applying it**, in case you need to modify the initial migration:
...
Then `0_` is important because Prisma Migrate applies migrations in a [lexographic order](https://en.wikipedia.org/wiki/Lexicographic_order). You can use a different value such as the current timestamp.
...
In order to fix these situations have a look at [Production Troubleshooting](/guides/database/production-troubleshooting#fixing-failed-migrations-with-migrate-diff-and-db-execute)
