...
1. Run the following command to create the first migration **without applying it**, in case you need to [modify the initial migration](#modify-initial-migration-to-include-feautres-not-supported-by-the-prisma-schema-language):
...
### Account for features not supported by Prisma Schema Language
...
1. Modify the generated SQL. For example:
   ...
   * Option 1: Add additional SQL - the following example creates a partial index:
   ...
   * Option 2: Replace the entire migration file with the result of a database dump ([`mysqldump`](https://dev.mysql.com/doc/refman/8.0/en/mysqldump.html), [`pg_dump`](https://www.postgresql.org/docs/12/app-pgdump.html))
...
1. Run the following command to baseline a migration - this example marks the migration genreated by the [Initialize migration history](#initialize-migration-history) step as already applied:
