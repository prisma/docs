...
However, when you `prisma migrate deploy` your migrations to databases that cannot be reset - such as production - you **do not want to include the initial migrations**. The target database is already in the state represented by the initial migrations, and attempting to create tables and fields that already exist will most likely result in an error.
