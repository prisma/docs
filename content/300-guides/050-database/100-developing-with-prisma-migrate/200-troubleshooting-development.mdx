---
title: Migration troubleshooting in development
...
* [Patching / hotfixing production databases](../patching-production)
...
In a development environment, switching between feature branches can result in a history conflict because the `_prisma_migrations` table contains migrations from `branch-1`, and switching to `branch-2` might cause some fo those migrations to disappear.

> **Note**: You should [never purposefully delete or edit a migration](../../../concepts/components/prisma-migrate#do-not-edit-or-delete-migrations-that-have-been-applied), as this might result in discrepancies between development and production.
...
- The database schema was changed _without_ using migrations - for example, by using [`prisma db push`](../../../reference/api-reference/command-reference#db-push--preview) or manually changing the database schema.

> **Note**: The [shadow database](../../../concepts/components/prisma-migrate/shadow-database) is required to detect schema drift, and can therefore only be done in a development environment.
...
- You [modify a migration before running it](advanced-migrate-scenarios) and introduce a syntax error
...
You will see the following error if you attempt to run Prisma Migrate commands in an environment that uses PgBouncer:
