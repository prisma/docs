...
One of the challenges when renaming fields is that it can lead to downtime in the time frame between applying the rename migration and deloying a new version of the application code which uses the renamed field.

This can be mitigated by breaking down the steps required to rename a field into a series of discrete steps designed to introduce the change gradually. This pattern is known as the _expand and contract pattern_.

The pattern takes two components into consideration: your application code accessing the database, and the database schema which you intend to alter.
...
7. _Contract_: remove the `bio` create a migration to remove the `bio` field
...
By using this approach, you avoid potential downtime that column renames are prone to and reduce the amount of coordination required between applying the migration and deploying the updated application code.
