...
When generating a migration SQL file, you may wish to also create a "down migration" SQL file that reverses the schema changes in your migration file.

This guide explains how to use Prisma Migrate's [`migrate diff` command](/reference/api-reference/command-reference#migrate-diff) to create a down migration, and how to then apply it to your production database with the [`db execute`](/reference/api-reference/command-reference#db-execute) command.
...
- The down migration will revert your schema, but other changes to data and application code that are carried out as part of the up migration will not be reverted. For example, if you have a script that changes data during the migration, this data will not be changed back when you run the down migration.
...
2. Generate the SQL file for the down migration. To do this, you will use `migrate diff` to make a comparison:
   ...
   and output this to a SQL script.
