...
3. Generate the "up" migration with a name of `add_profile`:
   ...
   This will create a new `<timestamp>_add_user` directory inside the `prisma/migrations` directory, with your new `migration.sql` up migration file inside.
...
2. Use `migrate resolve` to record that you ran the down migration in the migrations table:

   ```terminal
   npx prisma migrate resolve --rolled-back ??
   ```

Use db execute to apply the SQL script for the down migration.
Use prisma migrate resolve --rolled-back the-migration-nameto record that you ran the down migration in the migrations table.
