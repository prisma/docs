...
Migrations are **applied in the order they were created**. The creation date is part of the migration subfolder name - for example, `20210316081837-updated-fields` was created on `2021-03-16-08:08:37`.
...
1. Commits the changed schema and the new migration:
      ...
      * `./prisma/schema.prisma`
      * `./prisma/migration/20210316091837-new-model/migration.sql`
...
Your `schema.prisma` and local datatabase now includes your team's changes, and the migration history includes your migration:
...
If you are using Prisma Migrate, you must commit the following files to source control:
