...
- You now want to take your production environment from its current failed state back to the state specified in your local migrations history:
  ...
  - Run the following `prisma migrate diff` command:
    ...
    ```bash
     prisma migrate diff --from-url "$DATABASE_URL_PROD" --to-migrations ./migrations --script > backward.sql
    ```
  ...
  - Run the following `prisma db execute` command:
    ...
    ```bash
     prisma db execute --url "$DATABASE_URL_PROD" --file backward.sql
    ```
  ...
  - Run the following `prisma migrate resolve` command:
    ...
    ```bash
     prisma migrate resolve --rolled-back Unique
    ```
...
- Continue applying the rest of the failed migration to get to the data model defined in your `schema.prisma` file:
  ...
  - Run the following `prisma migrate diff` command:
    ...
    prisma migrate diff --from-url "$DATABASE_URL_PROD" --to-schema-datamodel schema.prisma --script > forward.sql
  ...
  - Run the following `prisma db execute` command:
    ...
    ```bash
    prisma db execute --url "$DATABASE_URL_PROD" --file forward.sql
    ```
  ...
  - Run the following `prisma migrate resolve` command:
    ...
    ```bash
    prisma migrate resolve --applied Unique
    ```
