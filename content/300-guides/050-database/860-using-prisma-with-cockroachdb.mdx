...
Prisma provides the [`autoincrement()`](/reference/api-reference/prisma-schema-reference#autoincrement) function for generating unique identifiers. For example, this `User` model has an `id` primary key, generated using the `autogenerate()` function:

```file=schema.prisma
model User {
 id   Int    @id @default(autogenerate())
 name String
}
```
