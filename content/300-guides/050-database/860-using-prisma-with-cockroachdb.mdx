...
- **Built-in scaling:** CockroachDB comes with automated replication, failover and repair capabilities, while still supporting consistent transactions
...
There are some Cockroach-specific differences to be aware of when working with Prisma's `cockroachdb` connector:
...
Prisma now has a `sequence()` function for CockroachDB that can take these sequence parameters as arguments. As an example, in the following `User` model the sequence for the `id` field has a `start` value of `2` and is incremented by `4` every time:
...
The `sequence()` function should be used on CockroachDB in preference to `autogenerate()`. For a full list of available options, see our [reference documentation](/reference/api-reference/prisma-schema-reference#sequence).
