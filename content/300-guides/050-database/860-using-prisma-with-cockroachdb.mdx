...
The `sequence()` function should be used on CockroachDB in preference to [`autoincrement()`](/reference/api-reference/prisma-schema-reference#autoincrement). If `autoincrement()` is used, CockroachDB will generate unique values which are not following a sequence.
