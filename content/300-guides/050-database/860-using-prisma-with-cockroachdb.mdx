...
The `sequence()` function should be used on CockroachDB in preference to [`autoincrement()`](/reference/api-reference/prisma-schema-reference#autoincrement). If `autoincrement()` is used, CockroachDB will generate unique primary keys which are not following a sequence.

For a full list of available options, see our [reference documentation](/reference/api-reference/prisma-schema-reference#sequence).
