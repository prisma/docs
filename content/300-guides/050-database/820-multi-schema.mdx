...
Currently it is not allowed to have models with the same name that use the `@@schema` attribute to reference different database schemas. All models in your Prisma schema must have unique names. Otherwise, your Prisma schema will fail to validate.
