...
Additionally, you can use the [`prisma debug`](/reference/api-reference/command-reference#debug) <span class="api"></span> command to retrieve debugging information. The `prisma debug` command provides debugging information that compliments the output of the `prisma -v` command. The information includes [environment variables](/reference/api-reference/environment-variables-reference) <span class="api"></span> used for Prisma Client, Prisma Migrate, Prisma CLI, and Prisma Studio.
