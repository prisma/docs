...
The errors mentioned above occur as a result of a bundling problem during Next.js's bundling process. The `schema.prisma` and Query Engine files are expected to be found next to the generated Prisma Client. The bundling process, however, does not copy over those files to the output location of the bundle.
