...
To convert the implicit relation to an explicit one, we need to create a [relation table](https://www.prisma.io/docs/concepts/components/prisma-schema/relations/many-to-many-relations#relation-tables). The relation table will contain foreign keys referencing both tables involved in the many-to-many relation. In our example, we'll create a new model called `UserPost`
...
Once we create the relation table, we need to update our existing models that now look like this:
