...
<details>   
  <summary>Alternative: Define the constraint as a <strong>table constraint</strong></summary>
<br />
...
Create a new file named `add-single-unique-constraint-later.sql` and add the following code:
...
Create a new file named `add-multi-unique-constraint-later.sql` and add the following code:
...
In this case, the `url` is [set via an environment variable](/concepts/components/prisma-schema#accessing-environment-variables-from-the-schema) which is defined in `prisma/.env`:
