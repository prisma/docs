...
- You [modify a migration before running it](/guides/migrate/developing-with-prisma-migrate/customizing-migrations) and introduce a syntax error
...
From version [3.12.0](https://github.com/prisma/prisma/releases/tag/3.12.0), `prisma migrate deploy` does not surface a warning if a migration is edited after it is edited. The warning will be surfaced by `prisma migrate dev`.
