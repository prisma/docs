...
### User-defined arguments for seeding
...
You can then provide the `environment` argument when using `prisma db seed` by adding an [option terminator](https://github.com/commandlineparser/commandline/wiki/CommandLine-Grammar#options-terminator-dashdash---) — `--` —, followed by your custom arguments:
...
- [Snaplet](https://docs.snaplet.dev/tutorials/prisma-seed)
