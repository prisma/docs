...
## Deploying migrations in production databases

To apply migrations in staging, testing, or production environments, run the `prisma migrate deploy` command as part of your CI/CD pipeline:

```terminal
npx prisma migrate deploy
```

Refer to the [Deploying database changes with Prisma Migrate](/guides/migrate/deployment/deploy-database-changes-with-prisma-migrate) guide for more details.
