...
This guide explains how to use Prisma Migrate's [`migrate diff` command](/reference/api-reference/prisma-cli/command-reference#migrate-diff) to create a down migration, and how to apply it to your production database with the [`db execute`](/reference/api-reference/prisma-cli/command-reference#db-execute) command in the case of a failed up migration.
...
- You will not be able to use `migrate diff` to revert manually changed or added SQL in your migration files. If you have any custom additions, such as a view or trigger, you will need to:
  ...
  - Create the up migration using [`migrate dev --create-only`](/reference/api-reference/prisma-cli/command-reference#options-6), so that it can be edited before it is applied to the database
