...
Next, generate Prisma Client for the Data Proxy with the `--no-engines` flag. Later, you use the [Data Proxy](/data-platform/classic-projects/data-proxy) to connect to your database over HTTP.

```terminal
deno run -A --unstable npm:prisma generate --no-engines
```
