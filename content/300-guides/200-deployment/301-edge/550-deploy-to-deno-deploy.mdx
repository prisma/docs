...
## 3. Generate Prisma Client for Data Proxy

Next, generate Prisma Client for Data Proxy with the `--no-engine` flag. Later, you use the [Data Proxy](/data-platform/classic-projects/data-proxy) to connect to your database over HTTP.
...
You now have a database schema and a locally generated Prisma Client for Data Proxy.
...
1. Click **New Project**.
1. Connect the Prisma Data Platform to your database and enable Data Proxy.
...
Next in this guide, you use the connection string in your local `.env` file to connect to your database through Data Proxy.
...
Prisma Client for Data Proxy does not read `.env` files by default, so you must also install `dotenv-cli` locally.
...
The configuration of your local environment is now ready to send Prisma queries to the database through Data Proxy.
...
You successfully deployed a Deno application that you created in TypeScript, which uses Prisma Client for Data Proxy to connect to a PostgreSQL database.
