...
Your application is [serverless](./serverless) if the Node.js processes of your application (or subsets of it broken into functions) are started as requests come in, and each function only handles one request at a time. Your application would most likely be deployed to a Function-as-a-Service (FaaS) offering, such as [AWS Lambda](./serverless/deploy-to-aws-lambda) or [Azure Functions](./serverless/deploy-to-azure-functions)
...
Your application is [edge deployed](./edge) if your application is [serverless](#serverless-functions-faas) and the functions are distributed across one or more regions close to the user.
