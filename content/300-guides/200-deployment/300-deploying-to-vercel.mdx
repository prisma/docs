...
When accessing a database from serverless functions, we recommended using a connection pooler like PgBouncer for scalability because every function invocation may result in a new connection to the database. For more information, check out our [serverless deployment guide](../deployment#serverless-faas).
