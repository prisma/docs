...
If you look in the `package.json`, you will see that the `prisma` dependency is defined in the `dependencies` rather than the more common approach of adding it to `devDependencies`.

This is neccessary due to Vercel pruning dev dependencies during the build, thereby preventing you from running `prisma migrate deploy` during the `vercel-build` step.

### Database migartions and deployments
...
Note that this prevents Preview deployments from working, as they have no access to the database. You can resolve this by creating a separate database for Preview deployments and setting a different `DATABASE_URL` env var, and exposing it only to Preview deployments.
