...
1. Push the migration to production **without running `migrate deploy`**. Instead, mark the migration as 'already applied' so that Prisma Migrate does not attempt to apply your hotfix a second time:
