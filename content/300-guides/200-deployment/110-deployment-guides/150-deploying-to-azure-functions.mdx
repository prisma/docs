...
**TBD: I am not sure what this section means. Is it a prerequisite? Is it something we're going to set up. We need to make this clear. **
...
In Azure, a resource group is a way to group together different cloud resources. Whenever you create a resource, such as an Azure Function, you must assign it a resource group.

Because the REST API uses Azure Functions and an Azure SQL database, you must first create the resource group. Use the following command:
...
To create the Azure SQL database server, run the following command:

Before you run the command, replace `UNIQUE_DB_SERVER_NAME` with a unique name for the database and replace `CHOOSE_A_PASSWORD` with a password, and note it down.
...
In this step, you create a storage account. You will use this account to information about your functions, including state information.
