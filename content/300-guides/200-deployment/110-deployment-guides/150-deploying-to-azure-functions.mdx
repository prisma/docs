...
In this guide, you create the necessary resources in Azure, create the database schema using Prisma Migrate. You then deploy a Node.js REST API with resource endpoints that use Prisma Client to handle database operations against the Azure SQL database.
...
Run the following command to list the contents of the folder:
...
```json no-copy
{
  ...
  "createMode": null,
  "creationDate": "2023-01-17T15:39:18.247000+00:00",
  ...
  "id": "/subscriptions/YOUR_SUBSCRIPTION_ID/resourceGroups/prisma-azure-example/providers/Microsoft.Sql/servers/abw-server/databases/prisma-azure-example",
  "identity": null,
...
```json no-copy
{
  ...
  "id": "/subscriptions/YOUR_SUBSCRIPTION_ID/resourceGroups/prisma-azure-example/providers/Microsoft.Sql/servers/abw-server/firewallRules/allow-function-access",
...
**TBD: change to action and result **

> **Checkpoint:** The command outputs a large JSON object. Verify that the status of `provisioningState` is `Succeeded`:
...
In this step, you create the function app, which provides the environment for executing your function code. A function app maps to your local function project and lets you group functions as a logical unit for easier management, deployment, and sharing of resources.
...
With the `DATABASE_URL` environment variable set, you create the database schema with the [`prisma migrate deploy`](/reference/api-reference/command-reference#migrate-deploy) command.

<Admonition type="info">

**Note:** The `prisma migrate deploy` command runs migration files from the `prisma/migrations` folder. The initial migration is already included in the example. To learn more about how to create migrations, check out the [Prisma Migrate](/concepts/components/prisma-migrate) docs.
...
**TBD Change to result - also note my text was slightly different - worth mentioning it will look similar to the following**
...
Congratulations! You have created all the necessary resources and configuration, which means that you can now deploy your API.
...
> **Checkpoint:** If the functions are successfully deployed, then you see the following output:
...
Congratulations! You successfully deployed a Prisma based REST API to Azure Functions, with Azure SQL as the database.
...
In this step, you will test the API's different endpoints with the URLs from the previous step.

### Steps

Make a POST HTTP request to the _CreateUser_ endpoint with curl, with the following command. Replace `FUNCTION_APP_NAME` in the URL with the app name you chose in step 8.
...
If the request succeeds, you see the created user object returned:
...
Now, create a _post_ associated with the user you created. Use the following command. Replace `FUNCTION_APP_NAME` in the URL with the app name you chose in step 8.
...
To update the `published` field of the post, make the following request. eplace `FUNCTION_APP_NAME` in the URL with the app name you chose in step 8.
...
If the request succeeds, you should see the post you created and the related author, similar to the following:
