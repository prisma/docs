...
Open your terminal and navigate to a location of your choice. Run the following to commands to set up your application.
...
To confiure webpack support, first create a `webpack.config.js` in the project root with the following code:
...
Next, point Wrangler to use this webpack config by adding the following line to your `wrangler.toml`:
...
If all goes well, you will be prompted to connect your database:
...
Next, you'll connect the Prisma Data Platform to your MongoDB Atlas database:
...
And if you check your database, you should see info logs written to your database.
