...
Today you'll be building an and deploying a Cloudflare Worker that uses Prisma to save every request to a MongoDB database for auditing later.
...
Next we'll want to authenticate the Wrangler CLI with our Cloudflare Worker account. To do this, run the following command in your terminal:
...
The Cloudflare Worker environment natively supports Javascript, Rust, C, and C++. To get Typescript working on Cloudflare Workers, you need to compile Typescript to Javascript before deploy to the worker.
...
<Admonition type="info">
  We need to pin `ts-loader` for the Wrangler CLI to work properly. See [this
...
This data model will be used to store incoming requests from the Cloudflare Workers.
...
<Admonition type="warning">
  Please be aware that the Data Proxy is in Early Access and subject to change.
</Admonition>
