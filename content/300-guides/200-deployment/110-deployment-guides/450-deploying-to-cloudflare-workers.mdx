...
After you create a MongoDB Atlas database cluster, you can get its connection string. By default, that connection string does not include a collection name.

When you import your project in the Prisma Data Platform at step [to be determined], you must provide a connection string that includes a collection name.

You do not need to manually create the collection in MongoDB Atlas even if it does not yet exist. The project creation workflow in the Prisma Data Platform will create the collection.
...
2. On the _Security Quickstart_ screen, complete the initial security configuration.  
   1. Add a new user and save the password you assign to it. 
      <Admonition>

      **Note**<br /><br />

      The user is later added to the connection string which does not include the password.
      </Admonition>
...
4. Add the collection name `Log` to the connection string.
   
   The code sample below shows how the collection name is added to a connection string that is provided by MongoDB Atlas.

   ```diff
   # The first line is a default-style MongoDB Atlas connection string.
   # The second line includes the insert `Log` collection name.
...
1. To get started, sign up for a free [Prisma Data Platform account](https://cloud.prisma.io/).
   ...
   **Note**<br /><br />
   You only need to have a GitHub account to automatically sign up when you log in for the first time.
   </Admonition>

   ![Signup for Prisma Data Platform](./450-02-signup-pdp.png)

2. Once you're in, click **New Project**, then **Import a Project**.
...
6. Go back to your code editor and paste the connection string into your `wrangler.toml` file:
...
## 9. Set the Data Proxy connection string in your local environment

You also need to add the Data Proxy connection string to your local environment `.env` file.
