...
At the start, you create a directory for your project, and use `deno run`
...
3. In the generated `prisma/schema.prisma`, add the following schema:
  ...
  Note that this already includes the preview feature flag `deno` to activate the current preview support for Deno in Prisma Client, and that it defines a custom `output` location for your Prisma Client. This is a requirement for Deno applications.
