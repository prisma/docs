...
This will under the hood install Prisma Client in your project, and as there is no `package.json` file yet, that will also be created.
...
As the Data Proxy Client does not read `.env` files by default, you also need to install `dotenv-cli` locally:
...
Now you can open a browser to [http://localhost:8000/](http://localhost:8000/) and it will write your request to the database.
...
- [https://dash.deno.com/new](https://dash.deno.com/new)
- Select your GitHub organization or user, repository and branch
- Select the "Automatic" deployment mode
- Select the `index.ts` as entry point
- Make sure that the project has a good name
- "Add Env Variable" to add `DATABASE_URL` with the Data Proxy connection string
- Click "Link" button
- Watch the deployment happen
- When it is done, it will redirect to a project view
- Click the blue "View" button on the top right to open your Deno Deploy deployment
