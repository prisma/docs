...
metaTitle: 'Deploying to Deno Deploy'
...
Inside the generated `prisma/schema.prisma`, add the following schema:
...
This data model will be used to store incoming requests from your application.
...
With the database schema created, and a local Prisma Client generated you can now create a minimal local Deno application. Create an index.ts with the following content:
...
You can't execute this script yet, as you are lacking the credentials for Data Proxy that allow you to use this Client with your database. Let's get that fixed.
...
4. Next, you connect the Prisma Data Platform to your database and set up the Data Proxy.
...
With the Data Proxy connection string known, you can replace the direct PostgreSQL connection string we used to create the database schema: 
...
Now you should be able to open a broweser to TODO and it will write your request to the database.
