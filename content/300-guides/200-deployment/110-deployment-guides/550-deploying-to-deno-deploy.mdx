...
3. Edit the `prisma/schema.prisma` file to define the data model and enable the `deno` preview feature flag.
  ...
  To use Deno, you need to add the preview feature flag `deno`. Also, Deno requires that Prisma Client is generated in a custom location and you enable that with the `output` parameter in the `generator client` block.
...
At this point, the command has 2 additional side effects. The command installs Prisma Client and creates the `package.json` file for the project which includes the `@prisma/client` package as a dependency.

## 3. Generate Prisma Client for the Data Proxy

Next, generate Prisma Client for the Data Proxy with the `--data-proxy` flag. Later, you use the [Data Proxy](/data-platform/data-proxy) to connect to your database over HTTP.
...
You cannot run this script yet, because you do not yet have the required Data Proxy credentials to use Prisma Client with your database. You obtain the required credentials when you next [add your application to the Prisma Data Platform](#6-create-your-project-in-the-prisma-data-platform).

After that, you can [test your application locally](#8-test-your-script-locally).

## 5. Create your project in the Prisma Data Platform

Sign up for the Prisma Data Platform and add your application as a project so that you can enable the [Data Proxy](/data-platform/data-proxy) for it.
...
1. Connect the Prisma Data Platform to your database and enable the Data Proxy.
    ...
    1. Click **Create project** to test the connection string and create your project.
      <img
      ...
      If the connection to the database is successful, Prisma Data Platform creates your project and redirects your browser to the **Get started** page. On the **Get started** page, you can [optionally link your GitHub repository](/data-platform/projects/create#optional-enable-schema-synchronization) to keep your Prisma schema up-to-date.<br /><br />
      For a project in Prisma Data Platform, you need a linked Prisma schema if you plan to use [Data Browser](/data-platform/data-browser) and [Query Console](/data-platform/query-console).<br /><br />
...
You can save the Data Proxy connection string in a password manager or another safe location. You can then use the connection string in your local `.env` file to configure your local environment to connect to your database through the Data Proxy.
...
The Data Proxy Client does not read `.env` files by default, so you must also install `dotenv-cli` locally.
...
Create a private repository for your application on GitHub. You need the GitHub repository to add your project to Deno Deploy and enable automated deployments on push.

<Admonition>

**Note**<br /><br />

Optionally, you can also [link your GitHub repository to your project](/data-platform/environments/create#projects-with-an-introspected-schema-or-with-no-schema) in the Prisma Data Platform so that you can keep it up-to-date with the latest changes you make in the `schema.prisma` file.
...
You can now add and deploy your application to Deno Deploy.
...
If it works correctly, you can see the same result as when you tested locally.
