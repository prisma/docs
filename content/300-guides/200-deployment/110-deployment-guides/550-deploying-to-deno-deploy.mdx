...
At this point, this command has 2 additional side effects. The command also installs Prisma Client and creates `package.json` file for the project which includes the `@prisma/client` package as a dependency.
...
You now have a database schema and a locally generate Prisma Client.
...
You can now start your application locally and test the creation of log entries. With your project in the Prisma Data Platform and with the Data Proxy connection string, your application uses the Data Proxy to to send queries to your database.
