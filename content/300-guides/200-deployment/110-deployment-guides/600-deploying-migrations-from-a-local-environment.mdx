...
- You are [baselining](/guides/database/developing-with-prisma-migrate/add-prisma-migrate-to-a-project#baseline-your-production-environment) a production environment
...
```bash file=.env highlight=1;delete|3;add copy
...
When you add Prisma Migrate to an **existing database**, you must [baseline](/guides/database/developing-with-prisma-migrate/add-prisma-migrate-to-a-project#baseline-your-production-environment) the production database. Baselining is performed **once**, and can be done from a local instance.
