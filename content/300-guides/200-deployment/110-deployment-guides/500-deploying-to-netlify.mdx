...
## 4. Set the <inlinecode>DATABASE_URL</inlinecode> varialbe in a <inlinecode>.env</inlinecode> file
...
```terminal highlight=2,3,4,5,6,7,8,9,10
$ ls -1
.env             <--
.git/
README.md
...
As a best practice, add the `.env` file to `.gitignore` to prevent `git` from monitoring it, and therefore, to avoid accidentally pushing it to a remote repository.
...
The `prisma/schema.prisma` file contains a pre-defined model for the app. It defines the `Post`, `Profile`, and `User` models which the `prisma db push` command uses to create the database tables and their relationships.
