...
Set the `DATABASE_URL` environment variable locally. The datasource url in the `prisma.schema` file is configured to use this environment variable.
...
The `prisma/schema.prisma` file contains a pre-defined model for the app. It defines the `Post`, `Profile`, and `User` models which the `prisma db push` command uses to create the database tables and relationships.
