...
With the help of the `dotenv` library, Prisma CLI loads the variables defined in the `.env` file into the environment. Later, when you run `prisma db push` later, the command reads the environment variables.
