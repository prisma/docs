...
AWS Lambda defines an upload limit which includes all application code and binaries like the [Prisma query engine](../../../concepts/components/prisma-engines/query-engine). The [deployment package (.zip) size limit for lambdas is 50MB](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html). As a general rule, keep .zip size as small as possible by only including what the function requires.
...
* The entire `node_modules/@prisma/engines` folder.
* The `native` engine binary from the `node_modules/.prisma/client`. For example, your schema defines the following `binaryTargets` if you develop on a Debian-based platform but deploy to AWS Lambda:
