...
1. The **local engine** for your development platform from the `node_modules/.prisma/client`. For example, your schema might define the following `binaryTargets` if you develop on Debian (`native`) but deploy to AWS Lambda (`rhel-openssl-1.0.x`):
   ...
   - Keep `node_modules/.prisma/client/query-engine-rhel-openssl-1.0.x`, which is the engine used by AWS Lambda
