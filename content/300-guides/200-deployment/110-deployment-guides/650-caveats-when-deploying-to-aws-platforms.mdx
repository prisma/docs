...
Right now we ship multiple query engine binaries: prisma-company.slack.com/archives/CRGDU19K6/p1615560739061000?thread_ts=1615496091.057100&cid=CRGDU19K6This is a problem in Lambda because there's a total upload limit for your function. This limit is about 50mb. This includes all your application code and the binaries.

* (preferred) but the first thing we should do is ask them to exclude the unnecessary binaries in @prisma/engines if they run into this. 
* Upload to S3 first and link the zip, 

prisma.io/docs/guides/deployment/deploying-to-aws-lambda
