...
In order to enable Preview deployment for pull requests, create a separate database for preview deployments, set the `DATABASE_URL` env var , and expose it **only** to Preview deployments. **Note that because each pull request shares the same preview database, if you have multiple pull requests with diverging schema migrations, you will encounter problems applying them.**
