...
metaDescription: 'Learn how to deploy a Next.js app to Vercel that uses a serverless REST API with Prisma Client.'
---
...
6. Click **Deploy**.
   ...
   Vercel will now deploy your application. The `package.json` file in the root directory of the example project defines a `vercel-build` script, which runs `prisma generate && prisma migrate deploy && next build` when you deploy the project to Vercel. This generates Prisma Client, applies the migrations, and builds the Next.js project.
...
To check that the API returns data successfully, click on the image of the deployed application. This will take you to your application at `https://__PROJECT-NAME__.vercel.app/`, where `__PROJECT-NAME__` is generated by Vercel.
