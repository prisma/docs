...
This guide takes you through the steps to set up and deploy a serverless application that uses Prisma to [Vercel](https://vercel.com/home). The application uses Next.js to provide a serverless REST API, and Prisma Client to handle fetching, creating, and deleting records from a database.

Vercel is a cloud platform that hosts static sites and serverless functions. Vercel integrates with a GitHub repository to allow you to deploy automatically when you make new commits. [Next.js](https://nextjs.org/) is a full-stack React-based serverless framework that integrates seamlessly with Vercel.
...
1. Go to [`deployment-example-vercel`](https://github.com/prisma/deployment-example-vercel) and click the Vercel **Deploy** button:
...
When the page loads, click **Check API status**. This should return:
...
You are now ready to test your application. Use the frontend at `https://__PROJECT_NAME__.vercel.app` to interact with the API.

### Steps

1. Click **Load users with profiles**. This API call uses Prisma to get all `users` in the database with their related `profiles`. The code is in the `pages/api/users.js` file in the project repository.

   You have not added any data to the database yet, so this should return an empty array:

   ```code no-copy
   []
   ```

2. Click **Seed data**. This API call uses Prisma to delete any existing database records and load the database with test `users`, `profiles`, and `posts`, then returns the created `users`. The code is in the `pages/api/seed.js` file.

3. Click **Load users with profiles** again. You should now see the `users` and `profiles` that you added in the previous step:
...
4. Click **Load posts**. This API call uses Prisma to get all `posts` in the database with their related `authors`. You should see the following output:
...
The code is in the `pages/api/posts.js` file.
...
Your application currently has a single _production_ environment, associated with the `main` Git branch of your repository. If you open a pull request to change your application, Vercel will create a new _preview_ environment.
...
In this section you will associate a second PostgreSQL database with a Vercel preview environment.
...
For more insight into Prisma Client's API, look at the function handlers in the [`api/` directory](https://github.com/prisma/deployment-example-vercel/tree/main/pages/api) of the example repository.
...
Therefore, it is beneficial to pool DB connections to get better performance.<br /><br />For some solutions to this issue, see our [connection management guide for serverless environments](/guides/performance-and-optimization/connection-management#serverless-environments-faas).
