...
The focus of this guide is showing how an API based on Prisma can be deployed to AWS Lambda. The starting point will be the [Prisma AWS example](https://github.com/prisma/prisma-examples/tree/latest/deployment-platforms/aws-lambda) which has a couple of REST endpoints pre-configured as serverless functions.
...
- [AWS](https://aws.amazon.com/) account and a corresponding [access key](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) for programmatic access.
...
For this, you need to define a `.env` file which the pre-configured `serverless-dotenv-plugin` will use to inject into the function runtime.
