...
In this guide, we create a new database from scratch. Prisma bases this on the Prisma schema (your `schema.prisma` file).

You will run `prisma db push` locally to create the database based on the Prisma schema.

** TBD: add a note about using migrate in future See discussion [here](https://prisma-company.slack.com/archives/C02JV0K2Q31/p1670942902655379)**
...
** Need to change connection string to use port Use port 5432, not the default 6543 - this is a supabase/prisma issue IF you use migrate - if we're using push, we don't need to **

** Obsolete note: Rewrite to use prisma migrate npx prisma migrate dev --name init. -> npx prisma migrate dev <- worked for me This creates a new SQL migration frile in the prisma/migrations irectory and runs the miration file against the database. Then it creates the database. We do this above - to create the database, then we can check for the tables**

** Although if we use migrate, then... we need a note -> Supabase allows you to create and drop databases automatically. If the provider doesn’t allow this you have to add a second database connection string manually https://www.prisma.io/docs/concepts/components/prisma-migrate/shadow-database#cloud-hosted-shadow-databases-must-be-created-manually **
...
** need to rewrite the above **

** Have a note about push vs. migrate - and link to a doc that explains the difference: /concepts/components/prisma-migrate/db-push#choosing-db-push-or-prisma-migrate For this guide, we use db push. In a production situation, you would use migrate. **

### Result

** TBW: this is not the result - instead, ask them to open prisma studio and inspect the database, or inspect it directly in Supabase. They should have three tbales - Post, Profile, and User **
** so we can remove the schema list below . In Supabase, go to their table editor. They can run npx prisma studio to view the data npx prisma studio **
...
## 5. Set the AWS Access key as an environment variable
...
The Serverless configuration file includes a package pattern that excludes all Prisma Engine binaries except for the one relevant for the Lambda runtime. This means that when Serverless packages your app for upload, it includes only one binary.
...
However, if you are using `serverless-webpack`, you cannot use that method.
...
we mention how to remove unneeded artifacts but only when using the serverless framework, I think we could improve that and mention how to do that when using AWS Lambda alone.

Example:

1. Create a zip script that only zips what's needed. For example, `prisma/ecosystem-tests@dev/platforms-serverless/lambda/zip.sh`.
2. Upload the zip.
