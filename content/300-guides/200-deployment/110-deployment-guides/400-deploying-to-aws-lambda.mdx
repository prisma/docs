...
For the purposes of this guide, we use `prisma db push` to create the database. However, in a production you would typically use `prisma migrate dev`. For more information on when to use each method, see [this](/concepts/components/prisma-migrate/db-push#choosing-db-push-or-prisma-migrate)
...
1. Create a zip script that only zips the files that your project needs. For example, `prisma/ecosystem-tests@dev/platforms-serverless/lambda/zip.sh`.
2. Upload the zip.
