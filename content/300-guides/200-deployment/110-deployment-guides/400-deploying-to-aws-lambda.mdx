...
** Ensure it's clear that we're saying what you will do in this guide - it's not steps to do right now **
** Need to write this up to use prisma db push - and have a note about using migrate in future See discussion [here](https://prisma-company.slack.com/archives/C02JV0K2Q31/p1670942902655379)**

So the new workflow will be:

1. Run `prisma db push` locally, which creates the database based on the Prisma schema.
...
** TBD: I used postgres:// as part of my string - if it worked, change the above **
...
** TBW: add a note here about conneciton URL formats (because you might need to do percentage encoding), and link to https://www.prisma.io/docs/reference/database-reference/connection-urls **

## 4. Create the database schema

** TBD: I'm not going to do this - if this works, we can remove this step - and renumber subsequent steps. We might want an explanation here though that the schema is already define in schema.prisma **
...
** TBW: change to use prisma db push **
...
** I got invalid port: let's check **
...
** Rewrite the above if we use prisma db push **
