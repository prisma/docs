...
This is necessary because the local runtime is different to the Lambda runtime. Add the `binaryTarget` to make the compatible Prisma Engine binary available. [Learn more](/reference/api-reference/prisma-schema-reference#binarytargets-options).
...
However, if you are using `serverless-webpack`, you cannot use this method. See [Deployment with serverless webpack](#deployment-with-serverless-webpack) for details.
...
** TBW - this is a separate docs ticket that we could do now **
