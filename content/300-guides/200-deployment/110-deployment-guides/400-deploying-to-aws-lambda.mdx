...
For the purposes of this guide, you will use `prisma db push` to push the initial Prisma schema to the database. This action does not generate a migration script, which you do not need for this guide.

In general, for a real project you start with a development database and use `prisma db push` as we suggest here. You can continue with `prisma migrate dev`, which generates a migration script for each change that you make to the Prisma schema. With multiple migration scripts, you build a migration history for your database and that makes it easy to later apply the changes to a production database with `prisma migrate deploy`. For more information, see [Prisma Migrate](/concepts/components/prisma-migrate).
...
## 2. Set the <inlinecode>DATABASE_URL</inlinecode> environment variable locally

### Steps

The `schema.prisma` file references the `DATABASE_URL` environment variable. This variable sets the connection string for the database. You must set the variable in your terminal session so that Prisma CLI can read it and then use it to connect to the database.

1. Set the `DATABASE_URL` environment variable locally as follows. This lets you create the database schema and ensures that Prisma can access the database.
...
2. Replace the uppercase placeholders with your database credentials. For example:
...
2. Open `.env` and set the `DATABASE_URL` to your database credentials from step 2.
...
To reduce your deployment footprint, you can change your deployment process so that you only upload the artifacts that your application needs. The method for doing so depends on whether you use the Serverless Framework or not.
