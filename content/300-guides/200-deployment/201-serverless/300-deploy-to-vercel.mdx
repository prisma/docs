...
Vercel is a cloud platform that hosts static sites and serverless functions. You can integrate a Vercel project with a GitHub repository to allow you to deploy automatically when you make new commits.
...
Another option to avoid an outdated Prisma Client is to use [a custom output path](/concepts/components/prisma-client/working-with-prismaclient/generating-prisma-client#using-a-custom-output-path) and check your client into version control.
...
4. Set the value to the connection string of your second database. As a reminder, a PostgreSQL connection string has the following format:
