...
Since your code is being bundled and deployed to Netlify's environment, which isn't necessarily the same as your local environment, you will need to set [`binaryTargets`](/reference/api-reference/prisma-schema-reference#binarytargets-options) in order to bundle the query engine that is compatible with Netlify. If you do not set this option, your deployed code will have an incorrect query engine deployed with it and will not function.
