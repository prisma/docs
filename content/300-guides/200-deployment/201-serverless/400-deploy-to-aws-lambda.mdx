...
metaTitle: 'Deploy your Prisma-backed project to AWS Lambda'
...
While a framework is not required to deploy to AWS Lambda, this guide covers deploying with [AWS SAM](https://aws.amazon.com/serverless/sam/), [Serverless Framework](https://www.serverless.com/framework/), and [SST](https://sst.dev/).

AWS SAM is a open-source framework from AWS that can be used in the creation of serverless applications. AWS SAM includes the [AWS SAM CLI](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-reference.html#serverless-sam-cli) which you will use to build, test, and deploy your application.
...
SST provides tools that make it easy for developers to define, test, debug, and deploy their application. Prisma works well with SST, but must be configured so that your schema is correctly packaged by SST.

</TopBlock>

## General AWS Lambda guidance

This section covers changes you will need to make to your application, regardless of framework. After following this guidance, follow the steps for your framework.

- [AWS SAM guidance](#aws-sam-guidance)
- [Serverless Framework guidance](#serverless-framework-guidance)
- [SST guidance](#sst-guidance)
...
While we do not recommend running migrations in an AWS Lambda context, some applications will require it. In these cases, you can use the [PRISMA_CLI_BINARY_TARGETS](https://www.prisma.io/docs/reference/api-reference/environment-variables-reference#prisma_cli_binary_targets) environment variable to make sure that `prisma migrate` has access to the correct schema engine.

In the case of AWS lambda, you will want to have the following line in your `.env` file
...
`prisma migrate` is a command in the `prisma` package. Normally, this package is installed as a dev dependency. Depending on your setup, you may need to install this package as a dependency instead in order for the command to be available in AWS Lambda
...
This section covers resolving issues when using the AWS SAM framework. Check the other sections for notes on [AWS Lambda in general](#general-aws-lambda-guidance), [Serverless Framework](#serverless-framework-guidance), or [SST](#sst-guidance).

### Loading environment variables

AWS SAM does not support loading values from a `.env` file directly. You will need to use "AWS Native" ways of storing and retrieving these parameters. [This guide](https://medium.com/bip-xtech/a-practical-guide-to-surviving-aws-sam-d8ab141b3d25) provides a great overview of your options and how to store and retrieve values in Parameters, SSM, Secrets Manager, and more.

### Loading required files

AWS SAM supports [esbuild](https://esbuild.github.io/) in order to bundle your TypeScript code. However, the full esbuild API is not exposed and esbuild plugins are not supported. This leads to problems when using Prisma in your application as certain files (like `schema.prisma`) must be available at runtime.

To get around this, we will directly reference needed files in our code so that they are correctly bundled. In your application, you could add the following lines to your application where Prisma is instantiated.
...
You will also need to tell esbuild how to bundle these files by adding the following lines to `Metadata.BuildProperties` in your `template.yaml`:
...
## Serverless Framework guidance

This section covers resolving issues when using Serverless Framework. Check the other sections for notes on [AWS Lambda in general](#general-aws-lambda-guidance), [AWS SAM](#aws-sam-guidance), or [SST](#sst-guidance).
...
This section covers what to know about using Prisma in an SST app. If you're looking for notes for [AWS Lambda in general](#general-aws-lambda-guidance) or [Serverless Framework](#serverless-framework-guidance), see the other relevant sections above.
