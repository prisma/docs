...
You can inject environment variables into the functions by adding them to the `Values` object in the `local.settings.json` file at the root of the project. You should, however, avoid adding secrets to that file as it's typically committed to the repository.
...
This will create a folder with the `index.js` and `function.json` files.
