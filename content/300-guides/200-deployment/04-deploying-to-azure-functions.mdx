...
This guide's focus is showing how Prisma can be used in the Azure cloud focusing on Azure Functions and Azure SQL. The starting point will the [Prisma Azure Functions example](#TODO) – a REST API for a simple blog with two models: `User` and `Post` (_1:n_). The example contains REST endpoints preconfigured as serverless functions.
...
This guide will start with an empty Azure SQL database and create the database schema with the Prisma `db push` command. The process looks as follows:
...
```terminal
az sql server create -l germanywestcentral -g prisma-azure-example --name CHOOSE_A_UNIQUE_DB_NAME --admin-user prisma --admin-password CHOOSE_A_PASSWORD --enable-public-network true
```

Before running the command, replace a unique name for the database in place of `CHOOSE_A_UNIQUE_DB_NAME`, set a password in place of `CHOOSE_A_PASSWORD`, and note it down.

The command does the following:

- Create the database server in the `germanywestcentral` region.
- Associate it with the `prisma-azure-example` resource group created in the previous step.
- Set the Azure SQL server name.
- Set the admin user to `prisma`.
- Set the admin password.
- Enable public network access so you can create the database schema from your machine.
...
Run the following command in the terminal, replace `UNIQUE_DB_NAME` with the database name you chose in the previous step:

```terminal
az sql db create --resource-group prisma-azure-example --server UNIQUE_DB_NAME --name prisma-azure-example --service-objective Basic
```
...
In this step, you will add a firewall rule to allow remote access from your public IP to the database on Azure. This is necessary so you can create the database schema and use the database for testing locally.
...
Copy the IP from the output and run the following command, replacing YOUR_PUBLIC_IP with the IP address:

```terminal
az sql server firewall-rule create --resource-group prisma-azure-example --server prisma-db --name allow-local-acccess --start-ip-address YOUR_PUBLIC_IP --end-ip-address YOUR_PUBLIC_IP
```
...
```terminal
az storage account create --name prismaazure --location germanywestcentral --resource-group prisma-azure-example --sku Standard_LRS
```
...
```json
{
  "id": "/subscriptions/YOUR_SUBSCRIPTION_ID/resourceGroups/prisma-azure-example/providers/Microsoft.Storage/storageAccounts/prismaazure",
  "provisioningState": "Succeeded",
  "resourceGroup": "prisma-azure-example",
...
Copy the following command and replace `APP_NAME` with a unique name for your function app

```terminal
az functionapp create --resource-group prisma-azure-example --consumption-plan-location germanywestcentral --runtime node --runtime-version 14 --functions-version 3 --name APP_NAME --storage-account prismaazure --os-type Linux
```
...
```terminal
az functionapp config appsettings set --name FUNCTION_APP_NAME_FROM_STEP_8 \
--resource-group prisma-azure-example \
...
```terminal
func azure functionapp publish prisma-azure-example
```

> **Checkpoint:** If the functions have been successfully deployed you should see the following:

```terminal
$ func azure functionapp publish prisma-azure-exampl
...
Functions in APP_NAME:
    CreatePost - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/post
    CreateUser - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/user
    DeletePost - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/post/{postid}
    FilterPosts - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/filterposts
    GetFeed - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/feed
    GetPost - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/post/{postid}
    PublishPost - [httpTrigger]
        Invoke url: https://APP_NAME.azurewebsites.net/api/publish/{postid}
```
...
```terminal
curl --request POST --data '"email":"alice@prisma.io","name":"Alice "}' https://APP_NAME.azurewebsites.net/api/user
```

> **Note:** Replace the **`APP_NAME`** in the URL with the app name you chose in step 8.
...
```terminal
curl --request POST --data '{"title":"Prisma with Azure","content":"","authorEmail":"alice@prisma.io"}' https://APP_NAME.azurewebsites.net/api/post
```
...
```terminal
curl --request PUT https://APP_NAME.azurewebsites.net/api/publish/1
```
...
```terminal
curl https://APP_NAME.azurewebsites.net/api/feed
```
