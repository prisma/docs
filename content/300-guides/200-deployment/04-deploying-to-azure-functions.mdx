...
Copy the following command and replace `FUNCTION_APP_NAME` with a unique name for your function app:

```terminal copy
az functionapp create --resource-group prisma-azure-example --consumption-plan-location germanywestcentral --runtime node --runtime-version 14 --functions-version 3 --name FUNCTION_APP_NAME --storage-account prismaazure --os-type Linux
```
