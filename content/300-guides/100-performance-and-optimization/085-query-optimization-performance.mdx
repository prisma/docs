...
You can use `include` to return each user's posts, that only results in **two** SQL queries - one to get users, and one to get posts. This is known as a [nested read](/concepts/components/prisma-client/relation-queries#nested-reads).
