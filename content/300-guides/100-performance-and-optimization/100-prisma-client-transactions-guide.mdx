...
- From version 4.5.0 to 5.0.0, you can filter on non-unique fields in `update` operations, so you can use optimistic concurrency control with `update` operations. To do so, use the [extendedWhereUnique preview feature](/reference/api-reference/prisma-client-reference#filter-on-non-unique-fields-with-userwhereuniqueinput).

- In versions 5.0.0 and later, the `extendedWhereUnique` preview feature is Generally Available and the preview feature is no longer needed.
