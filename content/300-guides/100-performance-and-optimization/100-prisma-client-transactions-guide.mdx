...
Since version 5.0.0 you are able to [filter on non-unique fields in `update` operations](/reference/api-reference/prisma-client-reference#filter-on-non-unique-fields-with-userwhereuniqueinput) so that optimistic concurrency control is being used. You can also enable this feature using the Preview flag `extendedWhereUnique` from versions 4.5.0 to 4.16.2.
