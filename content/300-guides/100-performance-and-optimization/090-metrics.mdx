...
You can [label your metrics data](#global-labels) to help you analyze them by category, for example to view separate sets of metrics for each database server that you operate.
...
You can [label your metrics data](#global-labels) to help you analyze them by category. For example, if you run Prisma Client on multiple servers, you can use labels like `{server: "us-server1", "app_version": "1"}` to view metrics at a server level. Global labels work with JSON and Prometheus-formatted metrics.
