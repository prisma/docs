...
- ☐ [Set pool size (`connection_limit`)](#recommended-connection-limit-1) based on whether you have an external connection pooler, and optionally [tune the pool size](#optimizing-the-connection-pool)
...
The recommended connection limit in serverless environments depends on:
...
> **Tip**: If you are using AWS Lambda and *not* configuring a `connection_limit`, refer to the following GitHub issue for information about the expected default pool size: https://github.com/prisma/docs/issues/667
...
If you rarely or never exceed the database connection limit with the `connection_limit` parameter set to 1, you can further optimize the connection pool size. Consider a function that is sends queries in parallel:
...
A potential solution is to **clean up idle connections** ([`serverless-mysql`](https://github.com/jeremydaly/serverless-mysql) implements this idea).

> **Note**: This is a suggested solution, not an official recommendation. Refer to the documentation for your chosen serverless platform for official recommendations.
