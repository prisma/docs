...
The [Data Proxy](/data-platform/data-proxy) is a managed external connection pooler built by Prisma that is integrated in the [Prisma Data Platform](https://www.prisma.io/dataplatform) and handles connection pooling for you.
...
Due to the way AWS RDS Proxy pins connections, [it does not provide any connection pooling benefits](/prisma-orm/deployment/deployment-guides/caveats-when-deploying-to-aws-platforms#aws-rds-proxy) when used together with Prisma Client.
