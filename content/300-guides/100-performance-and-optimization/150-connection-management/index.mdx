...
Databases have a limit on the number of open connections they can handle - for example, the default PostgreSQL connection limit is **100**. Although increasing the connection limit allows more processes to connect, there are significant performance cost on the database side. Moreover, many databases recommended not overpassing a certain connection number.
