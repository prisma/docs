...
## Connection URLs with PgBouncer

When you use Prisma Migrate, Prisma uses the `url` parameter in your `schema.prisma` file as the connection URL. This connection URL must connect directly to PostgreSQL and bypass PgBouncer. For example:
...
To use Prisma Client in conjunction with PgBouncer, provide a connection URL that connects to PostgreSQL with PGBouncer. When you instantiate Prisma Client, you can [specify a connection URL that overrides the one in your Prisma schema](/reference/api-reference/prisma-client-reference#datasources). For example:
...
The above example uses a URL in an [environment variable](/guides/development-environment/environment-variables). This makes it easy to refer to a different connection URL from the one we use with Prisma Migrate.
