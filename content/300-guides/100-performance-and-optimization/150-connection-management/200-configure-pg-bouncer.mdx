...
To work around this issue, you must connect directly to the database rather than going through PgBouncer. To achieve this, you can use the [`directUrl`](/reference/api-reference/prisma-schema-reference#datasource:~:text=the%20shadow%20database.-,directUrl,-No) attribute in your [`datasource`](/reference/api-reference/prisma-schema-reference#datasource) <span class="api"></span> block.
...
The block above uses a PgBouncer connection string as the primary URL, allowing Prisma Client to take advantage of the PgBouncer connection pooler.

It also provides a connection string directly to the database, without PgBouncer, using the `directUrl` attribute. This connection string will be used when commands that require a single connection to the database, such as `prisma migrate dev` or `prisma db push`, are invoked.
