...
- Prisma uses the datasource url in the `schema.prisma` file for migrations. This connection URL must connect directly to PostgreSQL, bypassing PgBouncer.
...
- When the prisma client is instantiated, it should use a connection URL which connects to PostgreSQL via PgBouncer in order to benefit from its connection pooling.

```ts highlight=8,14;normal
...
- Because of these conflicting requirements for these two connection URLs, you may find it easier to define them in two separate environment variables.
