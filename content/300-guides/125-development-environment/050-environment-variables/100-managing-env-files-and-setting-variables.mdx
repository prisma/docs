...
[Prisma creates an `.env` file for you upon installation](/guides/development-environment/environment-variable#how-does-prisma-use-environment-variables). You are not limited to using that file, some other options include:

- Do not use `.env` files and let Prisma use the system environment variables directly
- Use `.env` files from a location that the Prisma CLI does not check [by default](/guides/development-environment/environment-variable#using-env-files)
- [Use multiple `.env` file](/guides/development-environment/environment-variable/using-multiple-env-files)
...
- [Use multiple `.env` files](/guides/development-environment/environment-variable/using-multiple-env-files)
- Use `.env` files from a location that the Prisma CLI does not check [by default](/guides/development-environment/environment-variable#using-env-files)

### Using <inlinecode>dotenv-cli</inlinecode> via command line

The following steps show how to use the `dotenv-cli` package to use an alternative file to contain environment variables than the [default](/guides/development-environment/environment-variable#using-env-files) created by Prisma, which is then used to run Introspection.
