...
# will lead to DATABASE_URL_WITH_SCHEMA=postgresql://test:test@localhost:5432/test?schema=public
```
You can also use environment variables in the expansion that are set outside of the `.env` file, as for example a database URL that is set on a PaaS like Heroku or similar:

```terminal
# environment variable set in the environment of the system
