...
A **transaction** is a unit of work that only succeeds if every operation within that transaction succeeds. For example, if you update 200 users within a transaction, each update must succeed - if not, all changes are rolled back and the transaction fails as a whole. They also guarantee an order of execution for the operations provided to the transaction.
...
Its also worth noting that operations are executed according to the order they are placed in the transaction, however the order within the queries themselves are not guaranteed.
