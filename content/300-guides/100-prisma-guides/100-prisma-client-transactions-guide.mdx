...
In some cases you may need to perform custom logic as part of an atomic operation, such as writing to an external service or performing a check. However, all operations should still succeed or fail together.

### Idempotent APIs

Idempotency is the ability to run the same logic multiple times **without creating undesirable side-effects in the database**. For example:

* When you get a user by ID, you always get a user record or null - never a product record
* When you create a user with a unique email address, that user is created _once_ - attempting to create a second user with the same email address will fail
