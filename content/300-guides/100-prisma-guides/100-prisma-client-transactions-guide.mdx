...
Optimistic concurrency control (OCC) is a model for handling concurrent operations that does not rely on 🔒 locking. Instead, we **optimistically** assume that a record will remain unchanged in between reading and writing, and use some kind of concurrency token (a timestamp or version field) to detect if a record has changed - if the record has changed, cancel the transaction.

#### When to use optimistic concurrency control

* ✔ You
* ✔ You anticipate that conflicts will be rare

#### When to implement optimistic concurrency control

#### Scenario: Reserving a seat at the cinema

```terminal
/webinar subscribe 2943
```

1. Read: Retrieve the first available seat in a cinema
2. Modify: Reserve the seat by setting a `reserved` property to your session ID
3. Write: Write the updated `reserved` field to the database
