...
Writes are considered **dependent** one each other if:

* Operations depends on the result of a preceeding operation (for example, the database generating an ID)
...
* ✔ You want to write to create two or more records related by ID at the same time (for example, create a blog post and a user)
...
```ts
// Nested write
const createOne = client.team.create({
  data: {
    name: 'Aurora Adventures',
    members: {
      create: {
...
// Nested write
const createTwo = client.team.create({
  data: {
    name: 'Cool Crew',
    members: {
      create: {
...
```ts highlight=8;delete
  const deleteMany = await prisma.team.deleteMany({
    where: {
      id: {
        in: [2, 99, 2, 11]
      }
...
Instead of auto-generating IDs, change the `id` fields of `Team` and `User` to a `String` (if you do not provide a value, a UUID is generated automatically). We recommend using UUIDs:
...
In some cases you may need to perform custom logic as part of an atomic operation - also known as the [read-modify-write pattern](https://en.wikipedia.org/wiki/Read%E2%80%93modify%E2%80%93write). The following is an example of the read-modify-write pattern:s

* Read a value from the database
* Run some logic to manipulate that value (for example, contacting an external API)
* Write the value back to the databases

All operations should **succeed or fail together** without making unwanted changes to the databases, but you do not necessarily need to use an actual database transactions. This section of the guide describes two ways to work with Prisma Client and the read-modify-write pattern:
...
Idempotency is the ability to run the same logic with the same parameters multiple times with the same result: the **effect on the database** is the same whether you run the logic one or one thousand times. For example:

* **NOT IDEMPOTENT**: Create a user in the database with email address "letoya@prisma.io". The `User` table **does not** enforce unique email addresses. The effect on the database is different if you run the logic once (one user created) or ten times (ten users created).
* **IDEMPOTENT**: Create a user in the database with email address "letoya@prisma.io". The `User` table **does** unique email addresses. The effect on the database is the same if you run the logic once (one user created) or ten times (subsequent attempts to create a user with the same email address will fail due to `UNIQUE` constraint).

Idempotency is something you can actively design, and is not always needed or possible.
...
3. Associate the team with the Stripe custsomer ID to unlock paid features
...
4. Running the same code again either:
   ...
   * If `externalId` is not subject to a unique constraint, Stripe creates a yet another subscription (**not idempotent**)

You cannot re-run this code in case of error and you cannot change to another plan without being charged twice. 
...
const numTeammates = await client.user.count({
  where: {
    teams: {
      some: {
        id: teamId,
...
if (customer) {
  customer = await stripe.customers.update({ // If team already exists, update
    externalId: teamId,
    plan: "plan_id",
    quantity: numTeammates
  })
...
Optimistic concurrency control (OCC) is a model for handling concurrent operations on a single entity that does not rely on 🔒 locking. Instead, we **optimistically** assume that a record will remain unchanged in between reading and writing, and use some kind of concurrency token (a timestamp or version field) to detect if a record has changed. 

If a conflict ❌ occurs (someone else has changed the record since you read it), cancel the transaction. Depending on your scenario, you can then:

* Re-try the transaction (book another cinema seat)
* Throw an error (alert the user that they will overwrite made by someone else)

#### When to use optimistic concurrency control

* ✔ You anticipate that conflicts will be _rare_ - in some scenarios, such as editing shared documents, a lock (pessimistic concurrency control) might be more appropriate
...
The following sample code finds the first available seat and assigns that seat to a user:s
...
However, this code suppers from the "double booking problem" - it is possible for two people to book the same seats:
