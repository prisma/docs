...
This is not an efficient way to query. In a GraphQL context specifically, you are 
...
### Solving n+1 with <inlinecode>findUnique</inlinecode> and Prisma's dataloader

The Prisma Client dataloader **automatically** batches `findUnique` queries that:

* Occur in the same tick
* Have the same the same `where` and `include` parameters

This is particularly useful in a **GraphQL context** because GraphQL runs a separate resolver function for every field.
...
In this scenario, you should take advantage of Prisma's automatic batching of `findUnique`. For a detailed explanation, review our [example](#example-use-findunique-to-avoid-the-n1-issue-in-your-graphql-resolvers) or watch the following video:
...
Instead, use `findUnique` in combination with [the fluent API](../../../concepts/components/prisma-client/relation-queries#fluent-api) (`.posts()`) as shown to return a user's posts. Even though the resolver is called once per user, the Prisma dataloader **✔ batches the queries**.

> **Note**: Prisma's dataloader batches `findUnique` queries even if you do not use the fluent API. In this particular example, the fluent API is syntactically the best way to return a user's posts even though it results in an additional query to get all users a second time.
...
If you have a list of IDs, you can use the `in` filter to return all users, create a list of user IDs, then all posts where the `authorId` is `in` that list:
