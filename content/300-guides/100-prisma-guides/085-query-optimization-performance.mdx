...
> **Note**: Prisma's dataloader batches `findUnique` queries whether or not you use the fluent API. In this example, the fluent API is syntactically the best way to return a user's posts even though it results in an additional query to get all users a second time.
