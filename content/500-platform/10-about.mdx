...
An environment is an isolated space to provision PDP products for a given project. Environments typically map to development stages, like `Development`, `Staging` or `Production`. When a new project is created, it will have a _default_ environment called `Production`. When an environment is set as the default, it ensures that the project always has at least one active environment, as the default environment cannot be deleted unless another environment is designated as the default.
