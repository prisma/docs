...
Make sure you have your database [connection URL](../../concepts/database-connectors/connection-urls) (that includes your authentication credentials) at hand!
...
Next, set up your Prisma project by creating your [Prisma schema](../../concepts/tools-and-interfaces/prisma-schema/) file template with the following command:
...
To connect your database, you need to set the `url` field of the `datasource` block in your Prisma schema to your database [connection URL](../../concepts/database-connectors/connection-urls):
...
In this case, the `url` is [set via an environment variable](../../concepts/tools-and-interfaces/prisma-schema/#using-environment-variables) which is defined in `prisma/.env`:
...
As an example, for a MySQL database hosted on AWS RDS, the [connection URL](../../concepts/database-connectors/connection-urls) might look similar to this:
...
As a next step, you will introspect your database. The result of the introspection will be a [data model](../../concepts/tools-and-interfaces/prisma-schema/data-model) inside your Prisma schema.
...
- The `User` relation field is uppercased and therefore doesn't adhere to Prisma's [naming conventions](../../api-reference/prisma-schema#naming-conventions). To express more "semantics", it would also be nice if this field was called `author` to _describe_ the relationship between `User` and `Post` better.
- The `Post` and `Profile` relation fields on `User` as well as the `User` relation field on `Profile` are all uppercased. To adhere to Prisma's [naming conventions](../../../api-reference/prisma-schema#naming-conventions), both fields should be lowercased to `post`, `profile` and `user`.
- Even after lowercasing, the `post` field on `User` is still slightly misnamed. That's because it actually refers to a [list](../../concepts/tools-and-interfaces/prisma-schema/data-model#type-modifiers) of posts – a better name therefore would be the plural form: `posts`.

These changes are relevant for the generated Prisma Client API where using lowercased relation fields `author`, `posts`, `profile` and `user` will feel more natural and idiomatic to JavaScript/TypeScript developers. You can therefore [configure your Prisma Client API](../../concepts/tools-and-interfaces/prisma-client/configuring-the-prisma-client-api).

Because [relation fields](../../concepts/tools-and-interfaces/prisma-schema/relations#relation-fields) are _virtual_ (i.e. they _do not directly manifest in the database_), you can manually rename them in your Prisma schema without touching the database:
...
In this example, the database schema did follow the [naming conventions](../../api-reference/prisma-schema#naming-conventions) for Prisma models (only the virtual relation fields that were generated from introspection did not adhere to them and needed adjustment). This optimizes the ergonomics of the generated Prisma Client API. 
...
If you don't want to use the table and column names from your database in your Prisma Client API, you can configure them with [`@map` and `@@map`](../../concepts/tools-and-interfaces/prisma-schema/data-model#mapping-column-table-and-enum-names):
...
Learn more about this on the [Configuring your Prisma Client API](../../concepts/tools-and-interfaces/prisma-client/configuring-the-prisma-client-api) page.
...
Notice that the [`@prisma/client` node module](../../concepts/tools-and-interfaces/prisma-client/generating-prisma-client#the-prismaclient-npm-package) references a folder named `.prisma\client`. The `.prisma\client` folder contains your generated Prisma client, and is modified each time you change the schema and run the following command:
...
Depending on what your models look like, the Prisma Client API will look different as well. For example, if you have a `User` model, your `PrismaClient` instance exposes a property called `user` on which you can call [CRUD](../../concepts/tools-and-interfaces/prisma-client/crud) methods like `findMany`, `create` or `update`. The property is named after the model, but the first letter is lowercased (so for the `Post` model it's called `post`, for `Profile` it's called `profile`).
...
This code creates a new `User` record together with new `Post` and `Profile` records using a [nested write](../../concepts/tools-and-interfaces/prisma-client/relation-queries#nested-writes) query. The `User` record is connected to the two other ones via the `Post.author` ↔ `User.posts` and `Profile.user` ↔ `User.profile` [relation fields](../../concepts/tools-and-interfaces/prisma-schema/relations#relation-fields) respectively.

Notice that you're passing the [`include`](../../concepts/tools-and-interfaces/prisma-client/field-selection#include) option to `findMany` which tells Prisma Client to include the `posts` and `profile` relations on the returned `User` objects.
...
You can send a variety of queries with the Prisma Client API. Check out the [API reference](../../concepts/tools-and-interfaces/prisma-client/) and use your existing database setup from this guide to try them out.
