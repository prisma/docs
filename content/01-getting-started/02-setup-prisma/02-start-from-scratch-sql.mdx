...
Make sure your have your database [connection URL](../../concepts/database-connectors/connection-urls) (that includes your authentication credentials) at hand!
...
Next, set up your Prisma project by creating your [Prisma schema](../../concepts/tools-and-interfaces/prisma-schema/) file with the following command:
...
As an example, for a PostgreSQL database hosted on Heroku, the [connection URL](../../concepts/database-connectors/connection-urls) might look similar to this:
...
- The `User` relation field is uppercased and therefore doesn't adhere to Prisma's [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/data-model#model-names-and-table-names). To express more "semantics", it would also be nice if this field was called `author` to _describe_ the relationship between `User` and `Post` better.
- The `Post` and `Profile` relation fields on `User` as well as the `User` relation field on `Profile` are all uppercased. To adhere to Prisma's [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/data-model#model-names-and-table-names), both fields should be lowercased to `post`, `profile` and `user`.
...
In this example, the database schema did follow the [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/data-model#model-names-and-table-names) for Prisma models (only the virtual relation fields that were generated from introspection did not adhere to them and needed adjustment). This optimizes the ergonomics of the generated Prisma Client API. 
...
Also note that `allUsers` is _statically typed_ thanks to [Prisma Client's generated types](../../concepts/tools-and-interfaces/prisma-client/advanced-usage-of-generated-types). You can observe the type by hovering over the `allUsers` variable in your editor. It should be typed as follows:
