...
- The `User` relation field is uppercased and therefore doesn't adhere to Prisma's [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/models#naming-models). To express more "semantics", it would also be nice if this field was called `author` to _describe_ the relationship between `User` and `Post` better.
- The `Post` and `Profile` relation fields on `User` as well as the `User` relation field on `Profile` are all uppercased. To adhere to Prisma's [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/models#naming-models), both fields should be lowercased to `post`, `profile` and `user`.
...
In this example, the database schema did follow the [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/models#naming-models) for Prisma models (only the virtual relation fields that were generated from introspection did not adhere to them and needed adjustment). This optimizes the ergonomics of the generated Prisma Client API. 
