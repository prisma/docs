...
Notice that the [`@prisma/client` node module](../../reference/tools-and-interfaces/prisma-client/configuring-the-prisma-client-api#the-prisma-client-npm-module) references a folder named `.prisma\client`. The contents of the `.prisma\client` folder are modified by the following command:
