...
The data model definition part of the [Prisma schema](../) defines your application models (also called **Prisma models**). These models represent the entities of your _application domain_, map to the _tables_ in your database, and determine the shape of your generated [Prisma Client API](../prisma-client/). 

The following schema models a blogging platform:
...
- `model` primitives that define [Prisma models](#models)
- `enum` primitives that define [enums](#enums) (only available if enums are supported natively by your database)
...
- They are the foundation for the available **queries** in the generated [Prisma Client API](../prisma-client/).
...
Models represent the entities of your application domain. Models are represented by `model` blocks and define a number of [fields](#fields). In the [example data model](data-model#example), `User`, `Profile`, `Post` and `Category` are models. Here's the `User` model from the example for reference:
...
Here's an overview of these for the fields from the `User` model [above](#overview):
