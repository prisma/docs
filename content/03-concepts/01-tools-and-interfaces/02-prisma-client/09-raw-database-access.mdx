...
Both the `queryRaw` and `executeRaw` methods are implemented as a [tagged template](https://developer.mozilla.org/en-US/docs/Web/JavaScript/concepts/Template_literals#Tagged_templates). Therefore, you can also call `queryRaw` as follows:
...
To include variables in your SQL query, you can use JavaScript string interpolation with [template literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/concepts/Template_literals):
