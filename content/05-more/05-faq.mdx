...
Yes, Prisma Client provides has a method called [`.raw`](../cconcepts/tools-and-interfaces/prisma-client/raw-database-access) which you can use to send raw SQL strings to the database.

Note that Prisma currently doesn't support full [transactions](../cconcepts/tools-and-interfaces/prisma-client/transactions). If your application needs transactions, you can use Prisma Client alongside other lightweight query builder such as [knex.js](https://www.github.com/tgriesser/knex) to perform one-off queries as transaction.
...
Yes. Prisma Client is based on Prisma's [query engine](../cconcepts/tools-and-interfaces/prisma-client/query-engine) that can connect to any data source that provides a proper _connector implementation_. There will be built-in connectors such as the current ones for [PostgreSQL](../cconcepts/database-connectors/postgresql), [MySQL](../cconcepts/database-connectors/mysql) and [SQLite](../cconcepts/database-connectors/sqlite).
...
Learn more on the [Debugging](../cconcepts/tools-and-interfaces/prisma-client/debugging) page in the docs.

## How do schema migrations work with Prisma Client?

Prisma Client is not opinionated on how exactly you migrate your database schema (e.g. create new tables, alter columns, ...). You can keep your existing migration system and re-[introspect](../cconcepts/tools-and-interfaces/introspection) your database schema after each schema migration. You can also use Prisma Migrate to run your migrations based on Prisma's declarative [data model definition](../cconcepts/tools-and-interfaces/prisma-schema/data-model).
...
- delete your [Prisma schema file](../cconcepts/tools-and-interfaces/prisma-schema/prisma-schema-file)
