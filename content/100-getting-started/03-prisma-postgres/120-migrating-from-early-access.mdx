...
- A **Database connection string** or an **API Key** for your Prisma Postgres database.
...
1.	Set environment variables (with your actual EA database URL or API key):
```bash
...
```cmd
Prisma Postgres auth proxy listening on 127.0.0.1:63029 🚀
...
3. Copy the port number from the terminal output, you’ll need it in the next step for the `pg_dump` command.
...
3. Copy the port number from the terminal output, you’ll need it in the next step for the `pg_restore` command.
...
&& echo "-complete-"
```
Like before, you need to replace the `__PORT__` placeholder above with the port that was printed in terminal in the previous step.

Also, in this case the database name is `postgres_restore2`. You can replace it with your desired database name.
