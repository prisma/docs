...
This guide shows you how to import data from an existing PostgreSQL database into Prisma Postgres. You can accomplish this migration in three main steps:

1. Create a new Prisma Postgres database.
1. Export your existing database via `pg_dump`.
1. Import the previously exported database into Prisma Postgres via `pg_restore`.

We will be using the [TCP tunnel](/postgres/tcp-tunnel) to securely connect to your Prisma Postgres database during the restore step. 

## Prerequisites

- The connection URL to your existing PostgreSQL dartabase
...
In this step, you're going to export the data from your existing database into a file on your file system. 
...
- Make sure to get a database connection URL that uses [Supabase database](https://supabase.com/docs/guides/database/connecting-to-postgres#supavisor-session-mode).
- The connection URL should look similar to this:
    ```no-copy
...
Run the following command to export the data of your PostgreSQL database (replace the `__DATABASE_URL__` placeholder with your actual database connection URL):
...
- `-n`: Specifies the PostgreSQL schema
- `-f`: Specifies the output file name for the backup

Running this command will create a backup file named `db_dump.bak` which you will use to restore data into your Prisma Postgres database in the next step.

## 3. Import data into Prisma Postgres

In this step, you'll use the [TCP tunnel](/postgres/tcp-tunnel) to connect to your Prisma Postgres instance so you can import data via `pg_restore`.
...
Note that you don't need to provide username and password credentials to this command because the TCP tunnel already authenticated you via the API key in your Prisma Postgres connection URL.

You have successfully imported the data from your your existing PostgreSQL database into Prisma Postgres. 

You can validate that the import worked with Prisma Studio. Either open it in the [Platform Console](https://console.prisma.io) by clicking the **Studio** tab in the left-hand sidenav in your project or run this command to launch Prisma Studio locally:
...
Th Prisma Accelerate extension is required when using Prisma Postgres. If you are not currently using Prisma Accelerate with Prisma ORM and have the extension already installed, go through the following steps to make Prisma ORM work with Prisma Postgres.
...
### Option 2: You are not yet using Prisma ORM

If you are not yet using Prisma ORM, you'll need to go through the following steps to use Prisma Postrgres from your application:
...
You can find the detailled step-by-step instructions for this process in this guide: [Add Prisma ORM to an existing project](/getting-started/setup-prisma/add-to-existing-project/relational-databases-typescript-postgresql).