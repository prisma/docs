...
- [Amazon RDS Postgres](#import-data-from-amazon-rds-postgres-into-prisma-postgres)
- [Other Postgres provider](#import-data-from-different-postgres-provider-into-prisma-postgres)
...
Run the following command to create a backup of your Supabase database:

```terminal
pg_dump \                                           
  -Fc \
  -v \
  -d "postgresql://{USERNAME}:{PASSWORD}@{HOST}:5432/postgres" \
  -n public \
  -f supabase_db_dump.bak
```

Replace the connection string above with your actual Supabase database URL (including your username, password, and host).
...
## Import data from Amazon RDS Postgres into Prisma Postgres

You can follow the steps below to import data from your [Amazon RDS Postgres](https://aws.amazon.com/rds/postgresql/) database into Prisma Postgres.

### 1. Get the direct database connection string from Amazon RDS

Get the connection string for your [Amazon RDS Postgres database](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_CommonTasks.Connect.EndpointAndPort.html). 


Run the following command to create a backup of your Amazon RDS Postgres database:

```terminal
pg_dump \
  -Fc \
  -v \
  -d "postgresql://user:password@host.rds.amazonaws.com:5432/db" \
  -f amazon_rds_db_dump.bak
```

Replace the connection string above with your actual Amazon RDS Postgres database URL (including your username, password, and host).
...
Running this command will create a backup file named `amazon_rds_db_dump.bak` which you will use to restore data into your Prisma Postgres database.
...
### 3. Upload the Amazon RDS Postgres database backup to Prisma Postgres
...
pg_restore \
  ...
  -v \
  -d postgres \
  ./amazon_rds_db_dump.bak \
&& echo "-complete-"
```

You have successfully migrated your Amazon RDS Postgres database to Prisma Postgres. Connect to your new database using a favorite IDE or Prisma Client to ensure all tables, rows, and schemas match your old environment.

## Import data from different Postgres provider into Prisma Postgres
