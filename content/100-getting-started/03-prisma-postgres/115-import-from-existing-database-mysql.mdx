...
In this step, you'll use a secure [TCP tunnel](/postgres/database/direct-connections#tcp-tunnel-deprecated) to connect to your Prisma Postgres instance.
...
You can [specify a different host and port](/postgres/database/direct-connections#customizing-host-and-port) by providing your own host and port values using the `--port` and `--host` flags. Just be sure to use the same host and port values consistently throughout the guide.
