...
This page provides a step-by-step guide for using a Prisma Postgres database after setting it up with the `prisma init --db` command. Here's what it covers:
...
- A new Prisma Postgres instance was provisioned.
- The `prisma/` folder is created with an empty `schema.prisma` file.
