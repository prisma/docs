...
The [`src/queries.ts`](https://github.com/prisma/prisma-examples/blob/latest/databases/prismaPostgres/src/queries.ts) script contains a number of CRUD queries that will write and read data in your database. You can execute it by running the following command in your terminal:
...
The  [`src/realtime.ts`](https://github.com/prisma/prisma-examples/blob/latest/databases/prismaPostgres/src/realtime.ts) script contains a demo for receiving real-time change [events](/pulse/database-events) from your database. You can start the script as follows:
