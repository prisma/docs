---
title: 'Get Started'
metaTitle: 'Get started'
metaDescription: 'Get started'
hideTitle: true
tocDepth: 3
---

<TopBlock>

<BorderBox>

<BoxTitle>Getting started</BoxTitle>

Welcome 👋 Explore our products that make it easy to build data-driven applications:

[**<Database />Prisma ORM**](/orm/overview/introduction/what-is-prisma) is a next-generation Node.js and TypeScript ORM that unlocks a new level of developer experience when working with databases thanks to its intuitive data model, automated migrations, type-safety & auto-completion.

[**<Bolt />Prisma Accelerate**](/accelerate/what-is-accelerate) is a global database cache with scalable connection pooling.

[**<SignalStream />Prisma Pulse**](/pulse/what-is-pulse) allows you to build reactive, real-time applications in a type-safe manner.

</BorderBox>

</TopBlock>

## <Database color="#5A67D8" width="21" height="24" /> Prisma ORM

<Grid>
  <LinkCard
    icon="fa-solid fa-rabbit-running"
    title="Quickstart guide"
    desc="Set up Prisma ORM from scratch with a SQLite database in 5 minutes."
    link="/"
  />
  <LinkCard
    icon="fa-brands fa-github"
    title="Examples"
    desc="Explore our ready-to-run examples with your favorite frameworks and libraries."
    link="/"
  />
</Grid>

## Choose an option to get started

<TabbedContent gettingStarted tabs={['New database', 'Existing database']}>
  <tab>
    <Tab>
      <p>
        Set up Prisma ORM <b>from scratch</b> with your favorite database and
        learn basic workflows like data modeling, querying, and migrations.
      </p>
      <List>
        <SquareLogo image={<PostgresSQLSimple />} tech="Postgres SQL" />
        <SquareLogo image={<MySQLSimple />} tech="MySQL" />
        <SquareLogo image={<SQLServer />} tech="SQLServer" />
        <SquareLogo image={<PlanetScale />} tech="PlanetScale" />
        <SquareLogo image={<CockroachDBgradient />} tech="Cockroach DB" />
        <SquareLogo image={<MongoDBSimple />} tech="Mongo DB" />
      </List>
    </Tab>
  </tab>
  <tab>
    <Tab>
      <p>
        Get started with Prisma ORM and your <b>existing database</b> by
        introspecting your database schema and learn how to query your database.
      </p>
      <List>
        <SquareLogo image={<PostgresSQLSimple />} tech="Postgres SQL" />
        <SquareLogo image={<MySQLSimple />} tech="MySQL" />
        <SquareLogo image={<SQLServer />} tech="SQL Server" />
        <SquareLogo image={<PlanetScale />} tech="PlanetScale" />
        <SquareLogo image={<CockroachDBgradient />} tech="Cockroach DB" />
        <SquareLogo image={<MongoDBSimple />} tech="Mongo DB" />
      </List>
    </Tab>
  </tab>
</TabbedContent>

## <Bolt color="#16A394" width="21" height="24" /> Prisma Accelerate

<Grid>
  <LinkCard
    icon="fa-solid fa-arrow-right"
    title="Get started"
    desc="Add Accelerate to your app to use global database caching."
    link="/"
  />
  <LinkCard
    icon="fa-brands fa-github"
    title="Examples"
    desc="Run the Speed Test to see how Prisma Accelerate can make your app faster."
    link="/"
  />
</Grid>

## <SignalStream color="#16A394" width="27" height="24" /> Prisma Pulse

<Grid>
  <LinkCard
    icon="fa-solid fa-arrow-right"
    title="Get started"
    desc="Add Pulse to your app to subscribe to real-time updates from your database."
    link="/"
  />
</Grid>
