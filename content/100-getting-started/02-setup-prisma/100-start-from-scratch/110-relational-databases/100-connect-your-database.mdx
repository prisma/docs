...
Note that the default schema created by `prisma init` uses PostgreSQL as the `provider`. For PlanetScale, you need to use the `mysql` provider instead:
...
```bash file=.env
DATABASE_URL="mysql://janedoe:mypassword@planetscale-hostname/mydb?sslaccept=strict"
```
...
For a database hosted with PlanetScale, the [connection URL](/reference/database-reference/connection-urls) might look similar to this:

```bash file=.env
DATABASE_URL="mysql://johndoe:XXX@123456789abc.eu-west-3.psdb.cloud:3306/mydb"
```

When connecting to PlanetScale locally, your connection URL typically looks similar to this:

```bash file=.env
DATABASE_URL="mysql://johndoe:XXX@localhost:3306/mydb"
```
