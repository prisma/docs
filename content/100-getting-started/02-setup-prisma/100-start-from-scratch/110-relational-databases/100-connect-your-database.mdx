...
You now need to adjust the connection URL to point to your own database.
  ...
  > **_Attention:_**  For those are using MySQL, where is typed 'mydb', write the name of the schema. This happen because in  MySQL documentation, database (which is the system database which has a specific function) is labled as 'schema'.
