...
You will also need to add the [`referentialIntegrity` preview feature](/concepts/components/prisma-schema/relations/referential-integrity) to the `generator` block of your `schema.prisma` file, and set the relation mode type to `"prisma"` in the `datasource` block:
