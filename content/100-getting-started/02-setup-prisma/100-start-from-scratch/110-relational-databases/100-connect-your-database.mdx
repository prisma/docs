...
You will also need to add the [`referentialIntegrity` preview feature](/concepts/components/prisma-schema/relations/relation-mode) to the `generator` block of your `schema.prisma` file, and set the relation mode type to `"prisma"` in the `datasource` block:
