...
To use Prisma Migrate with PlanetScale, you will also need to add the [`referentialIntegrity` preview feature](/concepts/components/prisma-schema/relations/referential-integrity) to your `schema.prisma` file:
...
You will then need to connect to your PlanetScale database server using the [PlanetScale CLI](https://docs.planetscale.com/tutorials/automatic-prisma-migrations#:~:text=Install%20the-,PlanetScale%20CLI,-.). First, authenticate with the following command:
