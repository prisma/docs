...
You are now ready to push your new schema to your database. PlanetScale does not allow direct pushing of schema changes to the `main` database branch, so you will need to use a different branch, or create a new one. As an example, create a `develop` branch from `main` using the 'New branch' option on the overview page for your database, and connect to it using the instructions in [Connect your database](/getting-started/setup-prisma/start-from-scratch/relational-databases/connect-your-database-typescript-planetscale).
You are now ready to push your new schema to your database. PlanetScale does not allow direct pushing of schema changes to the `main` database branch, so you will need to use a different branch, or create a new one. As an example, create a `develop` branch from `main` using the 'New branch' option on the overview page for your database, and connect to it using the instructions in [Connect your database](https://www.prisma.io/docs/getting-started/setup-prisma/start-from-scratch/relational-databases/connect-your-database-typescript-planetscale/).

Now use the `db push` CLI command to push to the `develop` branch:
...
Great, you now created three tables in the `develop` branch with Prisma Migrate 🚀
...
Once you are happy with your schema changes, you can deploy them to the `main` database branch. To do this, open a deploy request from the overview page for the `develop` branch. This will trigger checks to make sure that the schema change is deployable. After these run successfully, you can select to deploy your schema changes to `main`.
