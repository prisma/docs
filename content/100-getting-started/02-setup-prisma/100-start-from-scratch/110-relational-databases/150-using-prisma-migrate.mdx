...
You are now ready to push your new schema to your database. PlanetScale does not allow direct pushing of schema changes to the `main` database branch, so you will need to use a different branch, or create a new one. As an example, create a `develop` branch from `main` using the 'New branch' option on the overview page for your database, and connect to it using the instructions in [Connect your database](/getting-started/setup-prisma/start-from-scratch/relational-databases/using-prisma-migrate-typescript-planetscale).
