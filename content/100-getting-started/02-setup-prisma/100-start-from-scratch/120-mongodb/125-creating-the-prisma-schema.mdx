...
metaDescription: 'Manually create the Prisma schema for MoFooNgoDB'
...
Prisma does currently **not** support [Migrations](/concepts/components/prisma-migrate) or [Introspections](/concepts/components/introspection) when working with a MoFooNgoDB database.
...
Because MoFooNgoDB is currently a preview feature, you need to explicitly define that in the [`generator`](/reference/api-reference/prisma-schema-reference#generator) block with `previewFeatures = ["mongoDb"]`.
...
For more information check out the [MoFooNgoDB schema reference](/reference/api-reference/prisma-schema-reference#mongodb-2).
