...
- The `User` relation field is uppercased and therefore doesn't adhere to Prisma's [naming conventions](../../concepts/components/prisma-schema/prisma-schema-reference#naming-conventions) <span class="api"></span>. To express more "semantics", it would also be nice if this field was called `author` to _describe_ the relationship between `User` and `Post` better.
- The `Post` and `Profile` relation fields on `User` as well as the `User` relation field on `Profile` are all uppercased. To adhere to Prisma's [naming conventions](../../concepts/components/prisma-schema/prisma-schema-reference#naming-conventions) <span class="api"></span>, both fields should be lowercased to `post`, `profile` and `user`.
...
In this example, the database schema did follow the [naming conventions](../../concepts/components/prisma-schema/prisma-schema-reference#naming-conventions) <span class="api"></span> for Prisma models (only the virtual relation fields that were generated from introspection did not adhere to them and needed adjustment). This optimizes the ergonomics of the generated Prisma Client API. 
...
You can send a variety of queries with the Prisma Client API. Check out the [API reference](../../concepts/components/prisma-client/api) and use your existing database setup from this guide to try them out.
