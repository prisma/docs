...
For the purposes of this guide, we'll setup a MongoDB database from scratch. If you have a MongoDB database already, feel free to jump to [Initializing Prisma](#initializing-prisma) in your project.
...
Now that we have a MongoDB database, let's create a new project and initialize Prisma:
...
We're now ready to introspect. Run the following command to introspect your database:
...
To be able to join data using the Prisma Client, we'll manually add the [`@relation`](/reference/api-reference/prisma-schema-reference#relation) attributes to our models:
