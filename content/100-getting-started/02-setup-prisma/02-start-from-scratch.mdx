...
> Note: `generate` is called when you run a migration, if the `prisma-client-js` generator is defined in your schema this will check if `@prisma/client` is installed and install it if it's missing.
