...
When `prisma migrate dev` is run the first time, it installs Prisma Client and runs `prisma generate` to generate a client based on your models. Subsequent changes to your database schema with `prisma migrate dev` will trigger `prisma generate` to regenerate Prisma Client.
...
Prisma has a huge [community](https://www.prisma.io/community) of developers. Join us on [Slack](https://slack.prisma.io) or [Discord](https://discord.gg/KQyTW2H5ca) and ask questions via [GitHub Discussions](https://github.com/prisma/prisma/discussions).
