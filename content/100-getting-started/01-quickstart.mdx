...
In this Quickstart guide, you'll learn how to get started with Prisma from scratch using a plain TypeScript project and a local SQLite database file.
...
If you want use Prisma with your own PostgreSQL, MySQL, MongoDB or any other supported database, follow these guides instead:

- [Start with Prisma from scratch](/getting-started/setup-prisma/start-from-scratch)
...
In order to be able to send queries to the database, you'll need a TypeScript file that you can execute. Create a new file called `script.ts` that you'll use to to write your Prisma Client queries:
...
```ts file=script.ts highlight=7-13;add
import { PrismaClient } from '@prisma/client'
...
```ts file=script.ts highlight=7-8;add
import { PrismaClient } from '@prisma/client'
...
```ts file=script.ts highlight=7-18;add
import { PrismaClient } from '@prisma/client'
...
In order to also retrieve the `Post` records that belong to a `User`, you can use the `include` the option via the `posts` relation field:

```ts file=script.ts highlight=7-12;add
import { PrismaClient } from '@prisma/client'
...
Notice that the objects in the `usersWithPosts` array are fully typed as well. This means you will get autompletion and the TypeScript compiler will prevent you from accidentally typing them.

## 5. Next steps

In this Quickstart guide, you learned how to get started with Prisma in a plain TypeScript project. Feel free to explore the Prisma Client API a bit more on your own, e.g. by including filtering, sorting and pagination options in the `findMany` query or exploring more operations like `update` and `delete` queries.
...
- [Build a REST API with NestJS](https://www.prisma.io/blog/nestjs-prisma-validation-7D056s1kOla1)

### Join the community

Prisma has a huge community of developers. Join us on [Slack](https://slack.prisma.io) or ask questions via [GitHub Discussions](https://github.com/prisma/prisma/discussions).
