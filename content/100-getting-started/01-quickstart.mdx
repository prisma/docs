...
techMetaTitles:
  [
    { name: 'node', value: 'Quickstart: Getting started with Node.js & SQLite' },
    { name: 'typescript', value: 'Quickstart: Getting started with TypeScript & SQLite' },
  ]
techMetaDescriptions:
  [
    {
      name: 'node',
      value: 'Get started with Prisma in 5 minutes. You will learn how to send queries to a SQLite database in a plain Node.js script using Prisma Client.',
    },
    {
      name: 'typescript',
      value: 'Get started with Prisma in 5 minutes. You will learn how to send queries to a SQLite database in a plain TypeScript script using Prisma Client.',
    },
  ]
---
...
```terminal copy
curl -L https://pris.ly/quickstart | tar -xz --strip=2 quickstart-master/javascript/starter
```
...
- `prisma`: The Prisma CLI which you can invoke with `npx prisma`
...
Let's write your first query with Prisma Client. For reference, this is what the [Prisma schema](../concepts/components/prisma-schema) that your Prisma Client was generated from looks like:
...
Also note that `allUsers` is _statically typed_ thanks to [Prisma Client's generated types](../concepts/components/prisma-client/advanced-type-safety). You can observe the type by hovering over the `allUsers` variable in your editor. It should be typed as follows:
...
```prisma file=schema.prisma highlight=15,18-23;add
model Post {
  id        Int     @id @default(autoincrement())
  title     String
  content   String?
