...
metaTitle: 'How to add Prisma ORM to an existing project using PlanetScale (15 min)'
metaDescription: 'Add Prisma ORM to an existing TypeScript project with PlanetScale and learn database introspection and querying.'
...
[PlanetScale](https://planetscale.com) is a serverless MySQL platform built on Vitess that offers database branching, non-blocking schema changes, and automatic backups. In this guide, you will learn how to add Prisma ORM to an existing TypeScript project, connect it to PlanetScale, introspect your existing database schema, and start querying with type-safe Prisma Client.
...
npm install @prisma/client @prisma/adapter-planetscale dotenv
```
...
const adapter = new PrismaPlanetScale({ connectionString })
