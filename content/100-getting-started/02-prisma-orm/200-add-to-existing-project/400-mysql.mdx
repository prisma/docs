...
npm install @prisma/client @prisma/adapter-mysql dotenv
```
...
- **`@prisma/adapter-mysql`** - The MySQL driver adapter that connects Prisma Client to your database
...
Update the `.env` file with your MySQL connection URL:

```bash file=.env
DATABASE_URL="mysql://user:password@localhost:3306/mydb"
```

The [format of the connection URL](/orm/reference/connection-urls) for MySQL looks as follows:
