...
- `url`: Specifies the [connection URL](#connection-url) for the Microsoft SQL Server database. In this case, an [environment variable is used](/concepts/components/prisma-schema#accessing-environment-variables-from-the-schema) to provide the connection URL.
