...
To connect to a SQL Server database, you need to configure a [`datasource`](../components/prisma-schema/data-sources) block in your [Prisma schema file](../components/prisma-schema):
...
- `url`: Specifies the [connection URL](#connection-url) for the SQL Server database. In this case, an [environment variable is used](../components/prisma-schema#accessing-environment-variables-from-the-schema) to provide the connection URL.

## Connection details

The connection URL used to connect to an SQL Server database follows the [JDBC standard](https://docs.microsoft.com/en-us/sql/connect/jdbc/building-the-connection-url?view=sql-server-ver15).
