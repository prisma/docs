...
This section describes which database features exist on the databases that are currently supported by Prisma. The **Prisma schema** column indicates how a certain feature can be represented in the [Prisma schema](../components/prisma-schema) and links to its documentation. Note that database features can be used in **Prisma Client** even though they might not yet be representable in the Prisma schema.
...
| ------------- | :--------: | :------ | :---: | :----: | :--------------------------------------------------------------------------------------------------: | :-----------: | :------------: |
| `PRIMARY KEY` |     ✔️     |  ✔️    |✔️    |   ✔️   |      [`@id` and `@@id`](../components/prisma-schema/data-model#defining-an-id-field)       |      ✔️       |       ✔️       |
| `FOREIGN KEY` |     ✔️     |  ✔️    |✔️   |   ✔️   |          [Relation fields](../components/prisma-schema/relations#relation-fields)          |      ✔️       |       ✔️       |
| `UNIQUE`      |     ✔️     |  ✔️†    |✔️   |   ✔️   | [`@unique` and `@@unique`](../components/prisma-schema/data-model#defining-a-unique-field) |      ✔️       |       ✔️       |
| `CHECK`       |     ✔️     |  ✔️    |✔\*  |   ✔️   |                                               Not yet                                                |      ✔️       |    Not yet     |
| `NOT NULL`    |     ✔️     |  ✔️    |✔️   |   ✔️   |                [`?`](../components/prisma-schema/data-model#type-modifiers)                |      ✔️       |       ✔️       |
| `DEFAULT`     |     ✔️     |  ✔️    |✔️   |   ✔️   |       [`@default`](../components/prisma-schema/data-model#defining-a-default-value)        |      ✔️       |       ✔️       |

* \*In [MySQL 8 and higher](https://mysqlserverteam.com/mysql-8-0-16-introducing-check-constraint/)
* † [Caveats apply when using the `UNIQUE` constraint with SQL Server](../components/preview-features/sql-server#cannot-use-column-with-unique-constraint-and-filtered-index-as-foreign-key)
...
| :-------------------------------- | :--------- | :--------- |:---- | :----- | :--------------------------------------------------------------------------------------------- | ------------- | -------------- |
| Autoincrementing IDs              | ✔️         | ✔️ | ✔️ |   | ✔️     | [`autoincrement()`](../components/prisma-schema/data-model#defining-a-default-value) | ✔️            | ✔️             |
| Arrays                            | ✔️         | No | No    | No     | [`[]`](../components/prisma-schema/data-model#type-modifiers)                        | ✔️            | ✔️             |
| Enums                             | ✔️         | No | ✔️    | No     | [`enum`](../components/prisma-schema/data-model#defining-enums)                      | ✔️            | ✔️             |
...
This section provides a high-level overview of the features that are provided by Prisma. Some features directly map to a feature in the underlying database, others are implemented by Prisma's [query engine](../components/prisma-client/query-engine) and therefore provide features that would not be available when using plain SQL.

### Data modeling

You can find a full reference of all data modeling features on the [Data model](../components/prisma-schema/data-model) page.
...
`uuid()` and `cuid()` are commonly used to set default values for [ID](../components/prisma-schema/data-model#defining-an-id-field) fields.
...
`autoincrement()` is commonly used to generated autoincrementing [ID](../components/prisma-schema/data-model
...
The following [attributes](../components/prisma-schema/data-model#defining-attributes) are available in Prisma. They either map to a database feature or are implemented by Prisma's [query engine](../components/prisma-client/query-engine):
...
| `@relation`  | n/a\*                   | Defines meta information about the relation. [Learn more](../components/prisma-schema/relations). |
...
\*\*Implemented by Prisma's [query engine](../components/prisma-client/query-engine)
