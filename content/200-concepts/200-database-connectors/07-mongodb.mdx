...
This section covers any limitations and known issues with the MongoDB connector.
...
The [`@@id`](/reference/api-reference/prisma-schema-reference#id-1) attribute (an `@id` for multiple fields) and [`autoincrement()`](/reference/api-reference/prisma-schema-reference#generate-autoincrementing-integers-as-ids) function (to create incrementing `@id` values) are not supported.
