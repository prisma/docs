...
When you try to use Prisma's MongoDB connector on a deployment that has no replica set configured, you will receive an `Error: Transactions are not supported by this deployment` message. The full text of the error message is the following:
