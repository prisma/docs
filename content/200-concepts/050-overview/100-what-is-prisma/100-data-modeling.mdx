...
- **Prisma Client and Prisma Migrate (Preview)**: Data modeling happens in the Prisma schema by _manually adding application models_ to it. Prisma Migrate maps these application models to tables in the underlying database.
...
> **Warning**: Prisma Migrate is currently in a Preview  state. It is ready to be tested and can be used in non-critical projects. However, it is not yet considered ready for production usage.

When using [Prisma Migrate](../../components/prisma-migrate), you define your application in the Prisma schema and use the `prisma migrate` subcommand to generation plain SQL migration files, which you can edit before applying.

Here is an overview of the main workflow:

1. Manually change your application models in the Prisma schema (e.g. add a new model, remove an existing one, ...)
1. Run `prisma migrate dev --preview-feature` to create and apply a migration (the Prisma Client is automatically generated)
