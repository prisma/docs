...
* If a field is mandatory but not present in MongoDB, you will get an error
...
- **Prisma Client and Prisma Migrate**: Data modeling happens in the Prisma schema by _manually adding application models_ to it. Prisma Migrate maps these application models to tables in the underlying database (currently not supported by the MongoDB connector).
...
When using [Prisma Migrate](../../components/prisma-migrate), you define your application in the Prisma schema and use the `prisma migrate` subcommand to generate plain SQL migration files, which you can edit before applying.
