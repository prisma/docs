...
If you're new to Prisma, be sure to check out the [Quickstart](../../getting-started/quickstart-typescript) and [Introduction](../overview/what-is-prisma) pages first.
...
As an example, Prisma Client connects to the [query engine](../components/prisma-client/query-engine) in order to read and write data in a database:
...
- You are behind a corperate firewall and need to use a whitelisted url to download the Prisma binaries.

Use the following environment variables to specify custom locations for your binaries:

- [`PRISMA_BINARIES_MIRROR`](../../reference/api-reference/environment-variables-reference/#prisma_binaries_mirror) <span class="api"></span>
...
Add the environment variable to the [`.env` file](../components/prisma-schema#using-env-files) (The Prisma CLI looks for [an .env file in`.env` file in several locations](../components/prisma-schema#using-env-files)):
...
#### Navigating a corperate firewall with <inlinecode>PRISMA_BINARIES_MIRROR</inlinecode>

You might find that your company operates behind a corporate firewall and you are unable to access the Prisma binaries via the default url `https://binaries.prisma.sh`.
In such situations the `PRISMA_BINARIES_MIRROR` environment variable can be used to bypass the corporate firewall by proxying the request to the Prisma domain and download the binaries.

```terminal
PRISMA_BINARIES_MIRROR=https://my-whitelisted-url
```
