...
- **Introspect**: Read a database schema and translate into [Prisma data model](../concepts/components/prisma-schema/data-model)
- **Schema**: Construct and modify a [Prisma schema file](../concepts/components/prisma-schema)
...
As an example, Prisma Client connects to the [query engine](../concepts/components/prisma-client/query-engine) in order to read and write data in a database:
...
> **Note**: It is possible to [use an `.env` file in a location outside the `prisma` folder](../concepts/components/prisma-schema#manage-env-files-manually).
