...
When building fullstack applications, Prisma Client can be used to query your database.

</TopBlock>

Fullstack frameworks, such as Next.js and Sveltekit, blur the lines between the server and the client. These frameworks also provide different patterns for fetching and mutating data on the server.
...
```ts
// (in /pages/index.tsx)
// You can use `getStaticProps` in place of `getServerSideProps`
