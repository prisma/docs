...
| ------------ | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| SQL Schema   | Migrations and model classes         | [Prisma schema](../../concepts/components/prisma-schema)                             |
| Tables       | Model classes                        | [Models](../../concepts/components/prisma-schema/data-model#defining-models) in the Prisma schema       |
| Columns      | Properties in model classes          | Model [fields](../../concepts/components/prisma-schema/data-model#defining-fields) in the Prisma schema |
| Records      | Instances of a model class (objects) | Plain JavaScript objects                                                                                           |
| Foreign keys | Foreign keys on model classes        | [Relations](../../concepts/components/prisma-schema/relations) between Prisma models                    |
...
Other ORMs have a workflow similar to this. Some, like [Django's ORM](https://docs.djangoproject.com/en/3.0/topics/migrations/#workflow), support a simpler workflow where you're required to only change the models and using a CLI tool, your models are scanned and compared to the versions currently contained in your migration files, and then a new set of migrations is written out.
...
- [**Prisma Client**](../../concepts/components/prisma-client/api): Auto-generated and type-safe database client
- [**Prisma Migrate**](../../concepts/components/prisma-migrate) (experimental): A CLI for declarative data modeling and migrations

Both Prisma Client and Prisma Migrate rely on the [Prisma schema](../../concepts/components/prisma-schema). The Prisma schema is a declarative representation of your database schema. It serves as the single source truth for both the database schema and your application models.
...
Prisma will generate Prisma Client inside the `node_modules` folder, from which it can be imported in your application. For more extensive usage documentation, see the [Prisma Client API](../../concepts/components/prisma-client/api) docs.
