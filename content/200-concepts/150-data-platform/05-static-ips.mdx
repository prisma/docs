...
<TopBlock>
  ...
  You can use the static IP address feature to connect your databases to the Prisma Data Platform securely whilst keeping them protected from the public internet.
</TopBlock>
...
## About Static IP addresses
  ...
  When properly setup, the platform will connect to databases from these static IPs. This works seamlessly across all features: databrowser, query console, data proxy.

  The following database offerings are known to be compatible with this feature: 
  * AWS RDS via [security groups](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Overview.RDSSecurityGroups.html)
  * AWS Aurora via [security groups](https://aws.amazon.com/premiumsupport/knowledge-center/aurora-private-public-endpoints/) (not Aurora Serverless)
  ...
  * [Google Cloud SQL](https://cloud.google.com/sql/docs/mysql/configure-ip#add)
  ...
  The following database offerings are known to be not compatible:
...
## Enabling the Static IPs feature
...
In the project or environment creation, or when modifying the settings of an existing environment, find the Static IPs section.
Select the option "Enabled (there is a firewall in front of my database)".

A list of three (3) IP addresses will be displayed below. Copy these into the clipboard.

#### Step 2. Allow traffic in your database provider

In your database provider, configure your database to allow traffic from the IP addresses. Please make sure to include all three (3) addresses.

#### Step 3. Save to test connectivity and finish

Back in your Prisma Data Platform, account, where you left off, submit the form.
At this point, the Platform will verify that it can connect to the database.
