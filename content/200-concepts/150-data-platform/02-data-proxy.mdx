...
Alternatively you can set the environment variable `PRISMA_GENERATE_DATAPROXY=true` to also choose to generate a Prisma Client with Data Proxy when a normal `prisma generate` command is executed.

<Admonition type="warning">

When deploying to [Vercel Serverless Functions](https://vercel.com/docs/concepts/functions/serverless-functions) you _need_ to use the environment variable approach and define `PRISMA_GENERATE_DATAPROXY=true` [as an environment variable in your Vercel project settings](https://vercel.com/docs/concepts/projects/environment-variables) for the correct Prisma Client for Data Proxy to be generated.

</Admonition>

When you use `--data-proxy` or `PRISMA_CLI_GENERATE_DATAPROXY=true`, you get a very lightweight Prisma Client runtime of approximately 250kb. This is because the Data Proxy does not need local engine files, so the generation process does not include the [query engine](/concepts/components/prisma-engines/query-engine).
...
You only need minimal changes to an existing project using Prisma Client with [Vercel Serverless Functions](https://vercel.com/docs/concepts/functions#serverless-functions) to start using the Prisma Data Proxy. Our Vercel Deployment sample repository has a [special branch](https://github.com/prisma/deployment-example-vercel/tree/data-proxy) showing how to deploy a Prisma application to Vercel Serverless Functions using the Data Proxy.
...
Depending on your [project plan](/concepts/data-platform/billing/plans-and-quotas), the Prisma Data Proxy provides a different [limit of concurrent proxy instances](/concepts/data-platform/about-platform/platform-limits-and-status#concurrency-limits-of-the-prisma-data-proxy).
...
[Contact Support](/concepts/data-platform/contact-support) if you are hitting the concurrency limits and have trouble scaling your Prisma application.
