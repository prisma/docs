...
**Not supported** <br />
You cannot use the Prisma Data Proxy at the same time as you use [Interactive Transactions](/concepts/components/prisma-client/transactions#interactive-transactions-in-preview) or [Metrics](/concepts/components/prisma-client/metrics#metrics-in-preview) preview features. To use Prisma Data Proxy, you must remove `interactiveTransactions` and `metrics` out of `previewFeatures` in your schema.
...
For security reasons, we strongly recommend that you always use environment variables in order to provide your database connection strings instead of hardcoding them into your `schema.prisma`:
...
The Data Proxy is able to work in Node.js as well as Edge runtimes such as Cloudflare Workers and Vercel Edge Functions (Middleware). Depending on where you're planning to deploy to, you may need to set the `DATASOURCE_URL` differently. 


##### Node.js

The environment variable is set and defined as usual. If you're not familiar with this, please give a read to our [Environment Variables](https://www.prisma.io/docs/guides/development-environment/environment-variables) guide.
...
Because Edge runtimes (eg. Cloudflare Workers, Vercel Edge Functions) cannot access files from the file system, the automatic loading of `.env` files cannot be handled by the Prisma Client. Instead, you can use environment variables to pass the connection string to the Data Proxy. Depending on your provider, you may need to set the `DATASOURCE_URL` differently, see the following sections.

###### Cloudflare Workers

In [Cloudflare Workers](https://developers.cloudflare.com/workers/), you can define environment variables via `wrangler` or via the dashboard. If you are not familiar with this, please give a read to their [Environment Variables](https://developers.cloudflare.com/workers/platform/environment-variables/) guide.

###### Vercel Edge Functions

In [Vercel Edge Functions](https://vercel.com/docs/concepts/functions/edge-functions) (Middleware), you can define environment variables as in Next.js. If you are not familiar with this, please give a read to their [Environment Variables](https://nextjs.org/docs/basic-features/environment-variables) guide.

#### Step 4: Generate the client

To generate the Prisma Client with the Data Proxy, call `prisma generate` with the option `--data-proxy`:

```terminal
prisma generate --data-proxy
```

One of the advantages of generating with `--data-proxy` is that you will get a very lightweight Prisma Client. In other words, no [query
engine](/concepts/components/prisma-engines/query-engine) will be included, and the Prisma Client runtime is only about `250kB`.
...
To use the Data Proxy with CloudFlare Workers or Vercel Edge Functions, you will need to import `@prisma/client/edge` instead of the usual `@prisma/client`.
...
The `@prisma/client/edge` can only be used with the Data Proxy, make sure to use the `--data-proxy` flag when generating the client.
