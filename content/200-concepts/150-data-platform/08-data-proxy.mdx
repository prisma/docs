...
The Data Proxy can work with Node.js, as well as with Edge runtimes like Cloudflare Workers and Vercel Edge Functions (Middleware). Depending on where you're planning to deploy to, you might need to set the `DATASOURCE_URL` differently.

##### Node.js

Set and define the environment variable as usual. For more information, see [Environment Variables](https://www.prisma.io/docs/guides/development-environment/environment-variables).
...
Edge runtimes, such as Cloudflare Workers and Vercel Edge Functions, cannot access files in the file system. This prevents Prisma Client from automatically loading `.env` files. You must use environment variables to pass the connection string to the Data Proxy. Depending on your provider, you might need to set the `DATASOURCE_URL` differently.

###### Cloudflare Workers

In [Cloudflare Workers](https://developers.cloudflare.com/workers/), you can define environment variables with either `wrangler` or the dashboard. For more information, see [Environment Variables](https://developers.cloudflare.com/workers/platform/environment-variables/).

###### Vercel Edge Functions

In [Vercel Edge Functions](https://vercel.com/docs/concepts/functions/edge-functions) (Middleware), you can define environment variables as in `Next.js`. For more information, see [Environment Variables](https://nextjs.org/docs/basic-features/environment-variables).
...
When you use `--data-proxy`, you get a very lightweight Prisma Client runtime of approximately 250kb. This is because the the Data Proxy does not need local engine files, so the generation process does not include the [query engine](/concepts/components/prisma-engines/query-engine).
