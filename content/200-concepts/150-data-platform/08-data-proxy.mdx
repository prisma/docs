...
To use the Data Proxy, use the version `3.15.2` or higher of the `prisma` and `@prisma/client` npm packages.
...
In Vercel Edge Functions, you can define environment variables as you would for Vercel Serverless Functions, e.g. in any `Next.js` project. For more information, see [Environment Variables](https://vercel.com/docs/concepts/projects/environment-variables) in the Vercel documentation.
...
You can use `@prisma/client/edge` only with the Data Proxy. Make sure to use the `--data-proxy` flag or `PRISMA_CLI_GENERATE_DATAPROXY=true` environment variable when you [generate the client](#step-4-generate-the-client).
...
The Prisma Data Proxy does not support the preview features [Interactive Transactions](/concepts/components/prisma-client/transactions#interactive-transactions-in-preview) or [Metrics](/concepts/components/prisma-client/metrics) yet. To use the Prisma Data Proxy, you must fist remove `interactiveTransactions` and `metrics` from the `previewFeatures` section of your `prisma.schema` file. Running `prisma generate --data-proxy` will error until these preview features are not enabled.
