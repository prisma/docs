...
In order to manually switch the database provider you need the following steps:
- switch the provider and the connection url 
- archive or remove your existing migration history so that there is no more folder named `migrations` in your directory
- run `prisma migrate dev` 

The last step will create a new initial migration going from an empty database to your current `schema.prisma`. This migration will only contain what is reflected in your `schema.prisma`. If you manually edited your previous migration files to add custom SQL you will need to again add this yourself. Also, the newly setup database on the changed provider will not contain any data. 
