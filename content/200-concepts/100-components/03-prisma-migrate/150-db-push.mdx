...
See [Schema prototyping with `db push`](/guides/migrations/prototyping-schema-db-push) <span class="guide"></span> for an example of how to use `db push` in this way.
...
- You want fine-grained control over how the schema changes are executed - for example, [renaming a column instead of dropping it and creating a new one](/guides/migrations/developing-with-prisma-migrate/customizing-migrations#renaming-a-field).
...
Yes, you can [use `db push` and Prisma Migrate together in your development workflow](/guides/migrations/prototyping-schema-db-push) <span class="guide"></span>. For example, you can:
