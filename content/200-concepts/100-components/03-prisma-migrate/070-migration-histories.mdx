---
title: 'Understanding migration histories'
metaTitle: 'Understanding migration histories'
metaDescription: 'Understanding migration histories'
tocDepth: 2
---
...
- A `_prisma_migrations` table in the database, which is used to check:
  ...
  If you change or delete a migration (**not** recommended), the next steps depend on whether you are in a [development environment](/concepts/components/prisma-migrate/using-migrate-development-production#development-environments) (and therefore using `migrate dev`) or a [production / testing environment](/concepts/components/prisma-migrate/using-migrate-development-production#production-and-testing-environments) (and therefore using `migrate deploy`).
...
- As you start to [customize migrations](/concepts/components/prisma-migrate/using-migrate-development-production#customizing-migrations), your migration history contains **information that cannot be represented in the Prisma schema**. For example, you can customize a migration to mitigate data loss that would be caused by a breaking change.
