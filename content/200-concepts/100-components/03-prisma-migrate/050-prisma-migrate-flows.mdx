...
Prisma Migrate cannot automatically detect that you want to _rename_ a field - instead, it will add a new field and remove the old one. To rename a field (for example, `name` &rarr; `fullname`) and maintain data, you must create and modify a **draft migration**:
