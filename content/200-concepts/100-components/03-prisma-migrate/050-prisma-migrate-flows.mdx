...
The `prisma migrate dev` command is reserved for development environments and requires the [shadow database](../#shadow-database), which is not available in production.
...
If Prisma Migrate detects a migration history conflict when you run `prisma migrate dev` (Development command), the CLI will ask to reset the database. Resetting the database:
...
1. Roll back the migration - this removes the migration from the `_prisma_migrations` table and allows it to be re-run:
...
The following example demonstrates how to manually complete the steps of a migration and mark that migration as resolved.
