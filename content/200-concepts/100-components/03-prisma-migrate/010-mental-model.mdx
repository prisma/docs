...
The `db push` command is useful when:
...
If the `db push` command detects a [schema drift](/guides/database/developing-with-prisma-migrate/troubleshooting-development#schema-drift), it will prompt you to reset your database. For example, this will happen when you add a required field to a table with existing content without providing a default value.

> A [schema drift](/guides/database/developing-with-prisma-migrate/troubleshooting-development#schema-drift) occurs when your database schema is out of sync with your migrations history and migrations table.
