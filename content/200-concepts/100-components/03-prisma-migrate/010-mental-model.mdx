...
**Does not apply to MongoDB** since it is a schema-less database.
...
- Feature branch (locally)
  ...
  1. Use one of the [Prisma CLI commands](/reference/api-reference/command-reference) to sync your Prisma schema with the database schema
...
### Prisma Migrate commands in a development environment

#### Prototype your schema

The [`prisma db push` command](/reference/api-reference/command-reference#db-push) allows you to sync your Prisma schema and database schema without persisting a migration (`/prisma/migrations`). The `db push` command tracks the state of the database using the following pieces of state:
...
The [`prisma migrate dev` command](/reference/api-reference/command-reference#migrate-dev) allows you to track the changes you make to your database. The `migrate dev` command automatically generates SQL migration files (saved in `/prisma/migrations`) and applies them to the database. When a migration is applied to the database, the migrations table (`_prisma_migrations`) in your database is also updated.
...
### Prisma Migrate commands in a production and staging environment
