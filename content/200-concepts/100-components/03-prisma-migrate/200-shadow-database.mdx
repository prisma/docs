...
- If you are developing against a cloud-based database (for example, on Heroku and Digital Ocean) and are currently **prototyping** such that you don't care about generated migration files and only need to apply your Prisma data model to the database schema, you can run [`prisma db push`](/reference/api-reference/prisma-cli/command-reference#db) instead of the `prisma migrate dev` command.
