...
To detect drift, Prisma Migrate:

1. Replays the **current** migration history in the shadow database.
1. **Introspects** the shadow database to generate the 'current state' of you Prisma schema.
...
> **Note**: The shadow database is not responsible for checking if a **migration file** has been edited or deleted.

### Generating new migrations

To generate new migrations, Prisma Migrate:

1. Compare the starting point and the target schema, and generate steps to get from one to the other. 
1. Render these steps to a SQL string and save it in the new migration file.
