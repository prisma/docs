...
\* If your database does not allow creation and deleting of databases (e.g. in a cloud-hosted environment), you need to [create and configured the shadow database manually](#cloud-hosted-shadow-databases-must-be-created-manually).
...
> **Note**: A shadow database is never used for MongoDB as `migrate dev` is not used there.
...
1. Creates a new shadow database (or performs a soft reset if the shadow database is cloud-hosted)
...
2. TODO evaluate data loss
3. Applies the generated migration to the development database (assuming you have not specified the `--create-only` flag)
4. Drops the shadow database (cloud-hosted databases cannot be dropped, but are reset at the start of the `migrate dev` command)
