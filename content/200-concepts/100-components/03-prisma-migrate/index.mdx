...
> **Tip**: If you are prototyping, consider using the [`db push` command](db-push) - see [Schema prototyping with `db push`](../../../guides/database/prototyping-schema-db-push) <span class="guide"></span> for examples.
...
Sometimes, you need to modify a migration **before applying it** to ensure that existing data is preserved when you run the migration. For example:

* You want to introduce a significant refactor, such as changing blog post tags from a `String[]` to a `Tag[]`
* You want to [rename a field](../../../guides/database/developing-with-prisma-migrate/advanced-migrate-scenarios#renaming-a-field) (by default, Prisma Migrate will drop the existing field)
* You want to [change the direction of a 1-1 relationship](../../../guides/database/developing-with-prisma-migrate/advanced-migrate-scenarios#changing-the-direction-of-a-1-1-relation)
...
Refer to the [Advanced migrations](../../../guides/database/developing-with-prisma-migrate/advanced-migrate-scenarios) <span class="guide"></span> for examples.
