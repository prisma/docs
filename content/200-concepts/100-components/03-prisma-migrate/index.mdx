...
Prisma Migrate throws the following error if it cannot create the shadow database:
...
* If you are developing against a cloud-based database (for example, on Heroku), we recomend that you change to a local web environment (such as running your development database on Docker) for the time being and follow the discussions on [GitHub issue #4571](https://github.com/prisma/prisma/issues/4571).
