...
### Rollbacks and down migrations not currently availble

### Resetting database results in data loss

- There is currently no way to rollback a migration without resetting the database. This could be deemed acceptable in development but certainly not in production environments. For production environments we have something in the works, for development, we are planning to deliver a database seeding functionality that can be used every time the database is reset.
- We are also considering supporting explicit down migrations (opt-in?). Most of the users we've talked to never roll back in production. For development, we think we may be able to provide productive workflows without the burden of maintaining down migrations in case they are ever used.
