...
Prisma detects when commands are run in non-interactive environments, such as Docker. When this happens a warning is displayed indicating that the environment is non-interactive and the `migrate dev` command is not supported.
