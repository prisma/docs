...
metaDescription: 'The Prisma Data Platform (Early Access) provides a collaborative, cloud-based framework for application developers who are using Prisma and other open source tools. The main features of the Platform include a visual data editor, query console, a data proxy (purpose-built for Prisma!), and integration with your Prisma schema and databases.
experimental: false
earlyaccess: true
---
...
These roles are the currently supported in the Platform:
...
The Prisma Data Proxy is an intermediary between your application and your database. It manages connection pooling, so you can reliably use traditional databases in serverless environments.
...
We assume you already have an account and a project on [the Prisma Data Platform](https://cloud.prisma.io). If you don't, please do that before continuing.
...
#### Step 2: Install the appropiate version of Prisma

To use the Data Proxy, you need to use version `3.3.0` or higher of the `prisma` and `@prisma/client` npm packages.
...
To generate the client for the deployment, set the environment variable `PRISMA_CLIENT_ENGINE_TYPE="dataproxy"` using the following command:
...
In the deployment in which you want to use the Data Proxy, make sure that the `DATABASE_URL` environment variable is set to the Data Proxy connection string.
...
If you are loading the connection string in your Prisma Schema using `env()`, there should be no further changes needed, other than verifying that the environment variables are properly set. For example:
...
### Data Proxy Important Considerations
...
If you are looking to scale up traffic beyond the limits, or if you are seeing `429` HTTP errors in your logs - this might mean you are reaching the quotas; [**please reach out to us**](#feedback-and-support). We'd love to know about your use case and work together with you to help your Prisma application scale.

To contact us find us on [Slack](prisma.slack.com) or you can submit a feedback form.

#### Cost of the service

The Data Proxy is provided for free during Early Access. We will use this period to learn about usage and integrate feedback. When the Prisma Data Platform is launched in General Availability, there will be an option to continue using the Data Proxy for free with some limitations, or move to paid options.
...
You can also use [this form](https://forms.gle/17uyd1raniRtGSPe9) to submit any feedback you might have or report issues. We appreciate your input!
