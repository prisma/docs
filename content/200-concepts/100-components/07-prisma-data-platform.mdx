...
In the generator block of your `Prisma.schema` file, enable the feature flag:
...
In the deployment you want to use the proxy, make sure that the `DATABASE_URL` environment variable is set to the proxy connection string. 
Currently, the only supported datasource name is `db`. Make sure your `prisma.schema` file includes the `datasource db { ... }` block.
