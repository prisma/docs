...
You can introspect your database using the Prisma CLI in order to generate the [data model](prisma-schema/data-model) in your [Prisma schema](prisma-schema). The data model is needed to [generate Prisma Client](prisma-client/configuring-and-initializing-prisma-client/use-custom-model-and-field-names-api).
...
If your goal is to exclude certain models from the [Prisma Client generation](prisma-client/configuring-and-initializing-prisma-client/generating-prisma-client), you can also manually delete the models that should be excluded from your Prisma schema. Note however that they will be added again when you run `prisma introspect`.
