...
You can introspect your database using the `prisma db pull` command of the [Prisma CLI](prisma-cli/installation). Note that using this command requires your [connection URL](/reference/database-reference/connection-urls) to be set in your [Prisma schema `datasource`](https://www.prisma.io/docs/concepts/components/prisma-schema/data-sources).
