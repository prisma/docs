...
The Prisma Language Server provides the following Code Actions to restructure code and fix issues:
...
Code Actions have the following key bindings in different code editors:
...
### Add a missing index in <inlinecode>prisma</inlinecode> relation mode

If you [emulate relations in Prisma Client](/concepts/components/prisma-schema/relations/relation-mode#emulate-relations-in-prisma-with-the-prisma-relation-mode) with the `prisma` relation mode, this Quick Fix allows you to add an index on any fields that are used in a relation and that do not yet have an index defined.

In the following example, the Quick Fix adds an index on the `userId` scalar field in the `Post` model:
...
model Post {
  ...
  //          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  // Quick Fix: Add an index for the relation's field(s)
}
```
...
For more information, see [Index validation](/concepts/components/prisma-schema/relations/relation-mode#index-validation).
...
![Rename a model in VS Code](code_action-rename.png)
