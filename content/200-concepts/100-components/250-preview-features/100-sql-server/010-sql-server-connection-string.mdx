...
| `connectionLimit` | No       | `num_cpus * 2 + 1`     | Maximum size of the [connection pool](./../prisma-client/working-with-prismaclient/connection-management#connection-pool)                                                    |
...
| `poolTimeout`     | No       | `5`                    | Maximum number of seconds to wait for a new connection from the pool. If all connections are in use, the database will return a `PoolTimeout` error after waiting for the given time. |
