...
Make sure you have your database [connection URL](sql-server-connection-url) (including authentication credentials) available.
...
1. Define the `DATABASE_URL` [environment variable](../../../components/prisma-schema#using-environment-variables) in the `prisma/.env` file - this is the connection URL to your database.

   The following example connection URL [uses SQL authentication](sql-server-local#enable-authentication-with-sql-logins-optional), but there are [other ways to format your connection URL](sql-server-connection-url):

   ```js file=prisma/.env
   DATABASE_URL =
     'sqlserver://localhost:1433;database=mydb;user=sa;password=r@ndomP@$$w0rd;trustServerCertificate=true'
   ```
...
1. Adjust the connection URL to match your setup - see [Microsoft SQL Server connection URL](sql-server-connection-url) for more information.
