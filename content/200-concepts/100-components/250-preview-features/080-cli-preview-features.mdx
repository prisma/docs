...
The `migrate diff` command can only compare features that are supported by Prisma in the Prisma Schema Language. If two databases differ only in features (such as views or triggers) that are not currently supported in the Prisma Schema Language, then `migrate diff` will not show any difference between them.
