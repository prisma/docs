...
Before using the `migrate diff` command, you must define a valid [`datasource`](/concepts/components/prisma-schema/data-sources) within your `schema.prisma` file.
...
| `--from-schema-datamodel` | Path to an empty schema file                    |
| `--from-empty`            | An empty data source                            |
...
| `--to-schema-datamodel` | Path to an empty schema file                    |
| `--to-empty`            | An empty data source                            |
...
- Compare the state of a database with a URL of `$DATABASE_URL` to the schema defined by the migrations in the `./migrations` directory, and output the differences to a script `script.sql`:
  ...
  prisma migrate diff \
   --preview-feature \
   --from-url "$DATABASE_URL" \
   --to-migrations ./migrations --script > script.sql
  ```
...
Before using the `db execute` command, you must define a valid [`datasource`](/concepts/components/prisma-schema/data-sources) within your `schema.prisma` file.
...
- Take the content of a SQL file located at `./script.sql` and execute it on the database specified by the `datasource` URL in your `schema.prisma` file:
