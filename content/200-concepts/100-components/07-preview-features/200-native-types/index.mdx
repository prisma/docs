...
* Introspection adds [type attributes](#native-database-type-attributes) to each field that **[enrich your schema](#enriched-schema)** with information about the underlying native database type
* Prisma Migrate uses [type attributes](#native-database-type-attributes) to **[infer which specific native type to create in the database](#control-native-types-created-by-prisma-migrate)** - for example, `@VarChar(90)` or `@VarChar(200)`
...
### Control native types created by Prisma Migrate

TODO

### Enriched schema

Mative database type attributes gives you an **enriched schema** when you introspect - you can see if  `String` is `varchar(200)` or just `text`.
