...
The `foreignKeys` relation mode maintains referential integrity at the database level using foreign key constraints and referential actions.
...
When you _update_ or _delete_ a record with a relation to another record, referential actions are triggered in the database. Referential actions maintain referential integrity in related records by preventing changes that would break referential integrity, cascading changes through to related records or setting the value of fields that reference the updated or deleted records to a `null` or default value.
...
One side effect of foreign key constraints in databases is that the database usually also creates an implicit index (for example [MySQL]([https://dev.mysql.com/doc/refman/8.0/en/constraint-foreign-key.html#:~:text=MySQL%20requires%20that%20foreign%20key%20columns%20be%20indexed%3B%20if%20you%20create%20a%20table%20with%20a%20foreign%20key%20constraint%20but%20no%20index%20on%20a%20given%20column%2C%20an%20index%20is%20created.](https://dev.mysql.com/doc/refman/8.0/en/create-table-foreign-keys.html#:~:text=MySQL%20requires%20indexes,as%20described%20previously.))) for the columns you create them for. This is so "foreign key checks can be fast and not require a table scan". 

As relation mode `prisma` does not use foreign keys when creating the table, there also no index created and without a manually added index, there is the danger for queries to require full table scans which can be slow or expensive (on database providers that bill per accessed row). To help our users to avoid that, Prisma renders a warning in Prisma Schema when relation mode `prisma` is enabled and a field that is used in an `@relation` does not have an index defined:

```
TODO Good, clean example for a warning
```

Prisma suggests that you create an index by adding [`@index`](TODO) (or [`@unique`](TODO), [`@@unique`](TODO) or [`@id`](TODO) if applicable) to your model to make sure your queries are fast and effecient. In the example above you could do so by adding the following index to your model:

```prisma
@@index([foo, bar, baz])
```

If you are using the [Prisma VS Code extension](TODO link extension) (or our [language server in another editor](TODO link editor setup)) the warning will be augmented with a Quick Fix that will add the required index for you:
