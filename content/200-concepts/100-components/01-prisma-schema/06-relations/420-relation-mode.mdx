...
### Indexes on emulated relations

With most relational database, once two tables are linked together with a foreign key, an index is automatically created to speed up table joins in SQL queries.
However, as the `prisma` relation mode gives up foreign key constraints altogether, users are responsible of creating the indexes they need by themselves.
Forgetting to do so can lead to performance issues. PlanetScale users, in particular, may also be impacted by unexpectedly high costs due to the service's [billing model](https://planetscale.com/docs/concepts/billing) (as documented by [this PlanetScale user](https://www.briananglin.me/posts/spending-5k-to-learn-how-database-indexes-work/)).

<Admonition type="warning">

Primary keys and unique indexes are fully-fledge indexes as well! Every field marked via the `@@index`, `@id`, `@unique`, `@@unique` attributes results in an index being stored in the database.
For the simple case of a `@relation` with a single field, Prisma checks that such field is marked as an index in the relation's model definition.
For a `@relation` with a several fields, however, Prisma verifies that those fields are appear at the beginning of a compound index, following the same order of declaration in the `@relation` attribute.

</Admonition>

<!-- TODO: add a couple of practical examples with actual Prisma schemas? -->g
