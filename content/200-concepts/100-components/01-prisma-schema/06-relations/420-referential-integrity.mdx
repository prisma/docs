...
Alternatively, Prisma can emulate referential integrity in the client. This is the default option when using MongoDB as a database, because it does not support foreign keys. Emulation can also be enabled for relational databases in cases where foreign keys are not supported, or where it is preferred for other reasons (for example, in cases where there are scaling concerns).
