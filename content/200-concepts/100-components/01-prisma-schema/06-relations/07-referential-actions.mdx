...
With the feature flag **enabled**, the `onDelete` clause now exists and has a default value of [`Restrict`](#restrict). This is represented in SQL as `ON DELETE RESTRICT`.
