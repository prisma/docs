...
- † On MySQL, `SetDefault` is ignored and is silently replaced with `NoAction` instead.
  On MySQL 8.0+, when a `SetDefault` referential action is triggered, a foreign key violation error is raised.  
  On older versions, like MySQL 5.6+, creating or altering a table with a `SetDefault` referential action is not possible at all.
  We therefore advise against using `SetDefault` at all on MySQL.
