...
When invoking the [`delete()`](../../../prisma-client/crud#delete-a-single-record) or [`deleteMany()`](../../../prisma-client/crud#delete-all-records) methods using Prisma Client on required relations, a runtime check is performed and the deletion of records prevented if they are referencing related objects. **This prevents cascade behavior**.
