...
The following documents represent a one-to-one self-relation between two users:
...
The following documents represent a one-to-many self-relation between three users - one teacher and two students with the same `teacherId`:
...
Note that this n-n-relation is [implicit](many-to-many-relations#implicit-many-to-many-relations). This means Prisma maintains a [relation table](../relations#relation-tables) for it in the underlying database:
...
The following documents represent a many-to-many self-relation between five users - two users that follow `"Bob"`, and two users that follow him:
