...
TODO THis is only correct for relational databases, the MongoDB example is using lists and is Mongo's m:n - not really "implicit" (There is already a mention of this further down where it shows the SQL/data structure)
Note that this many-to-many-relation is [implicit](many-to-many-relations#implicit-many-to-many-relations). This means Prisma maintains a [relation table](/concepts/components/prisma-schema/relations/many-to-many-relations#relation-tables) for it in the underlying database.

If you need the relation to hold other fields, on relational databases you can create an [explicit](many-to-many-relations#explicit-many-to-many-relations) many-to-many self relation as well. The explicit version of the self relation shown previously is as follows:
