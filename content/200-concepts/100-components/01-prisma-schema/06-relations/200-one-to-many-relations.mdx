...
One-to-many (1-n) relations refer to relations where one record on one side of the relation can be connected to zero or more records on the other side. In the example below, there's one 1-n-relation between `User` and `Post`:
...
In relational databases, you can also define this relation using [multi-field IDs](../../../reference/api-reference/prisma-schema-reference#id-1):
