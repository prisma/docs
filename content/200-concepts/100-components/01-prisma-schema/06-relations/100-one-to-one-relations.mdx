...
In **relational databases only**, you can also define use [multi-field IDs](../../../../reference/api-referencea/prisma-schema-reference#id-1) to define a 1-1 relation:
...
The following document represents a `User`:

```json
{ "_id": { "$oid": "60d58e130011041800d209e1" }, "name": "Bob" }
```

The following document represents a `Profile` - notice the `userId` field, which references the `User` document's `$oid`:
