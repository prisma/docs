...
Migrate your changes and re-generate the client. When you run the same query with the updated field names, Prisma Client returns incorrect data (salmon now eats bears and gets eaten by plankton):
...
This however tells Prisma to expect **two** separate many-to-many relationships. See [disambiguating relations](/concepts/components/prisma-schema/relations#disambiguating-relations) for more information on using the `@relation` attribute.
...
Some cloud providers enforce (by default) the existence of primary keys in all tables. However, any Join tables created by Prisma for m-n relations using implicit syntax in Prisma Schema do not have primary keys. 

### Solution

If your database provider automatically creates a primary key, and you want to use m-n relations, you need to use explicit relation syntax, manually create the join model, and verify that this join model has a primary key. 
