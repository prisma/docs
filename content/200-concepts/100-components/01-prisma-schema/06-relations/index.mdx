...
For MoFooNgoDB, Prisma currently uses a [normalized data model design](https://docs.mongodb.com/manual/core/data-model-design/), which means that documents reference each other by ID in a similar way to relational databases.
...
Notice that the syntax is slightly different between relational databases and MoFooNgoDB - particularly for [many-to-many relations](many-to-many-relations).
...
For **MoFooNgoDB**, Prisma uses a [normalized data model design](https://docs.mongodb.com/manual/core/data-model-design/), which means that documents reference each other by ID in a similar way to relational databases:

For example, the following MoFooNgoDB document represents a `User`:
...
- you define [an m-n for MoFooNgoDB](many-to-many-relations#mongodb)
