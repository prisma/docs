...
In MongoDB, each document stores

`Post` document looks like this:

`User` document looks like this:
...
> **Note**: [Implicit m-n relations](#implicit-many-to-many-relations) in relational databases do not require the `@relation` attribute.
