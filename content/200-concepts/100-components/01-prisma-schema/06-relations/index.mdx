...
1. Sets the `authorId` to `4` on the `Post` record where the `id` is `20`
...
When you define two relations between two same models, you need to add the `name` argument in the `@relation` attribute to disambiguate them. As an example for why that's needed, consider the following models:
