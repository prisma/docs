...
In relational databases, m-n-relations are typically modelled via [relation tables](../relations/many-to-many-relations#relation-tables). m-n-relations can be either [explicit](#explicit-many-to-many-relations) or [implicit](#implicit-many-to-many-relations) in the Prisma schema.
...
1. Creates a `Post`
2. Creates a category assignment, or `CategoryToPost` (assigned by Bob, assigned today)
...
2. Creates a new category assignment, or `CategoryToPost`
...
- Do **not** require the `@relation` attribute unless you need to [disambiguate relations](../relations#disambiguating-relations) (this is not specific to implicit n-m relations).
...
If you obtain your data model from [introspection](../../introspection), you can still use implicit many-to-many relations by following Prisma's [conventions for relation tables](#conventions-for-relation-tables-in-implicit-m-n-relations).
