...
In MongoDB, many-to-many (M:N) relations are represented by:

- Relation fields on both sides, include a `@relation` attribute on _each side_, with mandatory `fields` and `references` arguments
...
model Post {
  ...
  categories  Category[] @relation(fields: [b_ids], references: [id])
}

model Category {
  id      String   @id @default(auto()) @map("_id") @db.ObjectId
  postIDs String[] @db.ObjectId
  posts   Post[]   @relation(fields: [a_ids], references: [id])
}
```

### Validation rules

For MongoDB, Prisma validates M:N relations with the following rules:

- Both sides of the relation must be arrays (in the data model the fields categories and posts).
- The `@relation` attribute must define `fields` and `references` arguments
- The fields argument must have only one scalar field defined, this scalar field must be of an array type.
- The references argument must have only one scalar field defined, this scalar field must exist on the referenced model and must be of the same type as the scalar field in the fields argument, but singular (no array).
- The scalar field where references points to must be an id.
- No referential actions are allowed in this kind of relations (yet, support only after GA)
- The implicit M:N relations as seen in SQL databases are not allowed with MongoDB.
