...
In relational databases, m-n-relations are typically modelled via [relation tables](#relation-tables). m-n-relations can be either [explicit](#explicit-many-to-many-relations) or [implicit](#implicit-many-to-many-relations) in the Prisma schema.
...
The following query returns all categories where at last one (`some`) related `Post` record titles contain the words `"Cool stuff"` _and_ the category was assigned by Bob.
...
Implicit many-to-many relations define relation fields as lists on both sides of the relation. Although the relation table exists in the underlying database, **it is managed by Prisma and does not manifest in the Prisma schema**:

In the example below, there's one _implicit_ m-n-relation between `Post` and `Category`:

<TabbedContent tabs={[<FileWithIcon text="Prisma schema" icon="schema"/>, <FileWithIcon text="SQL" icon="database"/>]}>
<tab>
...
Implicit m-n relations makes the [Prisma Client API](../prisma-client/) for many-to-many relations a bit simpler (since you have one fewer level of nesting inside of [nested writes](../prisma-client/relation-queries#nested-writes)).
...
* Do **not** require the `@relation` attribute unless you need to [disambiguate relations](#disambiguating-relations) (this is not specific to implicit n-m relations).
...
#### Manually defining an m-n implicit relation
...
In this example, the `assignedAt` field stores additional information about the relation between `Post` and `Category` (i.e. it stores the point in time when "the post was added to the category").
