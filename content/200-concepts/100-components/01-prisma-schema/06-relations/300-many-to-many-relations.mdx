...
- Do **not** require the `@relation` attribute unless you need to [disambiguate relations](/concepts/components/prisma-schema/relations#disambiguating-relations) with a name, e.g. `@relation("MyRelationTable")` or `@relation(name: "MyRelationTable")`.
  - If you do use the `@relation` attribute for disambiguation, you cannot use the `references`, `fields`, `onUpdate` or `onDelete` arguments. This is because these take a fixed value for implicit m-n relations and cannot be changed.
...
The name of a relation table for an implicit-many-to-many relation is constructed from the following parts:

- It must start with an underscore `_`
- Then follows the alphabetically lower table name (in this case `Category`)
- After which `To` connects to the rest of the name
- Which is the alphavetically higher table name (in this case `Post`)

In the example the correct table name is `_CategoryToPost`.

(When creating an implicit m-n relation via Prisma itself, you can also [configure the name of the relation table to be different](#configuring-the-name-of-the-relation-table-in-implicit-many-to-many-relations) to be for example `_MyRelationTable`)
