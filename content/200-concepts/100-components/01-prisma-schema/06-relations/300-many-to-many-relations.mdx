...
- Both sides of the relation must be arrays (in the example data model above the fields `categories` and `posts`).
...
- The scalar field to which `references` points must be an `id`.
- No referential actions are allowed in this kind of relations (during Preview)
