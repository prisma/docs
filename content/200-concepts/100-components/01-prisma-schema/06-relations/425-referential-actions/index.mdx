...
```prisma file=schema.prisma highlight=4;normal copy
model Post {
...
```prisma file=schema.prisma highlight=10,16-17;normal copy
model User {
  id    Int    @id @default(autoincrement())
  posts Post[]
}
...
```prisma file=schema.prisma highlight=4;add copy
model Post {
  id       Int    @id @default(autoincrement())
  title    String
...
```prisma file=schema.prisma highlight=4,5;add copy
model Post {
  id             Int     @id @default(autoincrement())
  title          String
...
```ts highlight=14;delete|15;add copy
import { PrismaClient } from '@prisma/client'
