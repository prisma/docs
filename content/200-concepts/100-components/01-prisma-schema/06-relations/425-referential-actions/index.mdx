...
## Referential actions behavior before Prisma 3.0.0

> Warning: The following section is only relevant if you are using Prisma older than 3.0.0 (to be more precise: For all uses of Prisma 2.25.0 and earlier, and 2.26.0 until 3.0.0 _without_ the `referentialActions` preview flag). You can completely disregard it if you are using Prisma 3.0.0 or later. 

Before Prisma implemented support for configuring referential actions in Prisma 3.0.0 (and before that as a preview feature since 2.26.0 via the preview feature flag `referentialActions`), it already had some preventing behavior on `delete` and  `deleteAll` toa make sure not referential actions could be triggered, but also some default referential actions when using Prisma Migrations (which could be triggered via "raw queries" and other database clients):

When invoking the [`delete()`](../../../prisma-client/crud#delete-a-single-record) or [`deleteAll()`](../../../prisma-client/crud#delete-all-records) methods using the Prisma Client on _required relations_ (!), a runtime check is performed and the deletion of records prevented if they are referencing related objects. **This prevents cascade behavior**.
