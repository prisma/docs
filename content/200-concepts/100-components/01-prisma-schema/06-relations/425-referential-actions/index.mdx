...
Referential actions determine what should happen to a record when a related record is deleted or updated. In [2.26.0](https://github.com/prisma/prisma/releases/tag/2.26.0) and later, you can define referential actions on your relation fields in your Prisma schema.
...
The schema does not explicitly define referential actions on the mandatory `author` field, which means that the default referential actions of `Restrict` for `onDelete` and `Cascade` for `onUpdate` apply.
