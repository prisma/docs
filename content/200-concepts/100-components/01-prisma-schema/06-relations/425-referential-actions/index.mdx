...
Referential actions determine what should happen to a record when a related record is deleted or updated. In [2.26.0](https://github.com/prisma/prisma/releases/tag/2.26.0) and later, you can define referential actions on your relation fields.
...
The following model defines a one-to-many relation between `User` and `Post` and a many-to-many relation between `Post` and `Tag`:
...
- If you delete a `User`, the author is removed from all posts, using the `SetNull` referential action. To allow this, `User` and `userId` must be optional fields in `Post`.
...
For example, in the following model all `Post` records must be connected to an `author`:
...
The schema does not explicitly define referential actions on the mandatory `author` field, which means that the default referential actions of `Restrict` for `onDelete` and `Cascade` for `onUpdate` apply. See the SQL tab for details of how this is implemented.
...
From version 3.6.0, referential actions will also be available in MongoDB if the `[referentialIntegrity](/concepts/components/prisma-schema/relations/referential-integrity)` Preview feature is enabled:
...
The `Restrict` action is **not** available on [Microsoft SQL Server](/concepts/database-connectors/sql-server) and will trigger a schema validation error. Instead, you can use [`NoAction`](#noaction), which produces the same result and is compatible with SQL Server.
