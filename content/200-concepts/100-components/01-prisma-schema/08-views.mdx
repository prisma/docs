...
Some databases support updatable views (e.g. [PostgreSQL](https://www.postgresql.org/docs/current/sql-createview.html#SQL-CREATEVIEW-UPDATABLE-VIEWS), [MySQL](https://dev.mysql.com/doc/refman/8.0/en/view-updatability.html) and [SQL Server](https://learn.microsoft.com/en-us/sql/t-sql/statements/create-view-transact-sql?view=sql-server-ver16#updatable-views)) which means you can create, update or delete entries when interacting with these views. 

Currently Prisma treats all `view`s as updatable views. If the underlying database supports this functionality for the view, the operation should succeed. If the view is not marked as updatable, the database will return an error that Prisma Client will throw as an error.

In the future Prisma Client might support marking individual views as updatable or not updatable. Please [comment with your use case to our `views` feedback issue](https://github.com/prisma/prisma/issues/17335). Thanks.

### Materialized views

Some databases support materialized views (e.g. [PostgreSQL]([https://www.postgresql.org/docs/current/sql-createview.html#SQL-CREATEVIEW-UPDATABLE-VIEWS](https://www.postgresql.org/docs/current/rules-materializedviews.html)), [CockroachDB](https://www.cockroachlabs.com/docs/stable/views.html#materialized-views), [MongoDB](https://www.mongodb.com/docs/manual/core/materialized-views/) and [SQL Server under the name "indexed views"](https://learn.microsoft.com/en-us/sql/relational-databases/views/create-indexed-views?view=sql-server-ver16)) which means that the result of the view query is persisted for faster access and only updated on demand. 

Currently Prisma has no understanding of materialized views. But [when creating a view manually](#create-a-view-in-the-underlying-database) you can also create a materialized view by using the adequate command. You can then use Prisma's [raw query functionality](../prisma-client/raw-database-access) to execute the command to refresh the view manually.

In the future Prisma Client might support marking individual views as materializable and add a Prisma Client method to refresh the materialized view. Please [comment with your use case to our `views` feedback issue](https://github.com/prisma/prisma/issues/17335). Thanks.
