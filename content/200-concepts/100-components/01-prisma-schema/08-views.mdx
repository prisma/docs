...
Introspection of a database with one or more existing views will also create a new `views` folder within your `prisma` directory (starting with Prisma version 4.12.0). This folder will a folder named after your database's schema which contains a `.sql` file for each view that was introspected in that schema. Each file will be named after an individual view and will contain the query the related view defines.
