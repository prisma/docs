...
- [Add views to your Prisma schema](#add-views-to-your-prisma-schema)
  ...
  - [Use introspection](#use-introspection)
    - [Adding a unique identifier to an introspected view](#adding-a-unique-identifier-to-an-introspected-view)
...
Introspection of a database with one or more existing views will also create a new `views` folder within your `prisma` directory. This folder will contain a `.sql` file for each view that was introspected. Each file will be named after an individual view and will contain the query the related view defines.

For example, after introspecting a database using the model used above you will find a `prisma/views/UserInfo.sql` file was created with the following contents:
