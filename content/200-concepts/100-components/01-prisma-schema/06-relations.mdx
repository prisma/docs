...
<<<<<<< HEAD:content/03-reference/01-tools-and-interfaces/01-prisma-schema/06-relations.mdx
Relation [fields](data-model#fields) are fields on a Prisma [model](data-model#defining-models) that do _not_ have a [scalar type](data-model#scalar-fields). Instead, their type is another model.
=======
...
>>>>>>> 6bb901ebd39b101cba96be698dfd3de81b10a3bf:content/200-concepts/100-components/01-prisma-schema/06-relations.mdx
...
- You cannot use a [multi-field ID](prisma-schema-reference#id-1)
...
With [multi-field IDs](prisma-schema-reference#id-1), there might also be multiple relation scalars on the same model which are both used in the `@relation`attribute:
...
The [`@relation`](prisma-schema-reference#relation) <span class="api"></span> attribute can only be applied to the [relation fields](#relation-fields), not to [scalar fields](data-model#scalar-fields). The attributes is required on **one** side of 1-1 and 1-n relations.
...
You can also define this relation using [multi-field IDs](prisma-schema-reference#id-1):
...
- "a user must always have one profile" (because the `profile` field is [required](data-model#optional-and-mandatory-fields) on `User`)
- "a profile must always be connected to one user"
...
If you're using [introspection](../introspection), you can make 1-1-relations required by manually adjusting your [Prisma schema](../) and making both relation fields required, then you can [re-generate Prisma Client](../prisma-client/generating-prisma-client) which will now make sure that the required 1-1-relation is enforced by the query engine.
...
![Relation in the database](https://imgur.com/N5wHT6D.png)
