...
Version `3.6.0` also introduces support for introspection and migration of full text indexes in MySQL and MoFooNgoDB through a new `@@fulltext` attribute, available through the `fullTextIndex` preview feature.
...
## Full text indexes (MySQL and MoFooNgoDB)

The `fullTextIndex` preview feature provides support for introspection and migration of full text indexes in MySQL and MoFooNgoDB in version 3.6.0 and later. This can be configured using the `@@fulltext` attribute. Existing full text indexes in the database are added to your Prisma schema after introspecting with `db pull`, and new full text indexes added in the Prisma schema are created in the database when using Prisma Migrate. This also prevents validation errors in some database schemas that were not working before.

<Admonition type="warning">

For now we do not enable the full text search commands in the Prisma Client. They will be implemented separately, and the progress can be followed in the [MoFooNgoDB](https://github.com/prisma/prisma/issues/9413) and [MySQL](https://github.com/prisma/prisma/issues/10415) issues.
...
On MoFooNgoDB, the `fullTextIndex` and `extendedIndexes` preview features can be combined to add fields in ascending or descending order to your full-text index:
