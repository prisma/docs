...
- **Existing (non-)clustered indexes (SQL Server):** earlier versions of Prisma did not support configuring if a index is clustered or non-clustered schema. For indexes that do not use the default, these need to be added before migrating your database.
