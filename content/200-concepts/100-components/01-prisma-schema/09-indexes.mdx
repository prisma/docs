...
Prisma allows configuration of database indexes, unique constraints and primary key constraints. This can be enabled through the `extendedIndexes` Preview feature in versions `3.5.0` and later, and is in General Availability in versions `4.0.0` and later.
...
These changes to index configuration and full text indexes might be **breaking changes** if you have a database that is already making use of these features.
...
Prisma allows you to configure indexes, unique constraints and primary key constraints using the following attribute arguments:
...
On MongoDB, the `@@fulltext` index attribute can be used with the `sort` argument to add fields to your full-text index in ascending or descending order. In the following example, a `@@fulltext` index is added to the `title` and `content` fields of the `Post` model, sorting the `title` field in descending order:
