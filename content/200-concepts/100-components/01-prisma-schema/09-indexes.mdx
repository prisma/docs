...
- The `length` argument allows you to specify a maximum length for the subpart of the value to be indexed on `String` and `Bytes` types
  ...
  - Available on the `@id`, `@@id`, `@unique`, `@@unique` and `@@index` attributes **in MySQL only**, in versions 3.5.0 and later

- The `sort` argument allows you to specify the order that the entries of the constraint or index are stored in the database

  - Available on the `@unique`, `@@unique` and `@@index` attributes in all databases and on the `@id` and `@@id` attributes in SQL Server, in versions 3.5.0 and later

- The `type` allows you to support index access methods other than PostgreSQL's default `BTree` access method

  - Available on the `@@index` attribute **in PostgreSQL only**
  - The `Hash` index access method is available in versions 3.6.0 and later. Additionally the `Gist`, `Gin`, `SpGist` and `Brin` access methods are available in versions 3.14.0 and later

- The `clustered` argument allows you to configure whether a constraint or index is clustered or non-clustered
  - Available on the `@id`, `@@id`, `@unique`, `@@unique` and `@@index` attributes **in SQL Server only**, in version `3.13.0` and later. It allows Prisma to configure (non)clustered indexes.
