...
- The `type` allows you to support index access methods other than PostgreSQL's default `BTree` access method
  ...
  - The `Hash` index access method is available in versions 3.6.0 and later
  - Additionally the `Gist`, `Gin`, `SpGist` and `Brin` access methods are available in versions 3.14.0 and later
