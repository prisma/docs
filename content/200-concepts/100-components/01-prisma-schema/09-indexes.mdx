...
Indexes using a function to determine the indexed value, such as using of `to_tsvector` is not yet supported by Prisma. The indexes defined in such a way will not be visible with `prisma db pull`.
...
Read more about built-in operator classes from [official PostgreSQL documentation](https://www.postgresql.org/docs/14/gin-builtin-opclasses.html).
...
Now queries comparing IP addresses, such as `value > '10.0.0.2'` is using the index.
...
Read more about built-in operator classes from [official PostgreSQL documentation](https://www.postgresql.org/docs/14/gist-builtin-opclasses.html).
...
BRIN index is useful if having lots of data that does not change after inserted, such as date and time values. If the data fits well to the index, it can store large datasets into a minimal space.
...
Now a query, such as `value = 2` is using the index, which uses a fraction of a space compared to the `BTree` or `Hash` indexes.

##### Supported Operator Classes for BRIN

Prisma generally supports operator classes delivered with PostgreSQL version 10 forward; and some of the supported operators are only available from PostgreSQL version 14 forward. If the operator class requires field type to be of a type Prisma yet does not support, using the `raw` function with a string input allows using these operator classes without validation.
...
Read more about built-in operator classes from [official PostgreSQL documentation](https://www.postgresql.org/docs/14/brin-builtin-opclasses.html).
