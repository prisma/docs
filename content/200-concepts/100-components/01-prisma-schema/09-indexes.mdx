...
The `Hash` type will store the index data in a format that is much faster to search and insert, and that will use less disk space. However, only the `=` and `<>` can use the index, so other comparison operators such as `<` and `>` will be much slower than the default `BTree` type.
