...
The operator classes marked to work with CockroachDB are the only ones allowed on that database. The operator class can further not be changed from the one possible value, `ops` is not necessary or allowed for CockroachDB.
