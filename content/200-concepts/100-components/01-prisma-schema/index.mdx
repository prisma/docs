...
1.  The location specified by the [`--schema` flag](/reference/api-reference/prisma-cli/command-reference), which is available when you `introspect`, `generate`, `migrate`, and `studio`:
...
- Run the [`prisma format`](/reference/api-reference/prisma-cli/command-reference#format) <span class="api"></span> command.
