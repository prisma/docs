...
> **Note**: `///` comments are **not** present of the generated Prisma Client, but are present of the internal Data Model Meta Format (DMMF).
...
PSL syntax ships with a formatter for `.prisma` files. There are two ways to format `.prisma` files:
...
Like `gofmt` and unlike `prettier`, there are no configuration options - [formatting rules](#formatting-rules) are fixed.
