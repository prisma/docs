...
Environment variables can be used to provide configuration values in the schema, such as database connection urls.

By using environment variables instead of hardcoding values, you can **improve the portability of the schema** file, enabling you to use it in different environments.
This has the added benifit of **keeping secrets out of the schema file**, as secrets should never be hardcoded, shared or committed.

They can be accessed using the `env` function:
...
See [Environment variables](../../more/environment-variables) from more information.
