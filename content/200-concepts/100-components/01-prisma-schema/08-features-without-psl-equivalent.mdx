...
- You cannot or do not want to rely on functions such `uuid()`, `cuid()`, and `nanoid()` which are only implemented at Prisma level and do not manifest in the database.
