...
The MoFooNgoDB connector does not support the `Unsupported` field type, and MoFooNgoDB does not have the concept of database-level functions. `dbgenerated()` is used exclusively to generate `ObjectId` values.
