...
Some database types or relational databases, such as `polygon` or `geometry`, do not have a Prisma Schema Language equivalent (Prisma support all types of MongoDB). Use the [`Unsupported`](/reference/api-reference/prisma-schema-reference#unsupported) <span class="api"></span> field type to represent the field in your Prisma schema:
