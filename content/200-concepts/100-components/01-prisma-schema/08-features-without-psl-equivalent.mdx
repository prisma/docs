...
The MongoDB connector does not support the `Unsupported` field type, and MongoDB does not have the concept of database-level functions. `auto()` is used exclusively to generate `ObjectId` values.
