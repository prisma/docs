...
- Two [relation fields(../relations#relation-fields): `author` and `posts`. Relation fields define connections between models at the Prisma level and **do not exist in the database**. These fields are used to generate the Prisma Client.
...
At a Prisma level, a connection between two models is **always** represented by a [relation field(../relations#relation-fields) on **each side** of the relation.
...
- [One-to-one(../relations#one-to-one-relations) (also called 1-1-relation)
- [One-to-many(../relations#one-to-many-relations) (also called 1-n-relation)
- [Many-to-many(../relations#many-to-many-relations) (also called m-n-relation)
...
> **Note**: This schema is the same as the [example data model(../data-model) but has all [scalar fields(../data-model#scalar-fields) removed (except for the required [relation scalars(../relations#annotated-relation-fields-and-relation-scalar-fields)) so you can focus on the [relation fields(../relations#relation-fields).
...
> **Note**: Implicit many-to-many relations do **not** require the `@relation` attribute unless you need to [disambiguate relations(../relations#disambiguating-relations) (this is not specific to implicit n-m relations).
...
If you're not using Prisma Migrate but obtain your data model from [introspection(../../introspection), you can still make use of implicit many-to-many relations by following Prisma's [conventions for relation tables(../relations#conventions-for-relation-tables-in-implicit-m-n-relations).
...
Note that the same rules as for [1-n-relations(../relations#one-to-many-relations) apply (because `Post`↔ `CategoriesOnPosts` and `Category` ↔ `CategoriesOnPosts` are both in fact 1-n-relations), which means one side of the relation needs to be annotated with the `@relation` attribute.

When you don't need to attach additional information to the relation, you can model m-n-relations as [implicit many-to-many relations(../relations#implicit-many-to-many-relations). If you're not using Prisma Migrate but obtain your data model from [introspection(../../introspection), you can still make use of implicit many-to-many relations by following Prisma's [conventions for relation tables(../relations#conventions-for-relation-tables-in-implicit-m-n-relations).
...
The [`@relation`(../../../../reference/api-reference/prisma-schema-reference#relation) <span class="api"></span> attribute can only be applied to the [relation fields(../relations#relation-fields), not to [scalar fields(../data-model#scalar-fields). The attributes are required on **one** side of 1-1 and 1-n relations.

> **Note**: [Implicit m-n relations(../relations#implicit-many-to-many-relations) do not require the `@relation` attribute.
...
- you define a [self-relation(../relations#self-relations)
...
- the [annotated relation field(../relations#annotated-relation-fields-and-relation-scalar-fields) (including its relation scalar)
...
Many-to-many (m-n) relations refer to relations where zero or more records on one side of the relation can be connected to zero or more records on the other side. m-n-relations are typically modelled via [relation tables(../relations#relation-tables) in a relational database. m-n-relations can be either [explicit(../relations#explicit-many-to-many-relations) or [implicit(../relations#implicit-many-to-many-relations) in the Prisma schema.
...
Implicit many-to-many relations are maintained by Prisma with a [relation table(../relations#relation-tables) that's not reflected in the Prisma schema. An implicit m-n-relation does not require `@relation` attribute.

The implicit m-n-relation maps to the following tables (following Prisma's [conventions for relation tables(../relations#conventions-for-relation-tables-in-implicit-m-n-relations)):
...
- One relation field must be a [fully annotated(../relations#annotated-relation-fields-and-relation-scalar-fields). In this example, the `successor` field defines both the `field` and `references` arguments.
...
Note that this n-n-relation is [implicit(../relations#implicit-many-to-many-relations). This means Prisma maintains a [relation table(../relations#relation-tables) for it in the underlying database:
...
There are a couple of ways to define a m-n relationship, implicitly or explicity. Implicitly means letting Prisma handle the relation table (JOIN table) under the hood, all you have to do is define an array/list for the non scalar types on each model, see [implicit many-to-many relations(../relations#implicit-many-to-many-relations).

Where you might run into trouble is when creating an [explicit m-n relationship(../relations#explicit-many-to-many-relations), that is, to create and handle the relation table yourself. **It can be overlooked that Prisma requires both sides of the relation to be present**.
