...
```prisma highlight=10-45;normal copy
datasource db {
  provider = "postgresql"
...
```prisma highlight=2-4,8;normal copy
model Comment {
  id      Int    @id @default(autoincrement())
  title   String
  content String
...
```prisma highlight=4,10;normal copy
model Post {
  id       Int       @id @default(autoincrement())
  // Other fields
...
```prisma highlight=4,5;normal copy
model Post {
  id       Int       @id @default(autoincrement())
  // Other fields
...
```prisma highlight=2;normal copy
model User {
  id      Int      @id @default(autoincrement())
  email   String   @unique
  name    String?
...
```prisma highlight=3,5;normal copy
model Post {
  id         Int        @id @default(autoincrement())
  createdAt  DateTime   @default(now())
  title      String
...
```prisma highlight=10;normal copy
model Post {
  id         Int        @id @default(autoincrement())
  createdAt  DateTime   @default(now())
  title      String
...
```prisma highlight=5,8-11;normal copy
model User {
  id    Int     @id @default(autoincrement())
  email String  @unique
  name  String?
