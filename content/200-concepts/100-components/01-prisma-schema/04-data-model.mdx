...
- [Models](#defining-models) ([`model`](prisma-schema-reference#model) <span class="api"></span> primitives) that define a number of fields, including [relations between models](#relation-fields)
- [Enums](#defining-enums) ([`enum`](prisma-schema-reference#enum) <span class="api"></span> primitives) - only available if [enums are supported natively by your database](../../database-connectors/database-features#misc)
...
Models represent the entities of your application domain. Models are represented by [`model`](prisma-schema-reference#model) <span class="api"></span> blocks and define a number of [fields](prisma-schema-reference#model-fields). In the [example data model](data-model), `User`, `Profile`, `Post` and `Category` are models.
...
> **Note**: You can also [`@map`](prisma-schema-reference#map) <span class="api"></span> a column name or enum value, and `@@map` an enum.
...
- A **[field name](prisma-schema-reference#model-fields)** <span class="api"></span>
...
- [Scalar types](data-model#scalar-fields) (includes [enums](data-model#defining-enums)) - for example, [`String`](prisma-schema-reference#string) <span class="api"></span> or [`Int`](prisma-schema-reference#int) <span class="api"></span>
...
See [complete list of scalar field types](prisma-schema-reference#model-field-scalar-types) <span class="api"></span>.
...
Follow [`topic:native database types`](https://github.com/prisma/prisma/issues?q=is%3Aissue+is%3Aopen+label%3A%22topic%3A+native+database+types%22+) and [`topic:types`](https://github.com/prisma/prisma/issues?q=is%3Aissue+is%3Aopen+label%3A%22topic%3A+types%22+) for information about type support. 
...
* [`[]`](prisma-schema-reference#--modifier) <span class="api"></span> Make a field a list
* [`?`](prisma-schema-reference#-modifier) <span class="api"></span> Make a field optional
...
Attributes modify the behavior of fields or model blocks. The following example includes two field attributes ([`@unique`](prisma-schema-reference#unique) <span class="api"></span> and [`@default`](prisma-schema-reference#default)) <span class="api"></span> and one block attribute (`@@id`):
...
Some attributes accept [arguments](prisma-schema-reference#attribute-argument-types) <span class="api"></span> - for example, `@default` accepts `true` or `false`:

```prisma
isAdmin   Boolean @default(false) // short form of @default(value: false)
```

See [complete list of field and block attributes](prisma-schema-reference#attributes) <span class="api"></span>

### Defining an ID field

An ID uniquely identifies individual records of a model. IDs can be defined by a single field using the [`@id`](prisma-schema-reference#id) <span class="api"></span> attribute, or multiple fields using the [`@@id`](prisma-schema-reference#id-1) <span class="api"></span> attribute.
...
You can add unique attributes to your models to be able to uniquely identify individual records of that model. Unique attributes can be defined on a single field using [`@unique`](prisma-schema-reference#unique) <span class="api"></span> attribute, or on multiple fields (also called composite or compound unique constraints) using the [`@@unique`](prisma-schema-reference#unique-1) <span class="api"></span> attribute.
...
You can define default values for scalar fields of your models using the [`@default`](prisma-schema-reference#default) <span class="api"></span> attribute:
...
- [Functions](#using-functions), such as [`now()`](prisma-schema-reference#now) or [`uuid()`](prisma-schema-reference#uuid)
...
If you do not specify an index name, introspection generates [a name based on Prisma's naming convention for indexes (see `name`)](prisma-schema-reference#arguments-5).

## Defining enums

You can define enums in your data model [if they're supported by the data source you use](../../database-connectors/database-features#misc).
...
Enums are defined via the [`enum`](prisma-schema-reference#enum) <span class="api"></span> block. For example, a `User` has a `Role`:
...
The Prisma schema supports a number of [functions](prisma-schema-reference#attribute-functions) <span class="api"></span>. These can be used to specify [default values](prisma-schema-reference#default) on fields of a model. 

For example, the default value of `createdAt` is [`now()`](prisma-schema-reference#now) <span class="api"></span>:
