...
[Native database type attributes](../../preview-features/native-types) are a preview feature in 2.10.0. To enable this feature, add native types to the `generator` block as shown and run `npx prisma generate`:
...
When `nativeTypes` is enabled, each Prisma type (such as `String`):

* Maps to a default native database type, such as `text` (PostgreSQL). In this case, the field does **not** have a native database type attribute:
  
  ```prisma
  model Post {
  id      Int    @id
  | content String // Maps to text in PostgreSQL
  }
  ```
  
* Additionally maps to other native database types, such as `char(n)` or `varchar(x)` (PostgreSQL). Each `String` field that does not map to `text` has a **native database type attribute** that tells you what the underlying database type is:

  ```prisma
  model Post {
    id      Int    @id
  |  title   String @db.VarChar(200)
    content String // Maps to default 'text' type
...
* [`?`](../../../reference/api-reference/prisma-schema-reference#-modifier) <span class="api"></span> Make a field optional
