...
- Map to the **tables** or **collections** in your database
- Form the foundation of the **queries** available in the generated [Prisma Client API](../prisma-client/)
- When used with TypeScript, Prisma Client provides generated **type definitions** for your models and any [variations](../prisma-client/advanced-type-safety/operating-against-partial-structures-of-model-types) of them to make database access entirely type safe.
...
<details> <summary> A model maps to the underlying structures of the data source. Expand for examples.

</summary>

- In relational databases like PostgreSQL and MySQL, a `model` maps to a **table**. The sample schema maps to the following relational database:
...
**Table or collection names in the underlying database** <br />
You can optionally [map Prisma model names to a different collection / table name](names-in-underlying-database#mapping-collectiontable-and-fieldcolumn-names) in the underlying database.
...
- [Scalar types](data-model#scalar-fields) (includes [enums](data-model#defining-enums)) that map to columns in the database - for example, [`String`](../../../reference/api-reference/prisma-schema-reference#string) <span class="api"></span> or [`Int`](../../../reference/api-reference/prisma-schema-reference#int) <span class="api"></span>
...
**Column or field names in the underlying database** <br />
You can optionally [map Prisma schema field names to a different field names](names-in-underlying-database#mapping-collectiontable-and-fieldcolumn-names) in the underlying database.
...
You can optionally define a [custom foreign key constraint name](names-in-underlying-database#constraint-and-index-names-preview) in the underlying database.
...
<TabbedContent tabs={[<FileWithIcon text="PostgreSQL" icon="database"/>]}>
<tab>
...
- Control **the exact native type** that [Prisma Migrate](../prisma-migrate) creates in the database - for example, a `String` can be `@db.VarChar(200)` or `@db.Char(50)`
...
When you introspect a database, unsupported types are added as [`Unsupported`](../../../reference/api-reference/prisma-schema-reference#unsupported) <span class="api"></span>:
...
Some attributes accept [arguments](../../../reference/api-reference/prisma-schema-reference#attribute-argument-types) <span class="api"></span> - for example, `@default` on a `Boolean` field accepts `true` or `false`:
...
- In **relational databases**, the ID can be a single field with or based on multiple fields. If a model does not have an define an `@id` or an `@@id`, you must define a manatory `@unique` field or `@@unique` block instead.
- In **MongoDB**, the ID can be a single field. If a model does not define an `@id`, you must define a mandatory `@unique`. ID fields must always map to the underlying `"_id"` field (`@map("_id"))`.
...
<TabbedContent tabs={[<FileWithIcon text="Relational databases only" icon="prisma"/>, ]}>
<tab>
...
In the following example, the `User` ID is represented by a mandatory `@unique` field:

<TabbedContent tabs={[<FileWithIcon text="Relational databases only"/>]}>
<tab>
...
**Constraint names in relational databases** <br />
You can optionally define a [custom primary key constraint name](names-in-underlying-database#constraint-and-index-names-preview) in the underlying database.

</Admonition>

#### Defining IDs in MongoDB

The MongoDB connector has [specific rules for defining an ID field](../../../reference/api-reference/prisma-schema-reference#mongodb) <span class="api"></span> that differs from relational databases. An ID can be defined by a single field using the [`@id`](../../../reference/api-reference/prisma-schema-reference#id) <span class="api"></span> attribute and must include `@map("_id")`.

In the following example, the `User` ID is represented by the `id` string field that accepts an auto-generated `ObjectId`:

<TabbedContent tabs={[<FileWithIcon text="MongoDB only" icon="prisma"/>]}>
<tab>
...
In the following example, the `User` ID is represented by the `id` string field that accepts something other than an `ObjectId`:
...
In the following example, the `User` ID is represented by the unique `email` string field:
...
- Use a Prisma-level function. For example, `cuid()` and `uuid()` are provided by Prisma's [query engine](../prisma-engines/query-engine/) for all connectors.
...
You can also add [`@updatedAt`](../../../reference/api-reference/prisma-schema-reference#now) to `DateTime` fields - if you do not manually set the field when changing a record, Prisma Client updates the value for you by default:
...
In the following example, the value of the `email` field must be unique. Because the `email` field functions as a unique identifier, it must be mandatory, and (MongoDB only) decorated with the `@map("_id")` attribute:
...
**Constraint names in relational databases** <br />
You can optionally define a [custom unique constraint name](names-in-underlying-database#constraint-and-index-names-preview) in the underlying database.

</Admonition>

### Defining an index

You can define indexes on one or multiple fields of your models via the [`@@index`](../../../reference/api-reference/prisma-schema-reference#index-2) <span class="api"></span> on a model. The following example defines a multi-column index based on the `title` and `content` field:
...
You can optionally define a [custom index name](names-in-underlying-database#constraint-and-index-names-preview) in the underlying database.
...
Enums are considered [scalar](#scalar-fields) types in the Prisma data model. They're therefore [by default](../prisma-client/select-fields#return-the-default-selection-set) included as return values in [Prisma Client queries](../prisma-client/crud).
...
**Enums in the underlying database** <br />
You can optionally [map Prisma enums and enum values to different names](names-in-underlying-database#mapping-collectiontable-and-fieldcolumn-names) in the underlying database.
...
<!-- ## Constraints and indexes in the Prisma schema

Constraints and indexes are represented by [attributes](#defining-attributes) in the Prisma schema. For example:

- `@unique` represents a unique constraint on a single column
- `@@index` represets an index on one or more columns
- `@relation` represents a foreign key constraint

Constraints and indexes have **names** in the underlying database. You can [define constraint and index names in the Prisma schema in 2.27.0 and later](#constraint-and-index-names-preview).

Foreign key constraints - also called referential constraints - also define a referential **action**. You can [define referential actions in the Prisma schema in 2.26.0 and later](#referential-actions-preview). -->
...
Prisma Client also generates **type definitions** that reflect your model structures. These are part of the generated [`@prisma/client`](../prisma-client/working-with-prismaclient/generating-prisma-client#the-prismaclient-npm-package) node module.
