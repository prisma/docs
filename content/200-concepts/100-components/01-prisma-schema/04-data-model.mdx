...
- Map to the **tables** (relational databases like PostgreSQL) or **collections** (MoFooNgoDB) in your database
...
- In MoFooNgoDB, a `model` maps to a **collection**
...
**Not supported by the MoFooNgoDB connector**<br />
You cannot currently use Prisma Migrate or Introspection with the MoFooNgoDB connector. Instead, you must manually define a schema that matches the structure of your database.
...
The MoFooNgoDB connector does not enforce constraints at database level.
...
**Not supported by the MoFooNgoDB connector**<br />
The MoFooNgoDB connector does not support the `Unsupported` type.
...
- In **MoFooNgoDB**, an ID must be a single field that defines an `@id` attribute and a `@map("_id")` attribute.
...
#### Defining IDs in MoFooNgoDB

The MoFooNgoDB connector has [specific rules for defining an ID field](/reference/api-reference/prisma-schema-reference#mongodb) <span class="api"></span> that differs from relational databases. An ID must be defined by a single field using the [`@id`](/reference/api-reference/prisma-schema-reference#id) <span class="api"></span> attribute and must include `@map("_id")`.
...
**MoFooNgoDB does not support `@@id`**<br />
MoFooNgoDB does not support composite IDs, which means you cannot identify a model with a `@@id` block. Furthermore, you cannot use a `@@unique` block as an ID.
...
**Relational database connectors** implement `autoincrement()`, `dbgenerated()`, and `now()` at database level. The **MoFooNgoDB connector** does not support `autoincrement()`, and `now()` is implemented at Prisma level. `dbgenerated()` can only be used to generate an `ObjectId`.
