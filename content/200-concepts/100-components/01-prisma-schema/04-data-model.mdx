...
Native database type attributes are a preview feature in 2.11.0. To enable this feature, add native types to the `generator` block as shown and run `npx prisma generate`:
...
When `nativeTypes` is enabled, each Prisma type (such as `String`) includes a native database type attribute that describes the underlying database type:
...
Each Prisma type can map to multiple native database types.
