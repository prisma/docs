...
**Not supported by the MongoDB connector**<br />
The MongoDB connector does not require native type attributes, as each Prisma type maps to one BSON type. However, the MongoDB connector does use [`@db.ObjectId` to map a `String` or `Bytes` ID field](). // TODO: Link to ID section
...
<TabbedContent tabs={[<FileWithIcon text="Relational databases only" icon="schema"/>, ]}>
<tab>
...
<TabbedContent tabs={[<FileWithIcon text="MongoDB only" icon="schema"/>]}>
<tab>
