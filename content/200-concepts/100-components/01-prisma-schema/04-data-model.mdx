...
- Use a Prisma-level function. For example, `cuid()`, `uuid()`, and `nanoid()` are provided by Prisma's [query engine](/concepts/components/prisma-engines/query-engine) for all connectors.
