...
> **Note**: A model can only have _one_ ID, and that ID can either be a single field or based on multiple fields. If a model does not have an define an `@id` or an `@@id`, it must have a mandatory `@unique` or `@@unique` instead.
