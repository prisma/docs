...
You can define indexes on one or multiple fields of your models via the [`@@fulltext`](/reference/api-reference/prisma-schema-reference#fulltext) <span class="api"></span> attribute on a model. The following example defines a multi-column index based on the `title` and `content` field:
...
#### Defining full text indexes for composite type fields

In MongoDB in version `3.12.0` and later, you can define a full text index on a field `field` of a [composite type](#defining-composite-types) `compositeType` using the syntax `@@fulltext([compositeType.field])`. As with other fields, composite type fields can be used as part of a multi-column index.

The following example defines a multi-column full text index based on the `email` field of the `User` model and the `street` field of the `Address` composite type:
