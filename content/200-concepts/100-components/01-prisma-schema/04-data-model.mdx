...
### Defining native database types

TODO

In version [2.17.0]() and later, introspection adds a native database type attribute that describes the underlying database type:
...
Native database type attributes are:

- Specific to the underlying provider - for example, PostgreSQL uses `@db.Boolean` for `Boolean` whereas MySQL uses `@db.Bit(1)`
