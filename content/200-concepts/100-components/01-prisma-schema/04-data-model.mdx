...
Attributes modify the behavior of fields or model blocks. The following example includes two field attributes ([`@unique`](../../../reference/api-reference/prisma-schema-reference#unique) <span class="api"></span> and [`@default`](../../../reference/api-reference/prisma-schema-reference#default) <span class="api"></span>) and one block attribute ([`@@id`](../../../reference/api-reference/prisma-schema-reference#id-1) <span class="api"></span>):

```prisma highlight=4,5,7;normal
model User {
  firstName String
  lastName  String
  email     String @unique
...
An ID uniquely identifies individual records of a model. IDs can be defined by a single field using the [`@id`](../../../reference/api-reference/prisma-schema-reference#id) <span class="api"></span> attribute, or multiple fields using the [`@@id`](../../../reference/api-reference/prisma-schema-reference#id-1) <span class="api"></span> (relational databases only) attribute.

The syntax for defining an ID differs between relational databases and MongoDB. In the following example, the `User` ID is represented by the `id` field:
