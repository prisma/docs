...
<TabbedContent tabs={[<FileWithIcon text="With default value" icon="schema"/>, <FileWithIcon text="Without default value" icon="schema"/>]}>
<tab>
...
In the following example, the `User` ID is represented by the `id` field. The default value is an auto-generated [`ObjectId`](https://docs.mongodb.com/manual/reference/method/ObjectId/). The MongoDB connector has [specific rules for defining an ID field](../../../reference/api-reference/prisma-schema-reference#mongodb) <span class="api"></span> that differs from relational databases.

In the following example, the `User` ID is represented by the `id` string field that accepts an `ObjectId`:
