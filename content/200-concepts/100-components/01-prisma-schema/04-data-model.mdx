...
- In **relational databases**, the ID can be a single field with or based on multiple fields. If a model does not have an define an `@id` or an `@@id`, you must define a mandatory `@unique` field or `@@unique` block instead.
