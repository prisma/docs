...
The correct file is particularly important when [deploying](/prisma-orm/deployment/deployment) your application to production, which often differs from your local development environment.
...
> **Note**: The `native` binary target is the default. You can set it explicitly if you wish to [include additional binary targets for deployment to different environments](/prisma-orm/deployment/deployment-guides/deploying-to-aws-lambda#binary-targets-in-schemaprisma).
