...
- `binaryTargets`: Engine binary targets for `prisma-client-js` (for example, `debian-openssl-1.0.x` if you are deploying to Ubuntu 14.04, or `native` if you are working locally)
...
The correct file is particularly important when [deploying](../../../guides/deployment/deployment) your application to production - which often is another platform than your local development environment.
