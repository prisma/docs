...
> **Note**: The `native` binary target is the default. You can set it explicitly if you wish to [include additional binary targets for deployment to different environments](/guides/deployment/supported-platforms/deploying-to-aws-lambda#binary-targets-in-schemaprisma).
