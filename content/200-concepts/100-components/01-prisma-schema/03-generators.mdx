...
> **Note**: The `native` binary target is the default. You can set it explicitly if you wish to [include additional binary targets for deployment to different environments](/guides/deployment/deployment-guides/use-prisma-with-serverless-framework#reduce-the-package-footprint).
