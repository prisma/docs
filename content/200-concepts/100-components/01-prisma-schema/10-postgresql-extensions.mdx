...
PostgreSQL allows you to extend your database functionality by installing packages known as _extensions_. For example, the `citext` extension adds a case-insensitive string data type. For more information on extensions, see [the PostgreSQL documentation](https://www.postgresql.org/docs/current/external-extensions.html).
