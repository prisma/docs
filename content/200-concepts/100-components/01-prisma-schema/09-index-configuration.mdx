...
In version `3.6.0` and later, a new `type` argument is also available on the `@@index` field in PostgreSQL. This argument allows Prisma to support the `Hash` index type as well as the default `BTree` type.
...
This allows you to use `Hash` as the index algorithm, instead of the default `BTree` option. The `Hash` index algorithm can be much faster for inserts, but only supports checking for equality, whereas the `BTree` algorithm supports all comparison operators.
