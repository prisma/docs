...
From version `3.6.0` onwards, there will also be a new `@@fulltext` attribute available as a preview feature. This provides support for introspection and migration of full text indexes in MySQL and MongoDB. The `sort` argument is also available for `@@fulltext` on MongoDB.
