...
1. Make your schema reflect the current state of the database by running an [Introspection](../introspection). You now have the option to opt in and use the new referential actions if you wish.
2. Define your custom referential actions on your data model relation fields. You then update the database schema by running a [Migration](../prisma-migrate) or the [`db push`](../prisma-migrate/db-push) command.
...
To begin using referential actions you must opt in and add them to the `@relation` attribute in your schema.
