...
1. Make your schema reflect the current state of the database by running an [Introspection](../introspection). You can now see all relations in your data model using a different referential action compared to the new defaults.
...
3. Use the new default referential actions that exist when no custom actions are added manually by running a [Migration](../prisma-migrate), or [`db push`](../prisma-migrate/db-push).
...
The `Restrict` action will prevent the update or deletion of any parent records which have connected children. 
...
3. Upgrading Prisma to 2.26.0, _**enabling**_ the feature flag and **adding custom referential actions**.
  ...
  You can begin by running an [Introspection](../introspection) on your database. This will update your data models to use `ON DELETE CASCADE` and explicitly set `onDelete: Cascade` on your models `@relation` fields. 
  ...
  Next you can manually add a custom referential action to the `User` relation on the `Profile` model by adding the following line after the `references` property; `onDelete: Cascade`. 
