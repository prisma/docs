...
Typically PostgreSQL supports only a certain amount of concurrent connections and this limit can be reached quite fast when the service usage goes up – especially in serverless environments.

A connection pooler like PgBouncer prevents your application from exhausing the database's connection limit.

PgBouncer holds a connection pool to the database and proxies incoming client connections by sitting between Prisma Client and the database.

PgBouncer funnels client connections into a pooling application, which reduces the number of processes a database has to handle at any given time. PgBouncer passes on a limited number of connections to the database and queues additional connections for delivery when space becomes available.

To use Prisma Client with PgBouncer from a serverless function, add the `?pgbouncer=true` flag to the PostgreSQL connection string and set the connection limit to `1`:

```
postgresql://USER:PASSWORD@HOST:PORT/DATABASE?connection_limit=1&pgbouncer=true
```
...
_Transaction mode_ offers a connection for every transaction. With this mode in addition to the , the Prisma query engine is able to operate normally.

<details>
  <summary>How `pgbouncer` mode works in Prisma</summary>
- Prisma will clean up already present prepared statements in the connection by running `DEALLOCATE ALL` before trying to prepare and execute the Prisma Client query.
- Prisma will triggers a transaction for every query case –even when just reading data– which allows Prisma to use prepared statements.
in Prisma.
</details>
...
| Database connection pool exhaustion | Concurrent requests create multiple new connections to the database thereby exhausing the database's connection limit.                                                                                                                            | Add a connection pooler, e.g. [PgBouncer](#usage-with-pgbouncer) or limiting the serverless function concurrency to a number lower than the database's connection limit. |
