...
- Limiting the connection pool of the query engine with the database as described in [connection management](working-with-prismaclient/connection-management#connection-pool)
...
## AWS RDS Proxy & AWS Aurora Serverless Data API

At present, Prisma is not compatible with either RDS Proxy or Aurora Serverless Data API. Both these components enable FaaS applications to rely on AWS managed connection pooling to achieve reliable connection reuse. Support for these is being tracked here:
[RDS Proxy](https://github.com/prisma/prisma/issues/5866)
[Aurora Serverless Data API](https://github.com/prisma/prisma/issues/1964)
