...
Note: If you pass an empty value, instead of `null` or `undefined`, then the `OR` operator returns all items, as it does with the `AND` and `NOT` operators.
...
This is in contrast to the [`AND`](/reference/api-reference/prisma-client-reference#and) <span class="api"></span> and [`NOT`](/reference/api-reference/prisma-client-reference#not) <span class="api"></span> operators, which both return all the users if you pass in an `undefined` value or an empty value.

<Admonition type="info">

This is because if you pass an `undefined` value to an `AND` or `NOT` operator, it is the same as passing nothing at all. The query runs without any filters and returns all the users. However, if you pass an `undefined` value to an `OR` operator, this is **not** the same as passing nothing at all, and the query returns an empty list.
