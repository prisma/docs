...
> This is the same as passing `null`, which maps to `null` in the database. If you were to pass `undefined` that would represent no value at all and so would be the same as passing no conditions to the query.
