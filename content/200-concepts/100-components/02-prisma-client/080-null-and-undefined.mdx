...
There are some caveats to filtering with conditionals which might produce unexpected results. When you filter with conditionals, you might expect one result but receive another because of the way that Prisma treats nullable values.
...
| -------- | ------------------------------------------ | ---------------------- | -------------------- |
| `OR`     | return empty list (but see the note below) | validate single filter | validate all filters |
...
**Note:** If you pass an empty value, instead of `null` or `undefined`, then the `OR` operator returns all items, as with the `AND` and `NOT` operators.

The following example shows how an `undefined` parameter impacts the results returned by a query that uses the [`OR`](/reference/api-reference/prisma-client-reference#or) <span class="api"></span> operator.
...
The query receives filters from a formData object, which includes an optional email property.

- In this instance, the value of the email property is `undefined`. The query returns an empty list.
- If the value of the email property is empty, instead of `undefined`, then the query returns all the users.

This is in contrast to the [`AND`](/reference/api-reference/prisma-client-reference#and) <span class="api"></span> and [`NOT`](/reference/api-reference/prisma-client-reference#not) <span class="api"></span> operators, which will both return all the users if you pass in an `undefined` value or an empty value.

> This is because if you pass an `undefined` value to an `AND` or `NOT` operator, it is the same as passing nothing at all. This means that the `findMany` query in the above example runs without any filters and returns all the users. However, as shown in the above example, this is not the case if you pass an `undefined` value to an `OR` operator. In that situation, the query returns an empty list.
