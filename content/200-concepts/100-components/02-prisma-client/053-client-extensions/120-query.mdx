...
You can use the `query` [Prisma Client extensions](/concepts/components/prisma-client/client-extensions) component type to hook into the query life-cycle and modify an incoming query or its result.
...
`query` extensions modify the default behavior of Prisma Client methods. We recommend you create independent Prisma Client instances when creating `query` extensions.
...
await extendedPrisma.user.findMany() // returns users whose age is greater than 18
```

In the above example, a call to `extendedPrisma.user.findMany` triggers `query.user.findMany`. Each callback receives a type-safe `{ model, operation, args, query }` object that describes the query. This object has the following properties:
...
In the event a [raw query](/concepts/components/prisma-client/raw-database-access) is invoked, the `model` argument passed to the callback will be `undefined`.
