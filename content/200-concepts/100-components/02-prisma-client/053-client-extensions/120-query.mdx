...
await extendedClient.user.findMany() // returns users whose age is greater than 18
```

In the above example, a call to `extendedClient.user.findMany` triggers `query.user.findMany`. Each callback receives a type-safe `{ model, operation, args, query }` object that describes the query. This object has the following properties:
