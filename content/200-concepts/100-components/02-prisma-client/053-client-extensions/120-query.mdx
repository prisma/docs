...
In the event a [raw query](https://www.prisma.io/docs/concepts/components/prisma-client/raw-database-access) is invoked, the `model` argument passed to the callback will be `undefined`.
...
const xprisma = prisma.$extends({
  query: {
    $allModels: {
      $allOperations({ model, operation, args, query }) {
        /* your custom logic for modirying all operations on all models here */
        return query(args)
      },
    },
  },
