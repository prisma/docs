...
When you call a method in an extension, use the constant name from your `$extends` statement, not `prisma`. In the above example, `extendedPrisma.user.findFirst` computes the virtual field, but `prisma.user.findFirst` would not.
...
When you call a method in an extension, use the constant name from your `$extends`statement, not `prisma`. In the above example, `extendedPrisma.user.findUniqueOrThrow` works, but `prisma.user.findUniqueOrThrow` does not.
