...
const extendedPrisma = prisma.$extends(findOrCreate)
const user = await prisma.user.findOrCreate({ ... })

await extendedPrisma.user.findOrCreate()
```

When you call a method in an extension, use the constant name from your `$extends` statement, not `prisma`. In the above example,`extendedPrisma.user.findOrCreate` works, but `prisma.user.findOrCreate` does not, because the original `prisma` is not modified.
