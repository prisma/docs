...
const extendedClient = prisma.$extends(findOrCreate)
const user = await prisma.user.findOrCreate({ ... })

await prisma.user.findOrCreate()
```

When you call a method in an extension, use the constant name from your `$extends` statement, not `prisma`. In the above example,`extendedClient.user.findOrCreate` works, but `prisma.user.findOrCreate` does not, because the original `prisma` is not modified.
