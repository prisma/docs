...
const prisma = new PrismaClient().$extends({
  model: {
    $allModels: {
      // `T` is a generic type that corresponds to the current model
      // `A` is a generic type that corresponds to the argument for the operation
      findMany<T, A>(
        this: T,
        args: Prisma.Exact<
          A,
          // For the `exists` method, use the arguments from model `T` and the `findFirst` method
          // and intersect it with `customArg` as part of `exists` arguments
