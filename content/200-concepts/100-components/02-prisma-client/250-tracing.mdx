...
When you enable tracing, Prisma Client generates a trace for each operation that it makes. Each trace includes one or more [spans](https://www.opentelemetry.io/docs/reference/specification/trace/api/#span). Each span represents the length of time that one stage of the operation takes, such as a database statement or command, or serialization. Span durations are the round trip connection time from Prisma Client to the database and back.
...
- Trace (Prisma operation)
  ...
  You can send tracing output to the console, or analyze it in any OpenTelemetry-compatible tracing system, such as [Jaeger](https://www.jaegertracing.io/), [Honeycomb](https://www.honeycomb.io/trace/) and [Datadog](https://www.datadoghq.com/).

**TBA: maybe a screenshot in Jaeger with example**

## Output

Prisma Client can output any of the following spans. The number and type of spans varies depending on the Prisma operation.

**add one or more screenshots here showing spans - particularly a regular transaction and an interactive one**

- `prisma`: Represents the entire Prisma request, from Prisma Client to the database and back. It contains details such as the model and method called by Prisma Client.
...
If your application sends a large number of spans that are sent to a [collector](https://opentelemetry.io/docs/collector/), this can have a performance impact. For recommended approaches to reduce this performance impact, see [Reduce performance impact](#reduce-performance-impact).
...
export function otelSetup() {
  ...
  // exporter that works with Jaeger and other OpenTelemetry compliant collector
...
If you want to include your [middleware](/concepts/components/prisma-client/middleware) in your tracing, then set `middleware` to `true` in your `registerInstrumentations` statement:
