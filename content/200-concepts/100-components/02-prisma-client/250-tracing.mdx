...
A trace contains X amount of ‘spans’, e.g. 7. This number is dependant on the particular request for example - findMany might only contain 3 spans. Each span represents a different stage in the Prisma request lifecycle. The screenshot is the ‘trace’ expanded showing all the spans.
