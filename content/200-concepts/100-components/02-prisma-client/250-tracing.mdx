...
By default, tracing does not output spans for middleware. To include your [middleware](/concepts/components/prisma-client/middleware) in your traces, set `middleware` to `true` in your `registerInstrumentations` statement:
...
There might be some clock skew between the Prisma Client and the Query Engine.

### Some traces show 0μs

These spans most likely finished too fast for the spans to record meaningful durations.
