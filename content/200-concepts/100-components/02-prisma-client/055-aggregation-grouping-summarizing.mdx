...
This feature is available from version 4.1.0. To use it, you need to add the [preview feature](/concepts/components/preview-features/client-preview-features#enabling-a-prisma-client-preview-feature) `filteredRelationCount` and then run `prisma generate`.

</Admonition>

Use `where` to filter the the fields returned by the `_count` output type. You can do this on both [scalar](/concepts/components/prisma-schema/data-model#scalar-fields) and [relation](/concepts/components/prisma-schema/data-model#relation-fields) fields.
