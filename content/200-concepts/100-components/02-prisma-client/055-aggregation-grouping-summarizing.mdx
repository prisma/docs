...
Prisma Client's [`groupBy`](../../../reference/api-reference/prisma-client-reference#group-by-preview) <span class="api"></span> allows you to **group records** by one or more field values - such as `country`, or `country` and `city` and **perform aggregations** on each group.
...
Use [`count`](../../../reference/api-reference/prisma-client-reference#count) <span class="api"></span> to count the number of records. The following example query counts all users:
