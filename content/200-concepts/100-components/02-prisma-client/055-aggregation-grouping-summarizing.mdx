...
In [2.21.0](https://github.com/prisma/prisma/releases/2.21.0) and later, you can **order by aggregate group**. To enable this feature, add `orderByAggregateGroup` to `previewFeatures` in your schema:
...
The following example sorts results by the `country` with the most users:
