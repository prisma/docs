...
Suppose that you then run `prisma migrate deploy` to [deploy your database changes](/guides/deployment/deploy-database-changes-with-prisma-migrate) and regenerate your Prisma Client with `prisma generate`. Then, you run the following application code:
