...
TBA: link to this section above, at approipriate points - e.g. create, createMany, upseer

Use caution when you create a record with a unique composite index, or add or update data in such a record. MongoDB does not enforce unique values inside the same value (<-TERM).

If your schema has a composite index with a `@@unique` constraint <-CHECK TERM HERE, it is possible to store duplicate values into the field in the same record.

For example, in the following schema, `MailBox` has a composite type, `addresses`, which has a `@@unique` constraint.
...
The following code creates a record with two identical values in `address`. MongoDB does not throw an error in this situation.
...
Note that MongoDB does throw an error if you to store an identical value in the same unique field in two separate records. <-check terms here
