...
- `isSet` : Filter optional fields to include only results that have been set (either to a value, or to `null`). Setting this filter to `true` will exclude `undefined` results that are not set at all.
