...
If your schema has a composite type with a `@@unique` constraint, MongoDB prevents you from storing the same value for the constrained value in two or more of the records that contain this composite type. However, MongoDB does does not prevent you from storing multiple copies of the same field value in a single record. You can work around this by [**details TBA**](#workaround).
...
In the above example, MongoDB did not enforce the unique constraint on a nested address name. However, you can structure your data differently to enforce unique values in a record. To do so, use Prisma relationships for the composite type to turn the type into a collection. Establish a relationship to this collection and place the unique constraint on the field that you want to be unique.

In the following example, MongoDB enforces unique values in a record.
...
await prisma.MailBox.create({
  data: {
    name: 'Alice',
    addresses: {
      create: [{ name: 'alice@prisma.io' }, { name: 'alice@prisma.io' }],
    },
  },
})
```
