...
- [find](#find-records-that-contain-composite-types-with-find-and-findmany) records that contain composite types using `findFirst` and `findMany`
- [create](#create-records-with-composite-types-using-create-and-createmany) new records with composite types using `create` and `createMany`
...
You can also update all records that match a composite type. Let's update a shipping address on a list of orders:
...
Use the `set` and `push` operations to change a list of composite types:
...
You can also delete by a composite type. The example below shows how to delete products that contain a certain photo:
