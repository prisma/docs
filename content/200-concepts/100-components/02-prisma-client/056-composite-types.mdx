...
Be careful when you carry out any of the following operations on a record with a composite type that has a unique constraint. MongoDB does not enforce unique values inside a record.
