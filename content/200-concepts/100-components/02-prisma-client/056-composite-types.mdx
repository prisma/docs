...
If your schema has a composite type with a `@@unique` constraint, MongoDB prevents you from storing the same value in two or more records. However, MongoDB does does not prevent you from storing duplicate values in the same record. For example, in the following schema, `MailBox` has a composite type, `addresses`, which has a `@@unique` constraint.
...
Note that MongoDB does throw an error if you try to store the same value in two separate records. In our example above, if we tried to store the email address `alice@prisma.io` for the user Alice and for the user Bob, MongoDB does not allow this and throws an error.
