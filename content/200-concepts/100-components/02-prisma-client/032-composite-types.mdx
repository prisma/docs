...
Use the new `is`, `equals` and `isNot` methods to change a single composite type:
...
Use the new `equals`, `isEmpty`, `every`, `some` and `none` methods to filter for multiple composite types:
