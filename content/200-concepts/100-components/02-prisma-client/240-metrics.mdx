...
- Histograms (metrics data divided into a collection of values; we call each container in the collection a "bucket"):
  ...
  - `query_operation_total_elapsed_time_ms`: The execution time for all operations. This includes the time taken for all database queries executed, and all database engine activities, such as joining data, and transforming data to the correct format.
