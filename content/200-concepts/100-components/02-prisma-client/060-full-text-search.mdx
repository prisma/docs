...
The `search` field uses the database's native querying capabilities under the hood. This means that the Prisma Client [supports query operators the database supports](https://www.postgresql.org/docs/10/textsearch-controls.html#TEXTSEARCH-PARSING-QUERIES).
