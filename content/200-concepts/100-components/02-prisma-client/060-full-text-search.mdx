...
To speed up your full-text queries, you should add an index to your database. Prisma Schema currently doesn't support defining and Prisma Migrate doesn't support adding search indices, but you can easily add one yourself. The following example adds a `GIN` index to the table `posts` on the field `body`:
