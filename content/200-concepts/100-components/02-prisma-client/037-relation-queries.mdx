...
The are two main differences between the queries:

* The fluent API call is translated into two separate database queries while the other one only generates a single query (see this [GitHub issue](https://github.com/prisma/prisma/issues/1984))
* You can use the fact that `.findUnique({ where: { email: 'alice@prisma.io' } }).posts()` queries are automatically batched by the Prisma dataloader to [avoid the n+1 problem in GraphQL resolvers](../../../../guides/prisma-guides/query-optimization-performance#solving-n1-in-graphql-with-findunique-and-prismas-dataloader)

> **Note**: Note: Prisma's dataloader batches `findUnique` queries even if you do not use the fluent API.
