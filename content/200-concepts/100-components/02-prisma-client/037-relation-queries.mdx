...
You can use [`select` to customize which relation fields to return](#selet-specific-relation-fields). 
...
You can use a [nested `select`](#select-relation-fields) to choose a subset of relation fields to return. For example, the following query returns the user's `name` and the `title` of each related post:
