...
> **Note**: You can use the fact that `.findUnique({ where: { email: 'alice@prisma.io' } }).posts()` queries are automatically batched by the Prisma dataloader to [avoid the n+1 problem in GraphQL resolvers](/prisma-orm/performance-and-optimization/query-optimization#solving-n1-in-graphql-with-findunique-and-prismas-dataloader).
