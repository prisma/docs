...
Note that you **cannot** use `select` and `include` _on the same level_:
...
To `select` specific record and related record fields, use a [nested `select`](#select-relation-fields). The following example selects the user's `name` and the `title` of each related post:
