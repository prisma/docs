...
The code below shows the `UserSelect` generated type and how each property on the object has a type annotation.
...
You'll notice that if you hover your mouse over `userEmail` TypeScript wont infer the objects key or value (that is, `email: true`).

The same applies if you use dot notation on `userEmail` within the `getUser` query, you will be able to access all of the properties available to a `select` statement.
