...
<a name="variables-identifiers"></a>{' '}
...
- `$queryRaw` does not support dynamic table names in Postgres databases. [Learn more](#dynamic-table-names-in-postgres)
...
#### Dynamic table names in PostGres

[It is not possible to send a table name as a variable](#variables-identifiers). This means that you cannot use dynamic table names with `$queryRaw`. Instead, you must use [`$queryRawUnsafe`](#inlinecodequeryrawunsafeinlinecode), as follows:
...
Note that if you use `$queryRawUnsafe` in conjunction with user inputs, you risk SQL injection attacks. [Learn more](#raw-sql-injection).
