...
[It is not possible to send a table name as a variable](#variables-identifiers). This means that you cannot use dynamic table names with `$queryRaw`. Instead, you will need to use `$queryRawUnsafe`, as follows:
