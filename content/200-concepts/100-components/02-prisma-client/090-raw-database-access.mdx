...
### Using <inlinecode>improvedQueryRaw</inlinecode> (Preview)

In versions `3.14.0` and later, improvements to the raw query API are available:

- Values returned from raw queries are mapped to the corresponding JavaScript type. For more information, see [Raw query type mapping](#raw-query-type-mapping)
- Typecasting issues in PostgreSQL are fixed. For more information, see [PostgreSQL typecasting fixes](#postgresql-typecasting-fixes)

This is a breaking change, so has to be enabled as a Preview feature. To enable this, add the `improvedQueryRaw` feature flag to the `generator` block of your `schema.prisma` file:
...
This feature is currently not available for SQLite.

</Admonition>

With the `improvedQueryRaw` Preview feature enabled, the returned database values from raw queries will be mapped to their corresponding [JavaScript type](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures). This matches the behaviour of regular Prisma query methods such as `findMany`.
...
The `improvedQueryRaw` preview feature fixes a number of issues with typecasting in PostgreSQL. For example, the following raw query now works correctly, returning an integer result:
...
A consequence of this fix is that some subtle implicit casts are now handled more strictly, so some queries that previously were allowed will now fail with the `improvedQueryRaw` feature enabled. As an example, take the following query using PostgreSQL's `LENGTH` function, which only accepts the `text` type as an input:

```ts
await prisma.$queryRaw`SELECT LENGTH(${42});`
```

Without the `improvedQueryRaw` Preview feature enabled, Prisma silently coerces `42` to `text`. With the feature enabled, the query returns an error:
