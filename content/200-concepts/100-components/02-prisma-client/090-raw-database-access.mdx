...
**!!!does this section come out, due to impovedqueryraw, or is it situational?!!!**
...
Prisma 4.0.0 and later maps database values that are returned from raw queries to their corresponding [JavaScript types](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures). This behavior is the same as for regular Prisma query methods like `findMany`.

<Admonition type="info">

**Earlier versions:** In v3.14.x and v3.15.x, raw query type mapping was available with the preview feature `improvedQueryRaw`. We made raw query type mapping [Generally Available](/about/prisma/releases#generally-available-ga) in v4.0.0, so you do not need to use `improvedQueryRaw` in v4.0.0 or later.

</Admonition>

**!!! does this apply to queryRaw only, or also to queryRawUnsafe, etc? This is important, because it determines where this section should go. I am investigating this !!!**
...
Prisma 4.0.0 and later resolves a number of issues with typecasting in PostgreSQL.

<Admonition type="info">

**Earlier versions:** In v3.14.x and v3.15.x, these PostgreSQL fixes were available with the preview feature `improvedQueryRaw`. We made these fixes [Generally Available](/about/prisma/releases#generally-available-ga) in v4.0.0, so you do not need to use `improvedQueryRaw` in v4.0.0 or later.
