...
> **Note**: PostgreSQL serializes JavaScript numbers as `INT8`, so Prisma sends numeric values to PostgreSQL as `INT8`. If you use `$queryRaw` in conjunction with a PostgreSQL database, ensure that your custom functions use `INT8` numbers.
