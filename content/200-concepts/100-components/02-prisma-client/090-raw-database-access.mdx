...
- In v3.14.x and v3.15.x, raw query type mapping was available with the preview feature `improvedQueryRaw`. We made raw query type mapping [Generally Available](/about/prisma/releases#generally-available-ga) in v4.0.0, so you do not need to use `improvedQueryRaw` in v4.0.0 or later.
- Before v4.0.0, raw query type mapping was not available for SQLite.
