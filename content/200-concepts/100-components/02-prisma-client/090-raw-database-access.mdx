...
dbSwitcher: ['relational', 'mongodb']
---

<SwitchTech technologies={['*', 'relational']}>
...
<h2 id="queryraw"><inlinecode>$queryRaw</inlinecode></h2>
...
The method is implemented as a [tagged template](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#Tagged_templates), which allows you to pass a template literal where you can easily insert your <a href="#using-variables">variables</a>. In turn, Prisma creates prepared statements that are safe from SQL injections:
...
You can also use the <a href="#tagged-template-helpers">`Prisma.sql`</a> helper, in fact, the `$queryRaw` method will **only accept** a template string or the `Prisma.sql` helper.
...
You can also <a href="#typing-queryraw-results">type the results of `$queryRaw`</a>.
...
<h4>Type caveats when using raw SQL</h4>
...
<h2 id="queryrawunsafe"><inlinecode>$queryRawUnsafe</inlinecode></h2>
...
<h4>Parameterized PostgreSQL `ILIKE` query</h4>
...
<h2 id="executeraw"><inlinecode>$executeRaw</inlinecode></h2>
...
- [`PREPARE` does not support `ALTER`](https://www.postgresql.org/docs/current/sql-prepare.html) - see <a href="#alter-limitation-postgresql">workaround</a>.
...
<h2 id="executerawunsafe"><inlinecode>$executeRawUnsafe</inlinecode></h2>
...
- [SQL Template Tag](https://github.com/blakeembrey/sql-template-tag) give you access to <a href="#tagged-template-helpers">useful helpers</a>.
...
<h3 id="tagged-template-helpers">Tagged template helpers</h3>
...
<h4><inlinecode>ALTER</inlinecode> limitation (PostgreSQL)</h4>
...
- Use `<model>.findRaw` to find zero or more documents that match the filter.
- Use `<model>.aggregateRaw` to perform aggregation operations on a collection.

Use cases for raw queries include:

- You are a strong MongoDB user and want to run a heavily optimized query
- Prisma Client does not yet support the feature that you require (please [consider raising an issue](https://github.com/prisma/prisma/issues/new/choose))

</TopBlock>

<h2 id="findraw"><inlinecode>findRaw</inlinecode></h2>
...
<h2 id="aggregateraw"><inlinecode>aggregateRaw</inlinecode></h2>
...
<h2 id="runcommandraw"><inlinecode>$runCommandRaw</inlinecode></h2>
