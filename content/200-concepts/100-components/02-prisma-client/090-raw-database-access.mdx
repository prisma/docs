...
- queryRaw does not support dynamic table names in Postgres databases. [Learn more](#dynamic-table-names)
...
Here, we call queryRaw as a function, instead of as a tagged template varioable.
When you use the tagged template varibale version of this API, any variable you provide in the template literal body are cnvereted intop prepared statements like $1 or $2. SQL doesnt support naming tables as parepared variables to it throws the errors.
So you need to use the function version here.

!!! To be resolved:

- Does this apply to PostGres only?
- Does this also apply to queryRawUnsafe?
