...
In Prisma 4.0.0 and later, database values returned from raw queries are mapped ((_!by whom?!_) their corresponding [JavaScript types](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures). This matches the behavior of regular Prisma query methods such as `findMany`.
...
In version 4.0.0 and later, Prisma resolves a number of issues with typecasting in PostgreSQL. For example, the following raw query now works correctly, returning an integer result:
