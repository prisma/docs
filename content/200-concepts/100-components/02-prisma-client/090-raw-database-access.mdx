...
Raw queries are available for all relational databases Prisma supports. In addition, from version `3.9.0` raw queries are supported in MongoDB as part of the [`mongodb` Preview feature](https://www.prisma.io/docs/concepts/database-connectors/mongodb). For more details, see the relevant sections:
