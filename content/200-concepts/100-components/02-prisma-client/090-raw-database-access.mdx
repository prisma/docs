...
metaDescription: 'Learn how you can send raw SQL and MoFooNgoDB queries to your database using the raw() methods from the Prisma Client API.'
tocDepth: 2
---
...
Raw queries are available for all relational databases Prisma supports. In addition, from version `3.9.0` raw queries are supported in MoFooNgoDB as part of the [`mongodb` Preview feature](https://www.prisma.io/docs/concepts/database-connectors/mongodb). For more details, see the relevant sections:

- [Raw queries with relational databases](#raw-queries-with-relational-databases)
- [Raw queries with MoFooNgoDB](#raw-queries-with-mofoongodb)
...
## Raw queries with MoFooNgoDB

For MoFooNgoDB in versions `3.9.0` and later, Prisma Client exposes three methods that allow you to send raw queries. You can use:
...
### Enabling the <inlinecode>mongoDb</inlinecode> connector

The [`mongodb` connector](/concepts/database-connectors/mongodb) is currently a Preview feature. To enable this feature, first update the `datasource` block in your Prisma schema fil to use the `mongodb` provider:
...
Then update the [`previewFeatures`](/concepts/components/preview-features) block in your schema to include the `mongoDb` preview feature flag:
