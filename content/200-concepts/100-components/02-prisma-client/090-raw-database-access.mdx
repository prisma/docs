...
Prisma 4.0.0 and later maps database values returned from raw queries to their corresponding [JavaScript types](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures). This matches the behavior of regular Prisma query methods such as `findMany`.

<Admonition type="info">

Raw query type mapping was available from Prisma 3.14.0 with the preview feature `improvedQueryRaw`. It is now [Generally Available](/about/prisma/releases#generally-available-ga) from Prisma 4.0.0. You do not need to use `improvedQueryRaw` in Prisma 4.0.0 or later.
...
These PostgreSQL typecasting fixes were available from Prisma 3.14.0 with the preview feature `improvedQueryRaw`. They are now now [Generally Available](/about/prisma/releases#generally-available-ga) from Prisma 4.0.0. You do not need to use `improvedQueryRaw` in Prisma 4.0.0 or later.
