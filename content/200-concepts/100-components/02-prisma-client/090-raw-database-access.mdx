...
If you are selecting **specific fields** of the model or want to include relations, refer to the documentation about [leveraging Prisma Client's generated types](working-with-generated-types) if you want to make sure that the results are properly typed.
...
const result = await prisma.$queryRaw`SELECT * FROM User WHERE id IN (${join(ids)})`
```
