...
const email = 'bruce@avengers.io'
const result =
  await prisma.$queryRaw(Prisma.sql`SELECT * FROM User WHERE email = ${email}`)
```
...
```json5
[
  { id: 1, email: 'bruce@avengers.io', name: 'Bruce' },
  { id: 2, email: 'tony@avengers.io', name: 'Tony' },
]
```
...
**By using this method you open up the possibility for SQL injection attacks**. SQL injection attacks can expose your data, be it confidential or otherwise sensitive, to being deleted, modified, or even destroyed. For more information on SQL injection attacks, see the [OWASP SQL Injection guide](https://www.owasp.org/index.php/SQL_Injection).
...
You can also run a parameterized query. The following example returns all users whose email contains the string `bruce@avengers.io`:

```ts
prisma.$queryRawUnsafe("SELECT * FROM users WHERE email = $1", "bruce@avengers.io")
```
...
const result = await prisma.$executeRawUnsafe(`UPDATE User SET active = ${active} WHERE emailValidated = ${emailValidated}`);
``` 

The same can be written as a parameterized query:

```ts
const result = prisma.$executeRawUnsafe("UPDATE User SET active = $1 WHERE emailValidated = $2", "bruce@avengers.io", true)
```
