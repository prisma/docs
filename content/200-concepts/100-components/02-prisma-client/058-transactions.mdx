...
- The Prisma Client `$transaction` API:
  ...
  - [Interactive transactions](#interactive-transactions-in-preview) (in Preview): pass a function that can contain user code including Prisma Client queries, non-Prisma code and other control flow to be executed in a transaction, using `$transaction<R>(fn: (prisma: PrismaClient) => R): R`: P
...
- `isolationLevel`: Sets the [transaction isolation level](#transaction-isolation-level). By default this will be set to the value currently configured in your database.
...
In version 4.1.0 and earlier, you cannot configure the transaction isolation level at a Prisma level. Also, the isolation level is not explicitly set by Prisma, so the isolation level is set to the level configured in your database. For database-specific information on isolation levels, including database defaults, see the following resources:<br /><br />
...
By default Prisma Client will set the isolation level to the value currently configured in your database. For database-specific information on isolation levels, including database defaults, see the following resources:<br /><br />
