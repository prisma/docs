...
1. Transaction A: `createMany` operation. The newly-inserted rows conflict with the rows that transaction B added at step 4.

This conflict can occur at the isolation level `ReadCommited`, which is the default isolation level in PostgreSQL and Microsoft SQL Server. To avoid this problem, you can set a higher isolation level (`RepeatableRead` or `Serializable`).

From Prisma version 4.4.0, you do not need to set the isolation level in your database configuration. You can instead set the isolation level on a transaction. This overrides the isolation level on your database server for that transaction. [Learn more about isolation levels in Prisma tranasctions](#transaction-isolation-level).

To avoid transaction write conflicts and deadlocks:

1. On your transaction, use the `isolationLevel` parameter to set the transaciton to `Prisma.TransactionIsolationLevel.Serializable`.

   This ensures that your application commits multiple concurrent or parallel transactions as if they were run serially. Where a transaction fails due to a write conflict or deadlock, Prisma returns a [P2034 error](/reference/api-reference/error-reference#p2034).

2. In your application code, build a retry around your transaction to trap any P2034 errors, as shown in this example:
...
1. Transaction A: `deleteMany` operation removes a set of rows.
1. Transaction B: `deleteMany` operation removes a set of rows.
1. Transaction B: `createMany` operation creates a new set of rows.
1. Transaction B: The application commits transaction B.
1. Transaction A: `createMany` operation. The newly-inserted rows conflict with the rows that transaction B added at step 4.
...
From Prisma version 4.4.0, you do not need to set the isolation level in your database configuration. You can instead set the isolation level on a transaction. This overrides the isolation level on your database for that transaction. [Learn more about isolation levels in Prisma tranasctions](#transaction-isolation-level).
...
> > > > > > > parent of 1912b1f2 (Update 058-transactions.mdx)
