...
From Prisma version 4.4.0, you do not need to set the isolation level in your database configuration. You can instead set the isolation level on a transaction. This overrides the isolation level on your database for that transaction. [Learn more about isolation levels in Prisma transactions](#transaction-isolation-level).
