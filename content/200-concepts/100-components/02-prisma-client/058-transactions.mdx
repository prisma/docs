...
- `isolationLevel`: Sets the [transaction isolation level](#transaction-isolation-level). The default value [depends on your database connector](/concepts/components/prisma-client/transactions#isolation-levels-by-database).
...
In version 4.1.0 and earlier, you cannot configure the transaction isolation level
at a Prisma level. Also, the isolation level is not explicitly set by Prisma, so
the isolation level is set at the database level, at the default level for the database
type. For information on default isolation levels, see the following resources:<br /><br />
...
The default isolation level in Prisma Client is the same as the default for your database:
| Database | Default |
| ----------- | ---------------- |
