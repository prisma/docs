...
* `where` to filter **all records before grouping** - for example, all users where the email address contains `prisma.io`
* `having` to filter **entire groups by an aggregate value** - for example, only include groups where the average `profileViews` is greater than 100

## `groupBy` and ordering

The following constraints apply when you combine `groupBy` and `orderBy`:

* If skip or take is provided, orderBy has to be provided.
* Everything provided in orderBy needs to be in by
