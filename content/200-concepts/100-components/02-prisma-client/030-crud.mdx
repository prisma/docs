...
Prisma Client supports bulk inserts as a Preview feature in [2.16.0](https://github.com/prisma/prisma/releases/2.16.0) and later. To enable this feature, add `createMany` to `previewFeatures` in your schema:
...
The following [`createMany`](../../../reference/api-reference/prisma-client-reference#createmany-preview) <span class="api"></span> query creates multiple users and skips any duplicates (`email` must be unique):
...
The following video demonstrates how to use `createMany` and [faker.js](https://github.com/marak/Faker.js/) to seed a database with sample data:

<div class="videoWrapper">
<iframe width="560" height="315" src="https://www.youtube.com/embed/QxyqR4yh1GI" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
...
The following query uses a combinations of [operators](../../../reference/api-reference/prisma-client-reference#filter-conditions-and-operators) <span class="api"></span> to return users whose name start with `E` _or_ administrators with at least 1 profile view:
...
See [Working with relations](relation-queries#filter-the-relations-list) to find out how to combine [`include`](../../../reference/api-reference/prisma-client-reference#include) <span class="api"></span> and `where` for a filtered list of relations - for example, only include a user's published posts.
...
Sometimes you want to remove all data from all tables but keep the actual tables. You can accomplish this on a PostgreSQL database by doing a truncate cascade on all tables.
...
There are however some caveats to this approach. By doing a truncate cascade you are resetting the all the sequence ids (the auto-incremented ids column in your table), which can cause unexpected disruptions to your database. **This is only recommended in a development environment**.
