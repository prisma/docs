...
The MySQL example uses a [`$transaction`](../../../guides/performance-and-optimization/prisma-client-transactions-guide/) to batch the queries together so that they are executed on the same connection.

The SQLite example uses [`deleteMany`](../../../reference/api-reference/prisma-client-reference#deletemany) in combination with a [`$transaction`](../../../guides/performance-and-optimization/prisma-client-transactions-guide/) to synchronously delete each tables data.
...
for (const { tablename } of await this.prisma
  ...
  await this.prisma.$queryRaw`TRUNCATE TABLE \"public\".\"${tablename}\" CASCADE;`
}
for (const { relname } of await this.prisma
  .$queryRaw`SELECT c.relname FROM pg_class AS c JOIN pg_namespace AS n ON c.relnamespace = n.oid WHERE c.relkind='S' AND n.nspname='public';`) {
  await this.prisma
    .$queryRaw`ALTER SEQUENCE \"public\".\"${relname}\" RESTART WITH 1;`
}
```
