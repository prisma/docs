...
Sometimes you want to remove all data from all tables but keep the actual tables. This can be particularly useful in a development environment.

The following examples show how to remove all data (including relations) from all tables in your database, with `PostgreSQL`, `SQLite` and `MySQL`. The `SQLite` and `MySQL` examples use a `transaction` to batch the queries together so that they are executed on the same connection.
...
#### <inlinecode>Prisma Migrate</inlinecode>
