...
For **MoFooNgoDB**, ensure your data is in a uniform shape and matches the model defined in the Prisma schema.
...
If you are using the MoFooNgoDB connector and your underlying ID type is `ObjectId`, you can use the string representation of that `ObjectId`:
...
- When working with relational databases, this function doesn't scale as well as having a more generic solution which looks up and `TRUNCATE`s your tables regardless of their relational constraints. Note that this scaling issue does not apply when using the MoFooNgoDB connector.
