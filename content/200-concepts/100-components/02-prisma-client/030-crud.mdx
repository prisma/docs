...
> `findUnique` was previously `findOne`. `findOne` was deprecated in [version 2.12.0](https://github.com/prisma/prisma/releases/tag/2.12.0).

### Options

#### Type

`findUnique` takes an object with one of the following types as input:
...
`findUnique` returns a plain old JavaScript object or `null`. If `rejectOnNotFound` is `true`, `findUnique` throws a `NotFoundError` instead of returning `null`.

The type of the object that a `findUnique` API call returns depends on whether you use the `select` and `include` options.
...
const result = await prisma.user.findUnique({
  where: {
    fullname: {
      // name property of @@unique attribute
      firstName: 'Alice',
      lastName: 'Smith',
    },
  },
...
`findFirst` returns a plain old JavaScript object or `null`. If `rejectOnNotFound` is `true`, `findFirst` throws a `NotFoundError` instead of returning `null`.
...
#### Retrieve the first `User` record where the `name` is `Alice` and throw a `NotFoundError` if not found
...
To perform arithmetic operations (add, subtract, multiply, divide), use [atomic updates](#atomic-operations-on-update) to prevent race conditions.
...
Refer to [`findUnique`](#findunique), [`create`](#create) and [`update`](#update) to see what the generated types `UserWhereUniqueInput`, `UserCreateInput` and `UserUpdateInput` types look like.
...
To perform arithmetic operations (add, subtract, multiply), use [atomic updates](#atomic-operations-on-update) to prevent race conditions.
...
`count` either returns an `number` or a plain old Javascript object (if `select` is used in 2.15.0 and later).
...
const countUsers = await prisma.user.count({
  select: {
    _all: true, // Count all users
    city: true, // Count users with cities
    name: true, // Count users with names
  },
})
```
...
In 2.15.0 and later (Postgres only), you can filter scalar lists - for example, a list of `String` or `Enum` types. The following table describes available conditions:
...
Examples are based on the following schema:
...
The following example returns all posts where the tags list includes `databases` _and_ `typescript`:
...
Array fields with a `NULL` value are not included in the results returned by the following conditions:
