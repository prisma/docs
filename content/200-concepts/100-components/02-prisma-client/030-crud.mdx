...
See [Working with relations](#working-with-relations#filter-the-relations-list) to find out how to combine [`include`](../../../reference/api-reference/prisma-client-reference#include) <span class="api"></span> and `where` for a filtered list of relations - for example, only include a user's published posts.
