...
Prisma Client supports bulk inserts as a Preview feature in [2.16.0](https://github.com/prisma/prisma/releases/2.16.0) and later. To enable this feature, add `createMany` to the `previewFeatures` list as shown:
...
The following [`createMany`](../../../reference/api-reference/prisma-client-reference#createmany-preview) query creates multiple users and skips any duplicates (`email` must be unique):
