...
Prisma Client does not yet support batching at a database level. The following example does create two `User` records - however, this will result in two separate `INSERT` queries:
...
For more information about connecting and disconnecting related records, see [Working with relations](working-with-relations).

## Read

### Get record by ID or unique identifier

The following queries return a single record (`findUnique` <span class="api"></span>) by unique identifier or ID:
...
The following `findMany` <span class="api"></span> query returns _all_ `User` records:
...
The following `findFirst` <span class="api"></span> query returns the _most recently created user_ with at least one post that has more than 100 likes:
...
The following example returns users that have no posts (`none`) with more than 100 views:
...
The following example returns users that _only_ has posts (`every`) with more than 100 views:
...
The following example returns **all** users, but only returns posts that contain the word `Prisma`:
...
Refer to [Working with relations](working-with-relations) for information about disconnecting (`disconnect` <span class="api"></span>) and connecting (`connect` <span class="api"></span>) related records.
...
### Casading deletes (deleting related records)
...
* Delete the posts first, then the user:
