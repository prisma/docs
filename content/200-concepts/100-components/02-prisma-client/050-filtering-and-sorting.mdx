...
You can use (such as [`NOT`](../../../reference/api-reference/prisma-client-reference#not) <span class="api"></span> and [`OR`](../../../reference/api-reference/prisma-client-reference#not) <span class="api"></span>) to filter by a combination of conditions. The following query returns all users with an `email` that ends in `"prisma.io"` or `"gmail.com"`, but not `"hotmail.com"`:
...
Sorting by the properties of a relation is available in [2.16.0](https://github.com/prisma/prisma/releases/2.16.0) and later. Sorting by relations is a Preview feature - to enable this feature, add `orderByRelation` to `previewFeatures` as shown:
...
It is not yet possible to sort by an aggregate value.
