...
In [3.5.0](https://github.com/prisma/prisma/releases/3.5.0) and later, when using PostgresSQL you can sort records by relevance to the query. This uses PostgreSQL's full text search functionality which is explained further in [the PostgreSQL documentation](https://www.postgresql.org/docs/9.1/textsearch-controls.html).

For example, the following query sorts users by relevance of the search term `'developer'` to their bio:
