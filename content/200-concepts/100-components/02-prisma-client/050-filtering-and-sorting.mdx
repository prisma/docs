...
You can use (such as [`NOT`](../../../reference/api-reference/prisma-client-reference#not) <span class="api"></span> and [`OR`](../../../reference/api-reference/prisma-client-reference#or) <span class="api"></span>) to filter by a combination of conditions. The following query returns all users with an `email` that ends in `"prisma.io"` or `"gmail.com"`, but not `"hotmail.com"`:
...
It is not yet possible to sort by an aggregate value. Follow [issue #3821 on GitHub](https://github.com/prisma/prisma/issues/3821) for updates.
