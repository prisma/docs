...
Refer to Prisma Client's reference documentation for [a full list of operators](/reference/api-reference/prisma-client-reference#filter-conditions-and-operators) <span class="api"></span>, such as `startsWith` and `contains`.
...
For MySQL and PostgreSQL we are utilizing the [`LIKE`](https://www.w3schools.com/sql/sql_like.asp) (and [`ILIKE`](https://www.postgresql.org/docs/current/functions-matching.html#FUNCTIONS-LIKE)) operator to search for a given pattern. These operators have built-in pattern matching using symbols unique to `LIKE`. These include `%` for zero or more characters (similar to `*` in other regex implementations) and `_` for one character (similar to `.`).

If you would like to match the literal characters `%` or `_`, make sure to escape those characters. For example:
...
The above query will match any name that starts with any character. That is to say, any user where name is not `NULL`. If you instead wanted to find any user whose name starts with a literal `_` you could do

```ts highlight=4
const users = await prisma.user.findMany({
  where: {
    name: {
      startsWith: '_', // note that the `_` character is escaped
    },
  },
})
```
