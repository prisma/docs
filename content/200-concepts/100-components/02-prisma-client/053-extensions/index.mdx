...
- `model`: [add custom methods or fields to your models](/concepts/components/prisma-client/extensions/model)
- `client`: [add client-level methods to Prisma Client](/concepts/components/prisma-client/extensions/client)
- `query`: [create custom Prisma Client queries](/concepts/components/prisma-client/extensions/query)
- `result`: [add custom fields to your query results](/concepts/components/prisma-client/extensions/result)
...
You can [share your Prisma Client extensions](/concepts/components/prisma-client/extensions/shared-extensions) with other Prisma users, and [import Prisma Client extensions developed by other users](/concepts/components/prisma-client/extensions/shared-extensions#install-a-packaged-extension) into your Prisma project.
...
Use the `$extends` [client-level method](/reference/api-reference/prisma-client-reference#client-methods) to create an extended client. In your `$extends` structure, use the appropriate extension component or components ([`model`](/concepts/components/prisma-client/extensions/model), [`client`](/concepts/components/prisma-client/extensions/client), [`result`](/concepts/components/prisma-client/extensions/result) or [`query`](/concepts/components/prisma-client/extensions/query)).

For example, the following code uses a [`model` extension component](/concepts/components/prisma-client/extensions/model) to extend the `User` model.
