...
From version 3.5.0, you can perform soft deletes without middleware. To do so, include an `isDeleted` field in `where` statements. [Learn more](/reference/api-reference/prisma-client-reference/#use-non-unique-fields-with-where).
