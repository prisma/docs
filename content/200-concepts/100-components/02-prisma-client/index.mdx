...
Note that queries will only run if used with `await` or chained with `.then()`. When using [`$transaction`](transactions) this makes it possible to the client to pass all the queries on to the [query engine](../prisma-engines/query-engine) as a transaction.
The client methods are ["thenable"](https://masteringjs.io/tutorials/fundamentals/thenable) they return a PrismaPromise which is implemented like a Promise except that for the query to be executed they need to be awaited or chained with `.then()`.
