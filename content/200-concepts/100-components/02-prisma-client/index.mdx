...
Note that queries will only run if used with `await` or chained with `then()`. This is due to how [`$transactions`](transactions) work when passing the queries on to the [query engine](../prisma-engines/query-engine). The instantiated client has ["thenable"](https://masteringjs.io/tutorials/fundamentals/thenable) methods that act like promises, and the queries are executed as a batch job.
