...
const pets = jsonField.pets.pet1;
```
...
In 2.23.0 and later, you can filter on a specific property inside a block of JSON. In the following examples, the value of `pets` is a one-dimensional, unnested JSON object:
...
`string_startswith` and `string_endswith`.

### Filter on nested object property

You can filter on nested JSON properties. In the following examples, the value of `pets` is a JSON object with one level of nesting:
