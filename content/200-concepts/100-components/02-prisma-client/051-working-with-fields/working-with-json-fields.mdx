...
Supported by [PostgreSQL]() and [MySQL]() with different syntaxes for the `path` option. PostgreSQL does not support [filtering on object property values in arrays](#filtering-on-object-property-value-inside-array).
...
const jsonField = user?.pets as Prisma.JsonArray
...
```ts
var json = { pets: [{ name: 'Bob the dog' }, { name: 'Claudine the cat' }] } as Prisma.JsonObject
...
### Filtering on object property value inside array

Depending on your provider, you can filter on the property value of an object inside an array.

<Admonition type="alert">

Filtering on object property values within an array is **only** supported by the [MySQL database connector]().
...
* `$[*]` is the root array of pet objects
* `insurances[*]` mathes any `insurances` array inside any pet object
