...
## Filter on a <inlinecode>Json</inlinecode> field (simple)
...
## Filter on a <inlinecode>Json</inlinecode> field (advanced)

You can also filter rows by the data inside a `Json` field. We call this **advanced `Json` filtering**. This functionality is supported by [PostgreSQL](/concepts/database-connectors/postgresql) and [MySQL](/concepts/database-connectors/mysql) only with [different syntaxes for the `path` option](#path-syntax-depending-on-database).
