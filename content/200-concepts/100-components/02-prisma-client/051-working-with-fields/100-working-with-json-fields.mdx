...
The `Json` field type is a preview feature available since 2.23.0. You can enable it by editing the `previewFeatures` field in the `generator` block in your schema, then re-generating the Prisma Client:
...
Note that the `filterJson` feature is only available in the following databases:
