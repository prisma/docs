...
Advanced `Json` filtering is supported by [PostgreSQL](../../../database-connectors/postgresql) and [MySQL](../../../database-connectors/mysql) with different syntaxes for the `path` option. PostgreSQL does not support [filtering on object key values in arrays](#filtering-on-object-key-value-inside-array).
