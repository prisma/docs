...
Use the atomic [`push`](/reference/api-reference/prisma-client-reference#push) <span class="api"></span> method to add a single value to a scalar list:
...
Use [scalar list filters](/reference/api-reference/prisma-client-reference#scalar-list-filters) <span class="api"></span> to search within scalar lists. The following example returns all posts where the tags list includes `databases` _and_ `typescript`:
