...
Prisma Client under the hood users a _query engine_ to run queries against the database. This query engine is downloaded when `prisma generate` is invoked and stored in the `output` path together with the generated Client.
