...
Your application should generally only spawn one instance of `PrismaClient`. How to achieve this depends on whether you are using Prisma in a [long-running application](../../../../guides/performance-and-optimization/connection-management#prismaclient-in-long-running-applications) <span class="guide"></span> or in a [serverless environment](../../../../guides/performance-and-optimization/connection-management#prismaclient-in-serverless-environments) <span class="guide"></span>.

The technical reason behind this is that each instance of `PrismaClient` manages a [database connection pool](connection-pool), which means that a large number of clients can **exhaust the database connection limit**:
