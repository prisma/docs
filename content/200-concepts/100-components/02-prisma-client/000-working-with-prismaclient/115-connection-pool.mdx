...
1. Connection pool instantiated with a [congfigurable pool size]()
2. Queries come in and are added to queue (FIFO - First In First Out)
3. QE does its best to clear queue within [configurable pool timeout]() (5s by default)
4. If the pool has no connections available, 

Query engine creates pool with configurable pool size and pool timeout.
High volume of queries come in (e.g. many people using your app simultaneously or 1000 queries sent at once using Promise.all()
If all connections are busy, queries go into a queue (channel?) - queue is FIFO (first in, first out)
