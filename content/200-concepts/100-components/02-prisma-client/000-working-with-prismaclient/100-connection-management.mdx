...
* Runs infrequently (for example, a scheduled job to send emails each night), which means it does not benefit from a long-running connection to the database
* Exists in the context of a long-running application, such as a background service, which means that Prisma Client does not automatically disconnect when Node.JS process exits
...
In serverless environments, there are nuances that make it sensible to reuse database connections and _not_ call `$disconnect()`. Refer to the [deployment docs](../deployment) for more examples.
