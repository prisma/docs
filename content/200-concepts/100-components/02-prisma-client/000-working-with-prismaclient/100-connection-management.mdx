...
The `beforeExit` hook runs when Prisma is triggered externally (e.g. via a `SIGINT` signal) to shut down and allows you run code _before_ the Client disconnects - for example, to issue queries as part of a graceful shutdown of a service:
