...
You can use `await` and then mutate the result of the `query` promise, because its value is type-safe. TypeScript catches any unsafe mutations on the object.
...
While we want to show this is possible, for performance reasons, we recommend that you use the **`result` component to override existing fields**.
...
The following example shows how to wrap an extended query into a batch transaction:
