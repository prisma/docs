...
From version 4.7.0, you can use the `query` [Prisma Client extensions](/concepts/components/prisma-client/client-extensions) component type to hook into the query life-cycle and modify an incoming query or its result.

You can use the `query` extension component as an alternative to [middlewares](/concepts/components/prisma-client/middleware#performance-and-appropriate-use-cases). Unlike middlewares, the `query` extension component gives you end-to-end type safety, and you can use in to provide [user isolation](/concepts/components/prisma-client/client-extensions#extended-clients) in a row-level security (RLS) scenario. [Learn more](#query-extensions-versus-middlewares).
...
To extend all model queries, use `$allModels` instead of a specific model name. For example:
...
Use `$allOperations` to extend all operations at once instead of a specific one.
...
### Add a custom query to all operations for all existing models

Use `$allOperations` to apply your custom query to all operations, instead of to a specific operation name.
