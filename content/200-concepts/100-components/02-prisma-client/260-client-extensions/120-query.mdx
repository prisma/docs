...
You can use the Prisma Client extensions `query` component to create independent clients. This provides an alternative to [middlewares](/concepts/components/prisma-client/middleware#performance-and-appropriate-use-cases). You can bind one client to a specific filter or user, and another client to another filter or user. For example, you might do this to get [user isolation](/concepts/components/prisma-client/client-extensions#extended-clients) in a row-level security (RLS) extension. In addition, unlike middlewares the `query` extension component gives you end-to-end type safety. [Learn more about `query` extensions versus middlewares](#query-extensions-versus-middlewares).
...
Use the `query` extension component to create and modify queries. You can add or modify a custom query in the following:

- [A specific operation in a specific model](#add-or-modify-a-custom-query-for-a-specific-operation-in-a-specific-model)
- [A specific operation in all models of your schema](#add-or-modify-a-custom-query-for-a-specific-operation-in-all-models-of-your-schema)
- [All operations in a specific model](#add-or-modify-a-custom-query-for-all-operations-in-a-specific-model)
- [All operations in all models of your schema](#add-or-modify-a-custom-query-for-all-operations-in-all-models-of-your-schema)
...
### Add or modify a custom query for a specific operation in a specific model
...
### Add or modify a custom query for a specific operation in all models of your schema
...
### Add or modify a custom query for all operations in a specific model
...
### Add or modify a custom query for all operations in all models of your schema
...
The following example shows how to wrap an extended query into a batch transaction. It extends `findFirst` to accept an input and find the first result.

- So if you call xprisma.user.findFirst() it first calls `async findFirst({ args, query, operation }) {`
  - In there you get a chance to alter whatever input has been passed to `findFirst` (in this case nothing) so args will be an empty object {}.
  - When you call the query extension - query(...) -

Then you trigger the operation by calling `query(...)`
That either

- triggers the next query component or
- the original vanilla query with the modified args. And you can also re-wrap it in a transaction.
