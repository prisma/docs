...
You can use the `query` extension component as an alternative to [middlewares](/concepts/components/prisma-client/middleware#performance-and-appropriate-use-cases). Unlike middlewares, the `query` extension component gives you end-to-end type safety. You can also use it to provide [user isolation](/concepts/components/prisma-client/client-extensions#extended-clients) in a row-level security (RLS) scenario. [Learn more](#query-extensions-versus-middlewares).
...
- [A specific operation in a specific model of your schema](#add-or-modify-a-custom-query-for-a-specific-operation-in-a-specific-model)
...
- [All operations in a specific model of your schema](#add-or-modify-a-custom-query-for-all-operations-in-a-specific-model)
...
The `query` object can contain functions that map to the names of the [Prisma Client operations](/reference/api-reference/prisma-client-reference#model-queries), such as `findUnique`, `findFirst`, `findMany`, `count`, and `create`. The following example defines a custom query that finds users who are older than 18 years:
...
Use `$allOperations` to extend all operations in all models of your schema.
...
You can wrap your extended queries into a [batch transaction](/guides/performance-and-optimization/prisma-client-transactions-guide). An example use-case is to enact row-level security (RLS).
...
- You cannot instantiate middlewares in extended clients. You can only instantiate middlewares in the original client.
