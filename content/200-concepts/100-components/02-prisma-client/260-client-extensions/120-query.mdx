...
You can add a query to a specific operation in a model, to a specific operation to all operations in a model, or to all operations in all models of your schema.
...
- `data`: a promise for the result of the query.
  ...
  - You can use `await` and then mutate this promise, because its value is type-safe. TypeScript catches any unsafe mutations on the object.
...
Note: recursive wildcards just modify the query itself. They cannot return values.

** TBD - can we have an example of this note? **
...
** TBD: are the parameters model, operation, args, data or model, action, args, data? **
