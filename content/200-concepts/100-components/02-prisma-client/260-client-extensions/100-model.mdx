...
To help with authoring extensions that are highly type-safe, the Prisma Client provides a set of type utilities to be able to tap into input and output types. These are fully dynamic, which means they will adapt to any given model and schema, helping you deliver a great auto-completion and developer experience for your extension.
...
You can get the name of the current model at runtime with `Prisma.getExtensionContext(this).name`. You can use this to write out the model name to a log, to send the name to another service, or to branch your code based on the model.
...
- I'm not sure that this section should go here
