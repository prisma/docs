...
## Call a custom method in the same model

You can call a custom method that is declared in the same model as your method call. For example, in a custom method in the `user` model you can call another custom method that is declared in the `user` model. The method that you call can be declared in the same extension or in a different extension.

To do this, use `Prisma.getExtensionContext(this).methodName`. You cannot use the form `prisma.user.methodName`.
