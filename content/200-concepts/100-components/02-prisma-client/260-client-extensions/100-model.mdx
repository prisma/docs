...
## Call a different custom method in the same model

If you want to call a different custom method in the same model, then you cannot use the form `prisma.user.methodName`. Instead, use `Prisma.getExtensionContext(this).methodName`. This applies whether the method that you want to call is declared in the same extension, or in another extension.

For example, to call a method that is declared in the same extension:
