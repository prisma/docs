...
When you call a method in an extension, use the constant name from your `$extends` statement, not `prisma`. In the above example, `xprisma.user.signUp` works, but `prisma.user.signUp` does not as the original `prisma` is not modified.
...
this.user.validate(someData)
this.post.validate(someData)
```
