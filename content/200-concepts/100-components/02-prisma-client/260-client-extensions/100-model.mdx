...
## Advanced type safety on models
...
- `Exact`: Assists with strict type safety - narrows the input down. **TBA** - clarify what this specifically does - does it declare the objects/arguments that you want to apply this type safety to?
- `Args`: Dynamically retrieves the input type of a given model and a given operation. When you change the model, autocomplete indicates whether the fields ezists on it.
- `Result`: Takes the input types and provides the result for a given model and a given operation.
- `Payload`: Retrieves the entire structure of the result, as scalars and relations objects.
  ...
  model: {
    $allModels: {
      findOrCreate<T, A>(
        ...
        >
        // For the arguments A and findFirstorThrow,
        // on the model T, automcomplete gives you
        // the correct results.
...
## Get the current method name at runtime
