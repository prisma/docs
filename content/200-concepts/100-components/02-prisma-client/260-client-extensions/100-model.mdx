...
You can call a custom method in the same model, whether the method is declared in the same extension or in a different extension. However, you cannot use the form `prisma.user.methodName`. Instead, use `Prisma.getExtensionContext(this).methodName`.
