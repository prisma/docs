...
** TBA: It would be nice if we could populate the `...` above with some code to show how `email` is turned into a data structure that `prisma.user.create` accepts **
...
When you call a method in an extension, use the constant name from your `$extends` statement, not `prisma`. In the above example, `xprisma.user.signUp` works, but `prisma.user.signUp` does not.
...
When you call a method in an extension, use the constant name from your `$extends` statement, not `prisma`. In the above example, `xprisma.user.getClass` works, but `prisma.user.getClass` does not.
