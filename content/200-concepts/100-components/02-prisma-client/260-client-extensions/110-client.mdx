...
** TBW: a method to return a single metric that Prisma Client returns from `$metrics` (e.g. return only `prisma_client_queries_total`) . Also link to metrics page **

** Link to section below about call a custom method #call-a-custom-method-from-another-custom-method **
