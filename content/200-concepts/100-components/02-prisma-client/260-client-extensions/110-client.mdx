...
The following example uses the `client` component to add two methods to the client:
...
const prisma = new PrismaClient().$extends({
  client: {
    ...
    async $totalQueries() {
      ...
      ).counters // `Prisma.getExtensionContext(this)` returns the current client instance
