...
// Declare an extended client that has extensions A and B applied
const extendsAB = prisma.$extends(extensionA).$extends(extensionB)

//Declare an extended client that has a stacked combination of extensions A and B
const extendsAplusB = extensionA.$extends(extensionB)
```

### Conflicts in combined extensions

When you combine two or more extensions, then the _last_ extension that you declare takes precedence in any conflict. In the example above, suppose there is a method called `getName()` in `extendsA` and a method called `getName()` in `extendsB`. When you call `extendsAB.getName()`, then the client uses `getName()` as defined in `extendsB`.
