...
When you combine two or more extensions, then the last extension takes precedence in any conflict. In the example above, suppose there is a method called `getName` in `extendsA` and a method called `getName` in `extendsB`. When you call `extendsAB.getName`, then Prisma Clinet uses `getName` as defined in `extendsB`.
...
- Extensions are isolated: the functionality in each one is separate, unless you deliberately combine them
  - For example, this is useful if you want to implement row-level security (RLS), where each http request can have its own client with its own RLS extension, customized with session data. This can keep each user entirely separate, each in a separate client. With middlewares, all users would be active in the same client.
- All extended clients share the same connection pool, but their functionality is separate ** <-- is this different behavior from middlewares? **

** Need to emphasize this: **
When you do `const xprisma = prisma.$extends()`, `prisma` is not affected by your extension, but `xprisma` is.
Some users might expect `prisma` to be affected as well. Users need to use the correct instance.
