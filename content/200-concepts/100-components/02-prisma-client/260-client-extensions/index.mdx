...
In your `$extends` structure, use the appropriate extension type or types (`model`, `client`, `result` or `query`). For example, the following code uses a [`model` extension](/concepts/components/prisma-client/client-extensions/model) to extend the `user` model.
