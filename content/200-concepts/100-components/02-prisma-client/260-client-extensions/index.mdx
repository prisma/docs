...
To use a Prisma Client extension, you must create an associated _extended client_. An extended client is a wrapped variant of the standard Prisma Client with one or more extensions applied. The standard client does not change.
...
Use the `$extends` [client-level method](/reference/api-reference/prisma-client-reference#client-methods) to create an extended client. In your `$extends` structure, use the appropriate extension component or components (`model`, `client`, `result` or `query`). Write your extension logic in the `model`, `client`, `result` or `query` structure.
...
const xprisma = prisma.$extends({
  ...
  model: {        // This is a `model` component
    user: { ... } // The extension logic for the `model` component goes inside the curly braces
  },
})
```
