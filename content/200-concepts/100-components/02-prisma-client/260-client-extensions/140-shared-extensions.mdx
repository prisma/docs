...
For generic extensions, you cannot predict which `prisma` instance the extension user will want to use. Therefore, you cannot use `prisma.$extends`, which applies to a specific instance. Instead, use `Prisma.defineExtension`. `Prisma.defineExtension` declares the extension but does not apply the extension to a specific instance. It gives you autocomplete for the extension in your code editor.
