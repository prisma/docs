...
For generic extensions, you cannot predict which `prisma` instance the extension user will want to use. Therefore, you cannot use `prisma.$extends`, which applies to a specific instance. Instead, use `Prisma.createExtension`, which declares the extension but does not apply it to a specific instance. When someone uses your extension in their project, they can apply it to an instance with `prisma.$extends`.
