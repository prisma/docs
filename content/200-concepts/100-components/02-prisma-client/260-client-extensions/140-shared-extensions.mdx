...
Use `Prisma.defineExtension` to make your extensions shareable. `Prisma.defineExtension` can be used to move an extension to another file in your project, while still benefiting from autocompletion and type-checks. And finally, it can be used to package a a feature that you wish to share with other users. The result of `Prisma.defineExtension` can be exported and loaded via `$extends`.
