...
#### Refer to the client instance that the extension user applies the extension to

In your application code, you can refer to the client instance that the extension user applies your extension to.

For example:

```ts
Prisma.defineExtension((client) => {
  // This is the client that the user applies the extension to. You, as an npm-published extension author know very little about it
  return client.$extends({})
})
```

To give your generic extension access to the client instance it is applied to, use the following forms:
