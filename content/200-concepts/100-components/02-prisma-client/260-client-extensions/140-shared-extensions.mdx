...
For generic extensions, you cannot predict which `prisma` instance the extension user will want to use. Therefore, you cannot use `prisma.$extends`, which applies to a specific instance. Instead, use `Prisma.defineExtension`, which declares the extension but does not apply it to a specific instance. When someone uses your extension in their project, they can apply it to an instance with `prisma.$extends`.

If you want to package the extension in the above example, it would take the following form:
