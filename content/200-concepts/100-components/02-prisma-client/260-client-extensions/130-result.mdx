...
- `compute`: an object that contains methods that define how the virtual field is computed when it is accessed. You must declare the same properities in both `compute` and `needs`.
