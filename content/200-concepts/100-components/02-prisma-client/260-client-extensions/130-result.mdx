...
- `fields`: (required) an object that contains methods that define how the virtual field is computed when it is accessed. Properties declared in `fields` must also be declared in `needs`.
- `return`: the value of the computed field.

** we don't have return in the example above **
...
** Need to check this example Not sure it works, but result might be in the branch yet **
...
- For the `result` component, Prisma Client only computes a field if its dependencies are present. In the following example, `undefined` is returned for `fullName` if the user's `email` is undefined.
  ```ts
