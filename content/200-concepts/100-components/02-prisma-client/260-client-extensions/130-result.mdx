...
From version 4.7.0, you can use the `result` [Prisma Client extensions](/concepts/components/prisma-client/client-extensions) component type to add custom fields and methods to query results.
...
To add a custom [field](#add-a-custom-field-to-query-results) or [method](#add-a-custom-method-to-the-result-object) to query results, use the following structure. In this example, we add a custom field to the `user` model.
...
- You can only use computed fields with `select` and you cannot aggregate them.
...
When you call a method in an extension, use the constant name from your`$extends`statement, not `prisma`. Instead, use the constant name from your `$extends` statement. In the above example, `xprisma.user.findUniqueOrThrow` works, but `prisma.user.findUniqueOrThrow` does not.
