...
By default, all engine files are automatically downloaded into the `node_modules/@prisma/engines` folder when you install or update `prisma`, the Prisma CLI package. The [query engine](../../components/prisma-engines/query-engine) is also copied to the generated Prisma Client when you call `prisma generate`.
You might want to use a [custom library or binary](https://github.com/prisma/prisma-engines) file if:

- Automated download of engines is not possible.
- You have created your own engine binary or library (for testing, or for an OS that is not officially supported).
...
The [`PRISMA_ENGINES_MIRROR`](../../../reference/api-reference/environment-variables-reference#prisma_engines_mirror) <span class="api"></span> environment variable allows you to host engines via a private server, AWS bucket or other cloud storage.
This can be useful if you have a custom OS which requires you to use your own custom built engines.
