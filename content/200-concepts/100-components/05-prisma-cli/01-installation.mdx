...
When installing Prisma CLI, a [`postinstall`](https://github.com/prisma/prisma/blob/master/cli/sdk/package.json#L13) hook is being executed. It downloads Prisma's query and migration [engine binaries](https://github.com/prisma/prisma-engine). The query engine contains the [Prisma schema](../prisma-schema) parser which is used by the `prisma init` and the `prisma generate` commands. The migration engine is used by all `prisma migrate` commands.
