...
An environment variable is a key value pair stored on your machines local environment. Traditionally the name of the variable is uppercase, this is then followed by an equals sign then the value of the variable.
...
Prisma always reads environment variables from the systems environment. 

When you install Prisma for the first time it creates a convenience `.env` file for you to set your [`connection url`](../../reference/database-reference/connection-urls) as an environment variable. The `.env` file puts the variable in the systems environment and Prisma reads it from there.
...
Although Prisma creates a default `.env` file at your projects root, you can create additional (or replace) `.env` files at any location in your projects filesystem. The Prisma CLI looks for `.env` files, in order, in the following locations:
