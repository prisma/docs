...
Because Prisma reads from the systems environment when looking for environment variables, it's possible to skip using `.env` completely and create them manually on your local system. 

<Admonition type="info">

The following examples will use setting the `DATABASE_URL` environment variable
...
The following examples illustrate how to set the environment variable (for the current user) using both Command Prompt and Powershell, depending on your preference.
...
```terminal
setx DATABASE_URL "postgresql://test:test@localhost:5432/test?schema=public"
```
