...
**Drizzle** is a traditional SQL query builder which lets you compose SQL queries with JavaScript/TypeScript functions. It can be used to query a database or run migrations. Drizzle also offers a Queries API which offers a higher level abstraction from SQL and is useful for nested relations. Drizzle schema is defined in TypeScript files which are used to generate SQL migrations and are then executed against a database.
...
Select queries are natural to construct in both Drizzle and Prisma. Using Drizzle’s Queries API, the two approaches are very similar:
...
Both Drizzle and Prisma offer products alongside an ORM. Prisma Studio was released to allow for users to interact with their database via a GUI and also allows for limited self hosting for use within a team. Drizzle Studio was released to accomplish the same tasks.
...
For teams that use SQL daily, Drizzle offers a convenient wrapper that will feel familiar and is type safe. Prisma’s `$queryRaw` feature, while available, does not provide the same level of type safety.
