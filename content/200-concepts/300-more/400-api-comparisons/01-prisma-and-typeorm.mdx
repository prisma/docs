...
## Level of abstraction & API design
...
The following sections examine a few examples for how the Prisma's and TypeORM's APIs differ in certain scenarios and what the rationale of Prisma's API design is in these cases.
...
- [Nested writes](../../components/prisma-client/relation-queries#nested-writes) for creating and updating relationships
- The [`$transaction`](../../components/prisma-client/transactions#the-transaction-api) API for sending multiple operations in bulk
- [Atomic operators](../../components/prisma-client/crud#atomic-operations-on-update) for updating values in a single database query
...
- traversing relationships via the [fluent API](../../components/prisma-client/relation-queries#fluent-api)
- [nested writes](../../components/prisma-client/relation-queries#nested-writes) that enable updating/creating connected records
- applying [filters on related records](../../components/prisma-client/relation-filters)
- easy and type-safe [querying of nested data](../../components/prisma-client/relation-queries#nested-reads) without worrying about JOINs
- creating [nested TypeScript typings](../../components/prisma-client/advanced-usage-of-generated-types) based on models and their relations
- intuitive modeling of [relations in the data model](../../components/prisma-schema/relations) via relation fields
- [implicit handling of relation tables](../../components/prisma-schema/relations#implicit-many-to-many-relations) (also sometimes called JOIN, link, privot or junction tables)
