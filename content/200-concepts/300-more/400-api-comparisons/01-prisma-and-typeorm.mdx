...
- traversing relationships via the fluent API
- nested writes that enable updating/creating connected records
- applying filters on related records
- easy and type-safe querying of nested data without worrying about JOINs
- intuitive modeling of relations in the data model

### Data modeling and migrations

Prisma models are defined in the [Prisma schema]() while TypeORM uses TypeScript classes and decorators for model definitions. With the Active Record ORM pattern, TypeORM's approach often leads to complex model instances that are becoming hard to maintain as an application grows. Prisma on the other hand generates a database client that exposes a fully type-safe API to read and write data for the models that are defined in the Prisma schema.
