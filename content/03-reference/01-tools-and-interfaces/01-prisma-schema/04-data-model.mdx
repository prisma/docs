...
Any model can at most have one ID, no matter if it's defined on a single field or on multiple fields.
...
> **Note** A field that's annotated with `@id` can not be [optional](models#optional-vs-required).
