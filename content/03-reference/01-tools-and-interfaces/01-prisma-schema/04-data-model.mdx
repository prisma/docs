...
Models represent the entities of your application domain. Models are represented by [`model`](prisma-schema-reference#model) <span class="api"></span> blocks and define a number of [fields](#fields). In the [example data model](data-model#example), `User`, `Profile`, `Post` and `Category` are models.
...
The Prisma schema supports a number of [functions](prisma-schema-reference#attribute-functions) <span class="api"></span>. These can be used to specify [default values](#default-values) on fields of a model. 
