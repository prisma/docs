...
- [Scalar types](data-model#scalar-fields) (includes [enums](data-model#enums)) - for example, `String` or `Int` (scalar lists such as `String[]` are only supported if your database supports scalar lists)
...
* A [many-to-many](relations#many-to-many-relations) relation between posts and tags
* A [one-to-many](relations#one-to-many-relations) relation between comments and posts
...
model Tag {
  name   String  
|  posts  Post[] // A tag can have multiple posts
}
```

Refer to the [Relations documentation](relations) for more examples and information about relations in Prisma.
