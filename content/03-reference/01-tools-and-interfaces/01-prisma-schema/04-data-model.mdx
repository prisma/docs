...
`cuid()` and `uuid()` are implemented by Prisma and therefore are not "visible" in the underlying database schema. You can still use them when using [introspection]() by [manually changing your Prisma schema]() and [generating Prisma Client]().
...
**Map the `Role` enum to a native enum in the database called `_Role` its values to lowercase values in the databse**:
