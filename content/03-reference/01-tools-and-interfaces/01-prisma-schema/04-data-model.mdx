...
Some attributes accept [arguments](#attribute-arguments) - for example, `@default` accepts `true` or `false`:
...
- [Functions](#using-functions), such as `now()` or `uuid()`

There is a special [function](#using-functions) in the Prisma schema named `dbgenerated()` which is used in introspection results for default values that can not yet be represented in the Prisma schema.
