...
A blogging platform can also include the following models:
...
Learn more about relations between models on the relations page.
...
An ID uniquely identifies individual records of a model. IDs can be defined by a single field using the [`@id`](prisma-schema-reference#id) <span class="api"></span> attribute, or multiple fields using the [`@@id`](prisma-schema-reference#id-1) attribute.
