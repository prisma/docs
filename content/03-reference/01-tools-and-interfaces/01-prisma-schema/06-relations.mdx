...
```ts
const getAuthor = await prisma.post.findOne({
  where: {
    id: "20",
  },
  include: {
...
- [One-to-one](one-to-one) (also called 1-1-relation)
- [One-to-many](one-to-many) (also called 1-n-relation)
- [Many-to-many](many-to-may) (also called m-n-relation)
...
Relation [fields](data-model#fields are fields on a Prisma [model](data-model#models) that do _not_ have a [scalar type](data-model#scalar-types). Instead, their type is another model.
...
> **Note**: Implicit many-to-many relations do **not** require the `@relation` attribute unless you need to [disambiguate relations](relations#disambiguate-relations) (this is not specific to implicit n-m relations).

Implicit many-to-many relations require both models to have a single `@id`. Be aware that:

- You cannot use a [multi-field ID](data-model#multi-field-ids)
...
When using Prisma, you can create relation tables by defining [models](data-model#models) similar to how you would model them as tables. The main difference is that the fields of the relation table are both annotated relation fields with a corresponding relation scalar field.
...
The `@relation` attribute can only be applied to the [relation fields](#relation-fields), not to [scalar fields](data-model#scalar-types). The attributes is required on **one** side of 1-1 and 1-n relations.
...
| `fields`     | `FieldReference[]` | On [annotated](#annotated-relation-fields-and-relation-scalar-fields) relation fields | A list of [fields](data-model#fields of the _current_ model                                                             | `["authorId"]`, `["authorFirstName, authorLastName"]` |
| `references` | `FieldReference[]` | On [annotated](#annotated-relation-fields-and-relation-scalar-fields) relation fields | A list of [fields](data-model#fields of the model on _the other side of the relation_                                   | `["id"]`, `["firstName, lastName"]`                   |
...
In the example above, there's one 1-1-relation between `User` and `Profile`:
...
- "a user must always have one profile" (because the `profile` field is [required](data-model#optional-vs-required) on `User`)
- "a profile must always be connected to one user"
...
In the example above, there's one 1-n-relation between `User` and `Post`:
...
In the example above, there's one _implicit_ m-n-relation between `Post` and `Category`:
...
Note that you can also require each user to have a teacher by making the `teacher` field [required](data-model#optional-vs-required).
