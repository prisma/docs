...
- "a user can have one or zero predecessors" (for example, Sarah is Mary's predecessor)
- "a user can have one or zero successors" (for example, Mary is Sarah's successor)
...
- One relation field must be a [fully annotated](#annotated-relation-fields-and-relation-scalar-fields). In this example, the `successor` field defines both the `field` and `references` arguments. The `successor` field is backed by the `successorId` foreign key, and references the `id` field.
