...
A relation is a _connection_ between two models in the Prisma schema. For example, there is a one-to-many relationship between `User` and `Post` - a user can have many posts. The following Prisma schema includes a relationship between `User` and `Post`:
...
The schema has two _relation fields_: `author` and `posts`. Relation fields are used to model relationship at the Prisma level and **do not exist in the database**. The following diagram compares the generated schema and the original SQL:
