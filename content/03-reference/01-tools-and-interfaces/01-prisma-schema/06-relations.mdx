...
The [`@relation`](prisma-schema-reference#relation) <span class="api"></span> attribute can only be applied to the [relation fields](#relation-fields), not to [scalar fields](data-model#scalar-types). The attributes is required on **one** side of 1-1 and 1-n relations.
