...
A relation is a _connection_ between two models in the Prisma schema. A relation must always be defined with two _relation fields_, one for each model. In the case of one-to-one and one-to-many relations, an additional _relation scalar field_ must be added:
