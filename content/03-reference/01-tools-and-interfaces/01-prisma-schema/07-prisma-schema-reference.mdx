...
* If you specify [multiple providers](prisma-schema-reference#specify-multiple-data-sources), Prisma Client uses the provider that matches the connection string defined by the `url` field.
...
This example shows how to configure Prisma Client to run on `Ubuntu 19.04 (disco)` based on the table [above](prisma-schema-reference#linux-ubuntu).
...
Defines a Prisma [model](../../../reference/tools-and-interfaces/prisma-schema/data-model#models) <span class="concept"></span>.
...
- Every record of a model must be _uniquely_ identifiable. You must define at least _at least_ one of the following attributes per model:
  * [`@unique`](prisma-schema-reference#unique)
  * [`@@unique`](prisma-schema-reference#unique-1)
  * [`@id`](prisma-schema-reference#id)
  * [`@@id`](prisma-schema-reference#id-1)
...
> **Note**: You can use the [`@@map` attribute](prisma-schema-reference#map-1) to map a model (for example, `User`) to a table with a different name that does not match model naming conventions (for example, `users`).
...
> **Note**: You can use the [`@map` attribute](prisma-schema-reference#map) to map a field name (for example, `MyField`) to a column with a different name that does not match field naming conventions (for example, `myField`).
...
Attributes modify the behavior of a [field](prisma-schema-reference#model-fields) or block (e.g. [models](prisma-schema-reference#model)). There are two ways to add attributes to your data model:
...
* Can be annotated with a [`@default()`](prisma-schema-reference#default) value that uses [functions](prisma-schema-reference#attribute-functions) to auto-generate an ID:

  * [`autoincrement()`](prisma-schema-reference#autoincrement)
  * [`cuid()`](prisma-schema-reference#cuid)
  * [`uuid()`](prisma-schema-reference#uuid)
...
In most cases, you want to have the ID initialized by your database. To do so, you can annotate it with the `@default` attribute and initialize the field with a [function](prisma-schema-reference#attribute-functions).
...
* Default values can be a static value (`4`, `"hello"`) or one of the following [functions](prisma-schema-reference#attribute-functions):

  * [`autoincrement()`](prisma-schema-reference#autoincrement)
  * [`cuid()`](prisma-schema-reference#cuid)
  * [`uuid()`](prisma-schema-reference#uuid)
  * [`now()`](prisma-schema-reference#now)
...
An array of [field](prisma-schema-reference#model-fields) names: `[id]`, `[firstName, lastName]`
