...
Model names must start with a letter. They are typically spelled in [PascalCase](http://wiki.c2.com/?PascalCase) and use the singular form (e.g. `User` instead of `user`, `users` or `Users`).
...
There are a few exceptions to this since Prisma has a number of **reserved words** that are being used by Prisma internally and therefore can't be chosen as a model name. You can find the reserved words [here](https://github.com/prisma/prisma/blob/master/src/packages/client/src/generation/generateClient.ts#L463) and [here](https://github.com/prisma/prisma-engines/blob/master/libs/datamodel/core/src/transform/ast_to_dml/reserved_model_names.rs).
