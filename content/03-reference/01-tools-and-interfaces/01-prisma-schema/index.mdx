...
1. From the location of the schema taken from `"prisma": {"schema": "/path/to/schema.prisma"}` in `package.json`
...
You are trying to load env variables which are already present in your project root .env
  Root: C:\Prisma\Sandbox\mssql\.env
  Prisma: ./prisma/.env
  Env Conflicts:
          DATABASE_URL

You can fix this by removing the .env file from "./prisma/.env" and move its contents to your .env file at the root "C:\Prisma\Sandbox\mssql\.env" 
  ```
