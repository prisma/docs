...
1. From the location specified by the `--schema` argument
1. From the location of the schema taken from `"prisma": {"schema": "/path/to/schema.prisma"}` in `packge.json`
1. From the `./prisma` folder

If a `.env` file is located in step #1, but additional, clashing `.env` files are located in steps #2 - 4, the CLI will throw an error.
...
| `prisma [command] --schema=./a/b/schema.prisma` | `./a/b/schema.prisma`                                                           | `./a/b/.env`<br />`./prisma/.env` |
