...
2.  Create an `.env` file in, for example, the root of your project.

3.  Include a reference to `dotenv` at the top of your project's entry file.
...
4.  Use `dotenv` when you run any Prisma command and specify which `.env` file to use. The following example uses a file named `.env3`:
