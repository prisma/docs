...
The `@prisma/client` npm module consists of two key parts:

- The `@prisma/client` module itself, which only changes when you re-install the package
- The `.prisma/client` folder, which contains the unique Prisma client generated from your schema
...
Your Prisma client is generated from your Prisma schema and is unique to your project. Each time you change the schema (for example, by performing a [schema migration](../prisma-migrate)) and run `prisma generate`, the client code changes:

![The .prisma and @prisma folders](generating-prisma-client/prisma-client-node-module.png)

> **Note**: The `.prisma` folder is unaffected by [pruning](https://docs.npmjs.com/cli/prune.html) in Node.js package managers.
