...
A race conditions occurs when two or more operations must be done in sequence in order to complete a task. In the following example, two clients try to update the same field (`postCount`):
...
The post could *should* be `23`, but the two clients did not read and write to the `postCount` field in sequence. Atomic operations on update combine read and write into a single operation, which prevents a race condition:
...
| `set`    | Sets the current field value. Identical to `{ myField : 2 }`. | 
