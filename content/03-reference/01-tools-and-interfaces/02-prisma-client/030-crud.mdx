...
const createdUser = await prisma.user.create({
  data: {
    ...
    coinflips: set: [true, true, true, false, true],
  },
});
```

Scalar lists only supports setting a value - you cannot add or remove elements, only overwrite the entire value. The following example retrieves user, uses `push()` to add three new coin flips, and overwrites the `coinflips` property in an `update`:
