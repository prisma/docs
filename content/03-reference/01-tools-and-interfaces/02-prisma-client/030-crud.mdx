...
Atomic operations on update is availabler for `Float` and `Int` field types. This feature allows you to update a field based on its **current** value (such as *subtracting* or *dividing*) without risking a race condition.
