...
Offset pagination combines `skip` and `take` to skip a certain number of results and select a limited range. The following query skips the first 3 `Post` records and returns records 4 - 7:
...
![](pagination/offset-skip-take.png)
...
Cursor-based pagination combines `cursor` and `take` to return a limited set of results before or after a given **cursor**. A cursor is any unique, sequential column - such as an ID or a timestamp - that can be used to **bookmark** your location a result set. For example, you can `take` 10 results that come after the `Post` record with an `id` of 29.

> **Note**: Cursor-based pagination [requires that you sort by your 'bookmark'](#sorting-and-cursor-based-pagination) - the unique, sequential value that represents your cursor.

The following query returns the first 4 `Post` records that contain the word `"Prisma"`. The ID of the last `Post` in the result
...
> **Note**: The value of `myCursor` must come from your first query - if you guess, you will page to an unknown location in your result set. Although IDs are sequential, you cannot predict the rate of increment (`2`, `20`, `32` is more likely than `1`, `2` `3`, particularly in a filtered result set).
