...
Offset pagination combines `skip` and `first` to skip a certain number of results and select a limited range. The following query skips the first 40 `Post` records and returns records 41 - 51:
...
- Offset pagination **does not scale**. For example, if you skip 200,000 records and take the first 10, the database still has to traverse the first 200,000 records before returning the 10 you asked for.
...
### Filtering and off-set pagination
...
### Sorting and off-set pagination
...
Cursor-based pagination combines `after` and `first` or `before` and `last` to return a limited subset of results **before or after a cursor** that you supply. A cursor **bookmarks** your location in the result set as you paginate, and must be a unique, sequential column - such as an ID or a timestamp.
...
> **Note**: The value of `cursor` must come from your first query - if you guess, you will page to an unknown location in your result set. Although IDs are sequential, you cannot predict the rate of increment (`2`, `20`, `32` is more likely than `1`, `2` `3`, particularly in a filtered result set).
...
> \*_Note_ Cursor pagination does not [use cursors in the underlying database](https://www.postgresql.org/docs/9.2/plpgsql-cursors.html).
