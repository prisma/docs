...
Cursor-based pagination combines `cursor` and `take` to return a limited set of results before or after a given **cursor**. A cursor bookmarks your location in a result set and must be a unique, sequential column - such as an ID or a timestamp.
...
The following diagram shows the IDs of the first 4 results - or page 1. The cursor for the next cursor is **29**:

![](pagination/cursor-1.png)
...
No, cursor pagination does not [use cursors in the underlying database](https://www.postgresql.org/docs/9.2/plpgsql-cursors.html).
