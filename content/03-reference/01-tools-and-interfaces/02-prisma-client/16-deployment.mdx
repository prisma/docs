...
Projects using Prisma Client can be deployed to many different cloud deployment platforms. Given the large number of cloud offerings, it's useful to make a distinction between the two main deployment paradigms for a Node.js based project:

- **Long-running Node.js process**: The traditional paradigm where a Node.js process is constantly running and handles multiple requests.
- **Serverless**: A newer model where a Node.js process is started

Each paradigm has different tradeoffs. Specifically, the paradigm you choose affects the performance, scalability, and operational cost of your application.

Beyond the deployment paradigm, Prisma Client relies on the Prisma query engine– a binary that is started by Prisma Client.
