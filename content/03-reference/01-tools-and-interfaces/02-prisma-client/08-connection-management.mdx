...
**Note**: When deploying your application as a long-running process or when running a Node.js script that uses Prisma Client, it's recommended to always explicitly call `prisma.disconnect()` in your code even when an exception is thrown as shown below, otherwise, the Node.js process will not exit automatically. In serverless enviornments, there are nuances that make it sensible to reuse DB connections, i.e. not call disconnect. The [deployment docs](./deployment) cover those cases in more detail.
...
If the `connection_limit` argument is omitted, the default number of connecctions is calculated according to this formula: `num_physical_cpus * 2 + 1` where `num_physical_cpus` represents the number of physical CPUs on your machine.
