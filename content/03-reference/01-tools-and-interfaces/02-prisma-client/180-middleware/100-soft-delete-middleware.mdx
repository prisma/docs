...
| :----- | :-------------- | :---------- |
| `findOne`    | 🔧 Change query to to `findFirst` (because you cannot apply `deleted: false` filters to `findOne`) <br /> 🔧 Add `where: { deleted: false }` filter to exclude soft deleted posts | No change   |
