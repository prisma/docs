...
Prisma Client exposes two functions that allow you to send raw SQL queries to your database:

- Use `queryRaw` to return actual records (for example, using `SELECT`)
- Use `executeRaw` to return a count of affected rows (for example, using `UPDATE`)

Use cases for raw SQL include:

- Heavily optimized query
- Feature that you cannot currently do with Prisma Client

## `queryRaw`

Use `queryRaw` if you want to retrieve actual database records - for example, using `SELECT`:
...
Sometimes you are interested in the _number of affected rows of a database operation_ instead of retrieving records.
...
It's now possible to use parameters and they will be escaped.

The following helpers for template literals:
