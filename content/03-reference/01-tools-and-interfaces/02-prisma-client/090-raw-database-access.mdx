...
> **Note**: You cannot pass in table or column names as tagged template variables. 
...
* Escapes all variables when you use tagged templates or a parameterized query.
* Sends all queries as prepared statements.
...
We recommend that you use tagged templates and avoid using string concatenation to construct a query. The following example returns all users where `name` is `Sarah`:

```ts
const inputString = '"Sarah"';
...
However, because `inputString` is not escaped, you can extend the query with a `UNION` statement to include data from the `Post` table:
...
By contrast, the same query using tagged templates escapes `inputString` and the query does not return results:
