...
The same method is implemented as a tagged template, which makes it easier to use [variables](#using-variables) by providing **placeholders**. The following example includes an `${email}` placeholder:
...
The same method is implemented as a tagged template, which makes it easier to use [variables](#using-variables) by providing placeholders. The following example includes `${emailValidated}` and `${active}` placeholders:
...
> **Note**: `executeRaw` does not support [multiple queries in a single string]() (for example, `ALTER TABLE` and `CREATE TABLE` together).
...
> **Important**: You *must* use double backticks - the same query using `queryRaw()` is not necessarily secure.
...
* Tagged templates give you access to [useful helpers](#tagged-template-helpers).
* Embedded, named variables are easier to read.

> **Note**: You cannot pass a table or column name into a tagged template placeholder.

### Tagged template helpers

Prisma Client specifically uses [SQL Template Tag](https://github.com/blakeembrey/sql-template-tag
), which exposes a number of helpers. For example, the following query uses `join()` to pass in a list of IDs:
...
> **Note**: You cannot pass a table or column name as a variable into a parameterized query.

## SQL injection

Prisma Client mitigates the risk of SQL injection in the following ways:

* Prisma Client escapes all variables when you use tagged templates (``` queryRaw`` ``` or ``` executeRaw`` ``` ) or parameterized queries (`queryRaw(query, variables)` or `executeRaw(query, variables)`).
* Prima Client sends all queries as prepared statements.
...
The following example concatanates `query`and `inputString`. Prisma Client ❌ **cannot** escape `inputString` in this example, which makes it vulnerable to SQL injection:
