...
In the following example, if `emailInput` is **not** valid, the query sets `email` (a **mandatory** field) to `undefined` - which means ✔ **do not include this in the update**:
...
Howver, if you pass `null` values for `authorEmail` or `authorName` on to Prisma, the following will happen:
...
|     name: args.name != null ? args.name : undefined // If null, do nothing
     },
  })
},
```