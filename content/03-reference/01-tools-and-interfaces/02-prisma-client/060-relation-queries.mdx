...
- `connectOrCreate` (experimental): Create a new user and connect it to an existing profile, _or_ create a new profile if the supplied profile ID does not exist
...
- `connectOrCreate` (experimental): Update an existing user by connecting it to an existing profile, _or_ create a new profile if the supplied profile ID does not exist
...
You can provide any [unique](../prisma-schema/data-model#unique) or [ID](../prisma-schema/data-model#ids) property to the `connect` option, so in this case you could also provide the `id` field to identify the target `User` record:
...
The following example creates a `Profile`, then attempts to connect the profile to a `User` where the email address is `alice@prisma.io`. If the `User` does not exist, it is created:
...
#### Update an existing `User` record by connecting it to an existing `Profile` record, *or* create a new `Profile` record
...
- `connectOrCreate` (experimental): Create a new user and connect it to one or more existing posts, _or_ create a new posts if the supplied post ID does not exist
...
- `connectOrCreate` (experimental): Update an existing user by connecting it to one or more existing posts, _or_ create a new posts if the supplied post ID does not exist
