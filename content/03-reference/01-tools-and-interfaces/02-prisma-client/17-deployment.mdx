...
When the Prisma Client connects to the database, the [query engine](./query-engine) immediately creates a connection pool with the amount of connections that were specified as the `connection_limit` parameter in your database connection URL.
