...
| :----- | :-------------- | :---------- |
| `findOne`    | 🔧 Change query to to `findMany` (because you cannot apply `deleted: false` filters to `findOne`) <br /> 🔧 Add `where: { deleted: false }` filter to exclude soft deleted posts | `[]` instead of `Post` (`findMany` returns an array)   | test |
...
| `update`    | 🔧 Change query to to `updateMany` (because you cannot apply `deleted: false` filters to `update`) <br /> 🔧 Add `where: { deleted: false }` filter to exclude soft deleted posts | `{ count: n }` instead of `Post`    |
...
async function mainAsync() {
  ...
  prisma.$use(async (params, next) => {
    if (params.model == "Post") {
      if (params.action == "findOne") {
        // Change to findMany - you cannot filter
  ...
  const getOnePost = await prisma.post.findOne({
    where: {
...
* Return types of `findOne` and `update` affected because middleware changes the queries to `findMany` and `updateMany` respectively
