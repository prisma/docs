...
The following example uses middleware to perform a **soft delete**, which means that a record is marked as deleted rather than removed from the database.
...
* Intercept `delete` and `Post`
* Change the `params.action` to an `update`
* Introduce a `data` argument
