...
When you use `select` to load related records (for example, a user's posts), you can filter which related records should be returned. For example, you might want to return all users, but only include posts that have been published. Some user records will be returned without any posts.

### Examples

#### Retrieve _all_ `User` records, and select all `Post` records that have not been published yet
...
You can write the same query in a different way:
...
#### Retrieve `User` records with at least one `Post` that isn't published, and select all `Post` records that have not been published yet
