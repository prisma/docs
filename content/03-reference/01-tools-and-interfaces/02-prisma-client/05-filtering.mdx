...
| Operator | Input type      | Notes                                      |
| -------- | --------------- | ------------------------------------------ |
| `AND`    | `Enumerable<T>` | All conditions must return`true`.          |
| `NOT`    | `Enumerable<T>` | All conditions must return `false`.        |
| `OR`     | `Enumerable<T>` | One or more conditions must return `true`. |
...
The following format returns the same results:

> _Not_: Only valid for `AND`. `OR` and `NOT` require an array.
