...
> **Note**: This feature is currently in [Preview](../../../more/releases#preview). Features in Preview may still have some issues, and are not recommended for use in production environments. To access commands in Preview, you must opt-in via the `--preview-feature` flag.
...
- `db push` does not interact with or rely on migrations. The migrations table will no be updated, and no migration files will be generated. If migrations are detected, `db  push` will prompt for confirmation or require an explicit option (`--ignore-migrations`) 
...
- You want to keep track of changes made to the database schema over time. `db push` does not create any artifacts that allow you to keep track of this information.
...
Prisma Migrate introspects the database to anticipate the changes needed to reach the end-state of the migration and preview these changes as DDL statements in [`README.md`](http://readme.md) files.
...
2. the DDL statements in [`README.md`](http://readme.md) will **not** reflect the steps carried out in other environments where `db push` was not used - the statements can be different or entirely missing.
...
Before using the `db push` command, you must define a valid `[datasource](../../../prisma-schema/data-sources/)` within your `schema.prisma` file.
