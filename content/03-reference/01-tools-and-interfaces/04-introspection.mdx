...
### Table with no unique identifier or id

> The following models were commented out as they do not have a unique identifier or id. This is currently not supported by Prisma.

TODO

Workaround: Make a field unique or Primary Key
If your data has a column with unique data, but that does not have a unique index, you can try to uncomment the model and add @unique to this field manually. Caution: May break if you do have non unique data in that column.
...
### Field with unsupported name

> These fields were commented out because their names are currently not supported by Prisma.

(Please provide valid ones that match [a-zA-Z][a-zA-Z0-9_]*.)

TODO

Workaround: Use @map manually to give a supported field name that follows `[a-zA-Z][a-zA-Z0-9_]*.`



### Enum with unsupported name

> These enum values were commented out because their names are currently not supported by Prisma.

TODO

Workaround: Use @map manually to give a supported enum name that follows `[a-zA-Z][a-zA-Z0-9_]*.`
