...
Because the leading `5` in the table name as well as the leading underscores and the `$` on the columns are forbidden in Prisma, introspection adds the `@map` and `@@map` attributes so that these names adhere to Prisma's naming conventions:
...
This topic is covered extensivelty on the docs page about [relations]().
