...
1. Write data model into Prisma schema or [update existing schema](#re-introspection)

## Re-introspection

You **re-introspect** when you run the `prisma introspect` command with an existing Prisma schema. In version 2.6.0 and later, re-introspection merges manual changes made to the schema with changes made in the database. Re-introspection maintains the following manual changes:
...
The following properties of the schema are determined by the database:e

- Order of columns within `model` blocks
...
To overwrite manual changes and generate a schema based solely on the introspected database, add the `--force` flag to the `introspect` command:
