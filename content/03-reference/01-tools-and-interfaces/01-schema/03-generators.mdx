...
Prisma relies on several [engine binaries](https://github.com/prisma/prisma-engines). For example, Prisma Client uses the [query engine binary file]() under the hood to communicate with a database. Prisma Migrate uses the [migration engine binary file]() to run schema migrations against a database.
...
| Build OS              | Build name             | Build OS | OpenSSL |
| :-------------------- | :--------------------- | :------: | ------- |
