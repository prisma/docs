---
title: 'Start from scratch with Microsoft SQL Server'
metaTitle: 'Start from scratch with Microsoft SQL Server (Preview)'
metaDescription: ''
...
- A [Microsoft SQL Server](https://docs.microsoft.com/en-us/sql/?view=sql-server-ver15) database
  ...
  - [Microsoft SQL Server on Linux for Docker](Microsoft SQL Server-docker)
  - [Microsoft SQL Server on Windows (local)](Microsoft SQL Server-local)

Make sure you have your database [connection URL](Microsoft SQL Server-connection-string) (including authentication credentials) available.
...
1. Define the `DATABASE_URL` [environment variable](../../reference/tools-and-interfaces/prisma-schema/prisma-schema-file#using-environment-variables) in the `prisma/.env` file - this is the connection string to your database.
    ...
    The following example connection string [uses SQL authentication](Microsoft SQL Server-local#enable-authentication-with-sql-logins-optional), but there are [other ways to format your connection string]((Microsoft SQL Server-connection-string)):
...
1. Adjust the connection string to match your setup - see [Microsoft SQL Server connection string](Microsoft SQL Server-connection-string) for more information.
...
* Use Microsoft SQL Server Management Studio (Windows only)
...
1. Use the `sqlcmd` tool to run the `create.sql` script, replacing `mypassword` with your password:
  ...
  ```terminal
  /opt/Microsoft SQL Server-tools/bin/sqlcmd -S localhost -U SA -P "mypassword" -i create.sql
  ```

1. To confirm that your tables were created, log in to `sqlcmd`:

   ```
   /opt/Microsoft SQL Server-tools/bin/sqlcmd -S localhost -U SA -P "mypassword" 
   ```
  ...
  <Cmd>
   ...
   <CmdResult>
    ...
    ```
     ...
     --------------- --------------- --------------- --------------- ---------------
     mstest          dbo             _Migration      TABLE           NULL
     mstest          dbo             _PostToTag      TABLE           NULL
     mstest          dbo             Category        TABLE           NULL
     mstest          dbo             Post            TABLE           NULL
     mstest          dbo             Tag             TABLE           NULL
...
### Using Microsoft SQL Server Management Studio (Windows only)

To create tables Microsoft SQL Server Management Studio:

1. Log in to Microsoft SQL Server Server Management Studio.

1. Click the **New Query** button:

  ![The New Query button in Microsoft SQL Server Management Studio](../../../../doc-images/new-query.png)
...
1. Expand the database node in the Object Explorer to confirm that your tables were created:
  ...
  ![A list of database tables in the Object Explorer in Microsoft SQL Server Managmeent Studio](../../../../doc-images/database-tables.png)
