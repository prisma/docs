...
- A [MSSQL](https://docs.microsoft.com/en-us/sql/?view=sql-server-ver15) database

  - [Microsoft SQL Server on Linux for Docker](mssql-docker)
  - [MSSQL on Windows (local)](mssql-local)

Make sure you have your database [connection URL](mssql-connection-string) (including authentication credentials) available.
...
1. Define the `DATABASE_URL` [environment variable](../../reference/tools-and-interfaces/prisma-schema/prisma-schema-file#using-environment-variables) in the `prisma/.env` file - this is the connection string to your database.
    ...
    The following example connection string [uses SQL authentication](mssql-local#enable-authentication-with-sql-logins-optional), but there are [other ways to format your connection string]((mssql-connection-string)):

    ```js file=prisma/.env
    DATABASE_URL="sqlserver://localhost:1433;database=mydb;user=sa;password=r@ndomP@$$w0rd;trustServerCertificate=true"
    ```

    > To get around TLS issues, add `encrypt=DANGER_PLAINTEXT` if you are connecting to MSSQL from MacOS specifically.
...
* Use MSSQL Management Studio (Windows only)
...
### Using MSSQL Management Studio (Windows only)

To create tables MSSQL Management Studio:

1. Log in to MSSQL Server Management Studio.

1. Click the **New Query** button:

  ![The New Query button in MSSQL Management Studio](../../../../doc-images/new-query.png)
...
1. Expand the database node in the Object Explorer to confirm that your tables were created:
  ...
  ![A list of database tables in the Object Explorer in MSSQL Managmeent Studio](../../../../doc-images/database-tables.png)
        ...
        data: {
          ...
          posts: {
            create: [
              ...
              {
                title: "How to create an MSSQL database",
