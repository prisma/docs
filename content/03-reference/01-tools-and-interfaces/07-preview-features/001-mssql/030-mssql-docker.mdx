---
title: 'Microsoft SQL Server on Docker'
metaTitle: 'Microsoft SQL Server  on Docker (Preview)'
...
To run a Microsoft SQL Server Server container image with Docker:
...
1. Run the following command in your terminal to download the Microsoft SQL Server 2019 image:
   ...
   ```terminal
   docker pull mcr.microsoft.com/Microsoft SQL Server/server:2019-latest
   ```
   
1. Create an instance of the container image, replacing `SA_PASSWORD` with a password of your choice:

   ```terminal
   docker run --name sql_container -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=myPassword' -p 1433:1433 -d mcr.microsoft.com/Microsoft SQL Server/server:2019-latest
   ```
