...
To connect to a MySQL database server, you need to configure a [`datasource`](../prisma-schema/data-sources) block in your [Prisma schema file](../prisma-schema/prisma-schema-file):
...
- `url`: Specifies the [connection URL](#connection-url) for the MySQL database server. In this case, an [environment variable is used](../prisma-schema/prisma-schema-file#using-environment-variables) to provide the connection URL.
...
The MySQL connector maps the [scalar types](../prisma-schema/data-model#scalar-types) from the Prisma [data model](../prisma-schema/data-model) as follows to native column types:
...
During [introspection](../introspection), fields with types that **already have match in the Prisma schema but are not yet supported** will be added to the Prisma schema as comments, e.g. `macaddr` would be added to a model as follows:
