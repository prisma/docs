...
> **Note**: The Graphcool servers are gradually shut down from July 1st onwards. Please [reach out](mailto:burk@prisma.io) to us if you have a mission-critical application running on Graphcool and are not able to migrate away from Graphcool until July 1st.
...
Prisma unfortunately doesn't have a production-ready solution for running schema migrations against a database yet. This means that if you migrate to Prisma and want to evolve your application in the future, you'll need to migrate your database schema _manually_ using SQL or another migration tool like [Flyway](https://flywaydb.org/) (until [Prisma Migrate](../../reference/tools-and-interfaces/prisma-migrate) is considered production-ready).
...
1. [Introspect](../../reference/tools-and-interfaces/introspection) your database with Prisma 2.0
...
With your new npm project in place, you can follow the [guide](../../getting-started/setup-prisma/add-to-existing-project-typescript-postgres) that explains how to add Prisma to an existing project.

Note that Graphcool uses the same data modeling approach as Prisma 1. If you encounter any [schema incompatibilies](../upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities), you can resolve them in the same way as if you were upgrading from Prisma 1.
