...
1. Migrate the foreign key values from the current relation table into the new foreign key column on `Post`
1. Delete the relation table. T
...
1. Write a SQL query that reads all the rows from the `_PostToUser` relation table and for each row:
    ...
    1. Inserts the value from column `B` as the value for `authorId` into that `Post` record
    ```sql
    UPDATE "default$default"."Post" 
    SET "authorId" = "default$default"."_PostToUser"."B" 
    FROM "default$default"."_PostToUser" 
    WHERE "default$default"."_PostToUser"."A" = "default$default"."Post"."id";
    ```
1. Delete the `_PostToUser` relation table
    ```sql
    DROP TABLE "default$default"."_PostToUser";
...
At this point, you've resolved the schema incompatibilities with the Upgrade CLI. You can now exit the Upgrade CLI by typing <kbd>n</kbd> and hitting <kbd>RETURN</kbd>.
...
> **Note**: There are a few schema incompatibilities that were not yet resolved by the Prisma schema upgrade CLI. Note, at this point you still haven't fixed [scalar lists](./schema-incompatibilities#scalar-lists-arrays-are-maintained-with-extra-table) and [cascading deletes](./schema-incompatibilities#cascading-deletes-are-not-supported-in-prisma-20). You can find the recommended workarounds for these on the [Schema incompatibilities](./schema-incompatibilities) page. 
