...
> **Note**: With Prisma 1, it was usually recommended to install the CLI globally. We now recommend to [install the Prisma CLI locally](../../../cconcepts/tools-and-interfaces/prisma-cli/installation#local-installation-recommended) to prevent version conflicts.
...
For this guide, you'll first create a new Prisma schema using the `prisma init` command and then "fill" it with a data model using [introspection](../../../cconcepts/tools-and-interfaces/introspection).
...
- `schema.prisma`: Your Prisma schema file that specifies the [data source](../../../cconcepts/tools-and-interfaces/prisma-schema/data-sources), [generator](../../../cconcepts/tools-and-interfaces/prisma-schema/generators) and [data model](../../../cconcepts/tools-and-interfaces/prisma-schema/data-model) (note that the data model doesn't exist yet, it will be generated via introspection).
- `.env`: A [dotenv](https://github.com/motdotla/dotenv#readme) file to configure your database [connection URL](../../../cconcepts/database-connectors/connection-urls).
...
With Prisma 2, the HTTP layer isn't exposed any more and Prisma Client 2.0 is configured to run requests "directly" against the database (that is, requests are proxied by Prisma's [query engine](../../../cconcepts/tools-and-interfaces/prisma-client/query-engine), but there isn't an extra server any more).
...
Learn more on the [Connection URLs](../../../cconcepts/database-connectors/connection-urls) page.
...
Note that this data model has three [relations](../../../cconcepts/tools-and-interfaces/prisma-schema/relations):
...
For the above Prisma 1 datamodel, this results in the following Prisma 2 schema (note that the models and fields have been reorded to match the initial order of the Prisma 1 datamodel):
...
### 5.1.6. Breaking changes detected
...
If you did not resolve all schema incompatibilities, the Upgrade CLI now prints the remaining SQL statements. You can just ignore them at this point and continue to the last step by continuously typing <kbd>Y</kbd> and hitting <kbd>RETURN</kbd> when prompted.
...
One thing you'll notice with this version of the Prisma 2 schema is that all [relation fields](../../../cconcepts/tools-and-interfaces/prisma-schema/relations#relation-fields) are named after their respective models, e.g:
...
<!-- To make sure your Prisma Client queries feel natural and idiomatic, it's recommended to make a few adjustments to the Prisma in order to [configure your generated Prisma Client API](../../../cconcepts/tools-and-interfaces/prisma-client/configuring-the-prisma-client-api).

Mostly, this means renaming [relation fields](../../../cconcepts/tools-and-interfaces/prisma-schema/relations#relation-fields). -->
