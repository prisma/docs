...
- When adding the `nexus-plugin-prisma` via Nexus' `use()` function, you're disabling [Prisma Migrate](../../../cconcepts/tools-and-interfaces/prisma-migrate) and enabling the auto-generation of CRUD resolvers via `t.crud`.
...
Note that the `db` object that's attached to the `context` argument represents your [`PrismaClient`](../../../cconcepts/tools-and-interfaces/prisma-client/api) instance that you can use to send queries to your database.
