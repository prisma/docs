...
Also note that the Nexus API for defining Graphql types can seem a bit confusing at first. The above definition of the `Query` type is actually equivalent to writing the following:
...
However, the above version is the common convention for Nexus, so it'll be used throughout this guide.
