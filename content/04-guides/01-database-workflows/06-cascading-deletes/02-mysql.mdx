...
async function main() {
  ...
  console.log(
    `Before deleting the \`User\` record there are ${allUsers1.length} users and ${allPosts1.length} posts.`
  );
  ...
  console.log(
    `After deleting the \`User\` record there are ${allUsers2.length} users and ${allPosts2.length} posts.`
  );
}
