...
- Create a view that returns data from both tables.
...
- You must manually add each view as a model to the Prisma schema. Introspection does not add views to the schema.
...
In this section you will create a view named Drafts that returns the post title and author email of all posts that have not been published. To create a view:
...
## 5. Manually add the Drafts view to the Prisma schema
...
## 9. Validate the Drafts view in a Node.js script
...
1. Create a new file named `index.js` and add the following code to it:
   ...
   async function main() {
     ...
     const filteredDrafts = await prisma.drafts.findMany({
       where: {
         email: 'sarah@prisma.io',
       },
     })
