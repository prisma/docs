...
> **Note**: The quotes are added to the `"firstName"` and `"lastName"` fields to maintain the casing (otherwise SQLite interprets column names as all-lowercase). The `UNIQUE` constraint on `AnotherUser` is needed to be able to reference the columns as foreign keys.
