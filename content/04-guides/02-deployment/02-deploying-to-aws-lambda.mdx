...
- [Relation scalar fields](../..//concepts/tools-and-interfaces/prisma-schema/relations): these are used to store the foreign key, e.g. the `authorId` field in the `Post` model. Cannot be rename as it must match the field in the database.
...
Based on that logic, rename the relation fields to better adhere to the [naming conventions](../../concepts/tools-and-interfaces/prisma-schema/models#naming-fields):
