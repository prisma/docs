...
The focus of this guide is showing how an API based on Prisma can be deployed to AWS Lambda. The starting point will the [Prisma AWS example](https://github.com/prisma/prisma-examples/tree/prisma2/deployment-platforms/aws-lambda) which has a couple of REST endpoints preconfigured as serverless functions. While the example uses REST, the same principles apply to a GraphQL server, with the main difference being that you typically only need a single function to serve a GraphQL API.
...
```sh
mkdir prisma-aws-labmda
cd prisma-aws-labmda
curl https://codeload.github.com/prisma/prisma-examples/tar.gz/prisma2 | tar -xz --strip=3 prisma-examples-prisma2/deployment-platforms/aws-lambda/
...
Congratulations! You have successfully deployed the API to AWS Labmda.
