...
The `package.json` uses the `postinstall` hook script to run `prisma2 generate`.

Typically this would go in the `build` step, however, Zeit Now caches `node_modules` after the dependencies are installed.
