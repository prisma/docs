...
The first step to migrate any queries is to define the `Mutation` type of your GraphQL API. Once that's done, you can gradually add operations to it. Add the following definition to `graphql.ts`:
...
The `createUser` mutation from the sample GraphQL schema is defined and implemented as follows.
...
Similar to forwarding GraphQL queries, you can use the `crud` property on the `t` variable inside the `definition` function in order to mirror the same behaviour with Nexus. Similar to `model`, this property is available because you're using the `nexus-prisma-plugin` which leverages type information from your Prisma models and auto-generates resolvers under the hood. The `crud` property also support autocompletion, so you can explore all available queries in your editor again:
...
The `createDraft` mutation is defined and implemented as follows.
...
The `updateBio` mutation is defined and implemented as follows.
...
The `addPostToCategories` mutation is defined and implemented as follows.
