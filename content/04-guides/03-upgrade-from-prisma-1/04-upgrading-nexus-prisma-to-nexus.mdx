...
- To expose Prisma models via Nexus, you can use the `t.model` propery which is added to the `t` argument that's passed into Nexus' `definition` functions. `.t.model` gives you access to the properties of a Prisma model.
- Exposing CRUD operations for Prisma models via Nexus follows a similar approach. These are exposed via `t.crud` on the `definition` functions of your 
