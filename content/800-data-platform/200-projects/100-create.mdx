...
In the Prisma Data Platform, you can create new projects only with an available database connection string. If your database sits behind a firewall or only allows connections from specific IPs, you can enable the use of [static IPs of the platform](/data-platform/static-ips) and add them to your database allowlist. You can also select the [Data Proxy](/data-platform/data-proxy) region to prepare your project for connection pooling and load balancing in serverless environments.
...
The features in the [Data Browser](/data-platform/data-browser), [Query Console](/data-platform/query-console), and [Schema Viewer](/data-platform/schema-viewer) rely on an up-to-date schema that matches the latest data model in your database.<br /><br />

When you create a project with an existing database, the platform introspects your database and generates a Prisma schema for your project. If you make a change to your database model and the platform does not have a linked schema, you and your team might start seeing errors while you use [Data Browser](/data-platform/data-browser), [Query Console](/data-platform/query-console), and [Schema Viewer](/data-platform/schema-viewer).<br /><br />

If you create a project with an empty database, the [Data Browser](/data-platform/data-browser), [Query Console](/data-platform/query-console), and [Schema Viewer](/data-platform/schema-viewer) remain inactive for the project because the database introspection cannot read your database model and cannot generate a Prisma schema for the project.
...
4. From **Choose a Data Proxy region**, select the geographic location for the Data Proxy.
    ...
    **Note**<br /><br />
    ...
    After you create the project, you can [create a Data Proxy connection string](#optional-create-data-proxy-connection-string) and use that to set up connection pooling locally or in your deployment environment.
5. Click **Create project**.
    <img
        src="https://res.cloudinary.com/prismaio/image/upload/v1669396285/docs/pdp/create-manage-projects/create-project-01-paste-connection-string-select-data-proxy-region-enable-static-ips_p5wauf.png"
...
**If your database does not contain any models (tables or collections)**
...
As a result, the [Data Browser](/data-platform/data-browser) or [Query Console](/data-platform/query-console) are inactive. You can start using them after you [enable Prisma schema synchronization](#optional-enable-schema-synchronization).
<br />

**If your database contains models**

The platform introspects the database to read the data models so that it can generate the Prisma schema. The Prisma schema enables the features in the [Data Browser](/data-platform/data-browser), [Query Console](/data-platform/query-console), and [Schema Viewer](/data-platform/schema-viewer), where you can edit and query your data, or view your schema.


### What's next

* Click **Skip and continue to Data Platform** to go to the **Data Browser** for your project.<br /><br />
If your database is empty, you are redirected to the **Schema Viewer** where you can optionally provide a GitHub repository with your Prisma schema to set up schema synchronization.
* Alternatively, you can remain on the current screen and complete the optional configurations for your project.
    * [(Optional) Enable Prisma schema synchronization](#optional-enable-schema-synchronization)
    * [(Optional) Create a Data Proxy connection string](#optional-create-data-proxy-connection-string)

## (Optional) Enable schema synchronization

After you create the project, you can enable automated schema synchronization. This means that if you push your Prisma schema to a GitHub repository, you can link the Prisma schema and the GitHub repository in your project.

This enables the platform to keep your Prisma schema up-to-date for your project and also keep the data models in Data Browser and Query Console updated with the latest version of your Prisma schema.
...
7. Click **Link Prisma schema**.<br />
    <img
        src="https://res.cloudinary.com/prismaio/image/upload/v1669396711/docs/pdp/create-manage-projects/create-project-02-enable-schema-synchronization-select-github-account-branch-prisma-schema-path_xeeust.png"
...
You can now use [Data Browser](/data-platform/data-browser), [Query Console](/data-platform/query-console), and [Schema Viewer](/data-platform/schema-viewer) with the latest version of your Prisma schema.

## (Optional) Create Data Proxy connection string

After you create the project, you can create and copy a Data Proxy connection string for the project. You can then use the connection string in Prisma Client to enable connection pooling and load balancing of database connections locally or in your deployment environment.
...
4. Copy the `prisma://` connection string.
    ...
    <img
        src="https://res.cloudinary.com/prismaio/image/upload/v1669396884/docs/pdp/create-manage-projects/create-project-03-copy-data-proxy-connection-string_t5mpxt.png"
...
With your project created in the Prisma Data Platform, you can explore your project with the **Data Browser**, **Query Console**, and **Schema Viewer**.
...
Next, you can [develop your Prisma schema and your database models](https://www.prisma.io/docs/concepts/components/prisma-schema/data-model) further and [migrate any schema changes to your database](https://www.prisma.io/docs/concepts/components/prisma-migrate) with [`prisma migrate dev`](/reference/api-reference/command-reference#migrate-dev). 

You can also change your project settings or add new environments, if you want to isolate development activities with test databases, test branches, and select a different Data Proxy region for an environment.
