...
You see a notification that introspection failed and cannot generate a schema. This may be because the database is unreachable or run into a timeout. You will see a warning notification at the top you can close after providing a schema.

As a result, [Data Browser](/data-platform/data-browser) and [Query Console](/data-platform/query-console) are inactive. You can start using them after you provide a schema. You can [provide a schema](#optional-provide-a-schema) by either linking a schema from a repository or upload your schema.
...
1. On the **Or upload your schema** section, either click **Upload file** or Drag and drop your schema file to select files in the _drop zone_
...
A Prisma schema is now linked to your project. The platform updates the Prisma schema in your project every time you push a change to your repository if you link to a repository
