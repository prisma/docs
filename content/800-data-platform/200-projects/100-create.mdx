...
You will be prompted a notification telling that your project was created but introspection failed. This may be because the database is unreachable or run into a timeout. In order to utilize the Data Browser or Query Console, a project must have a valid Prisma schema that matches with the provided database.
...
As a result, [Data Browser](/data-platform/data-browser) and [Query Console](/data-platform/query-console) are disabled. You can start using those after you provide a schema. You can [provide a schema](#optional-provide-a-schema) by either linking a schema from a repository or uploading your schema.
...
The platform updates the Prisma schema in your project every time you push a change to your repository if you link to a repository. You can also manually update the Prisma schema linked to your project by refreshing the schema in the **Schema** tab of your project.
