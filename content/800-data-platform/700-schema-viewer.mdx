...
If the schema refresh fails, you can upload a schema file, an error notification pops up at the top of the page with the option to upload a schema file.
...
If you create your project with an empty database (one which contains no models), or no schema (one which introspection failed, and/ or uploaded a file), or and do not enable schema synchronization, the platform tries to introspect the database but because that contains no models, the platform cannot generate a Prisma schema for it.
...
For such projects, [**Data Browser**](/data-platform/data-browser) and [**Query Console**](/data-platform/query-console) remain inactive until you enable schema synchronization, upload a Prisma schema, or manually trigger a new introspection.
