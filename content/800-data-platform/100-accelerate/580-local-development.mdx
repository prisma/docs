...
Accelerate does not work with a local database. However, in a development environment, you can still use Prisma Client with the Accelerate client extension. This setup will not include Accelerate's connection pooling and caching features.

Here is how to use a local PostgreSQL database with Prisma ORM and Prisma Accelerate client extension:
...
2. Generate a Prisma Client:
   ...
   > Note: The `--no-engine` flag should only be used in preview and production environments. The command generates Prisma Client artifacts without a [Query Engine](/concepts/components/prisma-engines) file, which requires an Accelerate connection string.

3. Set up Prisma Accelerate to your project:
