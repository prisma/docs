...
## Are Accelerate and Prisma Data Platform the same?

Right now, you can use Accelerate in early access from the portal at [https://accelerate.prisma.io](https://accelerate.prisma.io). Prisma Data Platform continues to be available at [https://cloud.prisma.io](https://cloud.prisma.io). We plan to integrate Accelerate into the Prisma Data Platform before general availability.

## I want to use Accelerate. What happens to my Data Proxy connection string?

You can replace your Data Proxy connection string with the Accelerate connection string.

The Accelerate connection string provides both global cache and connection pooling. All Data Proxy features are available with the Accelerate connection string.
...
While Accelerate is in early access, you do not need a subscription or a monthly plan. With Accelerate, you can use the same Data Proxy connection pooling features on top of the Accelerate global cache.

In addition, you can get 6 months of free Accelerate usage when you opt-in for the credits program and enter your payment information (debit or credit card).
...
## VSCode does not recognize the `$extends` method

If you add the Prisma Client extension for Accelerate to an existing project that is currently open in VSCode, the editor might not immediately recognize the `$extends` method.
...
1. In VSCode, open the Command Palette. You can do so when you press F1 or select **View** > **Command Palette**.
2. Enter `typescript` and select and run the **TypeScript: Restart TS server** command.

VSCode should now recognize the `$extends` method.

## What regions are Accelerate available in?

Accelerate runs on Cloudflare’s network. Cache hits are served from Cloudflare’s 280+ locations. Cache misses are currently only served from `us-east-1`. We plan on adding at least 20 regions soon.
...
Accelerate does not currently provide any cache invalidation strategy. This is on our radar, and we would appreciate your feedback to help shape the feature by [scheduling some time with our team](https://cal.com/frederik).

## What is Accelerate’s consistency model?

Accelerate does not have a consistency model. Accelerate implements a [read-through caching strategy](https://www.prisma.io/dataguide/managing-databases/introduction-database-caching#read-through) particularly suitable for read-heavy workloads.

Accelerate is only a data cache. Accelerate is not a distributed system where nodes need to reach a consensus (because data is only stored in the node(s) closest to the user). However, the cached data doesn’t propagate to other nodes, so Accelerate lacks a consistency model.
...
## When should I not use the Accelerate/cache?
