...
### Providing the connection string to Prisma CLI

Currently, the Data Proxy can only be used to _query_ your database with [Prisma Client](/concepts/components/prisma-client).

CLI commands like `migrate`, `db`, `studio` can be only be run on a direct (non-proxied) database connection.
Assuming that your schema uses `env("DATABASE_URL")`, you'll need to override the `DATABASE_URL` environment variable before running a CLI command like `prisma migrate`.

You can declare an environment variable for the direct database connection string, for example `DIRECT_DATABASE_URL`, and use that to override the env 
var defined in your schema.

On your local environment:
1. Set the environment variables in your .env file.
```sh
DATABASE_URL="prisma://connection-string-from-prisma-data-platform"
DIRECT_DATABASE_URL="postgresql://direct-connection-string"

```

2 add a `prisma-cli` script in your package.json
```json
// package.json
{
  ...,
  "scripts": {
    "direct-url": "DATABASE_URL=\"$DIRECT_DATABASE_URL\" prisma"
  }
}
```

You could then call the script with `npm run direct-url prisma migrate dev` and the CLI would use the `DIRECT_DATABASE_URL` environment variable.
