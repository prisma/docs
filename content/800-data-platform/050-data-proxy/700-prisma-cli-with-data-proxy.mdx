...
Assuming that use have `env("DATABASE_URL")` in your `schema.prisma` file, you need to override the `DATABASE_URL` environment variable before you run `npx prisma migrate deploy` or any of the list commands.
