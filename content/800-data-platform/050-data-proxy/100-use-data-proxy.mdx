...
The _default_ value for the `connection_limit` in each Query Engine (QE) instance is _5_. This default value is determined using the calculation (`num_physical_cpus * 2 + 1`), where each QE instance consist of two CPUs.

```
prisma://region.prisma-data.com/?api_key=&connection_limit=10
```

In the example connection string, the connection limit of _10_ is used as an arbitrary value. However, it's important to understand that this connection limit applies to each individual Query Engine (QE) instance.
...
For example, if the `connection_limit` is set to _10_ and _5_ QE instances are
...
has sufficient capacity to handle this load of _50_ connections.
