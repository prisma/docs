...
> Note: The connection limit of **60** in the example connection string is used as an arbitray value. When there is a surge in traffic, multiple query engine instances are initiated.
>
> It's important to note that the connection limit specified in your connection string is applicable to each individual Query Engine instance. It is advisable to set a suitable value for the connection limit to prevent your database's connections from being exhausted.
>
> _For more detailed information regarding the limits of the Data Proxy and its concurrency limits, we recommend referring to the [Concurrency Limits documentation](https://www.prisma.io/docs/data-platform/about/limits-and-status#concurrency-limits-of-the-data-proxy)._
