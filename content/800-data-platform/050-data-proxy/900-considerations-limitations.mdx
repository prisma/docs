...
Data Proxy keeps several connection pools running to maintain open database connections even when no queries run from your application. The connection pools and any remaining idle connections close after six minutes of inactivity (or when no incoming queries are sent to the database). The number of idle connections to the database depends on the number of running connection pools at any point in time. 
