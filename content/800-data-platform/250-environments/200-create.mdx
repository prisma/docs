...
The table below shows the different project configurations and provides links to the correct steps to create a new environment for each configuration.

| Configuration | Steps |
| --- | --- |
| Projects with enabled schema synchronization | [Steps](#project-with-a-linked-prisma-schema) |
| Projects with introspected schema or no schema (empty database) | [Steps](#projects-with-introspected-schema-or-with-no-schema) |
...
- You are the [Project owner](/data-platform/members/user-roles) or have the [Admin](/data-platform/members/user-roles) role for the project.
...
- Create a new branch in the GitHub repository of the project and track changes to the Prisma schema for the environment in the created branch.
...
8. Click **Create environment**.
   <img
     src="https://res.cloudinary.com/prismaio/image/upload/v1670063711/docs/pdp/environments/create-environment-Case-linked-schema-01-name-connection-string-static-ips-data-proxy-region_kn04mu.png"
...
The platform links the Prisma schema from the selected branch so that it can keep your project up-to-date with the latest schema version. The updates of the Prisma schema keep the models and attributes up-to-date in **Data Browser**, **Query Console**, and also the schema contents in the **Schema Viewer**.

Your browser is redirected to the **Data Browser** for the new environment.
...
- After you switch to a different environment, the **Data Browser** and **Query Console** use the database and Data Proxy location configured for that environment.
...
## Projects with introspected schema or with no schema
...
You see a notification that the database for your environment is empty. Because the database does not contain any models, the introspection cannot generate a Prisma schema.

As a result, the **Data Browser** or **Query Console** for the environment are inactive. You can start using them after you enable Prisma schema synchronization for the environment.

Your browser is redirected to the **Schema Viewer** for the new environment.

**If your database contains models**

The platform introspects the database to read the data models so that it can generate the Prisma schema. The Prisma schema enables the features in the **Data Browser**, **Query Console**, and **Schema Viewer**, where you can edit and query your data, or view your schema.
