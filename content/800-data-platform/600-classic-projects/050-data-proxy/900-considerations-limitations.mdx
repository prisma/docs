...
metaDescription: 'When you use Data Proxy for applications in serverless and edge runtimes, bear in mind a few considerations and learn about the current limitations.'
tocDepth: 4
---
...
To use Data Proxy, you must first remove `metrics` from the `previewFeatures` section of your `schema.prisma` file.
...
With Data Proxy, the `PrismaClient` method `$connect` does not create a new connection pool and the `$disconnect` method does not close the connection pool.

Both methods are valid syntax in Prisma Client with Data Proxy, but they do not have any impact on the connections to the database. This is because Data Proxy manages the connections.
