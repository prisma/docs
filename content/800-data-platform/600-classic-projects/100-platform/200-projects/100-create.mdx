...
The features in [Data Browser](../data-browser) and [Query Console](../query-console) rely on an up-to-date schema that matches the latest data model in your database.<br /><br />

When you _create a project with an existing database_, the platform introspects your database and generates a Prisma schema for your project. If you make a change to your database model and the platform does not have a linked schema, you and your team might start seeing errors while you use [Data Browser](../data-browser) and [Query Console](../query-console).<br /><br />

If you _create a project with an empty database_, [Data Browser](../data-browser) and [Query Console](../query-console) remain inactive for the project because the database introspection cannot generate a Prisma schema from an empty database.

</details>

Later, if you want to enable connection pooling and load balancing of database connections with Data Proxy, you can copy the Data Proxy connection string for the project.
...
4. From **Choose a Data Proxy region**, select the geographic location for Data Proxy.

   <Admonition type="warning">

   **Important**<br /><br />
   Select a Data Proxy location that is close to the geographic location of your database.<br /><br />
   If you select a location that is remote from your database, then when your application sends requests to your database through Data Proxy, you might notice some delay in the responses.
...
The platform creates your project and assigns it a random name, such as **Grey&nbsp;fish&nbsp;<random-code\>**, where **<random-code\>** is an auto-generated code. You can later [change the project name](../projects/edit-settings) from the project settings.
...
As a result, [Data Browser](../data-browser) and [Query Console](../query-console) are inactive. You can start using them after you [enable Prisma schema synchronization](#optional-enable-schema-synchronization).

#### If your database contains models

The platform introspects the database to read the data models so that it can generate the Prisma schema. The Prisma schema enables the features in [Data Browser](../data-browser) and [Query Console](../query-console), where you can edit and query your data.
...
You can now use [Data Browser](../data-browser) and [Query Console](../query-console) with the latest version of your Prisma schema.
...
You can now [enable connection pooling with Data Proxy](/data-platform/classic-projects/data-proxy/use-data-proxy#enable-the-data-proxy-for-a-project) locally or in your deployment environment.
...
- [Manage data with Data Browser](../data-browser)
- [Test and run queries with Query Console](../query-console)
- [Manage schema updates with Schema Viewer](../schema-viewer)
