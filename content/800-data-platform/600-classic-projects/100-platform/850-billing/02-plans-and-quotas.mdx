...
metaDescription: 'The Prisma Data Platform provides a set of project-based plans (billed monthly) with quotas for the number of collaborators and environments per project, as well as a quota for the usage of Data Proxy (in CPU milliseconds) per project.'
---
...
All projects created or added to the Prisma Data Platform start on the [**Free**](#strongfreestrong-plan) plan.
...
### Quotas for Data Proxy

- Each plan includes specific Data Proxy CPU usage quotas which reset at the beginning of every month.
- The **Free** plan has a hard limit of monthly Data Proxy CPU usage.
- If a project reaches the quota limit on the **Free** plan, we will pause your project until the beginning of next month, when Data Proxy CPU usage quota resets.
...
- On the **Scale** and **Enterprise** plans, Data Proxy usage quota is unlimited (no hard limits). Both plans include an initial quota of Data Proxy usage and additional usage exceeding the quota is billed on a monthly basis.
