...
badge: preview
url: /docs/v6/postgres/database/local-development
...
If you must use a different port, append [`--port <number>`](/docs/v6/orm/reference/prisma-cli-reference#dev) (for example, `npx prisma migrate dev --port 5422`) and update your `DATABASE_URL` (or other connection settings) to match.
...
Local Prisma Postgres supports [direct TCP connections](/docs/v6/postgres/database/direct-connections), allowing you to connect to it via any tool.
...
[Prisma Postgres caching](/docs/v6/postgres/database/caching) is simulated locally. Queries always directly interact with the local Prisma Postgres instance, bypassing cache configurations:
