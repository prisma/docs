...
The Prisma Postgres serverless driver is currently in [Early Access](/docs/v6/orm/more/releases#early-access) and not yet recommended for production scenarios.
...
- Automatic connection pooling across [all available Prisma Postgres regions](/docs/v6/postgres/faq#what-regions-is-prisma-postgres-available-in) for optimal performance
...
For standard Node.js environments, use the [`node-postgres` driver](/docs/v6/orm/overview/databases/postgresql#using-the-node-postgres-driver) for lower latency with direct TCP connections.
...
If you don't have a Prisma Postgres database, create one using the [`create-db` CLI](/docs/v6/postgres/introduction/npx-create-db) tool:
...
The serverless driver automatically uses connection pooling [across all available Prisma Postgres regions](/docs/v6/postgres/faq#what-regions-is-prisma-postgres-available-in) for optimal performance and resource utilization.
...
- Requires a Prisma Postgres instance and does not work with [local development](/docs/v6/postgres/database/local-development) databases
...
- [Prisma Postgres documentation](/docs/v6/postgres/introduction)
