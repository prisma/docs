...
url: /docs/v6/postgres/query-optimization/recommendations/queries-on-unindexed-columns
...
The following queries targeting the `User` model use a [`where` property](/docs/v6/orm/prisma-client/queries/filtering-and-sorting) to filter on columns that do not have indexes:
...
- **Frequent queries with filtering or sorting:** Use indexes on columns that are frequently used for [filtering or sorting](/docs/v6/orm/prisma-client/queries/filtering-and-sorting#filtering).
- **Looking up related data:** Use indexes on foreign key columns to speed up the retrieval of related records, such as when using [`include`](/docs/v6/orm/prisma-client/queries/relation-queries#include-a-relation).
