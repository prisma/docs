...
url: /docs/v6/optimize/getting-started
...
- A project using [Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/introduction) version `5.0.0` or higher (we recommend using the latest version).
- A PostgreSQL, MySQL/MariaDB, CockroachDB, or MS SQL Server database.

:::note

Prisma Optimize is intended for use in local environments. Learn more in the [FAQ](/docs/v6/postgres/faq#can-i-enable-query-optimizations-for-prisma-postgres-in-production).
...
2. <a href="/docs/platform/about#accessing-the-optimize-dashboard" target="_blank">
...
<a href="/docs/platform/about#generating-an-optimize-api-key" target="_blank">
...
If you are using [Prisma Accelerate](/docs/v6/accelerate) in your application, make sure you apply it _after_ the Optimize extension. For example:
...
3. Explore [individual query details](/docs/v6/postgres/query-optimization/recordings#data-captured-in-a-recording-session) by clicking on them, and check the **Recommendations** tab for any suggested improvements to enhance query performance.

   :::info
   Use [Prisma AI](/docs/v6/postgres/query-optimization/prisma-ai) to understand recommendations and apply them within your Prisma model context.
   :::
