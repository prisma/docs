...
url: /docs/v6/orm/reference/prisma-schema-reference
...
| `engineType`      | No       | Enum (`library` or `binary`) | Defines the [query engine](/docs/v6/orm/more/internals/engines) type to download and use. **Default**: `library`                                                                                              |
| `binaryTargets`   | No       | List of Enums (see below)    | Specify the OS on which the Prisma Client will run to ensure compatibility of the [query engine](/docs/v6/orm/more/internals/engines). **Default**: `native`                                                  |
...
| Prisma Client JS ([before v6](/docs/v6/orm/more/upgrades/to-v7)) | [`Buffer`](https://nodejs.org/api/buffer.html)                                                              | See [examples of working with `Bytes`](/docs/v6/orm/prisma-client/special-fields-and-types#working-with-bytes) |
...
There is currently a known bug in Prisma ORM v7 where using mapped enum values with Prisma Client operations causes runtime errors. See the [Prisma 7 upgrade guide](/docs/v6/orm/more/upgrades/to-v7#mapped-enum-values-in-generated-typescript) for workarounds and details.
...
- Implemented by Prisma ORM and therefore not "visible" in the underlying database schema. You can still use `cuid()` when using [introspection](/docs/v6/orm/prisma-schema/introspection) by [manually changing your Prisma schema](/docs/v6/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma's [query engine](/docs/v6/orm/more/internals/engines).
...
- Compatible with `String`.
- Implemented by Prisma ORM and therefore not "visible" in the underlying database schema. You can still use `uuid()` when using [introspection](/docs/v6/orm/prisma-schema/introspection) by [manually changing your Prisma schema](/docs/v6/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma ORM's [query engine](/docs/v6/orm/more/internals/engines).
