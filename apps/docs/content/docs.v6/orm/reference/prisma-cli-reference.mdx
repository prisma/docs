...
url: /docs/v6/orm/reference/prisma-cli-reference
...
By default, the project sets up a [local Prisma Postgres](/docs/v6/postgres/database/local-development) instance but you can choose a different database using the `--datasource-provider` option.
...
| `--db`                   | No       | Shorthand syntax for `--datasource-provider prisma+postgres`; creates a new [Prisma Postgres](/docs/v6/postgres) instance. Requires authentication in the [PDP Console](https://console.prisma.io).     |                       |
...
| `--preview-feature`      | No       | Define the [Preview features](/docs/v6/orm/reference/preview-features) to use. To define multiple Preview features, you have to provide the flag multiple times for each Preview feature. See examples. |                       |
| `--output`               | No       | Specifies the [output location for the generated client](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path).                                       | `../generated/prisma` |
...
See the [Prisma Config reference](/docs/v6/orm/reference/prisma-config-reference) for more details.
...
The `generate` command generates assets like Prisma Client based on the [`generator`](/docs/v6/orm/prisma-schema/overview/generators) and [`data model`](/docs/v6/orm/prisma-schema/data-model/models) blocks defined in your `prisma/schema.prisma` file.
...
| ------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `--data-proxy`      | No       | The `generate` command will generate Prisma Client for use with [Prisma Accelerate](/docs/v6/accelerate) prior to Prisma 5.0.0. Mutually exclusive with `--accelerate` and `--no-engine`.                                                   |
| `--accelerate`      | No       | The `generate` command will generate Prisma Client for use with [Prisma Accelerate](/docs/v6/accelerate). Mutually exclusive with `--data-proxy` and `--no-engine`. Available in Prisma 5.1.0 and later.                                    |
| `--no-engine`       | No       | The `generate` command will generate Prisma Client without an accompanied engine for use with [Prisma Accelerate](/docs/v6/accelerate). Mutually exclusive with `--data-proxy` and `--accelerate`. Available in Prisma ORM 5.2.0 and later. |
...
The `prisma-client` generator creates a customized client for working with your database in a custom output directory specified by the `output` field - you can [customize the output folder](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path).

### `validate`

Validates the [Prisma Schema Language](/docs/v6/orm/prisma-schema/overview) of the Prisma schema file.
...
The `dev` command starts a [local Prisma Postgres](/docs/v6/postgres/database/local-development) database that you can run Prisma ORM commands against. It is useful for development and testing purposes and also allows you to switch to [Prisma Postgres](/docs/v6/postgres) in production easily.

### Arguments

| Argument              | Required | Description                                                                                                                                                 | Default |
| --------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `--name` (or `-n`)    | No       | Enables targeting a specific database instance. [Learn more](/docs/v6/postgres/database/local-development#using-different-local-prisma-postgres-instances). |         |
...
Starts existing [local Prisma Postgres](/docs/v6/postgres/database/local-development) instances in the background.
...
Lists all available [local Prisma Postgres](/docs/v6/postgres/database/local-development) instances:
...
Stops one or more [local Prisma Postgres](/docs/v6/postgres/database/local-development) databases:
...
Removes the data of one or more [local Prisma Postgres](/docs/v6/postgres/database/local-development) databases from your file system:
...
Introspection with the `db pull` command on the [MongoDB connector](/docs/v6/orm/overview/databases/mongodb) samples the data instead of reading a schema.
...
- [Conceptual overview of `db push` and when to use it over Prisma Migrate](/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema)
- [Schema prototyping with `db push`](/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema)
...
See [Seeding your database](/docs/v6/orm/prisma-migrate/workflows/seeding)
...
This command is currently not supported on [MongoDB](/docs/v6/orm/overview/databases/mongodb).
...
- [Migration troubleshooting in production](/docs/v6/orm/prisma-migrate/workflows/patching-and-hotfixing#fixing-failed-migrations-with-migrate-diff-and-db-execute)
...
Instead of `migrate dev` and related commands, [`db push`](/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/docs/v6/orm/overview/databases/mongodb).
...
1. Reruns the existing migration history in the [shadow database](/docs/v6/orm/prisma-migrate/understanding-prisma-migrate/shadow-database) in order to detect schema drift (edited or deleted migration file, or a manual changes to the database schema)
...
This command is not supported on [MongoDB](/docs/v6/orm/overview/databases/mongodb). Use [`db push`](/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema) instead.
...
- [Conceptual overview of Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started)
- [Developing with Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started)
...
If a [schema drift](/docs/v6/orm/prisma-migrate/understanding-prisma-migrate/shadow-database#detecting-schema-drift) is detected while running `prisma migrate dev` using `--create-only`, you will be prompted to reset your database.
...
This command is only partially supported for [MongoDB](/docs/v6/orm/overview/databases/mongodb). See the command options below for details.
...
The `migrate diff` command can only compare database features that are [supported by Prisma](/docs/v6/orm/reference/database-features). If two databases differ only in unsupported features, such as views or triggers, then `migrate diff` will not show any difference between them.
...
### `platform` ([Early Access](/docs/v6/platform/maturity-levels#early-access))
...
You can find the complete list of available commands with the arguments [here](/docs/v6/platform/platform-cli/commands).

### `mcp`

Starts the [Prisma MCP server](/docs/v6/postgres/integrations/mcp-server).
...
For detailed database support information, including SQLite requirements, see [Databases supported by Prisma Studio](/docs/v6/orm/tools/prisma-studio#databases-supported-by-prisma-studio).
...
- [`HTTP_PROXY`](/docs/v6/orm/reference/environment-variables-reference#http_proxy) or `http_proxy`: Proxy URL for http traffic, for example `http://localhost:8080`
- [`HTTPS_PROXY`](/docs/v6/orm/reference/environment-variables-reference#https_proxy) or `https_proxy`: Proxy URL for https traffic, for example `https://localhost:8080`

## `npx create-db`

The [`create-db`](https://create-db.prisma.io/) command provisions a temporary [Prisma Postgres](/docs/v6/postgres) database with a single command. This is a standalone utility that can be invoked using `npx`. It's ideal for quickly testing, prototyping, or integrating with Prisma Postgres.
...
For full usage details, options (such as `--region` and `--interactive`), and examples, see the [documentation](/docs/v6/postgres/introduction/npx-create-db).
