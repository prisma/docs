...
url: /docs/v6/orm/overview/introduction/should-you-use-prisma
...
Refer to the full list of databases (relational, NoSQL, and NewSQL) that Prisma ORM [supports](/docs/v6/orm/reference/supported-databases).
...
The declarative [Prisma schema](/docs/v6/orm/prisma-schema/overview) provides an overview of the current state of the database that's easy to understand for everyone. This is a major improvement to traditional workflows where developers have to dig through migration files to understand the current table structure.

[Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/introduction)'s minimal API surface enables developers to pick it up quickly without much learning overhead, so onboarding new developers to a team becomes a lot smoother.

The [Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started) workflows are designed in a way to cover database schema changes in collaborative environments. From the initial schema creation up to the point of deploying schema changes to production and resolving conflicts that were introduced by parallel modifications, Prisma Migrate has you covered.
...
- querying (with [Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/introduction))
- data modeling (in the [Prisma schema](/docs/v6/orm/prisma-schema/overview))
- migrations (with [Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started))
- prototyping (via [`prisma db push`](/docs/v6/orm/reference/prisma-cli-reference#db-push))
- seeding (via [`prisma db seed`](/docs/v6/orm/reference/prisma-cli-reference#db-seed))
- visual viewing and editing (with [Prisma Studio](https://www.prisma.io/studio))

### ... you value type-safety

Prisma ORM is the only _fully_ type-safe ORM in the TypeScript ecosystem. The generated Prisma Client ensures typed query results even for partial queries and relations. You can learn more about this in the [type-safety comparison with TypeORM](/docs/v6/orm/more/comparisons/prisma-and-typeorm#type-safety).
...
Prisma ORM is an abstraction. As such, an inherent tradeoff of Prisma ORM is a reduced amount of control in exchange for higher productivity. This means, the [Prisma Client API](/docs/v6/orm/prisma-client/setup-and-configuration/introduction) might have less capabilities in some scenarios than you get with plain SQL.
