...
To connect to a PostgreSQL database server, you need to configure a [`datasource`](/docs/v6/orm/prisma-schema/overview/data-sources) block in your [Prisma schema](/docs/v6/orm/prisma-schema):
...
- When [creating a migration](/docs/v6/orm/prisma-migrate) or [prototyping your schema](/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema) the table is also used - in the other direction.
...
By caching prepared statements, Prisma Client's [query engine](/docs/v6/orm/more/under-the-hood/engines) does not repeatedly compile the same query which reduces database CPU usage and query latency.
