...
description: "Learn how to manage the environment variables and settings in your Prisma ORM project"
---

An environment variable is a key value pair of string data that is stored on your machine's local environment. Refer to our [Environment variables reference documentation](/docs/v6/orm/reference/environment-variables-reference) for specific details.
...
With Prisma ORM v6.4.0, we released the `prisma.config.ts` file. This file allows you to manage your environment variables and settings in a more flexible way. [View our reference](/docs/v6/orm/reference/prisma-config-reference) for more information.
...
<Tabs items={["Command Prompt", "Powershell"]}>
...
When running tests, we advise you to [mock Prisma Client](/docs/v6/orm/prisma-client/testing/unit-testing#mocking-prisma-client). In doing so, you need to tell Jest which environment it should use when running its tests.
