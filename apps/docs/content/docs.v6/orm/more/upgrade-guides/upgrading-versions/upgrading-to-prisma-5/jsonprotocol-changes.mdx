...
[When filtering on JSON fields in a PostgreSQL model](/docs/v6/orm/prisma-client/special-fields-and-types/working-with-json-fields#filter-on-a-json-field-simple) the `path` argument now only accepts an array.
...
[Scalar list](/docs/v6/orm/prisma-schema/data-model/models#scalar-fields) values must be arrays in all operations.
...
Operations on lists of [Composite types](/docs/v6/orm/prisma-schema/data-model/models#defining-composite-types) (for [MongoDB](/docs/v6/orm/overview/databases/mongodb)) now only accept array values.
