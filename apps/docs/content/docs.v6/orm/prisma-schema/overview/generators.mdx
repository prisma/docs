...
url: /docs/v6/orm/prisma-schema/overview/generators
...
A Prisma schema can have one or more generators, represented by the [`generator`](/docs/v6/orm/reference/prisma-schema-reference#generator) block:
...
Update your [`generator`](/docs/v6/orm/prisma-schema/overview/generators) block:
...
The new generator includes both the TypeScript client code _and_ the [query engine](/docs/v6/orm/more/internals/engines#the-query-engine-file). Including the query engine in version control can cause compatibility issues on different machines. To avoid this, add the generated directory to `.gitignore`:
...
- `previewFeatures`: [Preview features](/docs/v6/orm/reference/preview-features) to include
...
As of [v6.16.0](https://pris.ly/release/6.16.0), Prisma ORM can be used without Rust engines in production applications. Learn more [here](/docs/v6/orm/prisma-client/setup-and-configuration/no-rust-engine).
...
Note that [driver adapters](/docs/v6/orm/overview/databases/database-drivers#driver-adapters) are required if you want to use Prisma ORM without Rust engines.
...
The correct file is particularly important when [deploying](/docs/v6/orm/prisma-client/deployment/deploy-prisma) your application to production, which often differs from your local development environment.
...
In that case, Prisma Client detects your operating system and finds the right binary file for it based on the [list of supported operating systems](/docs/v6/orm/reference/prisma-schema-reference#binarytargets-options) .
...
> **Note**: The `native` binary target is the default. You can set it explicitly if you wish to include additional [binary targets](/docs/v6/orm/reference/prisma-schema-reference#binarytargets-options) for deployment to different environments.

## Community generators

:::note

Existing generators or new ones should not be affected if you are using a [multi-file Prisma schema](/docs/v6/orm/prisma-schema/overview/location#multi-file-prisma-schema), unless a generator reads the schema manually.
