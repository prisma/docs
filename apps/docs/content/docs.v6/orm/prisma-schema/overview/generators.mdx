...
The new generator includes both the TypeScript client code _and_ the [query engine](/docs/v6/orm/more/under-the-hood/engines#the-query-engine-file). Including the query engine in version control can cause compatibility issues on different machines. To avoid this, add the generated directory to `.gitignore`:
