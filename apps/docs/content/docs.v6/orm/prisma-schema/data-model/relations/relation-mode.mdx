...
url: /docs/v6/orm/prisma-schema/data-model/relations/relation-mode
...
In Prisma schema, relations between records are defined with the [`@relation`](/docs/v6/orm/reference/prisma-schema-reference#relation) attribute. For example, in the following schema there is a one-to-many relation between the `User` and `Post` models:
...
- set [referential actions](/docs/v6/orm/prisma-schema/data-model/relations/referential-actions) that define how changes to records are handled
...
For more information, see the [referential actions](/docs/v6/orm/prisma-schema/data-model/relations/referential-actions) page.
...
The `prisma` relation mode does not use foreign keys, so no indexes are created when you use Prisma Migrate or `db push` to apply changes to your database. You instead need to manually add an index on your relation scalar fields with the [`@@index`](/docs/v6/orm/reference/prisma-schema-reference#index) attribute (or the [`@unique`](/docs/v6/orm/reference/prisma-schema-reference#unique), [`@@unique`](/docs/v6/orm/reference/prisma-schema-reference#unique-1) or [`@@id`](/docs/v6/orm/reference/prisma-schema-reference#id-1) attributes, if applicable).
...
If you use the [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma) (or our [language server in another editor](/docs/v6/orm/more/dev-environment/editor-setup)), the warning is augmented with a Quick Fix that adds the required index for you:

![The Quick Fix pop-up for adding an index on a relation scalar field in VS Code](/docs/v6/orm/prisma-schema/data-model/relations/quick-fix-index.png)
