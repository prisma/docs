...
The data model definition part of the [Prisma schema](/docs/v6/orm/prisma-schema) defines your application models (also called **Prisma models**). Models:
...
- Form the foundation of the **queries** available in the generated [Prisma Client API](/docs/v6/orm/prisma-client)
...
<Tabs items={["Relational databases", "MongoDB"]}>
<Tab value="Relational databases">
...
<Tabs items={["Query Example", "Copy-Paste Example"]}>
...
- **Write the data model manually and use Prisma Migrate**: You can write your data model manually and map it to your database using [Prisma Migrate](/docs/v6/orm/prisma-migrate). In this case, the data model is the single source of truth for the models of your application.
...
- Control **the exact native type** that [Prisma Migrate](/docs/v6/orm/prisma-migrate) creates in the database - for example, a `String` can be `@db.VarChar(200)` or `@db.Char(50)`
...
- Use a Prisma ORM-level function. For example, `cuid()` and `uuid()` are provided by Prisma Client's [query engine](/docs/v6/orm/more/under-the-hood/engines) for all connectors.
...
[`cuid()`](/docs/v6/orm/reference/prisma-schema-reference#cuid) and [`uuid()`](/docs/v6/orm/reference/prisma-schema-reference#uuid) are implemented by Prisma ORM and therefore are not "visible" in the underlying database schema. You can still use them when using [introspection](/docs/v6/orm/prisma-schema/introspection) by [manually changing your Prisma schema](/docs/v6/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma Client's [query engine](/docs/v6/orm/more/under-the-hood/engines)
...
Every model in the data model definition will result in a number of CRUD queries in the generated [Prisma Client API](/docs/v6/orm/prisma-client):
