...
url: /docs/v6/orm/prisma-schema/data-model/models
metaTitle: Models
...
The data model definition part of the [Prisma schema](/docs/v6/orm/prisma-schema/overview) defines your application models (also called **Prisma models**). Models:
...
- Form the foundation of the **queries** available in the generated [Prisma Client API](/docs/v6/orm/prisma-client/setup-and-configuration/introduction)
- When used with TypeScript, Prisma Client provides generated **type definitions** for your models and any [variations](/docs/v6/orm/prisma-client/type-safety/operating-against-partial-structures-of-model-types) of them to make database access entirely type safe.
...
- [Models](#defining-models) ([`model`](/docs/v6/orm/reference/prisma-schema-reference#model) primitives) that define a number of fields, including [relations between models](#relation-fields)
- [Enums](#defining-enums) ([`enum`](/docs/v6/orm/reference/prisma-schema-reference#enum) primitives) (if your connector supports Enums)
...
- **Write the data model manually and use Prisma Migrate**: You can write your data model manually and map it to your database using [Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started). In this case, the data model is the single source of truth for the models of your application.
- **Generate the data model via introspection**: When you have an existing database or prefer migrating your database schema with SQL, you generate the data model by [introspecting](/docs/v6/orm/prisma-schema/introspection) your database. In this case, the database schema is the single source of truth for the models of your application.

## Defining models

Models represent the entities of your application domain. Models are represented by [`model`](/docs/v6/orm/reference/prisma-schema-reference#model) blocks and define a number of [fields](/docs/v6/orm/reference/prisma-schema-reference#model-fields). In the example data model above, `User`, `Profile`, `Post` and `Category` are models.
...
However, you can still adhere to the naming convention without renaming the underlying `comments` table in the database by using the [`@@map`](/docs/v6/orm/reference/prisma-schema-reference#map-1) attribute:
...
> **Note**: You can also [`@map`](/docs/v6/orm/reference/prisma-schema-reference#map) a column name or enum value, and `@@map` an enum name.

`@map` and `@@map` allow you to [tune the shape of your Prisma Client API](/docs/v6/orm/prisma-client/setup-and-configuration/custom-model-and-field-names#using-map-and-map-to-rename-fields-and-models-in-the-prisma-client-api) by decoupling model and field names from table and column names in the underlying database.
...
- A **[field name](/docs/v6/orm/reference/prisma-schema-reference#model-fields)**
- A **[field type](/docs/v6/orm/reference/prisma-schema-reference#model-fields)**
...
- [Scalar types](#scalar-fields) (includes [enums](#defining-enums)) that map to columns (relational databases) or document fields (MongoDB) in the database - for example, [`String`](/docs/v6/orm/reference/prisma-schema-reference#string) or [`Int`](/docs/v6/orm/reference/prisma-schema-reference#int)
- Model types (the field is then called [relation field](/docs/v6/orm/prisma-schema/data-model/relations#relation-fields)) - for example `Post` or `Comment[]`.
...
See [complete list of scalar field types](/docs/v6/orm/reference/prisma-schema-reference#model-field-scalar-types) .
...
Refer to the [relations documentation](/docs/v6/orm/prisma-schema/data-model/relations) for more examples and information about relationships between models.
...
Furthermore, during [Introspection](/docs/v6/orm/prisma-schema/introspection) type attributes are _only_ added to the schema if the underlying native type is **not the default type**. For example, if you are using the PostgreSQL provider, `String` fields where the underlying native type is `text` will not have a type attribute.

See [complete list of native database type attributes per scalar type and provider](/docs/v6/orm/reference/prisma-schema-reference#model-field-scalar-types) .

#### Benefits and workflows

- Control **the exact native type** that [Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started) creates in the database - for example, a `String` can be `@db.VarChar(200)` or `@db.Char(50)`
...
- [`?`](/docs/v6/orm/reference/prisma-schema-reference#-modifier-1) Make a field optional
...
When you introspect a relational database, unsupported data types are added as [`Unsupported`](/docs/v6/orm/reference/prisma-schema-reference#unsupported) :
...
Fields of type `Unsupported` do not appear in the generated Prisma Client API, but you can still use Prisma ORM’s [raw database access feature](/docs/v6/orm/prisma-client/using-raw-sql/raw-queries) to query these fields.
...
Attributes modify the behavior of fields or model blocks. The following example includes three field attributes ([`@id`](/docs/v6/orm/reference/prisma-schema-reference#id) , [`@default`](/docs/v6/orm/reference/prisma-schema-reference#default) , and [`@unique`](/docs/v6/orm/reference/prisma-schema-reference#unique) ) and one block attribute ([`@@unique`](/docs/v6/orm/reference/prisma-schema-reference#unique-1)):
...
Some attributes accept [arguments](/docs/v6/orm/reference/prisma-schema-reference#attribute-argument-types) - for example, `@default` accepts `true` or `false`:

```prisma
isAdmin   Boolean @default(false) // short form of @default(value: false)
```

See [complete list of field and block attributes](/docs/v6/orm/reference/prisma-schema-reference#attributes)
...
In relational databases, an ID can be defined by a single field using the [`@id`](/docs/v6/orm/reference/prisma-schema-reference#id) attribute, or multiple fields using the [`@@id`](/docs/v6/orm/reference/prisma-schema-reference#id-1) attribute.
...
You can also provide your own name for the composite ID using the [`@@id`](/docs/v6/orm/reference/prisma-schema-reference#id-1) attribute's `name` field:
...
Refer to the documentation on [working with composite IDs](/docs/v6/orm/prisma-client/special-fields-and-types/working-with-composite-ids-and-constraints) to learn how to interact with a composite ID in Prisma Client.
...
You can optionally define a [custom primary key constraint name](/docs/v6/orm/prisma-schema/data-model/database-mapping#constraint-and-index-names) in the underlying database.

:::

#### Defining IDs in MongoDB

The MongoDB connector has [specific rules for defining an ID field](/docs/v6/orm/reference/prisma-schema-reference#mongodb) that differs from relational databases. An ID must be defined by a single field using the [`@id`](/docs/v6/orm/reference/prisma-schema-reference#id) attribute and must include `@map("_id")`.
...
You can define default values for scalar fields of your models using the [`@default`](/docs/v6/orm/reference/prisma-schema-reference#default) attribute:
...
- Use a Prisma ORM-level function. For example, `cuid()` and `uuid()` are provided by Prisma Client's [query engine](/docs/v6/orm/more/internals/engines) for all connectors.

Default values can be:

- Static values that correspond to the field type, such as `5` (`Int`), `Hello` (`String`), or `false` (`Boolean`)
- [Lists](/docs/v6/orm/reference/prisma-schema-reference#-modifier) of static values, such as `[5, 6, 8]` (`Int[]`) or `["Hello", "Goodbye"]` (`String`[]). These are available in Prisma ORM versions `4.0.0` and later, when using supported databases (PostgreSQL, CockroachDB and MongoDB)
- [Functions](#using-functions), such as [`now()`](/docs/v6/orm/reference/prisma-schema-reference#now) or [`uuid()`](/docs/v6/orm/reference/prisma-schema-reference#uuid)
...
Refer to the [attribute function reference documentation](/docs/v6/orm/reference/prisma-schema-reference#attribute-functions) for information about connector support for functions.

:::

### Defining a unique field

You can add unique attributes to your models to be able to uniquely identify individual records of that model. Unique attributes can be defined on a single field using [`@unique`](/docs/v6/orm/reference/prisma-schema-reference#unique) attribute, or on multiple fields (also called composite or compound unique constraints) using the [`@@unique`](/docs/v6/orm/reference/prisma-schema-reference#unique-1) attribute.
...
**Constraint names in relational databases** <br />
You can optionally define a [custom unique constraint name](/docs/v6/orm/prisma-schema/data-model/database-mapping#constraint-and-index-names) in the underlying database.

:::

By default, the name of this field in Prisma Client queries will be `authorId_title`.

You can also provide your own name for the composite unique constraint using the [`@@unique`](/docs/v6/orm/prisma-schema/data-model/database-mapping#constraint-and-index-names) attribute's `name` field:
...
Refer to the documentation on [working with composite unique identifiers](/docs/v6/orm/prisma-client/special-fields-and-types/working-with-composite-ids-and-constraints) to learn how to interact with a composite unique constraints in Prisma Client.
...
You can define indexes on one or multiple fields of your models via the [`@@index`](/docs/v6/orm/reference/prisma-schema-reference#index) on a model. The following example defines a multi-column index based on the `title` and `content` field:
...
You can optionally define a [custom index name](/docs/v6/orm/prisma-schema/data-model/database-mapping#constraint-and-index-names) in the underlying database.
...
You can define enums in your data model [if enums are supported for your database connector](/docs/v6/orm/reference/database-features#misc), either natively or at Prisma ORM level.

Enums are considered [scalar](#scalar-fields) types in the Prisma schema data model. They're therefore [by default](/docs/v6/orm/prisma-client/queries/select-fields#return-the-default-fields) included as return values in [Prisma Client queries](/docs/v6/orm/prisma-client/queries/crud).

Enums are defined via the [`enum`](/docs/v6/orm/reference/prisma-schema-reference#enum) block. For example, a `User` has a `Role`:
...
Composite types only support a limited set of [attributes](/docs/v6/orm/reference/prisma-schema-reference#attributes). The following attributes are supported:

- `@default`
- `@map`
- [Native types](/docs/v6/orm/reference/prisma-schema-reference#model-field-scalar-types), such as `@db.ObjectId`
...
The Prisma schema supports a number of [functions](/docs/v6/orm/reference/prisma-schema-reference#attribute-functions) . These can be used to specify [default values](/docs/v6/orm/reference/prisma-schema-reference#default) on fields of a model.

For example, the default value of `createdAt` is [`now()`](/docs/v6/orm/reference/prisma-schema-reference#now) :
...
[`cuid()`](/docs/v6/orm/reference/prisma-schema-reference#cuid) and [`uuid()`](/docs/v6/orm/reference/prisma-schema-reference#uuid) are implemented by Prisma ORM and therefore are not "visible" in the underlying database schema. You can still use them when using [introspection](/docs/v6/orm/prisma-schema/introspection) by [manually changing your Prisma schema](/docs/v6/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma Client's [query engine](/docs/v6/orm/more/internals/engines)

Support for [`autoincrement()`](/docs/v6/orm/reference/prisma-schema-reference#autoincrement), [`now()`](/docs/v6/orm/reference/prisma-schema-reference#now), and [`dbgenerated(...)`](/docs/v6/orm/reference/prisma-schema-reference#dbgenerated) differ between databases.

**Relational database connectors** implement `autoincrement()`, `dbgenerated(...)`, and `now()` at database level. The **MongoDB connector** does not support `autoincrement()` or `dbgenerated(...)`, and `now()` is implemented at the Prisma ORM level. The [`auto()`](/docs/v6/orm/reference/prisma-schema-reference#auto) function is used to generate an `ObjectId`.
...
Every model in the data model definition will result in a number of CRUD queries in the generated [Prisma Client API](/docs/v6/orm/prisma-client/setup-and-configuration/introduction):
...
- [`create()`](/docs/v6/orm/reference/prisma-client-reference#create)
- [`update()`](/docs/v6/orm/reference/prisma-client-reference#update)
- [`upsert()`](/docs/v6/orm/reference/prisma-client-reference#upsert)
- [`delete()`](/docs/v6/orm/reference/prisma-client-reference#delete)
- [`createMany()`](/docs/v6/orm/reference/prisma-client-reference#createmany)
- [`createManyAndReturn()`](/docs/v6/orm/reference/prisma-client-reference#createmanyandreturn)
- [`updateMany()`](/docs/v6/orm/reference/prisma-client-reference#updatemany)
- [`updateManyAndReturn()`](/docs/v6/orm/reference/prisma-client-reference#updatemanyandreturn)
- [`deleteMany()`](/docs/v6/orm/reference/prisma-client-reference#deletemany)
...
Prisma Client also generates **type definitions** that reflect your model structures. These are part of the generated [`@prisma/client`](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client#the-prismaclient-npm-package) node module.

When using TypeScript, these type definitions ensure that all your database queries are entirely type safe and validated at compile-time (even partial queries using [`select`](/docs/v6/orm/reference/prisma-client-reference#select) or [`include`](/docs/v6/orm/reference/prisma-client-reference#include) ).
