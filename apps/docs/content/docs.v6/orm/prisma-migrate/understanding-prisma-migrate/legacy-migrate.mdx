...
unlisted: true
url: /docs/v6/orm/prisma-migrate/understanding-prisma-migrate/legacy-migrate
...
> **Important!** This page documents legacy Prisma Migrate (Experimental) available in version 2.12.0 and earlier. [Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started) is available in version [2.13.0](https://github.com/prisma/prisma/releases/tag/2.13.0) and Generally Available in [2.19.0](https://github.com/prisma/prisma/releases/tag/2.19.0).

Legacy Prisma Migrate is a tool that lets you _change your database schema_, e.g. by creating new tables or adding columns to existing tables. These changes are called _schema migrations_. legacy Prisma Migrate is available as part of the [Prisma CLI](/docs/v6/orm/tools/prisma-cli#installation) via the `legacy Prisma Migrate` command.

## Documentation

### Legacy Prisma Migrate vs the `db push` command

If you want to prototype or iterate on a schema design in a development environment, consider the [`db push` command](/docs/v6/orm/reference/prisma-cli-reference#db-push).
...
- **legacy Prisma Migrate (declarative)**: Define the desired schema as a [Prisma schema data model](/docs/v6/orm/prisma-schema/data-model/models) (legacy Prisma Migrate takes care of generating the necessary _steps_).
...
With legacy Prisma Migrate, you write the desired database schema in the form of a [Prisma schema data model](/docs/v6/orm/prisma-schema/data-model/models) inside your [Prisma schema file](/docs/v6/orm/prisma-schema/overview). To map the data model to your database schema, you then have to run these two commands:
...
Add two models with [relation fields](/docs/v6/orm/prisma-schema/data-model/relations#relation-fields) to your Prisma schema:
...
Notice that in addition to the [annotated relation fields](/docs/v6/orm/prisma-schema/data-model/relations#annotated-relation-fields) and its relation scalar field (which represent the foreign keys), you must also specify the Prisma-level [relation fields](/docs/v6/orm/prisma-schema/data-model/relations#relation-fields) on the other side of the relation.
...
Add a [field](/docs/v6/orm/prisma-schema/data-model/models#defining-fields) to the `Post` model:
...
| Data validation                   | `CHECK`                         | No ([workaround](/docs/v6/orm/more/troubleshooting/check-constraints)) |
...
Before using the `migrate save` command, you must define a valid [`datasource`](/docs/v6/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
...
Before using the `migrate up` command, you must define a valid [`datasource`](/docs/v6/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
...
Before using the `migrate down` command, you must define a valid [`datasource`](/docs/v6/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
