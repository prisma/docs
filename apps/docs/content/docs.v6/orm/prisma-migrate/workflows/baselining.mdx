...
url: /docs/v6/orm/prisma-migrate/workflows/baselining
...
Baselining is part of [adding Prisma Migrate to a project with an existing database](/docs/v6/orm/prisma-migrate/getting-started#adding-prisma-migrate-to-an-existing-project).

:::warning

This guide **does not apply for MongoDB**.<br />
Instead of `migrate deploy`, [`db push`](/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/docs/v6/orm/overview/databases/mongodb).
...
![A migration history represented by three migration files (file icon and name), surrounded by a a box labelled 'migration history'. The first migration is marked 'do not apply', and the second two migrations are marked 'apply'. An arrow labelled with the command 'prisma migrate deploy' points from the migration history to a database labelled 'production'.](/docs/v6/orm/prisma-migrate/workflows/deploy-db.png)
