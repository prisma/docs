...
url: /docs/v6/orm/prisma-client/queries/select-fields
...
- Use [`select`](/docs/v6/orm/reference/prisma-client-reference#select) to return specific fields. You can also use a [nested `select`](/docs/v6/orm/prisma-client/queries/relation-queries#select-specific-fields-of-included-relations) by selecting relation fields.
- Use [`omit`](/docs/v6/orm/reference/prisma-client-reference#omit) to exclude specific fields from the result. `omit` can be seen as the "opposite" to `select`.
- Use [`include`](/docs/v6/orm/reference/prisma-client-reference#include) to additionally [include relations](/docs/v6/orm/prisma-client/queries/relation-queries#nested-reads).
...
You can also return relations by nesting `select` multiple times on [relation fields](/docs/v6/orm/prisma-schema/data-model/relations#relation-fields).
...
Be careful when deeply nesting relations because the underlying database query may become slow due it needing to access a lot of different tables. To ensure your queries always have optimal speed, consider adding a caching layer with [Prisma Accelerate](/docs/v6/accelerate) or use [Prisma Optimize](/docs/v6/optimize) to get query insights and recommendations for performance optimizations.

:::

For more information about querying relations, refer to the following documentation:

- [Include a relation (including all fields)](/docs/v6/orm/prisma-client/queries/relation-queries#include-all-fields-for-a-specific-relation)
- [Select specific relation fields](/docs/v6/orm/prisma-client/queries/relation-queries#select-specific-fields-of-included-relations)
