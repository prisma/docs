...
url: /docs/v6/orm/prisma-client/queries/filtering-and-sorting
...
Refer to Prisma Client's reference documentation for [a full list of operators](/docs/v6/orm/reference/prisma-client-reference#filter-conditions-and-operators) , such as `startsWith` and `contains`.

#### Combining operators

You can use operators (such as [`NOT`](/docs/v6/orm/reference/prisma-client-reference#not-1) and [`OR`](/docs/v6/orm/reference/prisma-client-reference#or) ) to filter by a combination of conditions. The following query returns all users whose `email` ends with `gmail.com` or `company.com`, but excludes any emails ending with `admin.company.com`
...
Prisma Client supports [filtering on related records](/docs/v6/orm/prisma-client/queries/relation-queries#relation-filters). For example, in the following schema, a user can have many blog posts:
...
Scalar lists (for example, `String[]`) have a special set of [filter conditions](/docs/v6/orm/reference/prisma-client-reference#scalar-list-filters) - for example, the following query returns all posts where the `tags` array contains `databases`:
...
See also: [Case sensitivity](/docs/v6/orm/prisma-client/queries/case-sensitivity)
...
Use [`orderBy`](/docs/v6/orm/reference/prisma-client-reference#orderby) to sort a list of records or a nested list of records by a particular field or set of fields. For example, the following query returns all `User` records sorted by `role` and `name`, **and** each user's posts sorted by `title`:
...
> **Note**: You can also [sort lists of nested records](/docs/v6/orm/prisma-client/queries/relation-queries#filter-a-list-of-relations)
...
**For PostgreSQL**, you need to enable order by relevance with the `fullTextSearchPostgres` [preview feature](/docs/v6/orm/prisma-client/queries/full-text-search):
...
- This feature is generally available in version `4.16.0` and later. To use this feature in versions [`4.1.0`](https://github.com/prisma/prisma/releases/tag/4.1.0) to [`4.15.0`](https://github.com/prisma/prisma/releases/tag/4.15.0) the [Preview feature](/docs/v6/orm/reference/preview-features/client-preview-features#enabling-a-prisma-client-preview-feature) `orderByNulls` will need to be enabled.
- This feature is not available for MongoDB.
- You can only sort by nulls on optional [scalar](/docs/v6/orm/prisma-schema/data-model/models#scalar-fields) fields. If you try to sort by nulls on a required or [relation](/docs/v6/orm/prisma-schema/data-model/models#relation-fields) field, Prisma Client throws a [P2009 error](/docs/v6/orm/reference/error-reference#p2009).
