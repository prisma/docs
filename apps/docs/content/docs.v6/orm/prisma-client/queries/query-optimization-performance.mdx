...
url: /docs/v6/orm/prisma-client/queries/query-optimization-performance
...
For more potential causes of performance issues, visit [this page](/docs/v6/optimize/recommendations).

:::

[Prisma Optimize](/docs/v6/optimize) offers [recommendations](/docs/v6/optimize/recommendations) to identify and address the inefficiencies listed above and more, helping to improve query performance.

To get started, follow the [integration guide](/docs/v6/optimize/getting-started) and add Prisma Optimize to your project to begin diagnosing slow queries.
...
You can also [log query events at the client level](/docs/v6/orm/prisma-client/observability-and-logging/logging#event-based-logging) to view the generated queries, their parameters, and execution times.
...
For serverless development environments with frameworks that use HMR (Hot Module Replacement), ensure you properly handle a [single instance of Prisma in development](/docs/v6/orm/more/troubleshooting/nextjs#best-practices-for-using-prisma-client-in-development).
...
Use `findUnique()` in combination with [the fluent API](/docs/v6/orm/prisma-client/queries/relation-queries#fluent-api) (`.posts()`) as shown to return a user's posts. Even though the resolver is called once per user, the Prisma dataloader in Prisma Client **✔ batches the `findUnique()` queries**.
...
You can perform the query with a [database join](/docs/v6/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) by setting `relationLoadStrategy` to `"join"`, ensuring that only **one** query is executed against the database.
...
- Use nested reads ([`include`](/docs/v6/orm/reference/prisma-client-reference#include) ) to return users and related posts
- Use the [`in`](/docs/v6/orm/reference/prisma-client-reference#in) filter
- Set the [`relationLoadStrategy`](/docs/v6/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) to `"join"`

#### Solving n+1 with `include`

You can use `include` to return each user's posts. This only results in **two** SQL queries - one to get users, and one to get posts. This is known as a [nested read](/docs/v6/orm/prisma-client/queries/relation-queries#nested-reads).
