...
In Prisma v6, FTS has been [promoted to General Availability on MySQL](/docs/v6/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-6#fulltextsearch). It still remains in Preview for PostgreSQL and requires using the [`fullTextSearchPostgres`](/docs/v6/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-6#full-text-search-on-postgresql) Preview feature flag.
...
1. Update the [`previewFeatures`](/docs/v6/orm/reference/preview-features) block in your schema to include the `fullTextSearchPostgres` preview feature flag:
...
Full-text search is currently in Preview, and due to a [known issue](https://github.com/prisma/prisma/issues/23627), you might experience slow search queries. If so, you can optimize your query using [TypedSQL](/docs/v6/orm/prisma-client/using-raw-sql).

### PostgreSQL

With [TypedSQL](/docs/v6/orm/prisma-client/using-raw-sql), you can use PostgreSQL's `to_tsvector` and `to_tsquery` to express your search query.

<Tabs items={["fullTextSearch.sql", "index.ts"]}>

  <Tab value="fullTextSearch.sql">
