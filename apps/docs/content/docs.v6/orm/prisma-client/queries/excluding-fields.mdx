...
By default Prisma Client returns all fields from a model. You can use [`select`](/docs/v6/orm/prisma-client/queries/select-fields) to narrow the result set, but that can be unwieldy if you have a large model and you only want to exclude a small number of fields.
...
<Tabs items={["Explicit Select", "Omit False"]}>

<Tab value="Explicit Select">
