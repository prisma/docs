...
Refer to the [Prisma Client API reference documentation](/docs/v6/orm/reference/prisma-client-reference) for detailed explanations of each method.
...
The following query creates ([`create()`](/docs/v6/orm/reference/prisma-client-reference#create)) a single user with two fields:
...
The user's `id` is auto-generated, and your schema determines [which fields are mandatory](/docs/v6/orm/prisma-schema/data-model/models#optional-and-mandatory-fields).
...
For more information about working with generated types, see: [Generated types](/docs/v6/orm/prisma-client/type-safety).
...
The following [`createMany()`](/docs/v6/orm/reference/prisma-client-reference#createmany) query creates multiple users and skips any duplicates (`email` must be unique):
...
See [Working with relations > Nested writes](/docs/v6/orm/prisma-client/queries/relation-queries#nested-writes) for information about creating a record and one or more related records at the same time.
...
The following queries return a single record ([`findUnique()`](/docs/v6/orm/reference/prisma-client-reference#findunique)) by unique identifier or ID:
...
The following [`findMany()`](/docs/v6/orm/reference/prisma-client-reference#findmany) query returns _all_ `User` records:

```ts
const users = await prisma.user.findMany();
```

You can also [paginate your results](/docs/v6/orm/prisma-client/queries/pagination).

### Get the first record that matches a specific criteria

The following [`findFirst()`](/docs/v6/orm/reference/prisma-client-reference#findfirst) query returns the _most recently created user_ with at least one post that has more than 100 likes:
...
Prisma Client supports [filtering](/docs/v6/orm/prisma-client/queries/filtering-and-sorting) on record fields and related record fields.
...
The following query uses a combination of [operators](/docs/v6/orm/reference/prisma-client-reference#filter-conditions-and-operators) to return users whose name start with `E` _or_ administrators with at least 1 profile view:
...
See [Working with relations](/docs/v6/orm/prisma-client/queries/relation-queries) for more examples of filtering on related field values.
...
For more information about including relations, see [Select fields and include relations](/docs/v6/orm/prisma-client/queries/select-fields).

### Select distinct field values

See [Select `distinct`](/docs/v6/orm/prisma-client/queries/aggregation-grouping-summarizing#select-distinct) for information about selecting distinct field values.
...
See [Working with relations](/docs/v6/orm/prisma-client/queries/relation-queries#filter-a-list-of-relations) to find out how to combine [`include`](/docs/v6/orm/reference/prisma-client-reference#include) and `where` for a filtered list of relations - for example, only include a user's published posts.

## Update

### Update a single record

The following query uses [`update()`](/docs/v6/orm/reference/prisma-client-reference#update) to find and update a single `User` record by `email`:
...
The following query uses [`updateMany()`](/docs/v6/orm/reference/prisma-client-reference#updatemany) to update all `User` records that contain `prisma.io`:
...
The following query uses [`upsert()`](/docs/v6/orm/reference/prisma-client-reference#upsert) to update a `User` record with a specific email address, or create that `User` record if it does not exist:
...
From version 4.6.0, Prisma Client carries out upserts with database native SQL commands where possible. [Learn more](/docs/v6/orm/reference/prisma-client-reference#database-upserts).
...
Use [atomic number operations](/docs/v6/orm/reference/prisma-client-reference#atomic-number-operations) to update a number field **based on its current value** - for example, increment or multiply. The following query increments the `views` and `likes` fields by `1`:
...
Refer to [Working with relations](/docs/v6/orm/prisma-client/queries/relation-queries) for information about disconnecting ([`disconnect`](/docs/v6/orm/reference/prisma-client-reference#disconnect)) and connecting ([`connect`](/docs/v6/orm/reference/prisma-client-reference#connect)) related records.

## Delete

### Delete a single record

The following query uses [`delete()`](/docs/v6/orm/reference/prisma-client-reference#delete) to delete a single `User` record:
...
The following query uses [`deleteMany()`](/docs/v6/orm/reference/prisma-client-reference#deletemany) to delete all `User` records where `email` contains `prisma.io`:
...
The following query uses [`deleteMany()`](/docs/v6/orm/reference/prisma-client-reference#deletemany) to delete all `User` records:
...
In [2.26.0](https://github.com/prisma/prisma/releases/tag/2.26.0) and later it is possible to do cascading deletes using the **preview feature** [referential actions](/docs/v6/orm/prisma-schema/data-model/relations/referential-actions).
...
When you know the order in which your tables should be deleted, you can use the [`deleteMany`](/docs/v6/orm/reference/prisma-client-reference#deletemany) function. This is executed synchronously in a [`$transaction`](/docs/v6/orm/prisma-client/queries/transactions) and can be used with all types of databases.
...
If you are comfortable working with raw SQL, you can perform a `TRUNCATE` query on a table using [`$executeRawUnsafe`](/docs/v6/orm/prisma-client/using-raw-sql/raw-queries#executerawunsafe).
