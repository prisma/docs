...
url: /docs/v6/orm/prisma-client/observability-and-logging/sql-comments
...
The trace context plugin requires [`@prisma/instrumentation`](/docs/v6/orm/prisma-client/observability-and-logging/opentelemetry-tracing) to be configured. The `traceparent` is only added when tracing is active and the span is sampled.
