...
See also: [Using the `Prisma.UserCreateInput` generated type](/docs/v6/orm/prisma-client/queries/crud#create-a-single-record-using-generated-types)
...
This query will also result in an error if an author with an `id` of `1` does not exist. In this case, Prisma Client will give a more descriptive error message. You can also use the [`connectOrCreate`](/docs/v6/orm/reference/prisma-client-reference#connectorcreate) API to safely create a new user if one does not already exist with the given `id`.
...
This is especially useful in [validating inputs](/docs/v6/orm/prisma-client/type-safety/prisma-validator) and [shared Prisma Client extensions](/docs/v6/orm/prisma-client/client-extensions/shared-extensions).
