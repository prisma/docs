...
url: /docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool
...
- By [explicitly calling `$connect()`](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/connection-management#connect) _or_
...
Starting with Prisma ORM v7, relational datasources instantiate Prisma Client with [driver adapters](/docs/v6/orm/overview/databases/database-drivers) by default. Driver adapters rely on the Node.js driver you supply, so connection pooling defaults (and configuration) now come from the driver itself.
...
Consider introducing [an external connection pooler like PgBouncer](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections#pgbouncer) to prevent your application or functions from exhausting the database connection limit.
...
If you consistently experience pool timeout errors, you need to [optimize the connection pool](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections#optimizing-the-connection-pool) .
...
Although the formula represents a good starting point, the [recommended connection limit](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections#recommended-connection-pool-size) also depends on your deployment paradigm - particularly if you are using serverless.

#### Setting the connection pool size

You can specify the number of connections by explicitly setting the `connection_limit` parameter in your database connection URL. For example, with the following `datasource` configuration in your [Prisma schema](/docs/v6/orm/prisma-schema/overview) the connection pool will have exactly five connections:
...
The number of connections Prisma Client uses can be viewed using [logging](/docs/v6/orm/prisma-client/observability-and-logging/logging) and built-in APIs provided by the driver adapter being used.

Using the `info` [logging level](/docs/v6/orm/reference/prisma-client-reference#log-levels), you can log the number of connections in a connection pool that are opened when Prisma Client is instantiated.
...
If you need even more insights into the size of your connection pool and the amount of in-use and idle connection, you can use the [metrics](/docs/v6/orm/prisma-client/observability-and-logging/metrics) feature (which is currently in Preview).
...
For more details on what is available in the metrics output, see the [About metrics](/docs/v6/orm/prisma-client/observability-and-logging/metrics#about-metrics) section.
...
You can choose to [disable the connection pool timeout if queries **must** remain in the queue](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections#disabling-the-pool-timeout) - for example, if you are importing a large number of records in parallel and are confident that the queue will not use up all available RAM before the job is complete.
