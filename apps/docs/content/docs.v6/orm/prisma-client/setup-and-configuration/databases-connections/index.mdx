...
url: /docs/v6/orm/prisma-client/setup-and-configuration/databases-connections
...
| ------------------------------------------------------------------------------ | ----------- | ------------------------------------------------ |
| [`migrate status`](/docs/v6/orm/reference/prisma-cli-reference#migrate-status) | 1           | Checks the status of migrations                  |
| [`migrate dev`](/docs/v6/orm/reference/prisma-cli-reference#migrate-dev)       | 1–4         | Applies pending migrations in development        |
| [`migrate diff`](/docs/v6/orm/reference/prisma-cli-reference#migrate-diff)     | 1–2         | Compares database schema with migration history  |
| [`migrate reset`](/docs/v6/orm/reference/prisma-cli-reference#migrate-reset)   | 1–2         | Resets the database and reapplies migrations     |
| [`migrate deploy`](/docs/v6/orm/reference/prisma-cli-reference#migrate-deploy) | 1–2         | Applies pending migrations in production         |
| [`db pull`](/docs/v6/orm/reference/prisma-cli-reference#db-pull)               | 1           | Pulls the database schema into the Prisma schema |
| [`db push`](/docs/v6/orm/reference/prisma-cli-reference#db-push)               | 1–2         | Pushes the Prisma schema to the database         |
| [`db execute`](/docs/v6/orm/reference/prisma-cli-reference#db-execute)         | 1           | Executes raw SQL commands                        |
| [`db seed`](/docs/v6/orm/reference/prisma-cli-reference#db-seed)               | 1           | Seeds the database with initial data             |
...
![An AWS Lambda function connecting to a database.](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/serverless-connections.png)
...
![Three AWS Lambda function connecting to a database.](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/serverless-connections-2.png)
...
If the query engine cannot [process a query in the queue before the time limit](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool#how-the-connection-pool-works) , you will see connection pool timeout exceptions in your log. A connection pool timeout can occur if:
...
As of Prisma ORM v7, [driver adapters](/docs/v6/orm/overview/databases/database-drivers) are the default for relational databases, providing better performance and developer experience. Learn more [here](/docs/v6/orm/prisma-client/setup-and-configuration/no-rust-engine).
...
With driver adapters, connection pool configuration is handled by the Node.js driver you provide (like `pg`, `mariadb`, or `mssql`). See the [connection pool guide](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool) for v7 defaults and configuration details.

Prisma ORM also has its own configurable timeouts that are separate from the database driver timeouts. If you run into a timeout error and are unsure whether it comes from the specific driver or from Prisma Client, see the [Prisma Client timeouts and transaction options documentation](/docs/v6/orm/prisma-client/queries/transactions#transaction-options).
...
Connection poolers like [Prisma Accelerate](/docs/v6/accelerate) and PgBouncer prevent your application from exhausting the database's connection limit.
...
[Prisma Accelerate](/docs/v6/accelerate) is a managed external connection pooler built by Prisma that is integrated in the [Prisma Data Platform](/docs/v6/platform) and handles connection pooling for you.
...
[PgBouncer](https://www.pgbouncer.org/) holds a connection pool to the database and proxies incoming client connections by sitting between Prisma Client and the database. This reduces the number of processes a database has to handle at any given time. PgBouncer passes on a limited number of connections to the database and queues additional connections for delivery when connections becomes available. To use PgBouncer, see [Configure Prisma Client with PgBouncer](/docs/v6/orm/prisma-client/setup-and-configuration/databases-connections/pgbouncer).

### AWS RDS Proxy

Due to the way AWS RDS Proxy pins connections, [it does not provide any connection pooling benefits](/docs/v6/orm/prisma-client/deployment/caveats-when-deploying-to-aws-platforms#aws-rds-proxy) when used together with Prisma Client.
