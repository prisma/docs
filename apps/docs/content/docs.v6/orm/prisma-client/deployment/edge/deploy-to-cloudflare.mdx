...
url: /docs/v6/orm/prisma-client/deployment/edge/deploy-to-cloudflare
...
Then setup helper scripts to perform migrations and generate `PrismaClient` as [shown in this section](/docs/v6/orm/prisma-client/deployment/edge/deploy-to-cloudflare#development).
...
When deploying a Cloudflare Worker that uses Prisma ORM, you need to use an [edge-compatible driver](/docs/v6/orm/prisma-client/deployment/edge/overview#edge-compatibility-of-database-drivers) and its respective [driver adapter](/docs/v6/orm/overview/databases/database-drivers#driver-adapters) for Prisma ORM.
...
- [Cloudflare D1](/docs/v6/orm/prisma-client/deployment/edge/deploy-to-cloudflare) is used to access D1 databases
...
If your application uses PostgreSQL, we recommend using [Prisma Postgres](/docs/v6/postgres). It is fully supported on edge runtimes and does not require a specialized edge-compatible driver. For other databases, [Prisma Accelerate](/docs/v6/accelerate) extends edge compatibility so you can connect to _any_ database from _any_ edge function provider.
...
- use the `@prisma/adapter-pg` database adapter (learn more [here](/docs/v6/orm/overview/databases/postgresql#using-the-node-postgres-driver))
...
- use the `@prisma/adapter-planetscale` database adapter (learn more [here](/docs/v6/orm/overview/databases/planetscale#how-to-use-the-planetscale-serverless-driver-with-prisma-orm-preview))
...
- use the `@prisma/adapter-neon` database adapter (learn more [here](/docs/v6/orm/overview/databases/neon#how-to-use-neons-serverless-driver-with-prisma-orm))
