...
url: /docs/v6/orm/prisma-client/deployment/serverless/deploy-to-aws-lambda
...
Lastly, you will want to exclude [Prisma ORM query engines](/docs/v6/orm/more/internals/engines) that do not match the AWS Lambda runtime. Update your `serverless.yml` by adding the following script that makes sure only the required query engine, `rhel-openssl-1.0.x`, is included in the final packaged archive.
