...
2. Generate a Prisma Client:
   ...
   > Note: The `--no-engine` flag should only be used in preview and production environments. The command generates Prisma Client artifacts without a [Query Engine](/docs/v6/orm/more/under-the-hood/engines) file, which requires an Accelerate connection string.
