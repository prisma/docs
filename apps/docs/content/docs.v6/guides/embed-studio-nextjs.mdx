...
url: /docs/v6/guides/embed-studio-nextjs
...
[**Embeddable Prisma Studio**](/docs/v6/postgres/database/prisma-studio/embedding-studio) is _free_ and licensed under Apache 2.0.
...
Currently, Embedded Prisma Studio supports [**Prisma Postgres**](/docs/v6/postgres), with additional databases coming soon.
...
1. **Add authentication**: Currently, anyone who can open your app has access to Prisma Studio. [Add user authentication](/docs/v6/postgres/database/prisma-studio/embedding-studio#adding-user-authentication) and only allow specific roles (for example, admins) to use the embedded Studio. You can do this by checking authentication tokens in your `/api/studio` endpoint before running queries.
...
3. **Apply custom styling**: The Studio component ships with a default look. Pass in [your own theme](/docs/v6/postgres/database/prisma-studio/embedding-studio#custom-styling) and adjust colors, typography, or branding to match the rest of your application. This helps Studio feel like a native part of your app rather than a standalone tool.
