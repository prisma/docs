...
<Tabs code groupId="packageManager">
  <Tab value="npm">```bash cd turborepo-prisma npm install turbo --save-dev ```</Tab>
  <Tab value="yarn">
    ```bash cd turborepo-prisma yarn add turbo --dev --ignore-workspace-root-check ```
  </Tab>
  <Tab value="pnpm">
    ```bash cd turborepo-prisma pnpm add turbo --save-dev --ignore-workspace-root-check ```
  </Tab>
</Tabs>
...
In the `schema.prisma` file, we specify a custom [`output`](/docs/v6/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path) path where Prisma will generate its types. This ensures Prisma's types are resolved correctly across different package managers.
...
You can add users to your database by creating a seed script or manually by using [Prisma Studio](/docs/v6/orm/tools/prisma-studio).
...
- [Prisma ORM Docs](/docs/v6/orm/overview/introduction)
