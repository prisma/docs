---
title: "Get Started"
description: "Welcome to Prisma! Choose your path below to get started."
---

## Prisma ORM

[**Prisma ORM**](/docs/v6/orm/overview/introduction/what-is-prisma) is an open-source ORM that provides fast, type-safe access to Postgres, MySQL, SQLite, and more databases.

```npm
npx prisma init --db
```

## Prisma Postgres

[**Prisma Postgres**](/docs/v6/postgres) is a fully managed PostgreSQL database that scales to zero, integrates with Prisma ORM and Prisma Studio.

```npm
npx create-db
```

---

## Quickstart

The fastest way to set up **Prisma ORM** with a ready-to-use **Prisma Postgres** database.

[**Start Quickstart →**](/docs/v6/getting-started/prisma-orm/quickstart/prisma-postgres)

---

## Have Your Own Database?

Set up Prisma ORM with your existing database:

| Database    | Guide                                                                       |
| ----------- | --------------------------------------------------------------------------- |
| PostgreSQL  | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/postgresql)  |
| MySQL       | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/mysql)       |
| SQL Server  | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/sql-server)  |
| SQLite      | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/sqlite)      |
| MongoDB     | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/mongodb)     |
| PlanetScale | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/planetscale) |
| CockroachDB | [Get started →](/docs/v6/getting-started/prisma-orm/quickstart/cockroachdb) |

---

## Add Prisma to Your Framework

Working with a popular framework? You can easily add Prisma to your setup:

| Framework      | Guide                                           |
| -------------- | ----------------------------------------------- |
| Next.js        | [Get started →](/docs/v6/guides/nextjs)         |
| Nuxt.js        | [Get started →](/docs/v6/guides/nuxt)           |
| Astro          | [Get started →](/docs/v6/guides/astro)          |
| SvelteKit      | [Get started →](/docs/v6/guides/sveltekit)      |
| React Router 7 | [Get started →](/docs/v6/guides/react-router-7) |
| TanStack Start | [Get started →](/docs/v6/guides/tanstack-start) |

---

## Add to Existing Project

Already have a project? Add Prisma ORM to your existing application:

[**Add to Existing Project →**](/docs/v6/getting-started/prisma-orm/add-to-existing-project/prisma-postgres)
