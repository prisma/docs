...
In the generated `.env` file, update `DATABASE_URL` to match your Prisma Postgres direct connection string that you received in [step 1](/docs/v6/getting-started/prisma-postgres/import-from-existing-database-mysql#1-create-a-new-prisma-postgres-database):
...
This command updates your `schema.prisma` file with models representing your migrated tables, so you can start using [Prisma Client](/docs/v6/orm/prisma-client) to query your data or [Prisma Migrate](/docs/v6/orm/prisma-migrate/getting-started) to manage future changes.
...
For a comprehensive guide on getting started with Prisma and Prisma Postgres, see [start from scratch with Prisma and Prisma Postgres](/docs/v6/getting-started/prisma-orm/quickstart/prisma-postgres).
