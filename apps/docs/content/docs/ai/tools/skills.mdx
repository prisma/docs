...
| [Prisma 8](/orm) | ships inside the `@prisma/orm-*` packages | `npx prisma@latest init` |
...
[Prisma 8](/orm) ships one skill, `prisma-8`. It travels inside the `@prisma/orm-postgres`, `@prisma/orm-sqlite`, and `@prisma/orm-mongo` packages, so the copy in your project always describes the version you installed.
