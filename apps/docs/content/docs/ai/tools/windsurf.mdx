...
[Windsurf Editor](https://windsurf.com/editor/) is an AI code editor with an agent (Cascade) and project rules. This guide covers how to use it with Prisma to:
...
The [`.windsurfrules` file](https://docs.windsurf.com/windsurf/cascade/memories) in Windsurf gives Windsurf persistent instructions for your Prisma project, so generated code follows your conventions without manual adjustments.

Create a `.windsurfrules` file in the root of your project. Below is an example configuration:
...
Windsurf's [context-awareness](https://docs.windsurf.com/context-awareness/overview) features let you add project files and external resources to the context Windsurf works from. Including your Prisma schema and relevant documentation helps Windsurf generate queries, tests, and seed data that match your database schema.

### Including Additional Prisma Resources

Windsurf has built-in knowledge of common libraries, but that knowledge can lag behind the current Prisma release. Reference external Prisma resources explicitly when you need current, authoritative context.
...
Windsurf can generate a Prisma schema from a description. The more specific the prompt (models, fields, relations, defaults), the closer the result matches what you need. Here is an example prompt and its result:
...
Windsurf can write queries against your Prisma schema. State the fields to include, the conditions to apply, and the relations to traverse, and Windsurf generates a query that type-checks against your schema. Here is an example prompt and its resulting query:
...
Windsurf can generate seed scripts that populate your database with realistic data, including the relations defined in your schema. Here is an example prompt and its resulting seed script:
...
The [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma) also works in Windsurf. It provides a UI for managing local and remote [Prisma Postgres](/postgres) instances: view, create, and delete instances, push local databases to the cloud, and visualize your schema.
