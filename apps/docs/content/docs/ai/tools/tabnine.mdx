---
title: Tabnine
description: Learn tips and best practices for using Prisma ORM with the Tabnine AI code editor
url: /ai/tools/tabnine
...
[Tabnine](https://www.tabnine.com/) is an AI software development platform that comes as an [extension](https://www.tabnine.com/install/) for your IDE (e.g. VS Code, WebStorm, IntelliJ, ...).
...
### Where Tabnine helps with Prisma ORM

Tabnine works well with the following Prisma workflows:
...
- Generating plain SQL queries that can be used with [TypedSQL](/orm/v7/prisma-client/using-raw-sql/typedsql)
- Generating code for unit and integrations tests
- Guiding through major refactorings
- Generating seeding scripts

### Starting from the Prisma schema

Because Prisma has its own [schema](/orm/v7/prisma-schema/overview), Tabnine can understand the basic shape of your data and help you write code for the other parts of your stack.
...
Prisma ORM provides a high-level API for database queries. Queries that combine filters, pagination, and relations can still take work to construct.
...
While Prisma provides a high-level API for querying, sometimes a developer may need to drop down to raw SQL for more low-level control of a query. In these cases, they can use [TypedSQL](/orm/v7/prisma-client/using-raw-sql/typedsql) which provides full type safety for raq SQL queries in Prisma ORM.
...
Tabnine can help with tests by:
...
Prisma ORM's type-safe queries make refactorings safer and more predictable: after making schema changes, the generated Prisma Client code will be updated and the TypeScript compiler will highlight all places that need a change.
...
If you use Tabnine in an editor that can install the [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma) and you work with [Prisma Postgres](/postgres), install the extension. It provides a UI for managing local and remote Prisma Postgres instances: view, create, and delete instances, push local databases to the cloud, and visualize your schema.
