...
description: 'A vibe coding tutorial: build a Linktree clone SaaS application from scratch using Next.js, Prisma ORM, Prisma Postgres, and Clerk authentication with AI assistance'
...
metaDescription: 'A vibe coding tutorial: build a Linktree clone SaaS application from scratch using Next.js, Prisma ORM, Prisma Postgres, and Clerk authentication with AI assistance.'
---

## Introduction

In this vibe coding tutorial, you'll build a **Linktree clone SaaS** application from scratch with an AI coding assistant. The stack is:
...
Once the setup is complete, add **Prisma** and **Prisma Postgres** to your project with the prepared setup prompt.
...
The AI sets up Prisma ORM, creates your database connection, and configures the project.
...
Add user authentication with [Clerk](https://clerk.com/), which provides sign-up, sign-in, and session management.
...
When a user signs in with Clerk, they exist in Clerk's system but **not** in your database. To bridge this gap, create a "Claim Username" flow where users pick their unique username (e.g., `yourapp.com/johndoe`) after signing in for the first time.
...
Give the app a cleaner, friendlier look inspired by platforms like Buy Me a Coffee.
...
Add the core functionality, managing links. Users should be able to add new links and delete existing ones from their dashboard.
...
Add a custom background pattern to the app.
...
If users sign in with Google or another OAuth provider, Clerk stores their profile photo. Display it on public profiles.
...
Add icons with Lucide React to make buttons and empty states easier to read.
...
📖 **Reference:** [Deploy to Vercel - Build Configuration](/orm/v7/prisma-client/deployment/serverless/deploy-to-vercel#build-configuration)
