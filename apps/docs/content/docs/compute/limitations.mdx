...
- The ingress closes a request when your service goes 60 seconds without sending data, and returns `504 Gateway Time-out` to the client. The clock restarts on every chunk, so a steady streaming response stays open. Your service is not notified either way. Move longer work out of the request; see [Request timeout](/compute/request-timeout).
