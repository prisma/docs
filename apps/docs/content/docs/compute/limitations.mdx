...
- Every request must be answered within 60 seconds. Past that the ingress returns `504 Gateway Time-out` to the client and closes the connection to your service. Your handler is not notified. Move longer work out of the request; see [Request timeout](/compute/request-timeout).
