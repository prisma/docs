...
Accelerate environments from Step 4 use `prisma://accelerate.prisma-data.net/?api_key=YOUR_API_KEY` connection strings; hosted Accelerate connections for Prisma Postgres use `prisma+postgres://accelerate.prisma-data.net/?api_key=YOUR_API_KEY`. The retirement covers both.
