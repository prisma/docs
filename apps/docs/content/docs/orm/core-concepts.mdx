...
1. **Every query goes through the same pipeline.** However a query is written (the ORM client, a query builder, a raw fragment, or an API added by an extension), it reaches the database as a plan. Middleware sees every query in the same shape and executes them all in the same way; query APIs can be mixed freely. In addition, one policy (an authorization check, for instance) can sit in one place and see everything.
...
- [How migrations work](/orm/migrations/how-migrations-work): change your contract, then plan, review, and apply the migration, hands-on.
