...
url: /docs/orm/prisma-schema/data-model/models
metaTitle: Models
...
The data model definition part of the [Prisma schema](/docs/orm/prisma-schema/overview) defines your application models (also called **Prisma models**). Models:
...
- Form the foundation of the **queries** available in the generated [Prisma Client API](/docs/orm/prisma-client/setup-and-configuration/introduction)
- When used with TypeScript, Prisma Client provides generated **type definitions** for your models and any [variations](/docs/orm/prisma-client/type-safety/operating-against-partial-structures-of-model-types) of them to make database access entirely type safe.
...
- [Models](#defining-models) ([`model`](/docs/orm/reference/prisma-schema-reference#model) primitives) that define a number of fields, including [relations between models](#relation-fields)
- [Enums](#defining-enums) ([`enum`](/docs/orm/reference/prisma-schema-reference#enum) primitives) (if your connector supports Enums)
...
- **Write the data model manually and use Prisma Migrate**: You can write your data model manually and map it to your database using [Prisma Migrate](/docs/orm/prisma-migrate). In this case, the data model is the single source of truth for the models of your application.
- **Generate the data model via introspection**: When you have an existing database or prefer migrating your database schema with SQL, you generate the data model by [introspecting](/docs/orm/prisma-schema/introspection) your database. In this case, the database schema is the single source of truth for the models of your application.

## Defining models

Models represent the entities of your application domain. Models are represented by [`model`](/docs/orm/reference/prisma-schema-reference#model) blocks and define a number of [fields](/docs/orm/reference/prisma-schema-reference#model-fields). In the example data model above, `User`, `Profile`, `Post` and `Category` are models.
...
However, you can still adhere to the naming convention without renaming the underlying `comments` table in the database by using the [`@@map`](/docs/orm/reference/prisma-schema-reference) attribute:
...
> **Note**: You can also [`@map`](/docs/orm/reference/prisma-schema-reference#map) a column name or enum value, and `@@map` an enum name.

`@map` and `@@map` allow you to [tune the shape of your Prisma Client API](/docs/orm/prisma-client/setup-and-configuration/custom-model-and-field-names#using-map-and-map-to-rename-fields-and-models-in-the-prisma-client-api) by decoupling model and field names from table and column names in the underlying database.

## Defining fields

The properties of a model are called _fields_, which consist of:

- A **[field name](/docs/orm/reference/prisma-schema-reference#model-fields)**
- A **[field type](/docs/orm/reference/prisma-schema-reference#model-fields)**
...
- [Scalar types](#scalar-fields) (includes [enums](#defining-enums)) that map to columns (relational databases) or document fields (MongoDB) - for example, [`String`](/docs/orm/reference/prisma-schema-reference#string) or [`Int`](/docs/orm/reference/prisma-schema-reference#int)
- Model types (the field is then called [relation field](/docs/orm/prisma-schema/data-model/relations#relation-fields)) - for example `Post` or `Comment[]`
...
See [complete list of scalar field types](/docs/orm/reference/prisma-schema-reference#model-field-scalar-types) .
...
Refer to the [relations documentation](/docs/orm/prisma-schema/data-model/relations) for more examples and information about relationships between models.
...
- Only added during [introspection](/docs/orm/prisma-schema/introspection) if the native type differs from the default

See [native database type attributes](/docs/orm/reference/prisma-schema-reference#model-field-scalar-types) for the complete list.
...
- [`?`](/docs/orm/reference/prisma-schema-reference) Make a field optional
...
When you introspect a relational database, unsupported data types are added as [`Unsupported`](/docs/orm/reference/prisma-schema-reference#unsupported):

```prisma
location    Unsupported("POLYGON")?
```

Fields of type `Unsupported` don't appear in the generated Prisma Client API, but you can still use [raw database access](/docs/orm/prisma-client/using-raw-sql/raw-queries) to query them.
...
Attributes modify the behavior of fields or model blocks. The following example includes three field attributes ([`@id`](/docs/orm/reference/prisma-schema-reference#id) , [`@default`](/docs/orm/reference/prisma-schema-reference#default) , and [`@unique`](/docs/orm/reference/prisma-schema-reference#unique) ) and one block attribute ([`@@unique`](/docs/orm/reference/prisma-schema-reference)):
...
Some attributes accept [arguments](/docs/orm/reference/prisma-schema-reference#attribute-argument-types) - for example, `@default` accepts `true` or `false`:

```prisma
isAdmin   Boolean @default(false) // short form of @default(value: false)
```

See [complete list of field and block attributes](/docs/orm/reference/prisma-schema-reference#attributes)
...
In relational databases, an ID can be defined by a single field using the [`@id`](/docs/orm/reference/prisma-schema-reference#id) attribute, or multiple fields using the [`@@id`](/docs/orm/reference/prisma-schema-reference) attribute.
...
You can also provide your own name for the composite ID using the [`@@id`](/docs/orm/reference/prisma-schema-reference) attribute's `name` field:
...
Refer to the documentation on [working with composite IDs](/docs/orm/prisma-client/special-fields-and-types/working-with-composite-ids-and-constraints) to learn how to interact with a composite ID in Prisma Client.
...
You can optionally define a [custom primary key constraint name](/docs/orm/prisma-schema/data-model/database-mapping#constraint-and-index-names) in the underlying database.

:::

#### Defining IDs in MongoDB

The MongoDB connector has [specific rules for defining an ID field](/docs/orm/reference/prisma-schema-reference#mongodb) that differs from relational databases. An ID must be defined by a single field using the [`@id`](/docs/orm/reference/prisma-schema-reference#id) attribute and must include `@map("_id")`.
...
You can define default values for scalar fields using the [`@default`](/docs/orm/reference/prisma-schema-reference#default) attribute:
...
- **Functions**: [`now()`](/docs/orm/reference/prisma-schema-reference#now), [`uuid()`](/docs/orm/reference/prisma-schema-reference#uuid), [`cuid()`](/docs/orm/reference/prisma-schema-reference#cuid)
- **JSON**: Use escaped strings, e.g., `@default("{ \"hello\": \"world\" }")`

See [attribute functions](/docs/orm/reference/prisma-schema-reference#attribute-functions) for connector support details.

### Defining a unique field

Unique attributes can be defined on a single field using [`@unique`](/docs/orm/reference/prisma-schema-reference#unique), or on multiple fields using [`@@unique`](/docs/orm/reference/prisma-schema-reference):
...
See [working with composite unique identifiers](/docs/orm/prisma-client/special-fields-and-types/working-with-composite-ids-and-constraints) for Prisma Client usage.
...
Define indexes via [`@@index`](/docs/orm/reference/prisma-schema-reference#index):
...
See [custom index names](/docs/orm/prisma-schema/data-model/database-mapping#constraint-and-index-names) for naming customization.

## Defining enums

Enums are defined via the [`enum`](/docs/orm/reference/prisma-schema-reference#enum) block when [supported by your database](/docs/orm/reference/database-features#misc):
...
The Prisma schema supports [functions](/docs/orm/reference/prisma-schema-reference#attribute-functions) for default values:
...
See [relations documentation](/docs/orm/prisma-schema/data-model/relations) for relationship details.

## Models in Prisma Client

### Queries (CRUD)

Every model generates CRUD queries in the [Prisma Client API](/docs/orm/prisma-client/setup-and-configuration/introduction):
