...
You can introspect your database using the Prisma CLI in order to generate the [data model](/docs/v7/orm/prisma-schema/data-model) in your [Prisma schema](/docs/v7/orm/prisma-schema). The data model is needed to [generate Prisma Client](/docs/v7/orm/prisma-client/setup-and-configuration/custom-model-and-field-names).

Introspection is often used to generate an _initial_ version of the data model when [adding Prisma ORM to an existing project](/docs/v7/getting-started/prisma-orm/add-to-existing-project/postgresql).

However, it can also be [used _repeatedly_ in an application](#introspection-with-an-existing-schema). This is most commonly the case when you're _not_ using [Prisma Migrate](/docs/v7/orm/prisma-migrate) but perform schema migrations using plain SQL or another migration tool. In that case, you also need to re-introspect your database and subsequently re-generate Prisma Client to reflect the schema changes in your [Prisma Client API](/docs/v7/orm/prisma-client).
...
- Map _tables_ in the database to [Prisma models](/docs/v7/orm/prisma-schema/data-model/models#defining-models)
- Map _columns_ in the database to the [fields](/docs/v7/orm/prisma-schema/data-model/models#defining-fields) of Prisma models
- Map _indexes_ in the database to [indexes](/docs/v7/orm/prisma-schema/data-model/models#defining-an-index) in the Prisma schema
- Map _database constraints_ to [attributes](/docs/v7/orm/prisma-schema/data-model/models#defining-attributes) or [type modifiers](/docs/v7/orm/prisma-schema/data-model/models#type-modifiers) in the Prisma schema

On MongoDB, the main functions are the following:

- Map _collections_ in the database to [Prisma models](/docs/v7/orm/prisma-schema/data-model/models#defining-models). Because a _collection_ in MongoDB doesn't have a predefined structure, Prisma ORM _samples_ the _documents_ in the collection and derives the model structure accordingly (i.e. it maps the fields of the _document_ to the [fields](/docs/v7/orm/prisma-schema/data-model/models#defining-fields) of the Prisma model). If _embedded types_ are detected in a collection, these will be mapped to [composite types](/docs/v7/orm/prisma-schema/data-model/models#defining-composite-types) in the Prisma schema.
- Map _indexes_ in the database to [indexes](/docs/v7/orm/prisma-schema/data-model/models#defining-an-index) in the Prisma schema, if the collection contains at least one document contains a field included in the index
...
- [PostgreSQL](/docs/v7/orm/overview/databases/postgresql#type-mapping-between-postgresql-and-prisma-schema)
- [MySQL](/docs/v7/orm/overview/databases/mysql#type-mapping-between-mysql-to-prisma-schema)
- [SQLite](/docs/v7/orm/overview/databases/sqlite#type-mapping-between-sqlite-to-prisma-schema)
- [Microsoft SQL Server](/docs/v7/orm/overview/databases/sql-server#type-mapping-between-microsoft-sql-server-to-prisma-schema)

## The `prisma db pull` command

You can introspect your database using the `prisma db pull` command of the [Prisma CLI](/docs/v7/orm/tools/prisma-cli#installation). Note that using this command requires your [connection URL](/docs/v7/orm/reference/connection-urls) to be set in your Prisma config [`datasource`](/docs/v7/orm/reference/prisma-config-reference#datasourceurl).

Here's a high-level overview of the steps that `prisma db pull` performs internally:

1. Read the [connection URL](/docs/v7/orm/reference/connection-urls) from the `datasource` configuration in the Prisma config
...
- [Naming models](/docs/v7/orm/reference/prisma-schema-reference#naming-conventions)
- [Naming fields](/docs/v7/orm/reference/prisma-schema-reference#naming-conventions-1)
- [Naming enums](/docs/v7/orm/reference/prisma-schema-reference#naming-conventions-2)
...
Prisma ORM translates foreign keys that are defined on your database tables into [relations](/docs/v7/orm/prisma-schema/data-model/relations).

#### One-to-one relations

Prisma ORM adds a [one-to-one](/docs/v7/orm/prisma-schema/data-model/relations/one-to-one-relations) relation to your data model when the foreign key on a table has a `UNIQUE` constraint, e.g.:
...
By default, Prisma ORM adds a [one-to-many](/docs/v7/orm/prisma-schema/data-model/relations/one-to-many-relations) relation to your data model for a foreign key it finds in your database schema:
...
[Many-to-many](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations) relations are commonly represented as [relation tables](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#relation-tables) in relational databases.

Prisma ORM supports two ways for defining many-to-many relations in the Prisma schema:

- [Implicit many-to-many relations](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations) (Prisma ORM manages the relation table under the hood)
- [Explicit many-to-many relations](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#explicit-many-to-many-relations) (the relation table is present as a [model](/docs/v7/orm/prisma-schema/data-model/models#defining-models))

_Implicit_ many-to-many relations are recognized if they adhere to Prisma ORM's [conventions for relation tables](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#conventions-for-relation-tables-in-implicit-m-n-relations). Otherwise the relation table is rendered in the Prisma schema as a model (therefore making it an _explicit_ many-to-many relation).

This topic is covered extensively on the docs page about [Relations](/docs/v7/orm/prisma-schema/data-model/relations).

#### Disambiguating relations

Prisma ORM generally omits the `name` argument on the [`@relation`](/docs/v7/orm/prisma-schema/data-model/relations#the-relation-attribute) attribute if it's not needed. Consider the `User` ↔ `Post` example from the previous section. The `@relation` attribute only has the `references` argument, `name` is omitted because it's not needed in this case:
...
In this case, Prisma ORM needs to [disambiguate the relation](/docs/v7/orm/prisma-schema/data-model/relations#disambiguating-relations) using a dedicated relation name:
...
Note that you can rename the [Prisma-ORM level](/docs/v7/orm/prisma-schema/data-model/relations#relation-fields) relation field to anything you like so that it looks friendlier in the generated Prisma Client API.
...
If your goal is to exclude certain models from the [Prisma Client generation](/docs/v7/orm/prisma-client/setup-and-configuration/generating-prisma-client), you can add the [`@@ignore` attribute](/docs/v7/orm/reference/prisma-schema-reference#ignore-1) to the model definition in your Prisma schema. Ignored models are excluded from the generated Prisma Client.

## Introspection warnings for unsupported features

The Prisma Schema Language (PSL) can express a majority of the database features of the [target databases](/docs/v7/orm/reference/supported-databases) Prisma ORM supports. However, there are features and functionality the Prisma Schema Language still needs to express.
