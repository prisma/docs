...
If you want to group custom functions together without hiding existing functionality, use `Object.assign` to extend Prisma Client:
...
Now you can use your custom `signup` method alongside `count`, `updateMany`, `groupBy()` and the other methods that Prisma Client provides, and the result stays type-safe.
