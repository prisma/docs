...
Be careful when deeply nesting relations because the underlying database query may become slow due to it needing to access a lot of different tables. To keep these queries fast, consider adding a caching layer with [Prisma Accelerate](/accelerate), or use [Query Insights](/postgres/database/query-insights) to identify slow queries and optimize them.
