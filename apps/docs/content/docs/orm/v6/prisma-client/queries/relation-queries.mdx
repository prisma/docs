...
Since version [5.8.0](https://github.com/prisma/orm/releases/tag/5.8.0), you can decide on a per-query-level _how_ you want Prisma Client to execute a relation query (i.e. what _load strategy_ should be applied) via the `relationLoadStrategy` option for PostgreSQL databases.

Since version [5.10.0](https://github.com/prisma/orm/releases/tag/5.10.0), this feature is also available for MySQL.
...
In [3.0.1](https://github.com/prisma/orm/releases/3.0.1) and later, you can [`include` or `select` a count of relations](/orm/v6/prisma-client/queries/aggregation-grouping-summarizing#count-relations) alongside fields - for example, a user's post count.
