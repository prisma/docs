...
**Use interactive transactions with caution**. Keeping transactions open for a long time hurts database performance and can even cause deadlocks. Try to avoid performing network requests and executing slow queries inside your transaction functions. Keep the transaction as short as possible.
...
During the transfer, you must make sure that:

- the amount doesn't disappear
- the amount isn't doubled

Interactive transactions fit this case because you need to run logic between the writes to check the balance.
...
transaction functions. Keep the transaction as short as possible.
