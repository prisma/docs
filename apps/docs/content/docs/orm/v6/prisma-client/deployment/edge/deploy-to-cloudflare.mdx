...
Use the [Prisma Postgres serverless driver](/postgres/database/serverless-driver) to connect from a Cloudflare Worker over HTTP and WebSockets. This path requires Prisma ORM 6.19 or later.

Configure an engine-less Prisma Client:
...
Install the Prisma Postgres adapter and driver:

```npm title="Terminal"
npm install @prisma/adapter-ppg @prisma/ppg
```

In the [Prisma Console](https://console.prisma.io), select your database, choose **Connect to your database**, generate a connection string, and store the direct value in the Worker's `DATABASE_URL` secret. The serverless driver accepts the direct connection-string format as a credential but communicates over HTTP and WebSockets rather than TCP.

Instantiate Prisma Client with `PrismaPostgresAdapter`:
...
Generate Prisma Client and configure the development and production secrets as [shown in this section](/orm/v6/prisma-client/deployment/edge/deploy-to-cloudflare#development). Keep database credentials in Worker secrets and never include them in browser-delivered code.
...
If your application uses Prisma Postgres, use the [Prisma Postgres serverless driver](/postgres/database/serverless-driver). For another database, choose a driver that supports your edge runtime and use its Prisma driver adapter.
...
Cloudflare has a [size limit of 3 MB for Workers on the free plan](https://developers.cloudflare.com/workers/platform/limits/). If your application bundle with Prisma ORM exceeds that size, use an engine-less Prisma Client with a compatible driver adapter or upgrade to a plan with a larger limit.
...
If you find that this has changed, let us know on [Discord](https://pris.ly/discord?utm_source=docs&utm_medium=inline_text).
