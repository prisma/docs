...
Sevalla is a developer-focused PaaS platform for deploying applications and servers. It hosts applications, databases, object storage and static sites.
...
Next, select your Git repository. Sevalla supports deploying from both private and public repositories. If you're using our example project, you don't need to fork it. Enter the URL `https://github.com/sevalla-templates/express-prisma-demo`.
...
On your Sevalla dashboard, click **Databases** \> **Add database**. Select **PostgreSQL** (or your preferred configured database type), and then provide a database name, username, and password, or keep the generated defaults.

![Create database interface in Sevalla](/img/v6/orm/prisma-client/deployment/traditional/images/sevalla-create-database.png)

Next, give the database a name you will recognize in your Sevalla dashboard. Ensure you select the **same region** as your application, choose an appropriate database size, and click **Create**.
...
You have deployed a Node.js application using Prisma ORM to Sevalla, with the app connected to its database over Sevalla's private network.
