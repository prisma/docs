...
The `metrics` preview feature has been **removed** as of [Prisma ORM v7.0.0](https://github.com/prisma/orm/releases/tag/7.0.0).

If you need visibility into query or connection pool behavior, we recommend using the _native metrics provided by your database driver_ (for example, pool statistics) or setting up _OpenTelemetry_ for observability across your whole application.
