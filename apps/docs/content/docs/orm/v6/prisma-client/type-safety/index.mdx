...
> **Note**: For more on advanced type safety with Prisma ORM, see this [blog post](https://www.prisma.io/blog/satisfies-operator-ur8ys8ccq7zb) about improving your Prisma Client workflows with the new TypeScript `satisfies` keyword.
...
Prisma Client provides a set of type utilities that tap into input and output types. These types are fully dynamic, which means that they adapt to any given model and schema. You can use them to improve type safety and auto-completion in your projects.
...
For example, you can enforce that the arguments to a function match what you will pass to `post.create`:
