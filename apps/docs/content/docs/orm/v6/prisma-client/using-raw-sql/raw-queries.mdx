...
With Prisma ORM `5.19.0`, we have released [TypedSQL](/orm/v6/prisma-client/using-raw-sql). TypedSQL lets you write SQL queries in `.sql` files and call them as type-safe functions.
...
In this example which is safe ✅ from SQL Injection, the `sql` helper method is used to build the query text including a parameter marker for the input value. Each variable is represented by a marker symbol (`?` for MySQL, `$1`, `$2`, and so on for PostgreSQL). The examples show PostgreSQL queries.
...
As an alternative to tagged templates, `$queryRawUnsafe` supports standard parameterized queries where each variable is represented by a symbol (`?` for MySQL, `$1`, `$2`, and so on for PostgreSQL). The examples show PostgreSQL queries.
