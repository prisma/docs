...
1. Add the following `generator` definition to your Prisma schema:
   ...
   You can change the output location to match your application. Common directories are `prisma`, `src`, or even the root of your project.
...
For improved compatibility with ECMAScript modules (ESM) and to ensure consistent behaviour of Prisma ORM across different Node.js runtimes, you can also use the newer [`prisma-client`](/orm/v6/prisma-schema/overview/generators#prisma-client) generator. This generator is built to handle module resolution and runtime differences, so it needs less configuration to work with bundlers.

:::

## Loading environment variables

To load environment variables in your Prisma application, you can use the `prisma.config.ts` file along with the `env` helper from `prisma/config`.
