...
Use the Rust-free version of Prisma ORM with hosted [Prisma Postgres](/postgres) through the [Prisma Postgres serverless driver](/postgres/database/serverless-driver). This requires `@prisma/adapter-ppg` instead of the Accelerate extension.
...
:::warning[Existing Accelerate integrations]

Hosted Accelerate connections will be retired on December 1, 2026. Existing Prisma Postgres applications using `prisma+postgres://accelerate.prisma-data.net` should follow [Connect to Prisma Postgres without Accelerate](/postgres/database/switch-from-accelerate). For other databases, follow [Keep your existing database](/accelerate/keep-your-database).
