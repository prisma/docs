...
The command output contains instructions on how to use the generated files and begin using Prisma ORM with your project.
...
The `dev` command starts a [local Prisma Postgres](/local-development/postgres) database that you can run Prisma ORM commands against. It is useful for development and testing, and lets you move to [Prisma Postgres](/postgres) in production later.

### Arguments

| Argument              | Required | Description                                                                                                                                                 | Default |
| --------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `--name` (or `-n`)    | No       | Enables targeting a specific database instance. [Learn more](/local-development/postgres#using-different-local-prisma-postgres-instances). |         |
...
Prisma ORM v7 introduces a new version of Prisma Studio.
...
The [`create-db`](https://create-db.prisma.io/) command provisions a temporary [Prisma Postgres](/postgres) database with a single command. This is a standalone utility that can be invoked using `npx`. It's useful for testing, prototyping, or trying out Prisma Postgres.
