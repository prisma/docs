...
There's a Turso preview feature that integrates with Prisma CLI workflows. See the [GitHub discussion](https://github.com/prisma/orm/discussions/21345) and share feedback there.
::::
...
With embedded replicas, read queries are served from the local copy, so they return faster than a round trip to the remote database.
