---
title: Supabase
description: Learn how to use Prisma ORM with Supabase PostgreSQL
url: /orm/v6/overview/databases/supabase
metaTitle: Supabase
...
[Supabase](https://supabase.com/) is a PostgreSQL hosting service and open source Firebase alternative. Unlike Firebase, Supabase is backed by PostgreSQL, which you can access directly using Prisma ORM.
...
We strongly recommend using connection pooling with Supavisor in addition to `DIRECT_URL`. The Prisma CLI keeps using the direct connection string from `prisma.config.ts`, while your application's connections are pooled regardless of your deployment strategy. While this is not strictly necessary for every app, serverless solutions will inevitably require connection pooling.

:::

## Getting started with Supabase

Supabase has a guide for connecting a Supabase database to your Prisma project [here](https://supabase.com/partners/integrations/prisma).

If you're running into issues integrating with Supabase, check out these [specific troubleshooting tips](https://supabase.com/partners/integrations/prisma) or [Prisma's GitHub Discussions](https://github.com/prisma/orm/discussions) for more help.
