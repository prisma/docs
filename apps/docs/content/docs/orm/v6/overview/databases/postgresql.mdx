...
url: /orm/v6/overview/databases/postgresql
...
If you need a PostgreSQL database, [Prisma Postgres](https://www.prisma.io/postgres?utm_source=docs&utm_campaign=postgresql) gives you one running on bare-metal in three clicks, with connection pooling, query caching, and automated backups included. [Visit the Console](https://console.prisma.io?utm_source=docs&utm_campaign=postgresql) to create one.

To create a Prisma Postgres database from your terminal instead, run `npx prisma init --db`.
...
As of [`v5.4.0`](https://github.com/prisma/orm/releases/tag/5.4.0), you can use Prisma ORM with database drivers from the JavaScript ecosystem (instead of using Prisma ORM's built-in drivers). You can do this by using a [driver adapter](/orm/v6/overview/databases/database-drivers).
