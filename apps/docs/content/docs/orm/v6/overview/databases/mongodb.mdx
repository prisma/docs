...
codeStyle: false
url: /orm/v6/overview/databases/mongodb
...
These iterative tasks of updating the schema and the database can result in inconsistencies between your schema and the actual data in the database. The following scenario shows how this can happen, followed by several strategies your team can use to handle these inconsistencies.
...
The MongoDB database connector uses transactions to support nested writes. Transactions **require** a [replica set](https://www.mongodb.com/docs/manual/tutorial/deploy-replica-set/) deployment. One way to deploy a replica set is with [Atlas](https://www.mongodb.com/docs/atlas/getting-started/), which has a free tier.
...
One way to do this is to use [MongoDB Atlas](https://www.mongodb.com/cloud/atlas) to launch a free instance that has replica set support out of the box.
