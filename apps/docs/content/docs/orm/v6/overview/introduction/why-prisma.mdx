...
Prisma ORM gives you a type-safe API for submitting database queries that returns _plain old JavaScript objects_, so query results are predictable and checked at compile time.
