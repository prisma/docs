---
title: Reading data
description: 'Fetch one record or many with Prisma 8, then filter, select, sort, paginate, and iterate the results.'
url: /orm/fundamentals/reading-data
metaTitle: Reading data with Prisma 8
metaDescription: 'Query PostgreSQL and MongoDB with Prisma 8. Filter with where, project with select, sort with orderBy, paginate with limit and offset, and iterate large results one record at a time.'
---

This page shows how to read data with Prisma 8: fetching [many records or one](#fetch-many-records-or-one), [filtering](#filter-records), [selecting fields](#select-fields), [sorting and paginating](#sort-and-paginate), [counting](#count-records), and [iterating large results](#iterate-a-large-result).
