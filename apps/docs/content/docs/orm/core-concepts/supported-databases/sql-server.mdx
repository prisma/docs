...
url: /docs/orm/core-concepts/supported-databases/sql-server
...
Use the `node-mssql` JavaScript database driver via [driver adapters](/docs/orm/core-concepts/supported-databases/database-drivers#driver-adapters):
...
See [connection pool guide](/docs/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool#sql-server-using-the-mssql-driver) for configuration.
...
With circular model references, you must use [`NoAction` referential actions](/docs/orm/prisma-schema/data-model/relations/referential-actions#special-rules-for-sql-server-and-mongodb) to avoid validation errors.
