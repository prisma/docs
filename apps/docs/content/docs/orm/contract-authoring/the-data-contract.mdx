...
The first line of every `.prisma` contract is `// use prisma-next`. It marks the file as a Prisma 8 contract rather than a Prisma 7 schema: the Prisma language server only offers diagnostics, completion, and formatting on files that carry it, and leaves unmarked files to Prisma 7.
