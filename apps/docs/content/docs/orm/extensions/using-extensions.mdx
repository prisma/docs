...
To add an extension, install its package and register it in two places: the config and the client. The steps below use pgvector, the vector search extension, as the example. Supabase is wired differently; see [the note on the catalog](#available-extensions).
...
pgvector contributes `fns.cosineSimilarity` alongside `fns.cosineDistance`. Both take two vector expressions and return a `float8`; similarity sorts descending where distance sorts ascending.
...
| --- | --- | --- |
| [pgvector](https://www.npmjs.com/package/@prisma/orm-extension-pgvector) | Vector columns and similarity search | `@prisma/orm-extension-pgvector` |
| [PostGIS](https://www.npmjs.com/package/@prisma/orm-extension-postgis) | Geometry columns and geo queries | `@prisma/orm-extension-postgis` |
| [ParadeDB](https://www.npmjs.com/package/@prisma/orm-extension-paradedb) | BM25 full-text search indexes | `@prisma/orm-extension-paradedb` |
| [Supabase](https://www.npmjs.com/package/@prisma/orm-extension-supabase) | Supabase auth and storage tables, role-bound clients | `@prisma/orm-extension-supabase` |
| [arktype-json](https://www.npmjs.com/package/@prisma/orm-extension-arktype-json) | JSON columns validated by an arktype schema | `@prisma/orm-extension-arktype-json` |

All target PostgreSQL. ParadeDB and Supabase are experimental (ParadeDB supports the `key_field` index option only so far). The rest ship with Prisma 8. Extension names link to each package on npm.

:::note[Supabase does not follow the recipe above]

Supabase ships no `/control` subpath, so it cannot be registered through `ormConfig({ extensions: [...] })`. Register the pack from `@prisma/orm-extension-supabase/pack` as `extensions: [supabasePack]` in the low-level config instead, and build the client with the `supabase()` factory from `@prisma/orm-extension-supabase/runtime` rather than `postgres()`. That factory is role-first (`asUser(jwt)`, `asAnon()`, `asServiceRole()`) and carries the JWT validation and RLS binding. See the [runnable example](https://github.com/prisma/orm/tree/main/examples/supabase) for the whole wiring.
