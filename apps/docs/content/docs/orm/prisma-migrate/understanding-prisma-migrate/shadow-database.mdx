...
[`migrate diff` command](/docs/v7/orm/reference/prisma-cli-reference#migrate-diff) also requires a shadow database when diffing against a local `migrations` directory with `--from-migrations` or `--to-migrations`.
...
1. Creates a fresh copy of the shadow database (or performs a soft reset if the shadow database is configured via [`shadowDatabaseUrl`](/docs/v7/orm/reference/prisma-schema-reference#datasource))
...
1. Drops the shadow database (shadow databases configured via [`shadowDatabaseUrl`](/docs/v7/orm/reference/prisma-schema-reference#datasource) are not dropped, but are reset at the start of the `migrate dev` command)
...
<Tabs items={["Prisma 7 (prisma.config.ts)", "Prisma 6 and below (schema.prisma)"]}>
<Tab value="Prisma 7 (prisma.config.ts)">
...
<Tab value="Prisma 6 and below (schema.prisma)">
