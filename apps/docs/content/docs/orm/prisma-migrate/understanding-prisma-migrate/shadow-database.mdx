...
url: /docs/orm/prisma-migrate/understanding-prisma-migrate/shadow-database
...
[`migrate diff` command](/docs/orm/reference/prisma-cli-reference#migrate-diff) also requires a shadow database when diffing against a local `migrations` directory with `--from-migrations` or `--to-migrations`.
...
- Creates a fresh copy of the shadow database (or performs a soft reset if the shadow database is configured via [`shadowDatabaseUrl`](/docs/orm/reference/prisma-schema-reference#datasource))
...
1. Drops the shadow database (shadow databases configured via [`shadowDatabaseUrl`](/docs/orm/reference/prisma-schema-reference#datasource) are not dropped, but are reset at the start of the `migrate dev` command)
