...
- **legacy Prisma Migrate (declarative)**: Define the desired schema as a [Prisma schema data model](/docs/v7/orm/prisma-schema/data-model/models) (legacy Prisma Migrate takes care of generating the necessary _steps_).
...
With legacy Prisma Migrate, you write the desired database schema in the form of a [Prisma schema data model](/docs/v7/orm/prisma-schema/data-model/models) inside your [Prisma schema file](/docs/v7/orm/prisma-schema). To map the data model to your database schema, you then have to run these two commands:
...
Add two models with [relation fields](/docs/v7/orm/prisma-schema/data-model/relations#relation-fields) to your Prisma schema:
...
Notice that in addition to the [annotated relation fields](/docs/v7/orm/prisma-schema/data-model/relations#annotated-relation-fields) and its relation scalar field (which represent the foreign keys), you must also specify the Prisma-level [relation fields](/docs/v7/orm/prisma-schema/data-model/relations#relation-fields) on the other side of the relation.
...
Add a [field](/docs/v7/orm/prisma-schema/data-model/models#defining-fields) to the `Post` model:
...
| Data validation                   | `CHECK`                         | No ([workaround](/docs/v7/orm/more/help-and-troubleshooting/check-constraints)) |
...
Before using the `migrate save` command, you must define a valid [`datasource`](/docs/v7/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
...
Before using the `migrate up` command, you must define a valid [`datasource`](/docs/v7/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
...
Before using the `migrate down` command, you must define a valid [`datasource`](/docs/v7/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
