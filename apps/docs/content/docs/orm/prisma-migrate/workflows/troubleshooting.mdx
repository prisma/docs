...
url: /docs/orm/prisma-migrate/workflows/troubleshooting
...
- [Patching / hotfixing production databases](/docs/orm/prisma-migrate/workflows/patching-and-hotfixing)
...
:::info[Note]
You should [never purposefully delete or edit a migration](/docs/orm/prisma-migrate/understanding-prisma-migrate/migration-histories#do-not-edit-or-delete-migrations-that-have-been-applied), as this might result in discrepancies between development and production.
:::
...
- The database schema was changed _without_ using migrations - for example, by using [`prisma db push`](/docs/orm/reference/prisma-cli-reference#db-push) or manually changing the database schema.

:::info[Note]
The [shadow database](/docs/orm/prisma-migrate/understanding-prisma-migrate/shadow-database) is required to detect schema drift, and can therefore only be done in a development environment.
:::
...
- You [modify a migration before running it](/docs/orm/prisma-migrate/workflows/customizing-migrations) and introduce a syntax error
...
See [Prisma Migrate and PgBouncer workaround](/docs/orm/prisma-client/setup-and-configuration/databases-connections/pgbouncer) for further information and a workaround.
