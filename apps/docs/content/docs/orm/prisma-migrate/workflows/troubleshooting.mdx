...
- [Patching / hotfixing production databases](/docs/v7/orm/prisma-migrate/workflows/patching-and-hotfixing)

:::warning

This guide **does not apply for MongoDB**.<br />
Instead of `migrate dev`, [`db push`](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/docs/v7/orm/overview/databases/mongodb).
...
> **Note**: You should [never purposefully delete or edit a migration](/docs/v7/orm/prisma-migrate/understanding-prisma-migrate/migration-histories#do-not-edit-or-delete-migrations-that-have-been-applied), as this might result in discrepancies between development and production.
...
- The database schema was changed _without_ using migrations - for example, by using [`prisma db push`](/docs/v7/orm/reference/prisma-cli-reference#db-push) or manually changing the database schema.

> **Note**: The [shadow database](/docs/v7/orm/prisma-migrate/understanding-prisma-migrate/shadow-database) is required to detect schema drift, and can therefore only be done in a development environment.
...
- You [modify a migration before running it](/docs/v7/orm/prisma-migrate/workflows/customizing-migrations) and introduce a syntax error
...
See [Prisma Migrate and PgBouncer workaround](/docs/v7/orm/prisma-client/setup-and-configuration/databases-connections/pgbouncer) for further information and a workaround.
