...
The Prisma CLI has a dedicated command for prototyping schemas: [`db push`](/docs/v7/orm/reference/prisma-cli-reference#db-push)
...
> - When working with PlanetScale, we recommend that you use `db push` instead of `migrate`. For details refer to our Getting started documentation, either [Start from scratch guide](/docs/v7/getting-started/prisma-orm/quickstart/planetscale) or [Add to existing project guide](/docs/v7/getting-started/prisma-orm/add-to-existing-project/planetscale) depending on your situation.
...
See [Schema prototyping with `db push`](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) for an example of how to use `db push` in this way.
...
- You want fine-grained control over how the schema changes are executed - for example, [renaming a column instead of dropping it and creating a new one](/docs/v7/orm/prisma-migrate/workflows/customizing-migrations#example-rename-a-field).
...
Yes, you can [use `db push` and Prisma Migrate together in your development workflow](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) . For example, you can:
