...
This guide describes how to generate a down migration SQL file that reverses a given [migration file](/docs/v7/orm/prisma-migrate/understanding-prisma-migrate/migration-histories).
...
This guide explains how to use Prisma Migrate's [`migrate diff` command](/docs/v7/orm/reference/prisma-cli-reference#migrate-diff) to create a down migration, and how to apply it to your production database with the [`db execute`](/docs/v7/orm/reference/prisma-cli-reference#db-execute) command in the case of a failed up migration.
...
- You will not be able to use `migrate diff` to revert manually changed or added SQL in your migration files. If you have any custom additions, such as a view or trigger, you will need to:
  ...
  - Create the up migration using [`migrate dev --create-only`](/docs/v7/orm/reference/prisma-cli-reference#options-6), so that it can be edited before it is applied to the database
