...
url: /docs/orm/prisma-client/deployment/deploy-database-changes-with-prisma-migrate
...
Instead of `migrate deploy`, [`db push`](/docs/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/docs/orm/core-concepts/supported-databases/mongodb).

:::

Exactly when to run `prisma migrate deploy` depends on your platform. For example, a simplified [Heroku](/docs/orm/prisma-client/deployment/traditional/deploy-to-heroku) workflow includes:
...
- [`migrate deploy` reference](/docs/orm/reference/prisma-cli-reference#migrate-deploy)
- [How `migrate deploy` works](/docs/orm/prisma-migrate/workflows/development-and-production#production-and-testing-environments)
