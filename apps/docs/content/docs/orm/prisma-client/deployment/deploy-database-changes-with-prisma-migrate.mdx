...
Instead of `migrate deploy`, [`db push`](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/docs/v7/orm/overview/databases/mongodb).

:::

Exactly when to run `prisma migrate deploy` depends on your platform. For example, a simplified [Heroku](/docs/v7/orm/prisma-client/deployment/traditional/deploy-to-heroku) workflow includes:
...
- [`migrate deploy` reference](/docs/v7/orm/reference/prisma-cli-reference#migrate-deploy)
- [How `migrate deploy` works](/docs/v7/orm/prisma-migrate/workflows/development-and-production#production-and-testing-environments)
