...
url: /docs/orm/prisma-client/deployment/serverless/deploy-to-vercel
...
In a more sophisticated CI/CD environment, you may additionally want to update the database schema with any migrations you have performed during local development. You can do this using the [`prisma migrate deploy`](/docs/orm/reference/prisma-cli-reference#migrate-deploy) command.
...
You can use [Prisma Postgres](/docs/postgres), which has built-in connection pooling, to reduce your Prisma Client bundle size, and to avoid cold starts.

For more information on connection management for serverless environments, refer to our [connection management guide](/docs/orm/prisma-client/setup-and-configuration/databases-connections#serverless-environments-faas).
