...
You can use Prisma Postgres in Vercel's edge runtime. Follow this guide for an end-to-end tutorial on [deploying an application to Vercel using Prisma Postgres](/docs/v7/guides/frameworks/nextjs).
...
When in **development**, you can configure your database connection via the `DATABASE_URL` environment variable (e.g. [using `.env` files](/docs/v7/orm/more/development-environment/environment-variables)).
