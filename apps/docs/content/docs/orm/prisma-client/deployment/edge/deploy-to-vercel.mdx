...
url: /docs/orm/prisma-client/deployment/edge/deploy-to-vercel
...
You can use Prisma Postgres in Vercel's edge runtime. Follow this guide for an end-to-end tutorial on [deploying an application to Vercel using Prisma Postgres](/docs/guides/frameworks/nextjs).
...
When in **development**, you can configure your database connection via the `DATABASE_URL` environment variable (e.g. [using `.env` files](/docs/orm/more/dev-environment/environment-variables)).
...
Use `attachDatabasePool` together with [Prisma's driver adapters](/docs/orm/core-concepts/supported-databases/database-drivers) to safely manage connections in Fluid:
