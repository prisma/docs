...
With this guide, you can learn how to build and deploy a simple application to [Deno Deploy](https://deno.com/deploy). The application uses Prisma ORM to save a log of each request to a [Prisma Postgres](/docs/v7/postgres) database.
...
Edit the `prisma/schema.prisma` file to define a `Log` model, add a custom `output` path and [the `prisma-client` generator](/docs/v7/orm/prisma-schema/overview/generators#prisma-client) with `deno` as the `runtime`:
