...
url: /docs/orm/prisma-client/deployment/edge/deploy-to-cloudflare
...
Then setup helper scripts to perform migrations and generate `PrismaClient` as [shown in this section](/docs/orm/prisma-client/deployment/edge/deploy-to-cloudflare#development).
...
- [Cloudflare D1](/docs/orm/prisma-client/deployment/edge/deploy-to-cloudflare) is used to access D1 databases
...
If your application uses PostgreSQL, we recommend using [Prisma Postgres](/docs/postgres). It is fully supported on edge runtimes and does not require a specialized edge-compatible driver. For other databases, [Prisma Accelerate](/docs/accelerate) extends edge compatibility so you can connect to _any_ database from _any_ edge function provider.
...
- use the `@prisma/adapter-pg` database adapter (learn more [here](/docs/orm/core-concepts/supported-databases/postgresql#using-driver-adapters))
...
- use the `@prisma/adapter-planetscale` database adapter (learn more [here](/docs/orm/core-concepts/supported-databases/mysql#planetscale))
...
- use the `@prisma/adapter-neon` database adapter (learn more [here](/docs/orm/core-concepts/supported-databases/postgresql#using-driver-adapters))
...
For step-by-step instructions on using Prisma ORM with [Cloudflare D1](https://developers.cloudflare.com/d1/) (schema setup, migrations, and deploying your Worker), see the dedicated [Cloudflare D1 deployment guide](/docs/guides/deployment/cloudflare-d1).
:::
