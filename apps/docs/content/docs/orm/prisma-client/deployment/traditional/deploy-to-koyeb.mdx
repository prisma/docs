...
url: /docs/orm/prisma-client/deployment/traditional/deploy-to-koyeb
...
In this guide, you will create the database schema with [Prisma Migrate](/docs/orm/prisma-migrate) to create the database schema. Prisma Migrate is based on the Prisma schema and works by generating `.sql` migration files that are executed against the database.
