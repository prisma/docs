...
In most cases, [TypedSQL](#writing-type-safe-queries-with-prisma-client-and-typedsql) allows you to express your query in SQL while still benefiting from Prisma Client's excellent user experience. However, since TypedSQL is statically typed, it may not handle certain scenarios, such as dynamically generated `WHERE` clauses. In these cases, you will need to use [`$queryRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#queryraw) or [`$executeRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#executeraw), or their unsafe counterparts.

## Writing type-safe queries with Prisma Client and TypedSQL

:::info

TypedSQL is available in Prisma ORM 5.19.0 and later. For raw database access in previous versions, see [our raw queries documentation](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries).
...
For a detailed guide on how to get started with TypedSQL, including setup instructions and usage examples, please refer to our [TypedSQL documentation](/docs/v7/orm/prisma-client/using-raw-sql/typedsql).
...
- [`$queryRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#queryraw)
- [`$executeRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#executeraw)
- [`$queryRawUnsafe`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#queryrawunsafe)
- [`$executeRawUnsafe`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#executerawunsafe)
...
- [`$runCommandRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#runcommandraw)
- [`<model>.findRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#findraw)
- [`<model>.aggregateRaw`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#aggregateraw)
