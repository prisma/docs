...
url: /docs/orm/prisma-client/queries/relation-queries
...
A key feature of Prisma Client is the ability to query [relations](/docs/orm/prisma-schema/data-model/relations) between two or more models. Relation queries include:

- [Nested reads](#nested-reads) (sometimes referred to as _eager loading_) via [`select`](/docs/orm/reference/prisma-client-reference#select) and [`include`](/docs/orm/reference/prisma-client-reference#include)
- [Nested writes](#nested-writes) with [transactional](/docs/orm/prisma-client/queries/transactions) guarantees
...
- Use [`include`](/docs/orm/reference/prisma-client-reference#include) to include related records, such as a user's posts or profile, in the query response.
- Use a nested [`select`](/docs/orm/reference/prisma-client-reference#select) to include specific fields from a related record. You can also nest `select` inside an `include`.
...
In [3.0.1](https://github.com/prisma/prisma/releases/3.0.1) and later, you can [`include` or `select` a count of relations](/docs/orm/prisma-client/queries/aggregation-grouping-summarizing#count-relations) alongside fields - for example, a user's post count.
...
- Are available for [relation fields](/docs/orm/prisma-schema/data-model/relations#relation-fields) when using the model's create or update query. The following section shows the nested write options that are available per query.
...
- Use a nested [`create`](/docs/orm/reference/prisma-client-reference#create) query
- Use a nested [`createMany`](/docs/orm/reference/prisma-client-reference#nested-createmany-options) query

In most cases, a nested `create` will be preferable unless the [`skipDuplicates` query option](/docs/orm/reference/prisma-client-reference#nested-createmany-options) is required. Here's a quick table describing the differences between the two options:
...
The following query uses nested [`create`](/docs/orm/reference/prisma-client-reference#create) to create:
...
The following query uses a nested [`createMany`](/docs/orm/reference/prisma-client-reference#createmany) to create:
...
If you want to create _all_ records in a single database query, consider using a [`$transaction`](/docs/orm/prisma-client/queries/transactions#the-transaction-api) or [type-safe, raw SQL](/docs/orm/prisma-client/using-raw-sql/typedsql).
...
The following query creates ([`create`](/docs/orm/reference/prisma-client-reference#create) ) a new `User` record and connects that record ([`connect`](/docs/orm/reference/prisma-client-reference#connect) ) to three existing posts:
...
You can [`connect`](/docs/orm/reference/prisma-client-reference#connect) an existing record to a new or existing user. The following query connects an existing post (`id: 11`) to an existing user (`id: 9`)
...
If a related record may or may not already exist, use [`connectOrCreate`](/docs/orm/reference/prisma-client-reference#connectorcreate) to connect the related record:
...
To [`disconnect`](/docs/orm/reference/prisma-client-reference#disconnect) _all_ related records in a one-to-many relation (a user has many posts), `set` the relation to an empty list as shown:
...
Prisma Client provides the [`some`](/docs/orm/reference/prisma-client-reference#some), [`every`](/docs/orm/reference/prisma-client-reference#every), and [`none`](/docs/orm/reference/prisma-client-reference#none) options to filter records by the properties of related records on the "-to-many" side of the relation. For example, filtering users based on properties of their posts.
...
Prisma Client provides the [`is`](/docs/orm/reference/prisma-client-reference#is) and [`isNot`](/docs/orm/reference/prisma-client-reference#isnot) options to filter records by the properties of related records on the "-to-one" side of the relation. For example, filtering posts based on properties of their author.
...
The fluent API lets you _fluently_ traverse the [relations](/docs/orm/prisma-schema/data-model/relations) of your models via function calls. Note that the _last_ function call determines the return type of the entire query (the respective type annotations are added in the code snippets below to make that explicit).
