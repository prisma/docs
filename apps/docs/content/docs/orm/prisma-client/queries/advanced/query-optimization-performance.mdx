...
url: /docs/orm/prisma-client/queries/advanced/query-optimization-performance
...
[Prisma Optimize](/docs/optimize) provides [recommendations](/docs/optimize/recommendations) to address these issues. Follow the [integration guide](/docs/optimize/getting-started) to get started.
...
- [`createMany()`](/docs/orm/reference/prisma-client-reference#createmany)
- [`createManyAndReturn()`](/docs/orm/reference/prisma-client-reference#createmanyandreturn)
- [`deleteMany()`](/docs/orm/reference/prisma-client-reference#deletemany)
- [`updateMany()`](/docs/orm/reference/prisma-client-reference#updatemany)
- [`updateManyAndReturn()`](/docs/orm/reference/prisma-client-reference#updatemanyandreturn)
- [`findMany()`](/docs/orm/reference/prisma-client-reference#findmany)

## Reuse `PrismaClient` or use connection pooling to avoid database connection pool exhaustion

Creating multiple instances of `PrismaClient` can exhaust your database connection pool, especially in serverless or edge environments, potentially slowing down other queries. Learn more in the [serverless challenge](/docs/orm/prisma-client/setup-and-configuration/databases-connections#the-serverless-challenge).
...
For serverless development environments with frameworks that use HMR (Hot Module Replacement), ensure you properly handle a [single instance of Prisma in development](/docs/orm/more/troubleshooting/nextjs#best-practices-for-using-prisma-client-in-development).
...
Use `findUnique()` in combination with [the fluent API](/docs/orm/prisma-client/queries/relation-queries#fluent-api) (`.posts()`) as shown to return a user's posts. Even though the resolver is called once per user, the Prisma dataloader in Prisma Client **✔ batches the `findUnique()` queries**.
...
You can perform the query with a [database join](/docs/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) by setting `relationLoadStrategy` to `"join"`, ensuring that only **one** query is executed against the database.
...
- Use nested reads ([`include`](/docs/orm/reference/prisma-client-reference#include) ) to return users and related posts
- Use the [`in`](/docs/orm/reference/prisma-client-reference#in) filter
- Set the [`relationLoadStrategy`](/docs/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) to `"join"`

#### Solving n+1 with `include`

You can use `include` to return each user's posts. This only results in **two** SQL queries - one to get users, and one to get posts. This is known as a [nested read](/docs/orm/prisma-client/queries/relation-queries#nested-reads).
