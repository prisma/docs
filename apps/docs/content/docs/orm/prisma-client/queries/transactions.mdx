...
A [nested write](/docs/v7/orm/prisma-client/queries/relation-queries#nested-writes) performs multiple operations on related records in a single transaction:
...
1. On your transaction, use the `isolationLevel` parameter to `Prisma.TransactionIsolationLevel.Serializable`.
   ...
   This ensures that your application commits multiple concurrent or parallel transactions as if they were run serially. When a transaction fails due to a write conflict or deadlock, Prisma Client returns a [P2034 error](/docs/v7/orm/reference/error-reference#p2034).
...
- Since version 5.0.0 you are able to [filter on non-unique fields in `update` operations](/docs/v7/orm/reference/prisma-client-reference#filter-on-non-unique-fields-with-userwhereuniqueinput) so that optimistic concurrency control is being used. The feature was also available via the Preview flag `extendedWhereUnique` from versions 4.5.0 to 4.16.2.
