...
See the [Prisma Client API reference](/docs/v7/orm/reference/prisma-client-reference) for detailed method documentation.
...
:::info
Available in Prisma ORM 5.14.0+ for PostgreSQL, CockroachDB, and SQLite.
:::
...
See [Nested writes](/docs/v7/orm/prisma-client/queries/relation-queries#nested-writes) for creating records with relations.
...
See [Filtering and sorting](/docs/v7/orm/prisma-client/queries/filtering-and-sorting) for more examples.
...
See [Select fields](/docs/v7/orm/prisma-client/queries/select-fields) and [Relation queries](/docs/v7/orm/prisma-client/queries/relation-queries) for more.
...
:::info
Available in Prisma ORM 6.2.0+ for PostgreSQL, CockroachDB, and SQLite.
:::
...
See [Relation queries](/docs/v7/orm/prisma-client/queries/relation-queries) for connecting and disconnecting related records.

## Delete\n

### Delete a single record

The following query uses [`delete()`](/docs/v7/orm/reference/prisma-client-reference#delete) to delete a single `User` record:
...
The following query uses [`deleteMany()`](/docs/v7/orm/reference/prisma-client-reference#deletemany) to delete all `User` records where `email` contains `prisma.io`:
...
The following query uses [`deleteMany()`](/docs/v7/orm/reference/prisma-client-reference#deletemany) to delete all `User` records:
...
In [2.26.0](https://github.com/prisma/prisma/releases/tag/2.26.0) and later it is possible to do cascading deletes using the **preview feature** [referential actions](/docs/v7/orm/prisma-schema/data-model/relations/referential-actions).
...
When you know the order in which your tables should be deleted, you can use the [`deleteMany`](/docs/v7/orm/reference/prisma-client-reference#deletemany) function. This is executed synchronously in a [`$transaction`](/docs/v7/orm/prisma-client/queries/transactions) and can be used with all types of databases.
...
If you are comfortable working with raw SQL, you can perform a `TRUNCATE` query on a table using [`$executeRawUnsafe`](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries#executerawunsafe).
