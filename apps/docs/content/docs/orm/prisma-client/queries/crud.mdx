...
See the [Prisma Client API reference](/docs/orm/reference/prisma-client-reference) for detailed method documentation.
...
See [Nested writes](/docs/orm/prisma-client/queries/relation-queries#nested-writes) for creating records with relations.
...
See [Filtering and sorting](/docs/v6/orm/prisma-client/queries/filtering-and-sorting) for more examples.
...
See [Select fields](/docs/v6/orm/prisma-client/queries/select-fields) and [Relation queries](/docs/orm/prisma-client/queries/relation-queries) for more.
...
See [Relation queries](/docs/orm/prisma-client/queries/relation-queries) for connecting and disconnecting related records.

## Delete\n

### Delete a single record

The following query uses [`delete()`](/docs/orm/reference/prisma-client-reference#delete) to delete a single `User` record:
...
The following query uses [`deleteMany()`](/docs/orm/reference/prisma-client-reference#deletemany) to delete all `User` records where `email` contains `prisma.io`:
...
The following query uses [`deleteMany()`](/docs/orm/reference/prisma-client-reference#deletemany) to delete all `User` records:
...
You can configure cascading deletes using [referential actions](/docs/orm/prisma-schema/data-model/relations/referential-actions).
...
When you know the order in which your tables should be deleted, you can use the [`deleteMany`](/docs/orm/reference/prisma-client-reference#deletemany) function. This is executed synchronously in a [`$transaction`](/docs/orm/prisma-client/queries/transactions) and can be used with all types of databases.
...
If you are comfortable working with raw SQL, you can perform a `TRUNCATE` query on a table using [`$executeRawUnsafe`](/docs/orm/prisma-client/using-raw-sql/raw-queries#executerawunsafe).
