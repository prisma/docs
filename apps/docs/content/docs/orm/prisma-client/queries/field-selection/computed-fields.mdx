...
The following example illustrates how to create a [Prisma Client extension](/docs/v7/orm/prisma-client/client-extensions) that adds a `fullName` computed field at runtime to the `User` model in a Prisma schema.
...
- Learn how you can use [Prisma Client extensions](/docs/v7/orm/prisma-client/client-extensions) to add a computed field to your schema — [example](https://github.com/prisma/prisma-client-extensions/tree/main/computed-fields).
- Learn how you can move the `computeFullName` function into [a custom model](/docs/v7/orm/prisma-client/queries/custom-models).
