...
url: /docs/orm/prisma-client/observability-and-logging/sql-comments
...
The trace context plugin requires [`@prisma/instrumentation`](/docs/orm/prisma-client/observability-and-logging/opentelemetry-tracing) to be configured. The `traceparent` is only added when tracing is active and the span is sampled.
