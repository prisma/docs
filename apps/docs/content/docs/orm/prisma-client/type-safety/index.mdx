...
url: /docs/orm/prisma-client/type-safety
...
See also: [Using the `Prisma.UserCreateInput` generated type](/docs/orm/prisma-client/queries/crud#create-a-single-record)
...
This query will also result in an error if an author with an `id` of `1` does not exist. In this case, Prisma Client will give a more descriptive error message. You can also use the [`connectOrCreate`](/docs/orm/reference/prisma-client-reference#connectorcreate) API to safely create a new user if one does not already exist with the given `id`.
...
This is especially useful in [shared Prisma Client extensions](/docs/orm/prisma-client/client-extensions/shared-extensions).
