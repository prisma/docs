...
url: /docs/orm/prisma-client/testing/integration-testing
...
See our [system requirements](/docs/orm/reference/system-requirements) for all minimum version requirements.
...
The guide uses a singleton pattern for Prisma Client setup. Refer to the [singleton](/docs/orm/prisma-client/testing/unit-testing#singleton) docs for a walk through of how to set that up.
...
The above `.env.test` file is used as part of a multiple `.env` file setup. Checkout the [using multiple .env files.](/docs/orm/more/dev-environment/environment-variables) section to learn more about setting up your project with multiple `.env` files
...
Before the test suite is run the database is seeded with data. After the test suite has finished a [`deleteMany`](/docs/orm/reference/prisma-client-reference#deletemany) is used to clear the database of its data.
...
However, this doesn't scale as well as having a more generic solution that maps over your models and performs a truncate on them. For those scenarios and examples of using raw SQL queries see [Deleting all data with raw SQL / `TRUNCATE`](/docs/orm/prisma-client/queries/crud#deleting-all-data-with-raw-sql--truncate)
