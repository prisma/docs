...
Three middleware ship with Prisma 8 today. Middleware is an extension like any other, so the table is generated from the [extension directory](https://www.prisma.io/extensions), which lists community middleware as well:
...
To write your own, follow the [authoring guide](/orm/middleware/authoring-custom-middleware): it builds a working query logger step by step. When it is on npm, [submit it to the directory](https://www.prisma.io/extensions/submit) so other Prisma 8 users can find it.
