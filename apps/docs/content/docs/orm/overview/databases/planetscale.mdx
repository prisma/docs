...
- **Enterprise scalability.** PlanetScale provides a highly available production database cluster that supports scaling across multiple database servers. This is particularly useful in a serverless context, as it avoids the problem of having to [manage connection limits](/docs/v7/orm/prisma-client/setup-and-configuration/databases-connections#serverless-environments-faas).
...
- model your database with the [Prisma Schema Language](/docs/v7/orm/prisma-schema)
- use Prisma ORM's existing [`mysql` database connector](/docs/v7/orm/overview/databases/mysql) in your schema, along with the [connection string PlanetScale provides you](https://planetscale.com/docs/concepts/connection-strings)
- use [Introspection](/docs/v7/orm/prisma-schema/introspection) for existing projects if you already have a database schema in PlanetScale
- use [`db push`](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) to push changes in your schema to the database
- use [Prisma Client](/docs/v7/orm/prisma-client) in your application to talk to the database server at PlanetScale
...
If you try to push to a production branch, you will get the [error message](/docs/v7/orm/reference/error-reference#p3022) `Direct execution of DDL (Data Definition Language) SQL statements is disabled on this database.`
...
In Prisma ORM versions 3.1.1 and later, you can [emulate relations in Prisma Client with the `prisma` relation mode](/docs/v7/orm/prisma-schema/data-model/relations/relation-mode#emulate-relations-in-prisma-orm-with-the-prisma-relation-mode), which avoids the need for foreign key constraints in the database.
...
To avoid this, you can define an index on the `postId` field using [Prisma ORM's `@@index` argument](/docs/v7/orm/reference/prisma-schema-reference#index):
...
In versions 4.7.0 and later, Prisma ORM warns you if you have a relation with no index on the relation scalar field. For more information, see [Index validation](/docs/v7/orm/prisma-schema/data-model/relations/relation-mode#index-validation).

:::warning

One issue to be aware of is that [implicit many-to-many relations](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations) cannot have an index added in this way. If query speed or cost is an issue, you may instead want to use an [explicit many-to-many relation](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#explicit-many-to-many-relations) in this case.
...
For a more detailed example, see the [Getting started guide for PlanetScale](/docs/v7/getting-started/prisma-orm/add-to-existing-project/planetscale).
...
As of [v6.10.0](https://github.com/prisma/prisma/releases/tag/6.10.0), Prisma ORM supports sharding on PlanetScale natively (as a [Preview](/docs/v7/orm/more/releases#preview) feature) via the [`@shardKey`](/docs/v7/orm/reference/prisma-schema-reference#shardkey) and [`@@shardKey`](/docs/v7/orm/reference/prisma-schema-reference#shardkey-1) attributes in the Prisma schema which you can apply to the fields in your models that should serve as shard keys in your database setup.
...
- [Start from scratch](/docs/v7/getting-started/prisma-orm/quickstart/planetscale)
- [Add to existing project](/docs/v7/getting-started/prisma-orm/add-to-existing-project/planetscale)
