...
To connect to a Microsoft SQL Server database, you need to configure a [`datasource`](/docs/v7/orm/prisma-schema/overview/data-sources) block in your [Prisma schema](/docs/v7/orm/prisma-schema):
...
| `connectionLimit`                                                                     | No                | `num_cpus * 2 + 1` | Maximum size of the [connection pool](/docs/v7/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool) (Prisma ORM v6 and before)                                                                                                                                                                                                                                                                                                                    |
...
In Prisma ORM v7, [driver adapters](/docs/v7/orm/overview/databases/database-drivers) are the default for relational databases. Connection pooling is now handled by the `mssql` driver, which has **different defaults** than Prisma ORM v6:

- **Connection timeout**: `15s` vs. v6's `5s`
- **Idle timeout**: `30s` vs. v6's `300s`

If you experience timeout issues after upgrading, you may need to configure your driver adapter to match v6 behavior. See the [connection pool guide](/docs/v7/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool#sql-server-using-the-mssql-driver) for detailed configuration examples.
...
For type mappings organized by Prisma ORM type, refer to the [Prisma schema reference](/docs/v7/orm/reference/prisma-schema-reference#model-field-scalar-types) documentation.

## Supported versions

See [Supported databases](/docs/v7/orm/reference/supported-databases).
...
Circular references can occur between models when each model references another, creating a closed loop. When using a Microsoft SQL Server database, Prisma ORM will show a validation error if the [referential action](/docs/v7/orm/prisma-schema/data-model/relations/referential-actions) on a relation is set to something other than [`NoAction`](/docs/v7/orm/prisma-schema/data-model/relations/referential-actions#noaction).

See [Special rules for referential actions in SQL Server](/docs/v7/orm/prisma-schema/data-model/relations/referential-actions/special-rules-for-referential-actions) for more information.
...
`String` query parameters in [raw queries](/docs/v7/orm/prisma-client/using-raw-sql/raw-queries) are always encoded to SQL Server as `NVARCHAR(4000)` (if your `String` length is \<= 4000) or `NVARCHAR(MAX)`. If you compare a `String` query parameter to a column of type `String @db.VarChar(N)`/`VARCHAR(N)`, this can lead to implicit conversion on SQL Server which affects your index performance and can lead to high CPU usage.
