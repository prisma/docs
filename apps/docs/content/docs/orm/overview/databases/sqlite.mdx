...
By default, the SQLite connector contains a database driver responsible for connecting to your database. You can use a [driver adapter](/docs/v7/orm/overview/databases/database-drivers#driver-adapters) (Preview) to connect to your database using a JavaScript database driver from Prisma Client.

## Example

To connect to a SQLite database file, you need to configure a [`datasource`](/docs/v7/orm/prisma-schema/overview/data-sources) block in your [Prisma schema](/docs/v7/orm/prisma-schema):
...
The SQLite connector maps the [scalar types](/docs/v7/orm/prisma-schema/data-model/models#scalar-fields) from the [data model](/docs/v7/orm/prisma-schema/data-model/models) to native column types as follows:
