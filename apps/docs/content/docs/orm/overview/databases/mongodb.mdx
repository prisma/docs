...
- [Quickstart with MongoDB](/docs/v7/getting-started/prisma-orm/quickstart/mongodb)
- [Add to existing MongoDB project](/docs/v7/getting-started/prisma-orm/add-to-existing-project/mongodb)
...
- connect to your database, using the [`mongodb` database connector](/docs/v7/orm/overview/databases)
- use [Introspection](/docs/v7/orm/prisma-schema/introspection) for existing projects if you already have a MongoDB database
- use [`db push`](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) to push changes in your schema to the database
- use [Prisma Client](/docs/v7/orm/prisma-client) in your application to query your database in a type safe way based on your Prisma Schema
...
For more information on how to use relations in Prisma ORM, see [our documentation](/docs/v7/orm/prisma-schema/data-model/relations).
...
To include missing fields as well, use the [`isSet` filter](/docs/v7/orm/reference/prisma-client-reference#isset) to explicitly search for fields which are either `null` or not set. This will return both records:
...
- [Start from scratch](/docs/v7/getting-started/prisma-orm/quickstart/mongodb)
- [Add to existing project](/docs/v7/getting-started/prisma-orm/add-to-existing-project/mongodb)
...
Further reference information is available in the [MongoDB connector documentation](/docs/v7/orm/overview/databases/mongodb).
...
To connect to a MongoDB server, configure the [`datasource`](/docs/v7/orm/prisma-schema/overview/data-sources) block in your [Prisma Schema](/docs/v7/orm/prisma-schema):
...
- `url`: Specifies the [connection URL](#connection-url) for the MongoDB server. In this case, an [environment variable is used](/docs/v7/orm/more/development-environment/environment-variables) to provide the connection URL.
...
You must [percentage-encode special characters](/docs/v7/orm/reference/connection-urls#special-characters).
...
See also: [Defining ID fields in MongoDB](/docs/v7/orm/prisma-schema/data-model/models#defining-ids-in-mongodb)
...
Currently, there are no plans to add support for [Prisma Migrate](/docs/v7/orm/prisma-migrate) as MongoDB projects do not rely on internal schemas where changes need to be managed with an extra tool. Management of `@unique` indexes is realized through `db push`.

### No support for `@@id` and `autoincrement()`

The [`@@id`](/docs/v7/orm/reference/prisma-schema-reference#id-1) attribute (an ID for multiple fields) is not supported because primary keys in MongoDB are always on the `_id` field of a model.

The [`autoincrement()`](/docs/v7/orm/reference/prisma-schema-reference#generate-autoincrementing-integers-as-ids-relational-databases-only) function (which creates incrementing `@id` values) is not supported because `autoincrement()` does not work with the `ObjectID` type that the `_id` field has in MongoDB.

### Cyclic references and referential actions

If you have cyclic references in your models, either from self-relations or a cycle of relations between models, and you use [referential actions](/docs/v7/orm/prisma-schema/data-model/relations/referential-actions), you must set a referential action of `NoAction` to prevent an infinite loop of actions.

See [Special rules for referential actions](/docs/v7/orm/prisma-schema/data-model/relations/referential-actions/special-rules-for-referential-actions) for more details.
...
The MongoDB connector maps the [scalar types](/docs/v7/orm/prisma-schema/data-model/models#scalar-fields) from the Prisma ORM [data model](/docs/v7/orm/prisma-schema/data-model/models) to MongoDB's native column types as follows:

> Alternatively, see [Prisma schema reference](/docs/v7/orm/reference/prisma-schema-reference#model-field-scalar-types) for type mappings organized by Prisma type.
...
When introspecting a MongoDB database, Prisma ORM uses the relevant [scalar types](/docs/v7/orm/prisma-schema/data-model/models#scalar-fields). Some special types also get additional native type annotations:

| MongoDB (Type \| Aliases) | Prisma ORM | Supported | Native database type attribute | Notes |
| ------------------------- | ---------- | :-------: | :----------------------------- | :---- |
| `objectId`                | `String`   |    ✔️     | `@db.ObjectId`                 |       |

[Introspection](/docs/v7/orm/prisma-schema/introspection) adds native database types that are **not yet supported** as [`Unsupported`](/docs/v7/orm/reference/prisma-schema-reference#unsupported) fields:
