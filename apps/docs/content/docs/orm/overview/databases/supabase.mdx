...
- Use Prisma ORM's existing [`postgresql` database connector](/docs/v7/orm/overview/databases/postgresql) in your schema, along with the [connection string Supabase provides you](https://supabase.com/docs/guides/database/connecting-to-postgres#connecting-to-external-libraries-and-tools)
- Use [Introspection](/docs/v7/orm/prisma-schema/introspection) for existing projects if you already have a database schema in Supabase
- Use [`db push`](/docs/v7/orm/prisma-migrate/workflows/prototyping-your-schema) to push changes in your schema to Supabase
- Use [Prisma Client](/docs/v7/orm/prisma-client) in your application to talk to the database server at Supabase
...
```text file=.env highlight=5-7;add showLineNumbers
# Connect to Supabase via connection pooling with Supavisor.
DATABASE_URL="postgres://postgres.[your-supabase-project]:[password]@aws-0-[aws-region].pooler.supabase.com:6543/postgres?pgbouncer=true"
