...
Prisma ORM support for Cloudflare D1 is currently in [Preview](/docs/v7/orm/more/releases#preview). We would appreciate your feedback [on GitHub](https://github.com/prisma/prisma/discussions/23646).

If you want to deploy a Cloudflare Worker with D1 and Prisma ORM, follow this [tutorial](/docs/v7/guides/deployment/cloudflare-d1).
...
- use Prisma ORM's existing [`sqlite` database connector](/docs/v7/orm/overview/databases/sqlite) in your schema
- use [Prisma Client](/docs/v7/orm/prisma-client) in your application to talk to the database server at D1
...
- **Making schema changes**. With Prisma ORM 7, you need to use D1's [migration system](https://developers.cloudflare.com/d1/reference/migrations/) via the Wrangler CLI combined with the [`prisma migrate diff`](/docs/v7/orm/reference/prisma-cli-reference#migrate-diff) command for your migration workflows. See the [Schema migrations with Prisma ORM on D1](#schema-migrations-with-prisma-orm-on-d1) section below for more information.
...
When using Prisma ORM with D1, you need to use the `sqlite` database provider and the `@prisma/adapter-d1` [driver adapter](/docs/v7/orm/overview/databases/database-drivers#driver-adapters).

If you want to deploy a Cloudflare Worker with D1 and Prisma ORM, follow these [step-by-step instructions](/docs/v7/guides/deployment/cloudflare-d1).
