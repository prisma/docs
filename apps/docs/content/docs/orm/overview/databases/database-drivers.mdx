...
One of Prisma Client's components is the [Query Engine](/docs/v7/orm/more/under-the-hood/engines) (which is implemented in Rust). The Query Engine is responsible for transforming Prisma Client queries into SQL statements. It connects to your database via TCP using built-in drivers that don't require additional setup.

:::note

As of Prisma ORM 7, the query compiler (client engine) is the default, which means Prisma Client is generated without a Rust-based query engine binary. This provides better performance and developer experience. Learn more [here](/docs/v7/orm/prisma-client/setup-and-configuration/no-rust-engine).
...
Note that [driver adapters](/docs/v7/orm/overview/databases/database-drivers#driver-adapters) are required when using the query compiler.
...
> **Note**: Driver adapters enable [edge deployments](/docs/v7/orm/prisma-client/deployment/edge/overview) of applications that use Prisma ORM.
...
- PostgreSQL
  - [`pg`](/docs/v7/orm/overview/databases/postgresql#using-the-node-postgres-driver)
- Prisma Postgres
  - [`@prisma/adapter-ppg`](/docs/v7/postgres/database/serverless-driver#use-with-prisma-orm)
- MySQL/MariaDB
  - [`mariadb`](/docs/v7/orm/overview/databases/mysql#using-the-mariadb-driver)
- SQLite
  - [`better-sqlite3`](/docs/v7/orm/overview/databases/sqlite#using-the-better-sqlite3-driver)
  - [`libSQL`](/docs/v7/orm/overview/databases/turso#how-to-connect-and-query-a-turso-database) (Turso)
- MS SQL Server
  - [`node-mssql`](/docs/v7/orm/overview/databases/sql-server#using-the-node-mssql-driver)
...
- [Prisma Postgres](/docs/v7/postgres/database/serverless-driver#use-with-prisma-orm)
- [Neon](/docs/v7/orm/overview/databases/neon#how-to-use-neons-serverless-driver-with-prisma-orm) (and Vercel Postgres)
- [PlanetScale](/docs/v7/orm/overview/databases/planetscale#how-to-use-the-planetscale-serverless-driver-with-prisma-orm-preview)
- [Cloudflare D1](/docs/v7/orm/overview/databases/cloudflare-d1)
...
- [PostgreSQL](/docs/v7/orm/overview/databases/postgresql#using-the-node-postgres-driver)
- [Prisma Postgres](/docs/v7/postgres/database/serverless-driver#use-with-prisma-orm)
- [MySQL/MariaDB](/docs/v7/orm/overview/databases/mysql#using-the-mariadb-driver)
- [MS SQL Server](/docs/v7/orm/overview/databases/sql-server#using-the-node-mssql-driver)
- [Neon](/docs/v7/orm/overview/databases/neon#how-to-use-neons-serverless-driver-with-prisma-orm)
- [PlanetScale](/docs/v7/orm/overview/databases/planetscale#how-to-use-the-planetscale-serverless-driver-with-prisma-orm-preview)
- [Turso](/docs/v7/orm/overview/databases/turso#how-to-connect-and-query-a-turso-database)
- [Cloudflare D1](/docs/v7/orm/overview/databases/cloudflare-d1)
...
In Prisma ORM 7, the database connection URL is configured in [`prisma.config.ts`](/docs/v7/orm/reference/prisma-config-reference). However, when using a driver adapter, the connection string needs to be provided in your _application code_ when the driver adapter is set up initially.
...
See the [connection pool guide](/docs/v7/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool) for the Prisma ORM v7 driver adapter defaults and how they map from Prisma ORM v6 URL parameters.

:::

:::tip[Prisma timeouts]

Prisma ORM also has its own configurable timeouts that are separate from the database driver timeouts. If you see a timeout error and are unsure whether it comes from the driver or from Prisma Client, see the [Prisma Client timeouts and transaction options documentation](/docs/v7/orm/prisma-client/queries/transactions#transaction-options).
...
[TypedSQL](/docs/v7/orm/prisma-client/using-raw-sql/typedsql) lets you write fully type-safe SQL queries that integrate directly with Prisma Client. This feature is useful if you want the flexibility of writing SQL while still benefiting from Prisma's type-safety.
