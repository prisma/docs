...
- [**Prisma Client**](/docs/v7/orm/prisma-client): Auto-generated and type-safe database client for use in your application.
- [**Prisma Migrate**](/docs/v7/orm/prisma-migrate): A declarative data modeling and migration tool.
- [**Prisma Studio**](/docs/v7/orm/tools/prisma-studio): A modern GUI for browsing and managing data in your database.

> **Note:** Since Prisma Client is the most prominent tool, we often refer to it as simply Prisma.

These three tools use the [Prisma schema](/docs/v7/orm/prisma-schema) as a single source of truth for the database schema, your application's object schema, and the mapping between the two. It's defined by you and is your main way of configuring Prisma ORM.
...
3. (Optional) Customize [field and model mappings](/docs/v7/orm/prisma-schema/data-model/models#mapping-model-names-to-tables-or-collections) between Prisma Client and the database.
4. Run `prisma generate`.

Prisma ORM will generate Prisma Client inside the `node_modules` folder, from which it can be imported in your application. For more extensive usage documentation, see the [Prisma Client API](/docs/v7/orm/prisma-client) docs.
...
The example only scratches the surface of Prisma Client's API for [CRUD operations](/docs/v7/orm/prisma-client/queries/crud) which you can learn more about in the docs. The main idea is that all queries and results are backed by types and you have full control over how relations are fetched.
