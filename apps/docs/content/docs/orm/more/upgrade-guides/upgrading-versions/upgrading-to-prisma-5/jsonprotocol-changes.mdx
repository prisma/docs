...
[When filtering on JSON fields in a PostgreSQL model](/docs/v7/orm/prisma-client/special-fields-and-types/working-with-json-fields#filter-on-a-json-field-simple) the `path` argument now only accepts an array.
...
[Scalar list](/docs/v7/orm/prisma-schema/data-model/models#scalar-fields) values must be arrays in all operations.
...
Operations on lists of [Composite types](/docs/v7/orm/prisma-schema/data-model/models#defining-composite-types) (for [MongoDB](/docs/v7/orm/overview/databases/mongodb)) now only accept array values.
