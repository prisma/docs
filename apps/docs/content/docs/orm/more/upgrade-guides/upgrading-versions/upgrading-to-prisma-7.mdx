...
For developers using AI Agents, we have a [migration prompt](/docs/v7/more/ai/prompts/prisma-7) that you can
...
Additionally other fields such as `url`, `directUrl`, and `shadowDatabaseUrl` in the `datasource` block are deprecated. You can configure them in the [Prisma Config](/docs/v7/orm/reference/prisma-config-reference).
...
**If you experience timeout issues after upgrading**, configure your driver adapter to match v6 behavior. See the [connection pool guide](/docs/v7/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool#prisma-orm-v7-driver-adapter-defaults) for detailed configuration examples for each database.
...
The client middleware API has been removed. If possible, use [Client Extensions](/docs/v7/orm/prisma-client/client-extensions).
