...
The guide assumes that you already went through the [guide for upgrading the Prisma ORM layer](/docs/v7/orm/more/upgrade-guides/upgrade-from-prisma-1/upgrading-the-prisma-layer-postgresql). This means you already:

- installed the Prisma ORM 2.0 CLI
- created your Prisma ORM 2.0 schema
- introspected your database and resolved potential [schema incompatibilities](/docs/v7/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql)
...
export const schema = makeSchema({
  types: [Query, User, Post, Profile, Category], // [!code highlight]
  plugins: [nexusSchemaPrisma()],
  outputs: {
    schema: __dirname + "/../schema.graphql",
    typegen: __dirname + "/generated/nexus.ts",
...
const Query = queryType({
  definition(t) {
    ...
    t.list.field("posts", {
      ...
      type: "Post", // [!code ++]
      nullable: false, // [!code ++]
      args: { searchString: stringArg() }, // [!code ++]
    }); // [!code ++]
  },
});
```
...
```graphql line-number
type Query {
  posts(searchString: String): [Post!]! // [!code highlight]
  users(after: UserWhereUniqueInput, before: UserWhereUniqueInput, first: Int, last: Int, orderBy: Enumerable<UserOrderByInput>, skip: Int, where: UserWhereInput): [User!]!
}
```
        ...
        return context.prisma.post.findMany({
          ...
          where: {
            ...
            OR: [
              ...
              {
                // [!code ++]
                title: { contains: args.searchString }, // [!code ++]
              }, // [!code ++]
              {
                // [!code ++]
                content: { contains: args.searchString }, // [!code ++]
              }, // [!code ++]
            ], // [!code ++]
          }, // [!code ++]
        }); // [!code ++]
...
import { inputObjectType, arg } from "@nexus/schema"; // [!code ++]

const UserUniqueInput = inputObjectType({
  // [!code ++]
  name: "UserUniqueInput", // [!code ++]
  definition(t) {
    // [!code ++]
    t.string("id"); // [!code ++]
    t.string("email"); // [!code ++]
...
```ts line-number highlight=2;normal
export const schema = makeSchema({
  types: [Query, User, Post, Profile, Category, UserUniqueInput], // [!code highlight]
  plugins: [nexusSchemaPrisma()],
  outputs: {
    schema: __dirname + "/../schema.graphql",
    typegen: __dirname + "/generated/nexus.ts",
...
type Query {
  posts(searchString: String): [Post!]
  user(userUniqueInput: UserUniqueInput!): User // [!code highlight]
  users(after: UserWhereUniqueInput, before: UserWhereUniqueInput, first: Int, last: Int, orderBy: Enumerable<UserOrderByInput>, skip: Int, where: UserWhereInput): [User!]!
}

input UserUniqueInput { // [!code highlight]
    ...
    t.field("user", {
      ...
      resolve: (_, args, context) => {
        ...
        return context.prisma.user.findUnique({
          ...
          where: {
            // [!code ++]
            id: args.userUniqueInput?.id, // [!code ++]
            email: args.userUniqueInput?.email, // [!code ++]
          }, // [!code ++]
        }); // [!code ++]
      }, // [!code ++]
    });
...
```ts line-number highlight=2;normal
export const schema = makeSchema({
  types: [Query, User, Post, Profile, Category, UserUniqueInput, Mutation], // [!code highlight]
  plugins: [nexusSchemaPrisma()],
  outputs: {
    schema: __dirname + "/../schema.graphql",
    typegen: __dirname + "/generated/nexus.ts",
...
const Mutation = mutationType({
  definition(t) {
    ...
    t.field("createDraft", {
      ...
      args: {
        // [!code ++]
        title: stringArg({ nullable: false }), // [!code ++]
        content: stringArg(), // [!code ++]
        authorId: stringArg({ nullable: false }), // [!code ++]
      }, // [!code ++]
    }); // [!code ++]
  },
});
...
type Mutation {
  createUser(data: UserCreateInput!): User!
  createDraft(title: String!, content: String, authorId: String!): Post! // [!code highlight]
}
```
    ...
    t.field("createDraft", {
      ...
      resolve: (_, args, context) => {
        ...
        return context.prisma.post.create({
          ...
          data: {
            // [!code ++]
            title: args.title, // [!code ++]
            content: args.content, // [!code ++]
            author: {
              // [!code ++]
              connect: { id: args.authorId }, // [!code ++]
            }, // [!code ++]
          }, // [!code ++]
        }); // [!code ++]
      }, // [!code ++]
  ...
  definition(t) {
    ...
    t.field("updateBio", {
      ...
      args: {
        ...
        bio: stringArg({ nullable: false }), // [!code ++]
      }, // [!code ++]
    }); // [!code ++]
  },
});
...
type Mutation {
  createUser(data: UserCreateInput!): User!
  createDraft(title: String!, content: String, authorId: String!): Post!
  updateBio(bio: String!, userUniqueInput: UserUniqueInput!): User // [!code highlight]
}
```
...
const Mutation = mutationType({
  definition(t) {
    ...
    t.field('updateBio', {
      ...
      resolve: (_, args, context) => { // [!code ++]
        return context.prisma.user.update({ // [!code ++]
          where: { // [!code ++]
            id: args.userUniqueInput?.id, // [!code ++]
            email: args.userUniqueInput?.email // [!code ++]
          }, // [!code ++]
...
const Mutation = mutationType({
  definition(t) {
    ...
    t.field("addPostToCategories", {
      ...
      args: {
        // [!code ++]
        postId: stringArg({ nullable: false }), // [!code ++]
...
type Mutation {
  ...
  createDraft(title: String!, content: String, authorId: String!): Post!
  updateBio(bio: String, userUniqueInput: UserUniqueInput!): User
  addPostToCategories(postId: String!, categoryIds: [String!]!): Post // [!code highlight]
}
```
...
const Mutation = mutationType({
  definition(t) {
    ...
    t.field("addPostToCategories", {
      ...
      resolve: (_, args, context) => {
        // [!code ++]
        const ids = args.categoryIds.map((id) => ({ id })); // [!code ++]
