...
import { makePrismaSchema, prismaObjectType } from "nexus-prisma"; // [!code --]
import datamodelInfo from "./generated/nexus-prisma"; // [!code --]
import { prisma } from "./generated/prisma-client"; // [!code --]
```

Instead, you now import the following into your application:

```ts line-number highlight=1-3;add
import { nexusSchemaPrisma } from "nexus-plugin-prisma/schema"; // [!code ++]
import { objectType, makeSchema, queryType, mutationType } from "@nexus/schema"; // [!code ++]
...
```ts file=./src/index.ts line-number highlight=2,12-14;add|1,8-11;delete showLineNumbers
 const schema = makePrismaSchema({ // [!code --]
 const schema = makeSchema({ // [!code ++]
  ...
  plugins: [nexusSchemaPrisma({ // [!code ++]
    experimentalCRUD: true, // [!code ++]
  })], // [!code ++]
...
Missing type Post, did you forget to import a type to the root query? # [!code --]
```
    ...
    t.field("user", {
      ...
      resolve: (_, args, context) => {
        return context.prisma.user.findUnique({
          ...
          where: {
            // [!code highlight]
            id: args.userUniqueInput?.id, // [!code highlight]
            email: args.userUniqueInput?.email, // [!code highlight]
          }, // [!code highlight]
        }); // [!code highlight]
      },
    });
    ...
    t.field("createDraft", {
      ...
      resolve: (_, args, context) => {
        return context.prisma.post.create({
          ...
          data: {
            // [!code highlight]
            title: args.title, // [!code highlight]
            content: args.content, // [!code highlight]
            author: {
              // [!code highlight]
              connect: { id: args.authorId }, // [!code highlight]
            }, // [!code highlight]
          }, // [!code highlight]
        }); // [!code highlight]
      },
      ...
      resolve: (_, args, context) => {
        return context.prisma.user.update({
          ...
          data: {
            ...
            profile: {
              // [!code highlight]
              create: { bio: args.bio }, // [!code highlight]
            }, // [!code highlight]
          }, // [!code highlight]
        }); // [!code highlight]
      },
    ...
    t.field("addPostToCategories", {
      ...
      resolve: (_, args, context) => {
        ...
        return context.prisma.post.update({
          ...
          where: {
            // [!code highlight]
            id: args.postId, // [!code highlight]
          }, // [!code highlight]
          data: {
            // [!code highlight]
            categories: { connect: ids }, // [!code highlight]
          }, // [!code highlight]
        }); // [!code highlight]
      },
    });
