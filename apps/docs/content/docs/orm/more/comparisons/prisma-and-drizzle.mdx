...
Drizzle exposes the underlying filter and conditional operators for a given SQL dialect. Prisma ORM on the other hand, provides a more [generic set of operators](/orm/v6/prisma-client/queries/filtering-and-sorting#combining-operators) that are intuitive to use.
