...
url: /orm/v7/prisma-client/observability-and-logging/opentelemetry-tracing
...
You can add the `traceparent` header to your SQL queries as comments using the [`@prisma/sqlcommenter-trace-context`](/orm/v7/prisma-client/observability-and-logging/sql-comments#trace-context) plugin. This enables correlation between distributed traces and database queries in your monitoring tools.
...
The `NodeSDK` class bundles common OpenTelemetry defaults into a single configuration.
...
OpenTelemetry lets you add more instrumentation with minimal changes to your application code.
