...
url: /orm/v7/prisma-client/type-safety
...
> **Note**: For advanced type safety topics with Prisma ORM, see this [blog post](https://www.prisma.io/blog/satisfies-operator-ur8ys8ccq7zb) about improving your Prisma Client workflows with the new TypeScript `satisfies` keyword.
...
See also: [Using the `Prisma.UserCreateInput` generated type](/orm/v7/prisma-client/queries/crud#create-a-single-record)
...
This query will also result in an error if an author with an `id` of `1` does not exist. In this case, Prisma Client will give a more descriptive error message. You can also use the [`connectOrCreate`](/orm/v7/reference/prisma-client-reference#connectorcreate) API to safely create a new user if one does not already exist with the given `id`.

We recommend using the "safe" `Input` types whenever possible.

## Type utilities

To help you create highly type-safe applications, Prisma Client provides a set of type utilities that tap into input and output types. These types are fully dynamic, which means that they adapt to any given model and schema. You can use them to improve autocompletion in your projects.

This is especially useful in [shared Prisma Client extensions](/orm/v7/prisma-client/client-extensions/shared-extensions).
