...
url: /orm/v7/prisma-client/queries/crud
...
See the [Prisma Client API reference](/orm/v7/reference/prisma-client-reference) for detailed method documentation.
...
- [Prisma Client API reference](/orm/v7/reference/prisma-client-reference) if you need the full method and option surface
- [Relation queries](/orm/v7/prisma-client/queries/relation-queries) if your CRUD workflow needs nested writes or relational reads
- [Prisma Migrate getting started](/orm/v7/prisma-migrate/getting-started) if you're still evolving the database schema behind these queries
- [Quickstart with Prisma Postgres](/v7/prisma-orm/quickstart/prisma-postgres) if you want a managed database to try these examples against quickly
...
See [Nested writes](/orm/v7/prisma-client/queries/relation-queries#nested-writes) for creating records with relations.
...
See [Filtering and sorting](/orm/v7/prisma-client/queries/filtering-and-sorting) for more examples.
...
See [Select fields](/orm/v7/prisma-client/queries/select-fields) and [Relation queries](/orm/v7/prisma-client/queries/relation-queries) for more.
...
See [Relation queries](/orm/v7/prisma-client/queries/relation-queries) for connecting and disconnecting related records.

## Delete

### Delete a single record

The following query uses [`delete()`](/orm/v7/reference/prisma-client-reference#delete) to delete a single `User` record:
...
The following query uses [`deleteMany()`](/orm/v7/reference/prisma-client-reference#deletemany) to delete all `User` records where `email` contains `prisma.io`:
...
The following query uses [`deleteMany()`](/orm/v7/reference/prisma-client-reference#deletemany) to delete all `User` records:
...
You can configure cascading deletes using [referential actions](/orm/v7/prisma-schema/data-model/relations/referential-actions).
...
When you know the order in which your tables should be deleted, you can use the [`deleteMany`](/orm/v7/reference/prisma-client-reference#deletemany) function. This is executed synchronously in a [`$transaction`](/orm/v7/prisma-client/queries/transactions) and can be used with all types of databases.
...
If you are comfortable working with raw SQL, you can perform a `TRUNCATE` query on a table using [`$executeRawUnsafe`](/orm/v7/prisma-client/using-raw-sql/raw-queries#executerawunsafe).
