...
url: /orm/v7/prisma-client/queries/transactions
...
A [nested write](/orm/v7/prisma-client/queries/relation-queries#nested-writes) performs multiple operations on related records in a single transaction:
...
1. On your transaction, use the `isolationLevel` parameter to `Prisma.TransactionIsolationLevel.Serializable`.
   ...
   This ensures that your application commits multiple concurrent or parallel transactions as if they were run serially. When a transaction fails due to a write conflict or deadlock, Prisma Client returns a [P2034 error](/orm/v7/reference/error-reference#p2034).
...
This section describes how to build your own optimistic concurrency control. See also: Plans for [application-level optimistic concurrency control on GitHub](https://github.com/prisma/orm/issues/4988)
