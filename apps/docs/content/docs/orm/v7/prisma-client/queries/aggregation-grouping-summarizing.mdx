...
url: /orm/v7/prisma-client/queries/aggregation-grouping-summarizing
...
Prisma Client allows you to [`aggregate`](/orm/v7/reference/prisma-client-reference#aggregate) on the **number** fields (such as `Int` and `Float`) of a model. The following query returns the average age of all users:
...
Prisma Client's [`groupBy()`](/orm/v7/reference/prisma-client-reference#groupby) allows you to **group records** by one or more field values - such as `country`, or `country` and `city` and **perform aggregations** on each group, such as finding the average age of people living in a particular city.
...
Use [`count()`](/orm/v7/reference/prisma-client-reference#count) to count the number of records or non-`null` field values. The following example query counts all users:
...
Use `where` to filter the fields returned by the `_count` output type. You can do this on [scalar fields](/orm/v7/prisma-schema/data-model/models#scalar-fields) and [relation fields](/orm/v7/prisma-schema/data-model/models#relation-fields).
...
Prisma Client allows you to filter duplicate rows from a Prisma Query response to a [`findMany`](/orm/v7/reference/prisma-client-reference#findmany) query using [`distinct`](/orm/v7/reference/prisma-client-reference#distinct) . `distinct` is often used in combination with [`select`](/orm/v7/reference/prisma-client-reference#select) to identify certain unique combinations of values in the rows of your table.
