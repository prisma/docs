...
url: /orm/v7/prisma-client/testing/integration-testing
...
The guide uses a singleton pattern for Prisma Client setup. Refer to the [singleton](/orm/v7/prisma-client/testing/unit-testing#singleton) docs for a walk through of how to set that up.
...
The above `.env.test` file is used as part of a multiple `.env` file setup. Checkout the [using multiple .env files.](/orm/v7/more/dev-environment/environment-variables) section to learn more about setting up your project with multiple `.env` files
...
Before the test suite is run the database is seeded with data. After the test suite has finished a [`deleteMany`](/orm/v7/reference/prisma-client-reference#deletemany) is used to clear the database of its data.
...
However, this doesn't scale as well as having a more generic solution that maps over your models and performs a truncate on them. For those scenarios and examples of using raw SQL queries see [Deleting all data with raw SQL / `TRUNCATE`](/orm/v7/prisma-client/queries/crud#deleting-all-data-with-raw-sql--truncate)
