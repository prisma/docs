...
url: /orm/v7/prisma-client/client-extensions/shared-extensions
...
Prisma 8 replaces Client extensions with [extensions](/orm/extensions) that add database capabilities and [middleware](/orm/middleware/how-middleware-works) that wraps every query. Browse both in the [extension directory](https://www.prisma.io/extensions).

:::

You can share your [Prisma Client extensions](/orm/v7/prisma-client/client-extensions) with other users, either as packages or as modules, and import extensions that other users create into your project.
...
To explore examples of Prisma's official Client extensions and those made by the community, visit [this](/orm/v7/prisma-client/client-extensions/extension-examples) page.
...
2. Use one of the methods that begin with the `$all` prefix such as [`$allModels`](/orm/v7/prisma-client/client-extensions/model#add-a-custom-method-to-all-models-in-your-schema) or [`$allOperations`](/orm/v7/prisma-client/client-extensions/query#modify-all-prisma-client-operations)
...
You do not need to use the `$all` prefix with the [`client`](/orm/v7/prisma-client/client-extensions/client) component, because the `client` component always applies to the client instance.
...
- [Modify all Prisma Client operations](/orm/v7/prisma-client/client-extensions/query#modify-all-prisma-client-operations)
- [Modify a specific operation in all models of your schema](/orm/v7/prisma-client/client-extensions/query#modify-a-specific-operation-in-all-models-of-your-schema)
- [Modify all operations in all models of your schema](/orm/v7/prisma-client/client-extensions/query#modify-all-operations-in-all-models-of-your-schema)
...
If you trigger the extension from inside a [transaction](/orm/v7/prisma-client/queries/transactions) (interactive or batched), the extension code will issue the queries in a new connection and ignore the current transaction context.

Learn more in this issue on GitHub: [Client extensions that require use of a client-level method silently ignore transactions](https://github.com/prisma/orm/issues/20678).

:::

In the following situations, you need to refer to a Prisma Client instance that your extension wraps:

- When you want to use a [client-level method](/orm/v7/reference/prisma-client-reference#client-methods), such as `$queryRaw`, in your packaged extension.
...
You can improve the type-safety of your shared extensions using [type utilities](/orm/v7/prisma-client/client-extensions/type-utilities).
