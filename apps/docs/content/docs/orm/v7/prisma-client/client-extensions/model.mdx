...
url: /orm/v7/prisma-client/client-extensions/model
...
You can use the `model` [Prisma Client extensions](/orm/v7/prisma-client/client-extensions) component type to add custom methods to your models.
...
Use the `$extends` [client-level method](/orm/v7/reference/prisma-client-reference#client-methods) to create an _extended client_. An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions. Use the `model` extension component to add methods to models in your schema.
...
You can improve the type-safety of `model` components in your shared extensions with [type utilities](/orm/v7/prisma-client/client-extensions/type-utilities).
