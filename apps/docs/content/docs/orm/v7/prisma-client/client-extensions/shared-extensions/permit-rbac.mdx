...
url: /orm/v7/prisma-client/client-extensions/shared-extensions/permit-rbac
...
:::info[Quick summary]
This page explains how to implement fine-grained authorization (FGA) in Prisma ORM applications using the `@permitio/permit-prisma` extension. It introduces the access control models supported by Permit.io (RBAC, ABAC, and ReBAC) and helps you choose the right model for your database operations.

:::

Prisma ORM does not include built-in authorization. This guide shows how to implement fine-grained authorization in your Prisma applications using the `@permitio/permit-prisma` extension.

Fine-grained authorization (FGA) controls which records a user can read or modify. Without it, your application might expose sensitive data or allow unauthorized modifications.
...
By integrating the `@permitio/permit-prisma` extension with your Prisma ORM application, you can implement authorization policies that protect your data and ensure users only access what they're permitted to see. The extension supports all major authorization models (RBAC, ABAC, ReBAC) and provides both automatic and manual permission enforcement.
