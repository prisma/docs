...
url: /orm/v7/prisma-client/deployment/edge/deploy-to-vercel
...
You can use Prisma Postgres in Vercel's edge runtime. Follow this guide for an end-to-end tutorial on [deploying an application to Vercel using Prisma Postgres](/guides/v7/frameworks/nextjs).
...
When deploying a Vercel Edge Function that uses Prisma ORM, you need to use one of these [edge-compatible drivers](/orm/v7/prisma-client/deployment/edge/overview#edge-compatibility-of-database-drivers) and its respective [driver adapter](/orm/v7/core-concepts/supported-databases/database-drivers#driver-adapters) for Prisma ORM.

:::note

For hosted Prisma Postgres, use the [Prisma Postgres serverless driver](/postgres/database/serverless-driver). For another database, choose a driver that supports Vercel Edge Functions and use its Prisma driver adapter.
...
When in **development**, you can configure your database connection via the `DATABASE_URL` environment variable (e.g. [using `.env` files](/orm/v7/more/dev-environment/environment-variables)).
...
Vercel has a [size limit of 1 MB on free accounts](https://vercel.com/docs/functions/limitations). If your application bundle with Prisma ORM exceeds that size, use an engine-less Prisma Client with a compatible driver adapter or upgrade to a plan with a larger limit.
...
- use the `@prisma/adapter-planetscale` database adapter (learn more [here](/orm/v7/core-concepts/supported-databases/mysql#planetscale))
...
- use the `@prisma/adapter-neon` database adapter (learn more [here](/orm/v7/core-concepts/supported-databases/postgresql#using-driver-adapters))
