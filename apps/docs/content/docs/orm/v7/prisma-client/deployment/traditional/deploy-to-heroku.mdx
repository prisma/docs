...
url: /orm/v7/prisma-client/deployment/traditional/deploy-to-heroku
...
At the core of Prisma ORM is the [Prisma schema](/orm/v7/prisma-schema/overview) – a declarative configuration where you define your data model and other Prisma ORM-related configuration. The Prisma schema is also a single source of truth for both Prisma Client and Prisma Migrate.

In this guide, you will use [Prisma Migrate](/orm/v7/prisma-migrate) to create the database schema. Prisma Migrate is based on the Prisma schema and works by generating `.sql` migration files that are executed against the database.
...
To learn more about how migrations are created with Prisma Migrate, check out the [start from scratch guide](/v7/prisma-orm/quickstart/postgresql)
...
* **Disable SSL certificate validation**: When using the `PrismaPg` adapter with `DATABASE_URL`, make sure to disable SSL certificate validation in line with [Heroku's guidelines](https://devcenter.heroku.com/articles/heroku-postgresql#connecting-to-heroku-postgres). Failing to do so can result in a `P1010 DriverAdapterError: DatabaseAccessDenied` error. You can handle this conditionally based on whether the database is local or hosted on Heroku:
...
You have deployed a Node.js app with Prisma ORM to Heroku.
