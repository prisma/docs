...
url: /orm/v7/prisma-client/deployment/edge/deploy-to-cloudflare
...
- [Cloudflare D1](/orm/v7/prisma-client/deployment/edge/deploy-to-cloudflare) is used to access D1 databases
...
For hosted Prisma Postgres, use the [Prisma Postgres serverless driver](/postgres/database/serverless-driver). For another database, choose a driver that supports Cloudflare Workers and use its Prisma driver adapter.
...
- use the `@prisma/adapter-pg` database adapter (learn more [here](/orm/v7/core-concepts/supported-databases/postgresql#using-driver-adapters))
...
To deploy the Worker, you first need to set the `DATABASE_URL` environment variable [via the `wrangler` CLI](https://developers.cloudflare.com/workers/configuration/secrets/#secrets-on-deployed-workers):
