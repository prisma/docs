...
url: /orm/v7/prisma-schema/data-model/database-mapping
metaTitle: Database mapping
metaDescription: Database mapping in Prisma schema
---

The [Prisma schema](/orm/v7/prisma-schema/overview) includes mechanisms that allow you to define names of certain database objects. You can:
...
A common approach for naming tables/collections in databases for example is to use plural form and [snake_case](https://en.wikipedia.org/wiki/Snake_case) notation. However, we recommended a different [naming convention (singular form, PascalCase)](/orm/v7/reference/prisma-schema-reference#naming-conventions).

`@map` and `@@map` allow you to [tune the shape of your Prisma Client API](/orm/v7/prisma-client/setup-and-configuration/custom-model-and-field-names) by decoupling model and field names from table and column names in the underlying database.

### Map collection / table names

As an example, when you [introspect](/orm/v7/prisma-schema/introspection) a database with a table named `comments`, the resulting Prisma model will look like this:
...
However, you can still choose `Comment` as the name of the model (e.g. to follow the naming convention) without renaming the underlying `comments` table in the database by using the [`@@map`](/orm/v7/reference/prisma-schema-reference) attribute:
...
You can also [`@map`](/orm/v7/reference/prisma-schema-reference#map) a column/field name:
...
You can optionally use the `map` argument to explicitly define the **underlying constraint and index names** in the Prisma schema for the attributes [`@id`](/orm/v7/reference/prisma-schema-reference#id), [`@@id`](/orm/v7/reference/prisma-schema-reference), [`@unique`](/orm/v7/reference/prisma-schema-reference#unique), [`@@unique`](/orm/v7/reference/prisma-schema-reference), [`@@index`](/orm/v7/reference/prisma-schema-reference#index) and [`@relation`](/orm/v7/reference/prisma-schema-reference#relation).
