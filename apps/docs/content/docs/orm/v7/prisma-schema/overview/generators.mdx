...
url: /orm/v7/prisma-schema/overview/generators
...
A Prisma schema can have one or more generators, represented by the [`generator`](/orm/v7/reference/prisma-schema-reference#generator) block:
...
The `prisma-client` generator outputs plain TypeScript that runs across JavaScript environments (such as ESM, Bun, Deno, ...).
...
Update your [`generator`](/orm/v7/prisma-schema/overview/generators) block:
...
**If you use `tsx` as your TypeScript runner** and see `Cannot find module './internal/class.js'` errors at runtime, set `importFileExtension = "ts"` in your generator block:
...
- `previewFeatures`: [Preview features](/orm/v7/reference/preview-features/client-preview-features) to include
...
Existing generators or new ones should not be affected if you are using a [multi-file Prisma schema](/orm/v7/prisma-schema/overview/location#multi-file-prisma-schema), unless a generator reads the schema manually.
...
- [`@lexmata/prisma-ent-generator`](https://github.com/Lexmata/prisma-ent-generator): Generates Go [Ent](https://entgo.io/) schema files from your Prisma schema.
- [`@lexmata/prisma-python-generator`](https://github.com/Lexmata/prisma-python-generator): Generates Python [Pydantic](https://docs.pydantic.dev/) model classes from your Prisma schema.
- [`@pegasusheavy/nestjs-prisma-graphql`](https://github.com/quinnjr/nestjs-prisma-graphql): Generates NestJS GraphQL types from your Prisma schema. ESM-first with support for Prisma 7+.
