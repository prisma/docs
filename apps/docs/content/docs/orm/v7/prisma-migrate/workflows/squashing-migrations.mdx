...
url: /orm/v7/prisma-migrate/workflows/squashing-migrations
...
In both cases, Prisma Migrate provides the tools for doing this, by using the [`migrate diff`](/orm/v7/reference/prisma-cli-reference#migrate-diff) command to compare two database schemas and output a single SQL file that takes you from one to the other. The rest of this guide gives detailed instructions on how to carry this out in these two scenarios.
...
- Mark this migration as having been applied on production, to prevent it from being run there:
   ...
   You can do this using the [`migrate resolve`](/orm/v7/reference/prisma-cli-reference#migrate-resolve) command to mark the migration in the `000000000000_squashed_migrations` directory as already applied:
