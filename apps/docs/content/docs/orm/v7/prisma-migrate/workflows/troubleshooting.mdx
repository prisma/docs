...
url: /orm/v7/prisma-migrate/workflows/troubleshooting
...
- [Patching / hotfixing production databases](/orm/v7/prisma-migrate/workflows/patching-and-hotfixing)
...
:::info[Note]
You should [never purposefully delete or edit a migration](/orm/v7/prisma-migrate/understanding-prisma-migrate/migration-histories#do-not-edit-or-delete-migrations-that-have-been-applied), as this might result in discrepancies between development and production.
:::
...
- The database schema was changed _without_ using migrations - for example, by using [`prisma db push`](/orm/v7/reference/prisma-cli-reference#db-push) or manually changing the database schema.

:::info[Note]
The [shadow database](/orm/v7/prisma-migrate/understanding-prisma-migrate/shadow-database) is required to detect schema drift, and can therefore only be done in a development environment.
:::
...
- You [modify a migration before running it](/orm/v7/prisma-migrate/workflows/customizing-migrations) and introduce a syntax error
...
The usual way to handle a failed migration in a development environment is to address the root cause and reset the database. For example:
...
See [Prisma Migrate and PgBouncer workaround](/orm/v7/prisma-client/setup-and-configuration/databases-connections/pgbouncer) for further information and a workaround.
