...
url: /orm/v7/prisma-migrate/understanding-prisma-migrate/limitations-and-known-issues
...
As of [2.15.0](https://github.com/prisma/orm/releases/2.15.0), Prisma Migrate detects when the migrations do not match the configured provider and prints a helpful error message. For example, if your migrations are for a PostgreSQL database but your `provider` is set to `mysql`:
...
:::info[Note]
To re-create data automatically when the database is reset, see the [seeding guide](/orm/v7/prisma-migrate/workflows/seeding).
::: 
