...
url: /orm/v7/prisma-migrate/getting-started
...
:::tip
You can use [native type mapping attributes](/orm/v7/prisma-migrate/workflows/native-database-types) in your schema to decide which exact database type to create (for example, `String` can map to `varchar(100)` or `text`).
:::
...
Your migration history can be [committed to version control](/orm/v7/prisma-migrate/understanding-prisma-migrate/migration-histories#committing-the-migration-history-to-source-control) and use to [deploy changes to test environments and production](/orm/v7/prisma-migrate/workflows/development-and-production#production-and-testing-environments).
...
To include [unsupported database features](/orm/v7/prisma-migrate/workflows/unsupported-database-features) that already exist in the database, you must replace or modify the initial migration SQL:
...
- Refer to the [Deploying database changes with Prisma Migrate](/orm/v7/prisma-client/deployment/deploy-database-changes-with-prisma-migrate) guide for more on deploying migrations to production.
- Refer to the [Production Troubleshooting](/orm/v7/prisma-migrate/workflows/patching-and-hotfixing#fixing-failed-migrations-with-migrate-diff-and-db-execute) guide to learn how to debug and resolve failed migrations in production using `prisma migrate diff`, `prisma db execute` and/ or `prisma migrate resolve`.

## Where to go next

- [Prisma Client CRUD guide](/orm/v7/prisma-client/queries/crud) once your schema changes are in place and you want to start querying data
- [Generating Prisma Client](/orm/v7/prisma-client/setup-and-configuration/generating-prisma-client) if you need to adjust client output or runtime behavior after migrations
- [Quickstart with Prisma Postgres](/v7/prisma-orm/quickstart/prisma-postgres) if you want a managed Postgres database without running one yourself
