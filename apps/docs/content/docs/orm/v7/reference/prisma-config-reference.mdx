...
url: /orm/v7/reference/prisma-config-reference
...
Configures how Prisma ORM locates and loads your schema file(s). Can be a file or folder path. Relative paths are resolved relative to the `prisma.config.ts` file location. See [here](/orm/v7/prisma-schema/overview/location#multi-file-prisma-schema) for more info about schema location options.
...
Learn more about the [`externalTables` feature here](/orm/v7/prisma-schema/data-model/externally-managed-tables).
...
This option allows you to define SQL statements that Prisma runs on the **shadow database** before creating migrations. It is useful when working with [external managed tables](/orm/v7/prisma-schema/data-model/externally-managed-tables), as Prisma needs to know about the structure of these tables to correctly generate migrations.
...
The path to the directory where Prisma should look for the SQL files used for generating typings via [`typedSql`](/orm/v7/prisma-client/using-raw-sql/typedsql).
...
Connection URL including authentication info. Uses [the syntax provided by the database](/orm/v7/reference/connection-urls#format).
...
As of [Prisma ORM v6.11.0](https://github.com/prisma/orm/releases/tag/6.11.0), the D1 adapter has been renamed from `PrismaD1HTTP` to `PrismaD1`.
...
Prisma Studio now uses the connection configuration from the `datasource` property automatically. See the [Prisma Studio documentation](/orm/v7/reference/prisma-cli-reference#studio) for more details.
...
To load environment variables in your Prisma application, use the `prisma.config.ts` file along with the `env` helper from `prisma/config`.
