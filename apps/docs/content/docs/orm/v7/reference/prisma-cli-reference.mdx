...
url: /orm/v7/reference/prisma-cli-reference
...
By default, the project sets up a [local Prisma Postgres](/local-development/postgres) instance but you can choose a different database using the `--datasource-provider` option.
...
| `--preview-feature`      | No       | Define the [Preview features](/orm/v7/reference/preview-features/cli-preview-features) to use. To define multiple Preview features, you have to provide the flag multiple times for each Preview feature. See examples. |                       |
| `--output`               | No       | Specifies the [output location for the generated client](/orm/v7/reference/prisma-schema-reference#fields-for-prisma-client-provider).                                                                                  | `../generated/prisma` |
...
See the [Prisma Config reference](/orm/v7/reference/prisma-config-reference) for more details.
...
The `generate` command generates assets like Prisma Client based on the [`generator`](/orm/v7/prisma-schema/overview/generators) and [`data model`](/orm/v7/prisma-schema/data-model/models) blocks defined in your `prisma/schema.prisma` file.
...
The `prisma-client` generator creates a customized client for working with your database in a custom output directory specified by the `output` field - you can [customize the output folder](/orm/v7/reference/prisma-schema-reference#fields-for-prisma-client-provider).

### `validate`

Validates the [Prisma Schema Language](/orm/v7/prisma-schema/overview) of the Prisma schema file.
...
For more information see our [environment variable documentation](/orm/v7/reference/environment-variables-reference)
...
The `dev` command starts a [local Prisma Postgres](/local-development/postgres) database that you can run Prisma ORM commands against. It is useful for development and testing, and lets you switch to [Prisma Postgres](/postgres) in production.

### Arguments

| Argument              | Required | Description                                                                                                                                                 | Default   |
| --------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `--name` (or `-n`)    | No       | Enables targeting a specific database instance. [Learn more](/local-development/postgres#using-different-local-prisma-postgres-instances). | `default` |
...
Starts existing [local Prisma Postgres](/local-development/postgres) instances in the background.
...
Lists all available [local Prisma Postgres](/local-development/postgres) instances:
...
Stops one or more [local Prisma Postgres](/local-development/postgres) databases:
...
Removes the data of one or more [local Prisma Postgres](/local-development/postgres) databases from your file system:
...
Introspection with the `db pull` command on the [MongoDB connector](/orm/v7/core-concepts/supported-databases/mongodb) samples the data instead of reading a schema.
...
- [Conceptual overview of `db push` and when to use it over Prisma Migrate](/orm/v7/prisma-migrate/workflows/prototyping-your-schema)
- [Schema prototyping with `db push`](/orm/v7/prisma-migrate/workflows/prototyping-your-schema)
...
See [Seeding your database](/orm/v7/prisma-migrate/workflows/seeding)
...
This command is currently not supported on [MongoDB](/orm/v7/core-concepts/supported-databases/mongodb).
...
- [Migration troubleshooting in production](/orm/v7/prisma-migrate/workflows/patching-and-hotfixing#fixing-failed-migrations-with-migrate-diff-and-db-execute)
...
Instead of `migrate dev` and related commands, [`db push`](/orm/v7/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/orm/v7/core-concepts/supported-databases/mongodb).
...
1. Reruns the existing migration history in the [shadow database](/orm/v7/prisma-migrate/understanding-prisma-migrate/shadow-database) in order to detect schema drift (edited or deleted migration file, or a manual changes to the database schema)
...
This command is not supported on [MongoDB](/orm/v7/core-concepts/supported-databases/mongodb). Use [`db push`](/orm/v7/prisma-migrate/workflows/prototyping-your-schema) instead.
...
- [Conceptual overview of Prisma Migrate](/orm/v7/prisma-migrate)
- [Developing with Prisma Migrate](/orm/v7/prisma-migrate)
...
If a [schema drift](/orm/v7/prisma-migrate/understanding-prisma-migrate/shadow-database#detecting-schema-drift) is detected while running `prisma migrate dev` using `--create-only`, you will be prompted to reset your database.
...
This command is only partially supported for [MongoDB](/orm/v7/core-concepts/supported-databases/mongodb). See the command options below for details.
...
The `migrate diff` command can only compare database features that are [supported by Prisma](/orm/v7/reference/database-features). If two databases differ only in unsupported features, such as views or triggers, then `migrate diff` will not show any difference between them.
...
| `-o`, `--output` | No       | Writes to a file instead of stdout                                                                                                                | Available since [5.12.1](https://github.com/prisma/orm/releases/tag/5.12.1) |
...
### `platform` [#platform-early-access]

The `platform` command is in [Early Access](/console/more/feature-maturity#early-access). It provides access to the Prisma Data Platform through the Prisma CLI starting in version `5.10.0` or later.
...
You can find the complete list of available commands with the arguments [here](/cli/v7/console).
...
Prisma ORM v7 ships a new version of Prisma Studio.
...
Prisma CLI supports [custom HTTP proxies](https://github.com/prisma/orm/issues/506). This is particularly relevant when being behind a corporate firewall.

To activate usage of the proxy, provide either of the following environment variables:

- [`HTTP_PROXY`](/orm/v7/reference/environment-variables-reference#http_proxy) or `http_proxy`: Proxy URL for http traffic, for example `http://localhost:8080`
- [`HTTPS_PROXY`](/orm/v7/reference/environment-variables-reference#https_proxy) or `https_proxy`: Proxy URL for https traffic, for example `https://localhost:8080`
