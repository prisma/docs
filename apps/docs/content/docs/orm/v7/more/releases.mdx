...
[Check out all the releases notes in GitHub](https://github.com/prisma/orm/releases).
...
> **Note**: As of version [2.13.0](https://github.com/prisma/orm/releases/2.13.0), 'Experimental' is no longer part of the product maturity scale.

### Early Access

If a feature or product is **Early Access**:

- It is ready to try out and represents the cutting edge of Prisma ORM.
- It can still evolve based on feedback from our users, so we welcome your input on how it works.
- Its API surface won't change drastically, though we may make refinements as it matures.

Early Access features and products are available to try today. Share your feedback to help shape them as they move toward General Availability.
...
Previews are typically available behind a feature flag or require some form of opt-in (for example, by providing a `--preview-feature` flag in the CLI or [adding them to a `previewFeatures` property in the `generator` block](/orm/v7/reference/preview-features/cli-preview-features) for Prisma Client in your Prisma schema).

Preview features and products are close to stable. We recommend validating them in a staging environment before rolling them out to production.

See also: [All currently available Preview features](/orm/v7/reference/preview-features/client-preview-features).
