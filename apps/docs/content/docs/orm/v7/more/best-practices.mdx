...
url: /orm/v7/more/best-practices
---
...
Bulk operations (`createMany`, `createManyAndReturn`, `updateMany`, `updateManyAndReturn`, and `deleteMany`) [automatically run as transactions](/orm/v7/prisma-client/queries/transactions#batch-operations), so all writes either succeed together or are rolled back if something fails.
...
- [Raw queries](/orm/v7/prisma-client/using-raw-sql/raw-queries)
