...
url: /orm/v7/more/dev-environment/environment-variables
...
An environment variable is a key value pair of string data that is stored on your machine's local environment. Refer to our [Environment variables reference documentation](/orm/v7/reference/environment-variables-reference) for specific details.
...
With Prisma ORM v6.4.0, we released the `prisma.config.ts` file. This file allows you to manage your environment variables and settings in a more flexible way. [View our reference](/orm/v7/reference/prisma-config-reference) for more information.
...
To use more than one `.env` file, see [Using multiple `.env` files](#using-multiple-env-files) for how to set them up in your application.
...
When running tests, we advise you to [mock Prisma Client](/orm/v7/prisma-client/testing/unit-testing#mocking-prisma-client). In doing so, you need to tell Jest which environment it should use when running its tests.
