...
codeStyle: false
url: /orm/v7/core-concepts/supported-databases/mongodb
...
- [Add to existing MongoDB project](/v7/prisma-orm/add-to-existing-project/mongodb)
...
- model your database with the [Prisma Schema Language](/orm/v7/prisma-schema/overview)
- connect to your database, using the [`mongodb` database connector](/orm/v7/core-concepts/supported-databases)
- use [Introspection](/orm/v7/prisma-schema/introspection) for existing projects if you already have a MongoDB database
- use [`db push`](/orm/v7/prisma-migrate/workflows/prototyping-your-schema) to push changes in your schema to the database
- use [Prisma Client](/orm/v7/prisma-client/setup-and-configuration/introduction) in your application to query your database in a type safe way based on your Prisma Schema
...
These iterative tasks of updating the schema and the database can result in inconsistencies between your schema and the actual data in the database. The following scenario shows one way this can happen, followed by several strategies you and your team can use to handle these inconsistencies.
...
For more information on how to use relations in Prisma ORM, see [our documentation](/orm/v7/prisma-schema/data-model/relations).
...
Prisma ORM returns the same result in both cases, because we currently don't have a way to specify this difference in MongoDB between fields that are `null` in the underlying database, and fields that are not defined at all — see [this Github issue](https://github.com/prisma/orm/issues/12555) for more information.
...
To include missing fields as well, use the [`isSet` filter](/orm/v7/reference/prisma-client-reference#isset) to explicitly search for fields which are either `null` or not set. This will return both records:
...
- [Start from scratch](/v7/prisma-orm/quickstart/mongodb)
- [Add to existing project](/v7/prisma-orm/add-to-existing-project/mongodb)
...
Further reference information is available in the [MongoDB connector documentation](/orm/v7/core-concepts/supported-databases/mongodb).
...
To connect to a MongoDB server, configure the [`datasource`](/orm/v7/prisma-schema/overview/data-sources) block in your [Prisma Schema](/orm/v7/prisma-schema/overview):
...
- `url`: Specifies the [connection URL](#connection-url) for the MongoDB server. In this case, an [environment variable is used](/orm/v7/more/dev-environment/environment-variables) to provide the connection URL.

:::warning

The MongoDB database connector uses transactions to support nested writes. Transactions **require** a [replica set](https://www.mongodb.com/docs/manual/tutorial/deploy-replica-set/) deployment. The quickest way to get a replica set is [Atlas](https://www.mongodb.com/docs/atlas/getting-started/), which configures one for you and has a free tier.
...
You must [percentage-encode special characters](/orm/v7/reference/connection-urls#special-characters).
...
See also: [Defining ID fields in MongoDB](/orm/v7/prisma-schema/data-model/models#defining-ids-in-mongodb)
...
Currently, there are no plans to add support for [Prisma Migrate](/orm/v7/prisma-migrate) as MongoDB projects do not rely on internal schemas where changes need to be managed with an extra tool. Management of `@unique` indexes is realized through `db push`.

### No support for `@@id` and `autoincrement()`

The [`@@id`](/orm/v7/reference/prisma-schema-reference) attribute (an ID for multiple fields) is not supported because primary keys in MongoDB are always on the `_id` field of a model.

The [`autoincrement()`](/orm/v7/reference/prisma-schema-reference#generate-autoincrementing-integers-as-ids-relational-databases-only) function (which creates incrementing `@id` values) is not supported because `autoincrement()` does not work with the `ObjectID` type that the `_id` field has in MongoDB.

### Cyclic references and referential actions

If you have cyclic references in your models, either from self-relations or a cycle of relations between models, and you use [referential actions](/orm/v7/prisma-schema/data-model/relations/referential-actions), you must set a referential action of `NoAction` to prevent an infinite loop of actions.

See [Special rules for referential actions](/orm/v7/prisma-schema/data-model/relations/referential-actions#special-rules-for-sql-server-and-mongodb) for more details.
...
One option is to use [MongoDB Atlas](https://www.mongodb.com/cloud/atlas) to launch a free instance, which comes with a replica set already configured.
...
The MongoDB connector maps the [scalar types](/orm/v7/prisma-schema/data-model/models#scalar-fields) from the Prisma ORM [data model](/orm/v7/prisma-schema/data-model/models) to MongoDB's native column types as follows:

> Alternatively, see [Prisma schema reference](/orm/v7/reference/prisma-schema-reference#model-field-scalar-types) for type mappings organized by Prisma type.
...
| `BigInt`   | `long`                                                                 |
| `Float`    | `double`                                                               |
| `Decimal`  | [Currently unsupported](https://github.com/prisma/orm/issues/12637) |
| `DateTime` | `timestamp`                                                            |
| `Bytes`    | `binData`                                                              |
| `Json`     |                                                                        |
...
When introspecting a MongoDB database, Prisma ORM uses the relevant [scalar types](/orm/v7/prisma-schema/data-model/models#scalar-fields). Some special types also get additional native type annotations:

| MongoDB (Type \| Aliases) | Prisma ORM | Supported | Native database type attribute | Notes |
| ------------------------- | ---------- | :-------: | :----------------------------- | :---- |
| `objectId`                | `String`   |    ✔️     | `@db.ObjectId`                 |       |

[Introspection](/orm/v7/prisma-schema/introspection) adds native database types that are **not yet supported** as [`Unsupported`](/orm/v7/reference/prisma-schema-reference#unsupported) fields:
