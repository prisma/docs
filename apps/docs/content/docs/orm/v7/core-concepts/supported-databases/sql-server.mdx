...
url: /orm/v7/core-concepts/supported-databases/sql-server
...
Use the `node-mssql` JavaScript database driver via [driver adapters](/orm/v7/core-concepts/supported-databases/database-drivers#driver-adapters):
...
See [connection pool guide](/orm/v7/prisma-client/setup-and-configuration/databases-connections/connection-pool#sql-server-using-the-mssql-driver) for configuration.
...
See [full type mapping reference](/orm/v7/reference/prisma-schema-reference#model-field-scalar-types) for complete details.
...
With circular model references, you must use [`NoAction` referential actions](/orm/v7/prisma-schema/data-model/relations/referential-actions#special-rules-for-sql-server-and-mongodb) to avoid validation errors.
