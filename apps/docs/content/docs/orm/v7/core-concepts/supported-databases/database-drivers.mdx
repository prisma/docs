...
url: /orm/v7/core-concepts/supported-databases/database-drivers
...
> **Note**: Driver adapters enable [edge deployments](/orm/v7/prisma-client/deployment/edge/overview) of applications that use Prisma ORM.
...
- PostgreSQL
  - [`pg`](/orm/v7/core-concepts/supported-databases/postgresql#using-driver-adapters)
- Prisma Postgres
  - [`@prisma/adapter-ppg`](/postgres/database/serverless-driver#use-with-prisma-orm)
- MySQL/MariaDB
  - [`mariadb`](/orm/v7/core-concepts/supported-databases/mysql#using-driver-adapters)
- SQLite
  - [`better-sqlite3`](/orm/v7/core-concepts/supported-databases/sqlite#using-driver-adapters)
  - [`libSQL`](/orm/v7/core-concepts/supported-databases/sqlite#using-driver-adapters) (Turso)
- MS SQL Server
  - [`node-mssql`](/orm/v7/core-concepts/supported-databases/sql-server#using-driver-adapters)
...
- [Neon](/orm/v7/core-concepts/supported-databases/postgresql#neon) (and Vercel Postgres)
- [PlanetScale](/orm/v7/core-concepts/supported-databases/mysql#planetscale)
- [Cloudflare D1](/orm/v7/core-concepts/supported-databases/sqlite#cloudflare-d1)
...
- [PostgreSQL](/orm/v7/core-concepts/supported-databases/postgresql#using-driver-adapters)
- [Prisma Postgres](/postgres/database/serverless-driver#use-with-prisma-orm)
- [MySQL/MariaDB](/orm/v7/core-concepts/supported-databases/mysql#using-driver-adapters)
- [MS SQL Server](/orm/v7/core-concepts/supported-databases/sql-server#using-driver-adapters)
- [Neon](/orm/v7/core-concepts/supported-databases/postgresql#neon)
- [PlanetScale](/orm/v7/core-concepts/supported-databases/mysql#planetscale)
- [Turso](/orm/v7/core-concepts/supported-databases/sqlite#turso-libsql)
- [Cloudflare D1](/orm/v7/core-concepts/supported-databases/sqlite#cloudflare-d1)
...
In [v6.6.0](https://github.com/prisma/orm/releases/tag/6.6.0), we introduced a simplified version for instantiating Prisma Client when using driver adapters. You now don't need to create an instance of the driver/client to pass to a driver adapter, instead you create the driver adapter directly (and pass the driver's options to it if needed).
...
In Prisma ORM 7, the database connection URL is configured in [`prisma.config.ts`](/orm/v7/reference/prisma-config-reference). However, when using a driver adapter, the connection string needs to be provided in your _application code_ when the driver adapter is set up initially.
...
See the [connection pool guide](/orm/v7/prisma-client/setup-and-configuration/databases-connections/connection-pool) for the Prisma ORM v7 driver adapter defaults and how they map from Prisma ORM v6 URL parameters.

:::

:::tip[Prisma timeouts]

Prisma ORM also has its own configurable timeouts that are separate from the database driver timeouts. If you see a timeout error and are unsure whether it comes from the driver or from Prisma Client, see the [Prisma Client timeouts and transaction options documentation](/orm/v7/prisma-client/queries/transactions#transaction-isolation-level).
...
For example, with `output` in your Prisma schema set to `../src/generated/prisma`:
...
You can now import your generated client from `db`:
...
[TypedSQL](/orm/v7/prisma-client/using-raw-sql/typedsql) lets you write fully type-safe SQL queries that integrate directly with Prisma Client. This feature is useful if you want the flexibility of writing SQL while still benefiting from Prisma's type-safety.
