...
- `count()` returns a `number` and has a zero-argument form; `countBigInt()` is the lossless `bigint` variant. `sum()` and `avg()` over an integer column return `number` and raise `RUNTIME.DECODE_FAILED` outside ±(2^53 − 1); `sumBigInt(field)` and `avgDecimal(field)` are the lossless forms.
