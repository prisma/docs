...
- Available for PostgreSQL and MongoDB.
- `all()` returns an [`AsyncIterableResult`](#asynciterableresult): you can `await` it to collect an array, or use `for await` to iterate rows one at a time.
...
| Return type | Example | Description |
|---|---|---|
| `AsyncIterableResult<Row>` | `await db.orm.public.User.all()` | Awaitable to `Row[]`, or iterable with `for await`, one row at a time. |
...
- Available for PostgreSQL and MongoDB.
- Returns an [`AsyncIterableResult`](#asynciterableresult): `await` for an array, or `for await` to iterate the inserted rows.
...
| Return type | Example | Description |
|---|---|---|
| `AsyncIterableResult<Row>` | `await db.orm.public.Tag.createAll([...])` | Awaitable to `Row[]`, or iterable with `for await`. |
...
##### Iterate inserted rows (PostgreSQL)
...
- **Iterated**: use `for await ... of` to take rows one at a time. On `postgres()` the driver still fetches the whole result before the first iteration; only decoding is per row. See [Iterate a large result](/orm/fundamentals/reading-data#iterate-a-large-result).
