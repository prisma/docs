...
url: /docs/orm/reference/prisma-config-reference
...
Connection URL including authentication info. Uses [the syntax provided by the database](/docs/orm/reference/connection-urls#format).
...
The `directUrl` property is supported by Prisma Studio from version 5.1.0 upwards. The `directUrl` property is not needed when using [Prisma Postgres](/docs/postgres) database.
...
Prisma Studio now uses the connection configuration from the `datasource` property automatically. See the [Prisma Studio documentation](/docs/orm/reference/prisma-cli-reference#studio) for more details.
