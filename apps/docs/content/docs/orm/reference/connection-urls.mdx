...
Prisma ORM needs a connection URL to be able to connect to your database, e.g. when sending queries with [Prisma Client](/docs/v7/orm/prisma-client) or when changing the database schema with [Prisma Migrate](/docs/v7/orm/prisma-migrate).

The connection URL is provided via the `url` field of a `datasource` block in your Prisma config (or Prisma schema if on version 6). It usually consists of the following components (except for SQLite and [Prisma Postgres](/docs/v7/postgres)):
...
Make sure you have this information at hand when getting started with Prisma ORM. If you don't have a database server running yet, you can either use a local SQLite database file (see the [Quickstart](/docs/v7/getting-started/prisma-orm/quickstart/sqlite)) or [setup a free PostgreSQL database with Prisma Postgres](/docs/v7/postgres).
...
- [Microsoft SQL Server](/docs/v7/orm/overview/databases/sql-server)
...
For Microsoft SQL Server, you must [escape special characters](/docs/v7/orm/overview/databases/sql-server#connection-details) in any part of your connection string.
...
[Prisma Postgres](/docs/v7/postgres) is a managed PostgreSQL service running on unikernels. There are several ways to connect to Prisma Postgres:

- via direct TCP connections (lets you connect via any ORM or database tool)
- via [Prisma Accelerate](/docs/v7/more/accelerate) (only supported with Prisma ORM)
- locally
...
The connection string for connecting to a [local Prisma Postgres](/docs/v7/postgres/database/local-development) instance mirrors the structure of a remote instance via Accelerate:
...
However, in this case the `API_KEY` doesn't provide authentication details. Instead, it encodes information about the local Prisma Postgres instance. You can obtain a local connection string via the [`prisma dev`](/docs/v7/orm/reference/prisma-cli-reference#dev) command.
