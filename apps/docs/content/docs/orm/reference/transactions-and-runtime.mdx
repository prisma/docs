...
- Calling `connect()` after the client has been closed rejects with a structured `DRIVER.NOT_CONNECTED` error (`Postgres client is closed`).
...
- After `close()`, calling `connect()` again rejects with `DRIVER.NOT_CONNECTED` (`Postgres client is closed`).
...
- After the block exits, the client is closed the same way `close()` closes it: a later `connect()` rejects with `DRIVER.NOT_CONNECTED`.
...
- Calling `connect()` when the client is already connected rejects with `DRIVER.ALREADY_CONNECTED`. The first connection can happen implicitly, the moment any query builds the runtime, so a later explicit `connect()` collides the same way. PostgreSQL raises the same code from the same situation.
- After the client has been closed, `connect()` rejects with `DRIVER.NOT_CONNECTED` (`Mongo client is closed`).
...
```typescript
await db.connect();
await db.connect(); // throws DRIVER.ALREADY_CONNECTED
```
...
- Returns an [`AsyncIterableResult`](#asynciterableresult): `await` it for an array, or `for await` to iterate rows.
...
- Returns a `Promise<void>`.
- After `close()`, any further use throws `DRIVER.NOT_CONNECTED` (`Mongo client is closed`). This is verified against both `db.runtime()` and ORM access such as `db.orm.users.first()`, since both route through the same runtime.
...
```typescript
await db.close();
await db.runtime(); // throws DRIVER.NOT_CONNECTED
```
...
- After the block exits, a later `connect()` rejects with `DRIVER.NOT_CONNECTED`.
...
- `prepare()` starts the connection itself, so calling `db.connect()` afterwards throws `DRIVER.ALREADY_CONNECTED`. Reach for the runtime with `db.runtime()` instead, which returns the one `prepare()` already built.
...
Read terminals (`all()`, `createAll()`, and the runtime's `query(...)`) return an `AsyncIterableResult`: `await` it to collect an array, or `for await` to iterate rows one at a time.
