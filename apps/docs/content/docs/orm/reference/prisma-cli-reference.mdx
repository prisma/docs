...
By default, the project sets up a [local Prisma Postgres](/docs/postgres/database/local-development) instance but you can choose a different database using the `--datasource-provider` option.
...
| `--db`                   | No       | Shorthand syntax for `--datasource-provider prisma+postgres`; creates a new [Prisma Postgres](/docs/postgres) instance. Requires authentication in the [PDP Console](https://console.prisma.io).                          |                       |
...
| `--preview-feature`      | No       | Define the [Preview features](/docs/orm/reference/preview-features/cli-preview-features) to use. To define multiple Preview features, you have to provide the flag multiple times for each Preview feature. See examples. |                       |
| `--output`               | No       | Specifies the [output location for the generated client](/docs/orm/reference/prisma-schema-reference#fields-for-prisma-client-provider).                                                                                  | `../generated/prisma` |
...
See the [Prisma Config reference](/docs/orm/reference/prisma-config-reference) for more details.
...
The `generate` command generates assets like Prisma Client based on the [`generator`](/docs/orm/prisma-schema/overview/generators) and [`data model`](/docs/orm/prisma-schema/data-model/models) blocks defined in your `prisma/schema.prisma` file.
...
| ------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `--data-proxy`      | No       | The `generate` command will generate Prisma Client for use with [Prisma Accelerate](/docs/accelerate) prior to Prisma 5.0.0. Mutually exclusive with `--accelerate` and `--no-engine`.                                                   |
| `--accelerate`      | No       | The `generate` command will generate Prisma Client for use with [Prisma Accelerate](/docs/accelerate). Mutually exclusive with `--data-proxy` and `--no-engine`. Available in Prisma 5.1.0 and later.                                    |
| `--no-engine`       | No       | The `generate` command will generate Prisma Client without an accompanied engine for use with [Prisma Accelerate](/docs/accelerate). Mutually exclusive with `--data-proxy` and `--accelerate`. Available in Prisma ORM 5.2.0 and later. |
...
The `prisma-client` generator creates a customized client for working with your database in a custom output directory specified by the `output` field - you can [customize the output folder](/docs/orm/reference/prisma-schema-reference#fields-for-prisma-client-provider).

### `validate`

Validates the [Prisma Schema Language](/docs/orm/prisma-schema/overview) of the Prisma schema file.
...
For more information see our [environment variable documentation](/docs/orm/reference/environment-variables-reference)
...
The `dev` command starts a [local Prisma Postgres](/docs/postgres/database/local-development) database that you can run Prisma ORM commands against. It is useful for development and testing purposes and also allows you to switch to [Prisma Postgres](/docs/postgres) in production easily.

### Arguments

| Argument              | Required | Description                                                                                                                                                 | Default   |
| --------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `--name` (or `-n`)    | No       | Enables targeting a specific database instance. [Learn more](/docs/postgres/database/local-development#using-different-local-prisma-postgres-instances). | `default` |
...
Starts existing [local Prisma Postgres](/docs/postgres/database/local-development) instances in the background.
...
Lists all available [local Prisma Postgres](/docs/postgres/database/local-development) instances:
...
Stops one or more [local Prisma Postgres](/docs/postgres/database/local-development) databases:
...
Removes the data of one or more [local Prisma Postgres](/docs/postgres/database/local-development) databases from your file system:
...
Introspection with the `db pull` command on the [MongoDB connector](/docs/orm/core-concepts/supported-databases/mongodb) samples the data instead of reading a schema.
...
See [Seeding your database](/docs/orm/prisma-migrate/workflows/seeding)
...
Instead of `migrate dev` and related commands, [`db push`](/docs/orm/prisma-migrate/workflows/prototyping-your-schema) is used for [MongoDB](/docs/orm/core-concepts/supported-databases/mongodb).
...
1. Reruns the existing migration history in the [shadow database](/docs/orm/prisma-migrate/understanding-prisma-migrate/shadow-database) in order to detect schema drift (edited or deleted migration file, or a manual changes to the database schema)
...
This command is not supported on [MongoDB](/docs/orm/core-concepts/supported-databases/mongodb). Use [`db push`](/docs/orm/prisma-migrate/workflows/prototyping-your-schema) instead.
...
If a [schema drift](/docs/orm/prisma-migrate/understanding-prisma-migrate/shadow-database#detecting-schema-drift) is detected while running `prisma migrate dev` using `--create-only`, you will be prompted to reset your database.
...
This command is only partially supported for [MongoDB](/docs/orm/core-concepts/supported-databases/mongodb). See the command options below for details.
...
The `migrate diff` command can only compare database features that are [supported by Prisma](/docs/orm/reference/database-features). If two databases differ only in unsupported features, such as views or triggers, then `migrate diff` will not show any difference between them.
...
### `platform` ([Early Access](/docs/console/more/feature-maturity#early-access))
...
You can find the complete list of available commands with the arguments [here](/docs/cli/console).

### `mcp`

Starts the [Prisma MCP server](/docs/ai/tools/mcp-server).
...
For detailed database support information, including SQLite requirements, see [Databases supported by Prisma Studio](/docs/studio#getting-started).
...
- [`HTTP_PROXY`](/docs/orm/reference/environment-variables-reference#http_proxy) or `http_proxy`: Proxy URL for http traffic, for example `http://localhost:8080`
- [`HTTPS_PROXY`](/docs/orm/reference/environment-variables-reference#https_proxy) or `https_proxy`: Proxy URL for https traffic, for example `https://localhost:8080`

## `npx create-db`

The [`create-db`](https://create-db.prisma.io/) command provisions a temporary [Prisma Postgres](/docs/postgres) database with a single command. This is a standalone utility that can be invoked using `npx`. It's ideal for quickly testing, prototyping, or integrating with Prisma Postgres.
...
For full usage details, options (such as `--region` and `--interactive`), and examples, see the [documentation](/docs/postgres/npx-create-db).
