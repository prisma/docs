...
If none of these apply yet, refer to [How migrations work](/orm/migrations/how-migrations-work) instead and revisit this when required.
...
The payoff is in the last column and the two merge edges. A database that follows Alice's branch is sitting at `93be6c2` and reaches `f9a41d7` through `merge_alice`. A database that follows Bob's is at `7e3fa7f` and takes `merge_bob`. Neither developer needed to know about the other's migration, and no files were renamed to make the timestamps line up.
...
The graph has been designed with these in mind, but the commands have not shipped yet. Each migrations page calls out what is missing rather than papering over it.
