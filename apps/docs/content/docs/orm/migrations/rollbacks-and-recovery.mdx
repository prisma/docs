...
“Rollback” refers to two different situations, which Prisma 8 treats differently:

- **The migration applied, but the change was wrong.** In this case, you need to revert the database *back* to an earlier state. This is a **rollback**, in Prisma 8 it's implemented by another migration.
- **The migration failed partway.** You need to get *unstuck*. This process is **recovery**, and it usually means fixing the cause and re-running, because re-running is safe.

## Rollback: a migration like any other

There is no `migrate down` command  and no separate "down migration" files. In the [graph model](/orm/migrations/the-migration-graph), the state you want to return to is a node that you've already visited, so rolling back means planning a new edge that points at it. If you know git, this is `git revert`, not `git reset`. History only ever grows; the ledger (the applied-history record every database keeps) retains the full round trip.
...
The planner diffs the two contract states: it writes the operations that undo the change and appropriately flags them as **destructive**. This is a real migration: review it, edit it (for example, to archive the column's data into another table before the `DROP`), commit it. Then apply it the same way as any other migration:
...
The database's marker (its record of which graph node it currently matches) is back at `705b1a6`; the ledger records both the apply and the rollback. Nothing is rewritten or deleted.

Please be aware of these two details:
...
- **You don't need to retrace every step.** An edge can jump from the current state directly to any earlier node, skipping intermediate states: one planned migration, one apply, even if you're rolling back three changes.

### One planning caveat after a rollback

A rollback edge creates a cycle in the graph (`A → B → A`), and with a cycle the planner can no longer infer "the latest state" on its own. The next time you run `migration plan`, pass `--from` explicitly (via a migration directory name or hash); otherwise, you’ll encounter the error `MIGRATION.NO_TARGET`.
