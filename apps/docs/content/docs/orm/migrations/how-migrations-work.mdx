...
`--advance-ref db` moves the [ref](/orm/migrations/the-migration-graph#name-important-states-with-refs) named `db` to the state you just applied, which is where the *next* `migration plan` starts from. Leave it off and the next plan has no origin: it refuses with `MIGRATION.PLAN_ORIGIN_UNKNOWN` rather than write a full-create package. In CI and production you leave it off on purpose, because migrations arrive already planned.
...
Migrations are one of the newest parts of Prisma 8. The core loop on this page (plan, edit, apply, roll back) works today and is exercised in the Prisma 8 test suite. Some things classic migration tools grew over years are not built yet: there is no squash command and no shadow-database dry run. Baselines do exist, both the automatic one and the explicit `--from @empty` retrofit; see [the migration graph](/orm/migrations/the-migration-graph#baselines). We call out what's missing on each page rather than papering over it.
