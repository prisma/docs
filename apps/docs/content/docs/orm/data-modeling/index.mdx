...
| `Decimal`  | Exact decimal, PostgreSQL `numeric`. The runtime value is a decimal string, so nothing is rounded. Write `Numeric(10, 2)` in type position when you want an explicit precision and scale |
| `Boolean`  | `true` / `false`                                                              |
| `DateTime` | Timestamp                                                                     |
| `Bytes`    | Binary data, PostgreSQL `bytea`. The runtime value is a `Uint8Array`          |
| `Json`     | Arbitrary JSON value, stored in PostgreSQL's native `json`                     |
| `Jsonb`    | Arbitrary JSON value, stored in PostgreSQL's `jsonb`. This is what `field.json()` and value objects use, and what a `Json` field meant before 0.17 |
| `ObjectId` | MongoDB document identifier                                                   |

Filtering on a `Json` or `Jsonb` field is whole-value equality only: you can ask whether the column equals a given document, not whether a key inside it matches. Reach into the document with [raw SQL](/orm/reference/raw-queries) when you need a path query.
