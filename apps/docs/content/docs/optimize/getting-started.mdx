...
- A project using [Prisma Client](/docs/v7/orm/prisma-client) version `5.0.0` or higher (we recommend using the latest version).
...
If you are using [Prisma Accelerate](/docs/v7/more/accelerate) in your application, make sure you apply it _after_ the Optimize extension. For example:
...
3. Explore [individual query details](/docs/v7/postgres/query-optimization/recordings#data-captured-in-a-recording-session) by clicking on them, and check the **Recommendations** tab for any suggested improvements to enhance query performance.
