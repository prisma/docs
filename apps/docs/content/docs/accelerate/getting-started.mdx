...
- A project that uses [Prisma Client](/docs/v7/orm/prisma-client) `4.16.1` or higher. If your project is using interactive transactions, you need to use `5.1.1` or higher. (We always recommend using the latest version of Prisma.)
...
If you require IP allowlisting or firewall configurations with trusted IP addresses, enable Static IP for enhanced security. Learn more on [how to enable static IP for Accelerate in the Platform Console](/docs/v7/more/accelerate/static-ip).
...
```text file=.env highlight=3;add showLineNumbers
DATABASE_URL="prisma://accelerate.prisma-data.net/?api_key=__API_KEY__"
...
💡 Accelerate requires [Prisma Client](/docs/v7/orm/prisma-client) version `4.16.1` or higher and [`@prisma/extension-accelerate`](https://www.npmjs.com/package/@prisma/extension-accelerate) version `1.0.0` or higher.
...
If VS Code does not recognize the `$extends` method, refer to [this section](/docs/v7/more/accelerate/faq#vs-code-does-not-recognize-the-extends-method) on how to resolve the issue.

#### Using the Accelerate extension with other extensions

Since [extensions are applied one after another](/docs/v7/orm/prisma-client/client-extensions#conflicts-in-combined-extensions), make sure you apply them in the correct order. Extensions cannot share behavior and the last extension applied takes precedence.

If you are using [Prisma Optimize](/docs/v7/more/optimize) in your application, make sure you apply it _before_ the Accelerate extension. For example:
...
For information about which strategy best serves your application, see [Select a cache strategy](/docs/v7/postgres/database/caching#selecting-a-cache-strategy).
