...
- [Switch an existing Prisma Postgres database away from its hosted Accelerate connection](/postgres/database/switch-from-accelerate)
...
If you require IP allowlisting or firewall configurations with trusted IP addresses, enable Static IP. Learn more on [how to enable static IP for Accelerate in the Platform Console](/accelerate/static-ip).
...
💡 Accelerate requires [Prisma Client](/orm/v7/prisma-client/setup-and-configuration/introduction) version `4.16.1` or higher and [`@prisma/extension-accelerate`](https://www.npmjs.com/package/@prisma/extension-accelerate) version `1.0.0` or higher.
...
Since [extensions are applied one after another](/orm/v7/prisma-client/client-extensions#conflicts-in-combined-extensions), make sure you apply them in the correct order. Extensions cannot share behavior and the last extension applied takes precedence.
