...
url: /docs/accelerate/getting-started
...
- A project that uses [Prisma Client](/docs/orm/prisma-client/setup-and-configuration/introduction) `4.16.1` or higher. If your project is using interactive transactions, you need to use `5.1.1` or higher. (We always recommend using the latest version of Prisma.)
...
If you require IP allowlisting or firewall configurations with trusted IP addresses, enable Static IP for enhanced security. Learn more on [how to enable static IP for Accelerate in the Platform Console](/docs/accelerate/static-ip).
...
💡 Accelerate requires [Prisma Client](/docs/orm/prisma-client/setup-and-configuration/introduction) version `4.16.1` or higher and [`@prisma/extension-accelerate`](https://www.npmjs.com/package/@prisma/extension-accelerate) version `1.0.0` or higher.
...
If VS Code does not recognize the `$extends` method, refer to [this section](/docs/accelerate/more/faq#vs-code-does-not-recognize-the-extends-method) on how to resolve the issue.

#### Using the Accelerate extension with other extensions

Since [extensions are applied one after another](/docs/orm/prisma-client/client-extensions#conflicts-in-combined-extensions), make sure you apply them in the correct order. Extensions cannot share behavior and the last extension applied takes precedence.

If you are using [Prisma Optimize](/docs/optimize) in your application, make sure you apply it _before_ the Accelerate extension. For example:
...
For information about which strategy best serves your application, see [Select a cache strategy](/docs/postgres/database/caching#selecting-a-cache-strategy).
