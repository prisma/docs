...
The [`@prisma/management-api-sdk`](https://www.npmjs.com/package/@prisma/management-api-sdk) is a TypeScript SDK for the [Prisma Data Platform Management API](/docs/management-api). Use the simple client for direct API access, or the full SDK with built-in OAuth authentication and automatic token refresh.
...
For usage with an existing access or [service token](/docs/management-api/authentication#service-tokens).
