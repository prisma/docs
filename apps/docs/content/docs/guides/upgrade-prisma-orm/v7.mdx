---
title: Upgrade to v7
description: Guide for upgrading to Prisma ORM v7
...
Additionally other fields such as `url`, `directUrl`, and `shadowDatabaseUrl` in the `datasource` block are deprecated. You can configure them in the [Prisma Config](/orm/v7/reference/prisma-config-reference).
...
**If you experience timeout issues after upgrading**, configure your driver adapter to match v6 behavior. See the [connection pool guide](/orm/v7/prisma-client/setup-and-configuration/databases-connections/connection-pool#prisma-orm-v7-driver-adapter-defaults) for detailed configuration examples for each database.
:::
...
Standalone Prisma Accelerate and hosted Prisma Postgres `prisma+postgres://accelerate.prisma-data.net` connections will be retired on December 1, 2026.

- If the URL connects to Prisma Postgres, [switch to pooled TCP or the Prisma Postgres serverless driver](/postgres/database/switch-from-accelerate).
- If the URL connects to another database, [keep the database and remove Accelerate](/accelerate/keep-your-database).

:::

If you complete the Prisma 7 upgrade before removing Accelerate, you can keep the Accelerate URL and extension temporarily. Do **not** pass the Accelerate URL to a driver adapter, because `PrismaPg` expects a direct database connection string and will fail with `prisma://` or `prisma+postgres://`.
...
Remove this temporary integration before December 1, 2026 by following the transition guide that matches your database.
...
For more details, see [GitHub issue #28795](https://github.com/prisma/orm/issues/28795).
...
The Metrics preview feature was deprecated in [Prisma ORM 6.14.0](https://github.com/prisma/orm/releases/tag/6.14.0) and has been removed for Prisma ORM 7.0.0.
...
The client middleware API has been removed. If possible, use [Client Extensions](/orm/v7/prisma-client/client-extensions).
