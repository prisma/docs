...
This guide targets `prisma@8.0.0-rc.9`, `@prisma/orm-postgres@8.0.0-rc.9`, `@prisma/cli-engine@0.3.0`, and `@prisma/prisma7@7.10.0-dev.58`, with a Prisma 7 baseline on `7.9.1`.
...
**Expected result:** `8.0.0-rc.9` (or newer).
...
Without a ref, the next `migration plan` has no origin. Because migrations now exist on disk, it refuses with `MIGRATION.PLAN_ORIGIN_UNKNOWN` rather than plan a full `CREATE TABLE` package. With the ref set, plans chain from the baseline and contain only your actual changes.
