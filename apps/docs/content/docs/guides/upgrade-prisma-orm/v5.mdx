---
title: Upgrade to v5
description: Guide for upgrading to Prisma ORM v5
...
Prisma ORM v5.0.0 introduces a number of changes, including the usage of our new JSON Protocol, [which makes Prisma Client faster by default](https://www.prisma.io/blog/prisma-5-f66prwkjx72s). A full list of these changes can be found [in our release notes](https://github.com/prisma/orm/releases/tag/5.0.0).
...
In most cases, the fix is to wrap the existing value in an array. The shortcuts removed in Prisma ORM 5 are:
...
If you were using [native database types](/orm/v7/reference/prisma-schema-reference#postgresql) and also the `postgresql` provider, you will need to [baseline your database](/v7/prisma-orm/add-to-existing-project/cockroachdb#5-baseline-your-database) from PostgreSQL to CockroachDB:
...
5. Delete your existing migrations. We will be [performing a baseline](/v7/prisma-orm/add-to-existing-project/cockroachdb#5-baseline-your-database) in order to make your local setup agree with your existing CockroachDB instance.
6. Perform the [baselining steps](/v7/prisma-orm/add-to-existing-project/cockroachdb#5-baseline-your-database). After these steps, you'll have migrated successfully from the `postgresql` provider to the `cockroachdb` provider!
...
We highly discourage the use of internal private APIs as they can change without warning and are not guaranteed to be supported. If your usage requires a private API that was previous available [please reach out to us on GitHub.](https://github.com/prisma/orm/discussions/new?category=q-a)
...
The `prisma2` executable has been deprecated for some time and is now removed. If your scripts use Prisma CLI as `prisma2`, replace it with `prisma`.

### Removed `experimentalFeatures`

The `previewFeatures` field of the [generator block](/orm/v7/reference/prisma-schema-reference#generator) used to be called `experimentalFeatures`. We are removing that deprecated property.
