...
The deployment flow relies on these concepts:
...
The region where your Prisma Postgres database will be deployed. Choose a region close to your users to reduce latency.
...
For production deployments requiring multiple databases, we recommend the `partnerEntry` plan which provides higher database creation limits. [Apply for partner access](https://www.prisma.io/partners) to get these limits.
:::
...
The complete example above shows the entire deployment process. The sections below break down each step.
...
- `region`: Database region; pick one close to your users
...
You can call the deployment flow from your existing platform code:
...
- **Performance metrics**: Measure deployment time and time to first claim
...
Yes. The [Prisma Postgres REST API](/rest-api) lets you create and manage databases directly, which is useful for platforms that host applications elsewhere but still want programmatic database provisioning.
