...
- [Prisma CLI](/orm/v7/reference/prisma-cli-reference) (`npx prisma`)
...
Before continuing with the setup, define how access control will work in your application.
...
- New tasks inherit project access, so you do not need to update permissions manually.
...
:::info
These rules are used by the @permitio/permit-prisma extension to determine which records to return for each user.
:::
...
Thanks to the Permit-Prisma integration, **you won't need to add any manual filtering logic**; the extension handles it.
...
- Keeps permission checks out of your Prisma queries
