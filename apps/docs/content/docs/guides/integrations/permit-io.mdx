...
url: /docs/guides/integrations/permit-io
...
- [Prisma CLI](/docs/orm/reference/prisma-cli-reference) (`npx prisma`)
...
Integrate your API with an auth provider (e.g., [Clerk](/docs/guides/authentication/clerk/nextjs), Auth0) and replace the `x-user-email` header with a secure identity mechanism (like a JWT token).
