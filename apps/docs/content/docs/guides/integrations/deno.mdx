...
description: Learn how to integrate Prisma Postgres in a Deno Deploy project using a minimal Deno application
image: /img/guides/prisma-deno-integration-cover.png
url: /guides/integrations/deno
...
Update the `main.ts` file to create a minimal HTTP server that responds with "Hello, World!". You will add database access to this handler later.
...
Each time you refresh the page, a new log entry is created in your database.
