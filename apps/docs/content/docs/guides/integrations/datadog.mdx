...
Prisma ORM generates the SQL for you, which can hide how each query performs unless you instrument it. By integrating Datadog with Prisma using `@prisma/instrumentation` and [`dd-trace`](https://www.npmjs.com/package/dd-trace), you can automatically capture spans for every database query.
...
If you're integrating tracing into an existing Prisma project, you can skip this step and directly follow from [the setup tracing section](#4-set-up-datadog-tracing). Apply the changes in your project's equivalent folder structure.
...
- [Prisma tracing docs](/orm/v7/prisma-client/observability-and-logging/opentelemetry-tracing).
