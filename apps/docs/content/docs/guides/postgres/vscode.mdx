...
url: /docs/guides/postgres/vscode
...
1. Authenticate with the [Prisma Console](https://console.prisma.io) via the popup and select a target [workspace](/docs/console/concepts#workspace)
...
Beyond managing your database instances, the Prisma VS Code extension embeds Prisma Studio directly in your editor making it easy to perform create, update, delete steps on your database from right inside of VS Code. Follow these [easy steps](/docs/studio/integrations/vscode-integration) to get started.

## Prisma MCP server

Prisma provides its own [Model Context Protocol](/docs/ai/tools/mcp-server) (MCP) server that lets you manage Prisma Postgres databases, model database schemas and chat through migrations.
...
Learn more about the MCP server in our [MCP server documentation](/docs/ai/tools/mcp-server).
...
:::note
Currently, the agent mode uses your default [workspace](/docs/console/concepts#workspace) in the Prisma Data Platform.
:::
...
For more details, refer to our [GitHub Copilot documentation](/docs/ai/tools/github-copilot).
