...
The extension sets `DATABASE_URL` to a Prisma Postgres [connection string](/postgres/database/connecting-to-your-database) that starts with `postgres://`. Netlify Functions run on a conventional Node.js runtime, so use the pooled connection string for application queries:
...
1. Generate a connection string and copy the pooled value, which starts with `postgres://` and uses the `pooled.db.prisma.io` host.
1. Save it as the `DATABASE_URL` environment variable, you'll need it in the next section.
...
1. Open the newly created `.env` file and paste the pooled `DATABASE_URL` environment variable from the previous section into it.
