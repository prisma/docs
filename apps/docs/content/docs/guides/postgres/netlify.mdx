...
The extension sets `DATABASE_URL` to a Prisma Postgres [connection string](/postgres/database/connecting-to-your-database) that starts with `postgres://`. Netlify Functions run on a conventional Node.js runtime, so use the pooled connection string for application queries and the direct connection string for Prisma CLI commands:
...
If an existing Netlify site still receives a hosted `prisma+postgres://accelerate.prisma-data.net` URL in `DATABASE_URL`, switch it to pooled TCP or the Prisma Postgres serverless driver and update the environment variable on the site.
...
1. Generate a connection string and copy both the pooled value (host `pooled.db.prisma.io`) and the direct value (host `db.prisma.io`). Both start with `postgres://`.
1. Save both values, you'll need them in the next section: the pooled value becomes `DATABASE_URL`, the direct value becomes `DIRECT_URL`.
...
4. Install the PostgreSQL driver adapter that Prisma Client uses to connect over TCP:

```npm title="Terminal"
npm install @prisma/adapter-pg pg
```

5. Initialize Prisma ORM to create Prisma schema, config and `.env` file:
...
1. Open the newly created `.env` file and add the pooled connection string as `DATABASE_URL` and the direct connection string as `DIRECT_URL`, as shown [above](#ensure-your-project-uses-the-database_url-environment-variable). Prisma Client uses the pooled value, the Prisma CLI uses the direct one.
1. Open the newly created `prisma.config.ts` file and point the datasource at `DIRECT_URL`. `prisma init` generates it with `DATABASE_URL`, which would send the migration and Studio commands below through the connection pooler:
...
5. Create (at least) one `User` record in the database with Prisma Studio:
