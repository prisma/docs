...
url: /guides/v7/frameworks/nextjs
...
```npm
npm create prisma@stable -- --template next
```
...
This gives you a basic page with a title and a list of users. However, that list is static with hardcoded values. Update the page to fetch the users from your database and make it dynamic.
...
Now `localhost:3000/posts` will load, but the content is hardcoded again. Update it to be dynamic, similarly to the home page:
...
As before, this page is static with hardcoded content. Update it to be dynamic based on the `params` passed to the page:
...
There's a lot of changes here, so here is what each part does:
...
The form renders, but it doesn't do anything yet. Update the `createPost` function to save the post to the database:
...
When you submit the form, the `createPost` action creates a new post in the database and redirects you to the posts list page.
...
After the deployment is complete, you can visit your application at the URL that Vercel provides. Your Next.js application with Prisma ORM is now live on Vercel.
