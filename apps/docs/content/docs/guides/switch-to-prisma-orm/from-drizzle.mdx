...
url: /docs/guides/switch-to-prisma-orm/from-drizzle
...
You can learn how Prisma ORM compares to Drizzle on the [Prisma ORM vs Drizzle](/docs/orm/more/comparisons/prisma-and-drizzle) page.
...
this migration guide uses Neon PostgreSQL as the example database, but it equally applies to any other relational database that are [supported by Prisma ORM](/docs/orm/reference/supported-databases).
...
The first step to adopt Prisma ORM is to [install the Prisma CLI](/docs/orm/reference/prisma-cli-reference) in your project:
...
Before you can introspect your database, you need to set up your [Prisma schema](/docs/orm/prisma-schema/overview) and connect Prisma to your database. Run the following command in the root of your project to create a basic Prisma schema file:
...
Once that's done, you can configure your [database connection URL](/docs/orm/reference/connection-urls) in the `.env` file. Drizzle and Prisma ORM use the same format for connection URLs, so your existing connection URL should work fine.
...
With your connection URL in place, you can [introspect](/docs/orm/prisma-schema/introspection) your database to generate your Prisma models:
...
To continue using Prisma Migrate to evolve your database schema, you will need to [baseline your database](/docs/orm/prisma-migrate/getting-started).
...
Models that are generated via introspection currently _exactly_ map to your database tables. In this section, you'll learn how you can adjust the naming of the Prisma models to adhere to [Prisma ORM's naming conventions](/docs/orm/reference/prisma-schema-reference#naming-conventions).
...
In this section, we'll show a few sample queries that are being migrated from Drizzle to Prisma Client based on the example routes from the sample REST API project. For a comprehensive overview of how the Prisma Client API differs from Drizzle, check out the [comparison page](/docs/orm/more/comparisons/prisma-and-drizzle#).
...
Unlike Drizzle, Prisma ORM allows you to [model many-to-many relations _implicitly_](/docs/orm/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations). That is, a many-to-many relation where you do not have to manage the [relation table](/docs/orm/prisma-schema/data-model/relations/many-to-many-relations#relation-table-conventions) (also sometimes called JOIN table) _explicitly_ in your schema. Here is an example comparing Drizzle with Prisma ORM:
