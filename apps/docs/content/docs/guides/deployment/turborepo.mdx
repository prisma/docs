...
In the `schema.prisma` file, we specify a custom [`output`](/docs/v7/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path) path where Prisma will generate its types. This ensures Prisma's types are resolved correctly across different package managers.
...
You can add users to your database by creating a seed script or manually by using [Prisma Studio](/docs/v7/orm/tools/prisma-studio).
...
- [Prisma ORM Docs](/docs/v7/orm/overview/introduction)
