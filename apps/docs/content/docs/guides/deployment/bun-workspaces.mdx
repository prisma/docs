...
url: /docs/guides/deployment/bun-workspaces
...
- A [Prisma Postgres](/docs/postgres) database (or another [supported database](/docs/orm/reference/supported-databases))
...
Initialize Prisma ORM with an instance of [Prisma Postgres](/docs/postgres) in the `database` package by running the following command:
...
We're going to be using [Prisma Postgres](/docs/postgres) in this guide. If you're not using a Prisma Postgres database, you won't need to add the `--db` flag.
...
Use [Prisma Migrate](/docs/orm/prisma-migrate) to migrate your database changes:
...
From the monorepo root, run `bun run seed` to add sample users. Run `bun run studio` to open [Prisma Studio](/docs/studio) at [`http://localhost:5555`](http://localhost:5555) to view and edit your data.
...
Open your browser at [`http://localhost:3000`](http://localhost:3000) to see your app in action. You can run `bun run studio` to open [Prisma Studio](/docs/studio) at [`http://localhost:5555`](http://localhost:5555) to view and edit your data.
...
For further exploration and to enhance your setup, consider reading the [How to use Prisma ORM with Turborepo](/docs/guides/deployment/turborepo) guide.
