...
Initialize Prisma ORM with an instance of [Prisma Postgres](/docs/v7/postgres) in the `database` package by running the following command:
...
We're going to be using [Prisma Postgres](/docs/v7/postgres/getting-started) in this guide. If you're not using a Prisma Postgres database, you won't need to add the `--db` flag.
...
Edit the `schema.prisma` file to define a `User` model in your database and specify a [custom `output` directory](/docs/v7/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path) to generate the Prisma Client. This ensures that generated types are resolved correctly:
...
Use [Prisma Migrate](/docs/v7/orm/prisma-migrate) to migrate your database changes:
...
There shouldn't be data in your database yet. You can execute `pnpm run studio` in your CLI to start a [Prisma Studio](/docs/v7/orm/tools/prisma-studio) in [`http://localhost:5555`](http://localhost:5555) to interact with your database and add data to it.
...
For further exploration and to enhance your setup, consider reading the [How to use Prisma ORM with Turborepo](/docs/v7/guides/deployment/turborepo) guide.
