...
url: /docs/guides/deployment/pnpm-workspaces
...
Edit the `schema.prisma` file to define a `User` model in your database and specify a [custom `output` directory](/docs/orm/reference/prisma-schema-reference#fields-for-prisma-client-provider) to generate the Prisma Client. This ensures that generated types are resolved correctly:
...
There shouldn't be data in your database yet. You can execute `pnpm run studio` in your CLI to start a [Prisma Studio](/docs/studio) in [`http://localhost:5555`](http://localhost:5555) to interact with your database and add data to it.
