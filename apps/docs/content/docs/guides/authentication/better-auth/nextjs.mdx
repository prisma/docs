...
Next, build the user interface for authentication. In the `src/app` directory, create the following folder structure:
...
Add loading and redirecting states so the page does not flash an empty dashboard while the session is being verified.
...
Finally, update the home page to link to the sign-in and sign-up pages. Replace the contents of `src/app/page.tsx` with the following:
...
You now have a working authentication system built with Better Auth, Prisma, and Next.js.
