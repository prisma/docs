...
You can learn how Prisma ORM compares to Drizzle on the [Prisma ORM vs Drizzle](/docs/v7/orm/more/comparisons/prisma-and-drizzle) page.
...
this migration guide uses Neon PostgreSQL as the example database, but it equally applies to any other relational database that are [supported by Prisma ORM](/docs/v7/orm/reference/supported-databases).
...
The first step to adopt Prisma ORM is to [install the Prisma CLI](/docs/v7/orm/tools/prisma-cli#installation) in your project:
...
If you are using a different database provider (MySQL, SQL Server, SQLite), install the corresponding driver adapter package instead of `@prisma/adapter-pg`. For more information, see [Database drivers](/docs/v7/orm/overview/databases/database-drivers).
...
Before you can introspect your database, you need to set up your [Prisma schema](/docs/v7/orm/prisma-schema) and connect Prisma to your database. Run the following command in the root of your project to create a basic Prisma schema file:
...
Once that's done, you can configure your [database connection URL](/docs/v7/orm/reference/connection-urls) in the `.env` file. Drizzle and Prisma ORM use the same format for connection URLs, so your existing connection URL should work fine.
...
With your connection URL in place, you can [introspect](/docs/v7/orm/prisma-schema/introspection) your database to generate your Prisma models:
...
To continue using Prisma Migrate to evolve your database schema, you will need to [baseline your database](/docs/v7/orm/prisma-migrate/getting-started).
...
Models that are generated via introspection currently _exactly_ map to your database tables. In this section, you'll learn how you can adjust the naming of the Prisma models to adhere to [Prisma ORM's naming conventions](/docs/v7/orm/reference/prisma-schema-reference#naming-conventions).
...
In this section, we'll show a few sample queries that are being migrated from Drizzle to Prisma Client based on the example routes from the sample REST API project. For a comprehensive overview of how the Prisma Client API differs from Drizzle, check out the [comparison page](/docs/v7/orm/more/comparisons/prisma-and-drizzle#).
...
Unlike Drizzle, Prisma ORM allows you to [model many-to-many relations _implicitly_](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations). That is, a many-to-many relation where you do not have to manage the [relation table](/docs/v7/orm/prisma-schema/data-model/relations/many-to-many-relations#relation-tables) (also sometimes called JOIN table) _explicitly_ in your schema. Here is an example comparing Drizzle with Prisma ORM:
