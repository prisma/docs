...
Initialize Prisma with a [Prisma Postgres](/docs/v7/postgres) instance by running:
...
Open `prisma-user-database/schema.prisma` file and update it to define a `User` model. Also, set the environment variable and specify a [custom `output` directory](/docs/v7/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path) for the generated Prisma Client:
...
Edit the `prisma-post-database/schema.prisma` file to define a `Post` model. Also, update the datasource URL and set a [custom `output` directory](/docs/v7/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path):
...
- `migrate`: Applies pending migrations in development mode for both databases using [Prisma Migrate](/docs/v7/orm/prisma-migrate), updating their schemas based on changes in your Prisma files.
...
In a separate terminal window, open two instances of [Prisma Studio](/docs/v7/orm/tools/prisma-studio) to add data to your databases by running the script:
...
- [How to use Prisma ORM in a pnpm workspaces monorepo](/docs/v7/guides/deployment/pnpm-workspaces)
- [How to use Prisma ORM with Turborepo](/docs/v7/guides/deployment/turborepo)
