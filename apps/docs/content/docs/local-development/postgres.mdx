...
url: /local-development/postgres
...
[Prisma Postgres](/postgres) is a hosted database for your staging and production environments. For local iteration and isolated testing, you can run a _local_ Prisma Postgres instance (powered by [PGlite](https://pglite.dev)) with the `prisma dev` command. This page explains how to launch and manage a local Prisma Postgres database.
...
If you must use a different port, append [`--port <number>`](/orm/v7/reference/prisma-cli-reference#dev) (for example, `npx prisma migrate dev --port 5422`) and update your `DATABASE_URL` (or other connection settings) to match.
