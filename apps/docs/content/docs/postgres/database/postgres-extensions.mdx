...
description: "Use PostgreSQL extensions with Prisma Postgres"
badge: early-access
---
...
Postgres extensions support in Prisma Postgres is currently in [Early Access](/docs/v7/more/platform/maturity-levels#early-access) and not yet recommended for production scenarios.

:::

## Using extensions with Prisma ORM

Some extensions may already be supported by Prisma Postgres but not yet by Prisma ORM. Native support for some Postgres extensions in Prisma ORM is coming soon. In the meantime, you can still use these extensions with Prisma ORM by using [customized migrations](/docs/v7/orm/prisma-migrate/workflows/customizing-migrations) and [TypedSQL](/docs/v7/orm/prisma-client/using-raw-sql/typedsql) (or another mechanism to send [raw SQL](/docs/v7/orm/prisma-client/using-raw-sql) via in Prisma ORM).
...
The [warning in the CLI output of the command is expected](/docs/v7/orm/prisma-schema/introspection#introspection-warnings-for-unsupported-features) because Prisma ORM doesn't natively support the `VECTOR` type yet.
...
Because the `VECTOR` type is not yet natively supported by Prisma ORM, it's represented as an [`Unsupported`](/docs/v7/orm/prisma-schema/data-model/models#unsupported-types) type.
...
You can also use [TypedSQL](/docs/v7/orm/prisma-client/using-raw-sql/typedsql) for type-safe SQL queries against your database.
