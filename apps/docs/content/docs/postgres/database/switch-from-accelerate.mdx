...
For Prisma 7, point `prisma.config.ts` at the direct URL so that Prisma CLI commands stop using the Accelerate URL, then configure Prisma Client with the pooled URL and the `pg` driver adapter. Adjust the Prisma Client import path to match the `output` directory in your generator block:
...
For Prisma 7, point `prisma.config.ts` at the same direct URL so that Prisma CLI commands stop using the Accelerate URL, then instantiate Prisma Client with `PrismaPostgresAdapter`:
