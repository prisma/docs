...
url: /docs/postgres/database/caching
...
This feature is powered by an internal caching layer enabled through [Prisma Accelerate](/docs/accelerate), but you do not need to interact with Accelerate directly unless you're using your own database.
...
**When to invalidate:** If there are critical updates to the catalog, such as a major price change or product availability adjustment, the [cache should be invalidated](/docs/postgres/database/caching#on-demand-cache-invalidation) immediately to prevent customers from seeing outdated information.
...
You can invalidate the cache using the [`$accelerate.invalidate` API](/docs/accelerate/reference/api-reference#accelerateinvalidate):
