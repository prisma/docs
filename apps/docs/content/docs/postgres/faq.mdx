...
Yes, you can use Prisma Postgres with any database library or tool via a [direct connection](/docs/postgres/database/direct-connections).
...
- a SQL query (when using a [direct TCP connection](/docs/postgres/database/direct-connections))
...
The fundamental principle of operations-based pricing remains the same for Prisma ORM and [direct TCP connection](/docs/postgres/database/direct-connections). However, depending on whether you use Prisma ORM or direct SQL to interact with your database, an operation is something different:
...
- [**Batch your writes**](/docs/orm/prisma-client/queries/transactions#bulk-operations) with `createMany`, `updateMany`, or `deleteMany` instead of looping over single-row calls.
...
- **Use nested-relation helpers** such as [`connectOrCreate`](/docs/orm/reference/prisma-client-reference#connectorcreate) or [`set`](/docs/orm/reference/prisma-client-reference#set) to create or link related records in a single operation.
...
- **Prefer regular (array) [transactions](/docs/orm/prisma-client/queries/transactions#transaction-api) over [interactive transactions](/docs/orm/prisma-client/queries/transactions#interactive-transactions)** when the individual queries don't depend on each other.
...
The freshness of the data served by the cache depends on the cache strategy defined in your query. Refer to [this section](/docs/postgres/database/caching#selecting-a-cache-strategy) for more information on selecting the right cache strategy for your query.
...
The [Time-to-live](/docs/postgres/database/caching#time-to-live-ttl) (`ttl`) parameter can be set for up to a _year_. However, it's important to note that items within the cache may be evicted if they are not frequently accessed.
...
[On-demand cache invalidation](/docs/postgres/database/caching#on-demand-cache-invalidation) lets applications instantly update specific cached data when it changes, instead of waiting for regular cache refresh cycles. This keeps information accurate and up-to-date for users.

### When should I use the cache invalidate API?

The [cache invalidate API](/docs/postgres/database/caching#on-demand-cache-invalidation) is essential when data consistency cannot wait for the cache’s standard expiration or revalidation. Key use cases include:
...
:::warning
While you can increase these limits based on your subscription plan, it's _still_ recommended to optimize your database operations. [Learn more in our troubleshooting guide.](/docs/postgres/error-reference)
:::

## Query optimization

Prisma Postgres allows query optimization via [Prisma Optimize](/docs/optimize) and provides performance recommendations to help improve your database queries during development. You can enable it with Prisma Postgres or [also use it with your own database](/docs/optimize/getting-started), but setup and integration steps differ.
