...
This command is only partially supported for [MongoDB](/orm/v7/core-concepts/supported-databases/mongodb). See options below for details.
...
The `migrate diff` command can only compare database features [supported by Prisma](/orm/v7/reference/database-features). Differences in unsupported features (views, triggers, etc.) won't be shown.
