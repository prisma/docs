---
title: rm
description: Remove local Prisma Postgres servers
metaTitle: prisma dev rm | Remove Local Prisma Postgres Databases
metaDescription: 'Remove local Prisma Postgres database data with prisma dev rm. Use --force to stop running servers first. Supports glob patterns.'
url: /docs/cli/dev/rm
---

The `prisma dev rm` command removes the data of one or more [local Prisma Postgres](/docs/postgres/database/local-development) databases from your file system.

## Usage

```bash
prisma dev rm [options] <name>
```

## Arguments

| Argument | Description                                           |
| -------- | ----------------------------------------------------- |
| `<name>` | Name(s) or glob pattern(s) of the server(s) to remove |

## Options

| Option    | Description                                   | Default |
| --------- | --------------------------------------------- | ------- |
| `--debug` | Enable debug logging                          | `false` |
| `--force` | Stop any running servers before removing them | `false` |

Without `--force`, the command fails if any server is running.

## Examples

### Remove a specific database

```npm
npx prisma dev rm mydb
```

### Remove multiple databases with a pattern

Remove all databases starting with `mydb`:

```npm
npx prisma dev rm mydb*
```

### Force remove a running database

Stop and remove a database in one command:

```npm
npx prisma dev rm --force mydb
```

:::note

The `rm` command is interactive and includes safety prompts to prevent accidental data loss.

:::
