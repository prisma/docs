...
url: /docs/cli/generate
---

The `prisma generate` command generates assets like Prisma Client based on the [`generator`](/docs/orm/prisma-schema/overview/generators) and [`data model`](/docs/orm/prisma-schema/data-model/models) blocks defined in your `schema.prisma` file.
...
The `prisma-client` generator creates a customized client for working with your database. You can [customize the output folder](/docs/orm/reference/prisma-schema-reference#fields-for-prisma-client-provider) using the `output` field in the generator block.
