...
url: /docs/prisma-orm/add-to-existing-project/cockroachdb
...
- **`@prisma/adapter-pg`** - The [`node-postgres` driver adapter](/docs/orm/core-concepts/supported-databases/postgresql#using-driver-adapters) that connects Prisma Client to your database (CockroachDB is PostgreSQL-compatible)
...
Set up your Prisma ORM project by creating your [Prisma Schema](/docs/orm/prisma-schema/overview) file with the following command:
...
The generated schema uses [the ESM-first `prisma-client` generator](/docs/orm/prisma-schema/overview/generators#prisma-client) with a custom output path:
...
The [format of the connection URL](/docs/orm/reference/connection-urls) for CockroachDB looks as follows:
...
To use Prisma Migrate with your existing database, you need to [baseline your database](/docs/orm/prisma-migrate/getting-started).
...
[Prisma Studio](/docs/studio) does not currently support CockroachDB. Support may be added in a future release. See [Databases supported by Prisma Studio](/docs/studio#supported-databases) for more information.
...
- **Learn more about Prisma Client**: Explore the [Prisma Client API](/docs/orm/prisma-client/setup-and-configuration/introduction) for advanced querying, filtering, and relations
- **Database migrations**: Learn about [Prisma Migrate](/docs/orm/prisma-migrate) for evolving your database schema
- **Performance optimization**: Discover [query optimization techniques](/docs/orm/prisma-client/queries/advanced/query-optimization-performance)
- **Build a full application**: Check out our [framework guides](/docs/guides) to integrate Prisma ORM with Next.js, Express, and more
- **Join the community**: Connect with other developers on [Discord](https://pris.ly/discord)


## More info

- [CockroachDB database connector](/docs/orm/core-concepts/supported-databases/postgresql#cockroachdb)
- [Prisma Config reference](/docs/orm/reference/prisma-config-reference)
- [Database introspection](/docs/orm/prisma-schema/introspection)
