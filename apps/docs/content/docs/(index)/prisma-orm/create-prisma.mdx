...
The scaffold adds these scripts to `package.json`, next to the template's own `dev`, `build`, and `start`:
...
| `postinstall` | `prisma skills sync \|\| exit 0`, added by the `prisma init` the scaffold runs, so the skills re-sync on every install |
