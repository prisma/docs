...
The prompts cover the project name, the app template, the database provider, the contract authoring style, the package manager, and whether to deploy right away. Generated Node.js projects expect Node.js 22.18 or newer; Node.js 24 is recommended.
...
Set `DATABASE_URL` in your environment, then initialize the database and start the dev server. For PostgreSQL the scaffold writes no `.env` and the generated `prisma.config.ts` reads no dotenv file, so the connection string has to be exported in the shell. MongoDB projects are the exception: the scaffold writes a `.env` with a starter `DATABASE_URL` you can edit.
...
Deno projects get the Prisma scripts only, each wrapped in `deno run -A npm:...`, and no Composer scripts.
