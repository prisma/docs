...
If you work with a coding agent, run [`npx prisma@latest init`](/cli/init) once. The scaffold ran it already, so on a fresh project `init` confirms the setup and reports each step as already done: the [Prisma agent skills](/ai/tools/skills) that ship inside the Prisma packages are synced, and a `skills:sync` script is in `package.json` for refreshing them. Running `init` again is what repairs the setup after you upgrade a Prisma package. See [`skills`](/cli/skills).
