...
- Node.js 22.18 or newer, 24 recommended (or Bun)
...
Answer the prompts for contract authoring style and package manager, or pass `--authoring psl|typescript` and `--package-manager npm|pnpm|yarn|bun|deno` to skip them. Then enter the project:

```npm
cd my-app
```

If you work with a coding agent, run [`npx prisma@latest init`](/cli/init) once. The scaffold ran it already, so on a fresh project `init` confirms the setup and reports each step as already done: the [Prisma agent skills](/ai/tools/skills) that ship inside the Prisma packages are synced, a `postinstall` hook (`prisma skills sync || exit 0`) re-syncs them on every install, and a `skills:sync` script is in `package.json` for refreshing them by hand. Running `init` again is what repairs the setup after you upgrade a Prisma package. See [`skills`](/cli/skills).
