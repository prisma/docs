...
This guide assumes you set up [Prisma Postgres](/docs/v7/postgres) instance with `prisma init --db`:
...
If you ran the `prisma init --db` command inside a folder where you want your project to live, you can skip this step and [proceed to the next section](/docs/v7/getting-started/prisma-postgres/from-the-cli#2-set-up-your-project).
...
- **`@prisma/adapter-pg`** - The [`node-postgres` driver adapter](/docs/v7/orm/overview/databases/postgresql#using-the-node-postgres-driver) that connects Prisma Client to your database
...
After adding the models, migrate your database using [Prisma Migrate](/docs/v7/orm/prisma-migrate):
...
If you need to query your database via HTTP from an edge runtime (Cloudflare Workers, Vercel Edge Functions, etc.), use the [Prisma Postgres serverless driver](/docs/v7/postgres/database/serverless-driver#use-with-prisma-orm).
...
One of the main features of Prisma ORM is the ease of working with [relations](/docs/v7/orm/prisma-schema/data-model/relations). In this section, you'll learn how to create a `User` and a `Post` record in a nested write query. Afterwards, you'll see how you can retrieve the relation from the database using the `include` option.
...
You just got your feet wet with a basic Prisma Postgres setup. If you want to explore more complex queries, such as [adding caching functionality](/docs/v7/postgres/database/caching#setting-up-caching-in-prisma-postgres), check out the official [Quickstart](/docs/v7/getting-started/prisma-orm/quickstart/prisma-postgres).
...
- [Build a fullstack app with Next.js 15](/docs/v7/guides/frameworks/nextjs)
