...
url: /docs/prisma-postgres/from-the-cli
...
This guide assumes you set up [Prisma Postgres](/docs/postgres) instance with `prisma init --db`:
...
If you ran the `prisma init --db` command inside a folder where you want your project to live, you can skip this step and [proceed to the next section](/docs/prisma-postgres/from-the-cli#2-set-up-your-project).
...
After adding the models, migrate your database using [Prisma Migrate](/docs/orm/prisma-migrate):
...
One of the main features of Prisma ORM is the ease of working with [relations](/docs/orm/prisma-schema/data-model/relations). In this section, you'll learn how to create a `User` and a `Post` record in a nested write query. Afterwards, you'll see how you can retrieve the relation from the database using the `include` option.
...
You just got your feet wet with a basic Prisma Postgres setup. If you want to explore more complex queries, such as [adding caching functionality](/docs/postgres/database/caching#setting-up-caching-in-prisma-postgres), check out the official [Quickstart](/docs/prisma-orm/quickstart/prisma-postgres).
...
- [Build a fullstack app with Next.js 15](/docs/guides/frameworks/nextjs)
