...
url: /docs/prisma-postgres/import-from-existing-database-postgresql
...
In the third step, you will be using a [direct connection](/docs/postgres/database/direct-connections) to securely connect to your Prisma Postgres database to run `pg_restore`.
...
Make sure to have the connection URL for your existing database ready, it should be [structured](/docs/orm/reference/connection-urls) like this:
...
In this section, you'll use your [direct connection string](/docs/postgres/database/direct-connections) to connect to your Prisma Postgres instance and import data via `pg_restore`.
...
To validate that the import worked, you can use [Prisma Studio](/docs/guides/postgres/viewing-data#viewing-and-editing-data-in-prisma-studio). Either open it in the [Platform Console](https://console.prisma.io) by clicking the **Studio** tab in the left-hand sidenav in your project or run this command to launch Prisma Studio locally:
...
For a complete guide on setting up Prisma ORM with Prisma Postgres from scratch, including driver adapter configuration and best practices, see the [Prisma ORM with Prisma Postgres quickstart](/docs/prisma-orm/quickstart/prisma-postgres).
...
You can find the detailed step-by-step instructions for this process in this guide: [Add Prisma ORM to an existing project](/docs/prisma-orm/add-to-existing-project/prisma-postgres).
