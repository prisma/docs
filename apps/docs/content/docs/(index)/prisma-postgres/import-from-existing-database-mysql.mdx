...
1. Connect directly to a Prisma Postgres instance using a [direct connection](/docs/v7/postgres/database/direct-connections).
...
1. In a [workspace](/docs/v7/more/platform/about#workspace) of your choice, click the **New project** button.
...
In this step, you'll use the [direct connection string](/docs/v7/postgres/database/direct-connections) you obtained in step 1 to connect to your Prisma Postgres instance.
...
You also can use [Prisma Studio](/docs/v7/guides/postgres/viewing-data#viewing-and-editing-data-in-prisma-studio) and verify whether the migration was successful:
...
In the generated `.env` file, update `DATABASE_URL` to match your Prisma Postgres direct connection string that you received in [step 1](/docs/v7/getting-started/prisma-postgres/import-from-existing-database-mysql#1-create-a-new-prisma-postgres-database):

```bash file=.env no-copy
DATABASE_URL="postgres://USER:PASSWORD@db.prisma.io:5432/?sslmode=require"
```

[Introspect](/docs/v7/orm/prisma-schema/introspection) your newly migrated database by running:

```npm
npx prisma db pull
```

This command updates your `schema.prisma` file with models representing your migrated tables, so you can start using [Prisma Client](/docs/v7/orm/prisma-client) to query your data or [Prisma Migrate](/docs/v7/orm/prisma-migrate/getting-started) to manage future changes.
...
For a comprehensive guide on getting started with Prisma and Prisma Postgres, see [start from scratch with Prisma and Prisma Postgres](/docs/v7/getting-started/prisma-orm/quickstart/prisma-postgres).
...
If you were using [Prisma Migrate](/docs/v7/orm/prisma-migrate/getting-started) before:

- Delete your existing `migrations` folder in the `prisma` directory.
- [Baseline your database](/docs/v7/orm/prisma-migrate/workflows/baselining#baselining-a-database) to begin creating new migrations.
