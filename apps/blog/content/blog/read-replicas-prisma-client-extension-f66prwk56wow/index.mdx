...
In the Node.js ecosystem, Prisma is one of the most popular libraries for implementing the application data layer. [Support for read replicas](https://github.com/prisma/orm/issues/172) in Prisma Client is one of the most requested features.
...
<Image
  ...
  alt="Create a read replica on Neon"
...
<Image
  ...
  alt="Connect to a read replica on Neon"
...
Any write queries (e.g., `create`, `update`, ...) as well as [`$transaction`](https://www.prisma.io/docs/orm/v7/prisma-client/queries/transactions#the-transaction-api) queries are forwarded to the primary instance of your database, which would consequently propagate the resulting changes to the existing database replicas.
...
Be sure also to try out [Prisma Client extensions](https://www.prisma.io/docs/orm/v7/prisma-client/client-extensions) and share with us what you build on [Twitter](https://pris.ly/x) or [Discord](https://pris.ly/discord). 🙌
