...
Our engineers have been working hard, issuing new [releases](https://github.com/prisma/orm/releases/) with many improvements and new features. You can stay up-to-date about all upcoming features on our [roadmap](https://pris.ly/roadmap).

We once shipped a regression (and fixed it quickly, of course!), a new major version, [Prisma 4](https://github.com/prisma/orm/releases/tag/4.0.0) coupled with breaking changes, promoted a ton of features to [General Availability](https://www.prisma.io/docs/orm/v7/more/releases#generally-available-ga), released a couple of new [Preview features](https://www.prisma.io/docs/orm/v7/more/releases#preview), and improvements in the last three months.
...
Refer to our docs to learn how you can [configure indexes](https://www.prisma.io/docs/orm/v7/prisma-schema/data-model/indexes) in your Prisma schema and the [supported indexes for the different databases](https://www.prisma.io/docs/orm/v7/reference/database-features).

⚠️ **Breaking change**: If you previously configured the index properties at the database level, refer to the [upgrade guide](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#index-configuration) for a detailed explanation and steps to follow.
...
```
Learn more in [our documentation](https://www.prisma.io/docs/orm/v7/prisma-client/special-fields-and-types/working-with-json-fields#filter-on-a-json-field).
...
⚠️ **Breaking change**: To learn how you can smoothly upgrade to version 4.0.0, refer to our upgrade guide: [Raw query type mapping: scalar values are now deserialized as their correct JavaScript types](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#raw-query-type-mapping-scalar-values-are-now-deserialized-as-their-correct-javascript-types) and [Raw query mapping: PostgreSQL type-casts](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#raw-query-mapping-postgresql-type-casts).
...
If you're upgrading from Prisma version `3.9.0`+ or the PostgreSQL connector, you can now run `npx prisma db pull` and review the changes to your schema. To learn more about CockroachDB-specific native types we support, refer to [our docs](https://www.prisma.io/docs/orm/v7/core-concepts/supported-databases/postgresql#type-mapping-limitations-in-cockroachdb).

To learn more about the connector and how it differs from PostgreSQL, head to our [documentation](https://www.prisma.io/docs/orm/v7/core-concepts/supported-databases/postgresql).
...
- [Fixing failed migrations](https://www.prisma.io/docs/orm/v7/prisma-migrate/workflows/patching-and-hotfixing#fixing-failed-migrations-with-migrate-diff-and-db-execute)
- [Squashing migrations](https://www.prisma.io/docs/orm/v7/prisma-migrate/workflows/squashing-migrations)
- [Generating down migrations](https://www.prisma.io/docs/orm/v7/prisma-migrate/workflows/generating-down-migrations)
- Command reference for [`migrate diff`](https://www.prisma.io/docs/orm/v7/reference/prisma-cli-reference#migrate-diff) and [`db execute`](https://www.prisma.io/docs/orm/v7/reference/prisma-cli-reference#db-execute)
...
console.log(metrics)
```
To learn more, check out the metrics [documentation](https://www.prisma.io/docs/orm/v7/prisma-client/observability-and-logging/logging). Give it a try and [let us know what you think](https://github.com/prisma/orm/issues/13579).
...
```
Learn more in [our documentation](https://www.prisma.io/docs/orm/fundamentals/reading-data#sort-with-null-records-first-or-last) and don't hesitate to share your feedback in [this issue](https://github.com/prisma/orm/issues/14377).
...
Refer to the API reference in our docs to learn how [`findUniqueOrThrow`](https://www.prisma.io/docs/orm/v7/reference/prisma-client-reference#finduniqueorthrow) and [`findFirstOrThrow`](https://www.prisma.io/docs/orm/v7/reference/prisma-client-reference#findfirstorthrow) differ from `findUnique` and `findFirst` respectively.

### General improvements

#### Prisma Client for Data Proxy improvements

The [Prisma Data Proxy](https://www.prisma.io/docs/postgres) provides connection management and pooling for database connections for efficiently scaling database connections in serverless environments. The Prisma Client for Data Proxy provides support for connecting to the Prisma Data Proxy using HTTP.
...
We introduced Prisma Client for Data Proxy in version [`3.3.0`](https://github.com/prisma/orm/releases/tag/3.3.0) and we have been shipping features, fixes and improvements.
...
```
2. Running Prisma Client using the Data Proxy in [Cloudflare Workers and Edge environments](https://www.prisma.io/docs/orm/v7/prisma-client/deployment/edge/deploy-to-cloudflare).
...
```
To learn more, check out our [documentation](https://www.prisma.io/docs/postgres).
...
```
To learn more about default values for scalar lists, refer to [our docs](https://www.prisma.io/docs/orm/v7/reference/prisma-schema-reference#define-a-scalar-list-with-a-default-value).

**⚠️ Breaking change:** Refer to the [upgrade guide](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#scalar-list-defaults) for a detailed explanation and steps to follow.
...
```
Refer to our docs to learn more on [default values for required fields on composite types](https://www.prisma.io/docs/orm/v7/prisma-client/special-fields-and-types/composite-types#default-values-for-required-fields-on-composite-types).

**⚠️ Breaking change:** Refer to our [upgrade guide](https://www.prisma.io/docs/orm/v7/prisma-client/special-fields-and-types/composite-types#default-values-for-required-fields-on-composite-types) for detailed explanation and steps when working with default fields on composite types in MongoDB from version `4.0.0`.
...
```
**⚠️ Breaking change:** Refer to our [upgrade path](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#explicit-unique-constraints-on-one-to-one-relations) for a detailed explanation and steps to follow.
...
Refer to our docs to learn more about [implicit m:n relations](https://www.prisma.io/docs/orm/v7/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations).

**⚠️ Breaking change:** Refer to the [upgrade guide](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#remove-references-syntax-for-implicit-many-to-many-relations) for a detailed explanation and steps to follow.
...
**⚠️ Breaking change:** To learn how to upgrade to version `4.0.0`, refer to our [upgrade guide](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#enforced-use-of-unique-or-id-attribute-for-one-to-one-and-one-to-many-relations-mysql-and-mongodb).
...
**⚠️ Breaking change:** To learn how to update your existing schema, refer to the [upgrade guide](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#better-grammar-for-string-literals).
...
Refer to the [`findUniqueOrThrow`](https://www.prisma.io/docs/orm/v7/reference/prisma-client-reference#finduniqueorthrow) and [`findFirstOrThrow`](https://www.prisma.io/docs/orm/v7/reference/prisma-client-reference#findfirstorthrow) docs to learn how you can upgrade.
...
Prisma will now check numbers in the query's response to verify they fit within the boundaries of an integer. If a number does not fit, Prisma will throw a [`P2023`](https://www.prisma.io/docs/orm/reference/error-reference#p2023) error:
```
Inconsistent column data: Conversion failed:
Value 9223372036854775807 does not fit in an INT column,
try migrating the 'int' column type to BIGINT
```
To learn more on rounding errors with big numbers on SQLite, refer to our [docs](https://www.prisma.io/docs/orm/v7/core-concepts/supported-databases/sqlite#rounding-errors-on-big-numbers).
...
Refer to the [Prisma 4 upgrade guide](https://www.prisma.io/docs/guides/upgrade-prisma-orm/v4#dbnull-jsonnull-and-anynull-are-now-objects) in case you run into any type errors.
...
Refer to our [system requirements](https://www.prisma.io/docs/orm/v7/reference/system-requirements) for the minimum versions Prisma requires
...
If the new default doesn't work for you, please [create an issue](https://github.com/prisma/orm/issues/new) and use the `statement_cache_size=x` parameter in your connection string to override the default value.
...
If you're using `@prisma/sdk` (now `@prisma/internals`), it would be helpful if you could help us understand where, how, and why you are using it by giving us feedback in this [GitHub discussion](https://github.com/prisma/orm/discussions/13877). Your feedback will be valuable to us in defining a better API.
...
1. Prisma Client now correctly releases memory on Prisma Client instances that are no longer being used. Learn more in this [GitHub issue](https://github.com/prisma/orm/issues/8989)
2. Reduced CPU usage spikes when disconnecting Prisma Client instances while using Prisma Client. You can learn more in this [GitHub issue](https://github.com/prisma/orm/issues/12516)
...
You can learn more about it in [this blog post](/series-b-announcement-v8t12ksi6x).
### Prisma FOSS Fund
...
You can learn more about the FOSS Fund initiative in [this blog post](/prisma-foss-fund-announcement-XW9DqI1HC24L).
...
- [Database access on the Edge with Next.js, Vercel & Prisma Data Proxy](/database-access-on-the-edge-8F0t1s1BqOJE)
- [Announcing the Prisma FOSS Fund](/prisma-foss-fund-announcement-XW9DqI1HC24L)
- [Building a REST API with NestJS and Prisma: Input Validation & Transformation](/nestjs-prisma-validation-7D056s1kOla1)
- [The Prisma Data Platform is now Generally Available](/prisma-data-platform-now-generally-available-8D058s1BqOL1)
- [Building a REST API with NestJS and Prisma](/nestjs-prisma-rest-api-7D056s1BmOL0)
- [Prisma Support for CockroachDB Is Production Ready 🪳](/cockroach-ga-5JrD9XVWQDYL)
- [How Prisma helps Amplication evolutionize backend development](/amplication-customer-story-nmlkBNlLlxnN)
- [Build A Fullstack App with Remix, Prisma & MongoDB (series)](/series/fullstack-remix-prisma-mongodb)
