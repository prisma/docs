...
```shell
npm install @prisma/client@7.9.1 express cors
```
...
Inside the `/prisma` directory is a file called `schema.prisma`. This file uses the [Prisma Schema Language (PSL)](https://www.prisma.io/docs/orm/contract-authoring/psl-syntax#syntax) and is the place we describe all of our database tables and the relationships between them.
...
With our model in place, we can now run a command to create and run a [migration](https://www.prisma.io/docs/orm/migrations/how-migrations-work).
