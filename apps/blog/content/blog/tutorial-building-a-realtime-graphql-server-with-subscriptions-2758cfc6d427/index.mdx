...
metaDescription: "Learn how to build a realtime GraphQL server with subscriptions so clients can receive live event updates."
...
Consider again the sample `updatePost`-mutation from the section [\*\*2.3.3](/tutorial-building-a-realtime-graphql-server-with-subscriptions-2758cfc6d427)\**. But let’s now assume, the subscription query includes *all\* the fields we just discussed:
...
The first step, as usual when adding new features to a GraphQL API, is to express the new operation as a [_root field_](/graphql-server-basics-the-schema-ac5e2950214e) in the GraphQL schema.
