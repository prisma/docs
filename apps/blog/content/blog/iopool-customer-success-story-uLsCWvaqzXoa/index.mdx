...
The improvements in the development cycle are due, in large part, to the type safety that Prisma provides. The ability for iopool's developers to get intellisense, autocompletion, and type checking when making database access calls with [Prisma Client](https://www.prisma.io/orm) has been crucial for the speed gains.

[Prisma Migrate](https://www.prisma.io/docs/orm/migrations/how-migrations-work) has also been key for iopool in their version 2 upgrade. Migrate has given the team confidence in their approach to database schema changes, allowing multiple developers to collaborate on a central schema and have the changes applied seamlessly across environments.
