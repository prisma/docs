...
Joining data from multiple tables is a complicated topic. There are two main strategies: database-level and application-level joins. [Prisma ORM offers both options](/prisma-orm-now-lets-you-choose-the-best-join-strategy-preview). In this article you’ll learn the tradeoffs between the two so you can pick the best strategy for your use case.
...
When Prisma ORM was [initially released in 2021](https://www.prisma.io/docs/orm/v6/overview/introduction/why-prisma), it implemented the application-level join strategy for all its relation queries. 
...
That's why [we've added the smart DB-level joins as an alternative one year ago](/prisma-orm-now-lets-you-choose-the-best-join-strategy-preview), so developers have the option to always choose the most performant join strategy for their individual use case.
...
[Community feedback is one of the major drivers that helps us prioritize](/prisma-orm-manifesto#2-clearer-issue-management-community-prioritization-and-engagement) what we're working on to improve Prisma ORM.
